...
class BinSchedule:
	...
	def _gen(self) -> Generator[pl.LazyFrame, None, None]:
		lf = pl.LazyFrame({'symbol': pl.Enum(list(self.symbols))})
	...
	def _check_input_types(self):
		for (name, field_type) in self.__annotations__.items():
			if not isinstance(self.__dict__[name], field_type):
				raise TypeError(f'{name} should be of type {field_type}')
	...
	def __call__(
		...
		*,
		...
		idx = next(self._schedule).filter(
		...
		return idx.join(data, on=['symbol', 'date', 'time'], how='left')
