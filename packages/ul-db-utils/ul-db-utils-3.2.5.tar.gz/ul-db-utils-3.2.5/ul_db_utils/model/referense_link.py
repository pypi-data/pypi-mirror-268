...
class ReferenceLink(BaseUserLogModel):
    ...
    short_id = db.Column(db.String(255), unique=True, comment="Короткая ссылка, сгенерированная в format(uuid4().time_low, 'x')")
    expiration_date = db.Column(db.DateTime(), comment="Дата и время истечения срока действия ссылки")
    origin_link = db.Column(db.Text, comment="Ссылка на исходный ресурс, на которой пользователь будет перенаправлен по короткой ссылке")
