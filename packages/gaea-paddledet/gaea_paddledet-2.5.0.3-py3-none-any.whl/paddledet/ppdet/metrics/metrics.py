...
from .coco_utils import get_infer_results, cocoapi_eval, cocoapi_detail_eval
...
class COCOMetric(Metric):
    def __init__(self, anno_file, **kwargs):
        ...
        # self.tracking_uri = os.environ.get("TRACKING_URI", "")
    ...
    def accumulate(self):
        if len(self.results['bbox']) > 0:
            ...
            if self.save_prediction_only:
                ...
                #else:
                # gaea-tracker
                classwise_metrics, bbox_stats = cocoapi_detail_eval(
                    output,
                    'bbox',
                    anno_file=self.anno_file,
                    confusion_matrix=True,
                )
                self.eval_results['GAEA_TRACKER_METRICS'] = classwise_metrics
