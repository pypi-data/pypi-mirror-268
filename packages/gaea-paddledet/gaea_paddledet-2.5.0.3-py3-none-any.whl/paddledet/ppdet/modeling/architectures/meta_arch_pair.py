...
__all__ = ['BaseArchPair']


@register
class BaseArchPair(nn.Layer):
    def __init__(self, data_format='NCHW'):
        super(BaseArchPair, self).__init__()
    ...
    def forward(self, inputs):
        if self.data_format == 'NHWC':
            image = inputs['image']
            tmp_image = inputs['tmp_image']
            inputs['image'] = paddle.transpose(image, [0, 2, 3, 1])
            inputs['tmp_image'] = paddle.transpose(tmp_image, [0, 2, 3, 1])

        if self.fuse_norm:
            image = inputs['image']
            self.inputs['image'] = image * self.scale + self.bias
            self.inputs['tmp_image'] = tmp_image * self.scale + self.bias
        ...
        if self.training:
            ...
            for inp in inputs_list:
                if self.fuse_norm:
                    ...
                    self.inputs['tmp_image'] = inp['tmp_image'] * self.scale + self.bias
