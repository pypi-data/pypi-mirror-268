...
@register_op
class DecodePair(BaseOperator):
    def __init__(self, suffix=None):
        """ Transform the image data to numpy format following the rgb format
        """
        super(DecodePair, self).__init__()
        self.suffix = suffix
    ...
    def apply(self, sample, context=None):
        ...
        if 'image' not in sample:
            with open(sample['im_file'], 'rb') as f:
                # print(f"sample['im_file']:{sample['im_file']}")
                sample['image'] = f.read()

            if self.suffix:
                sample["tmp_file"] = sample['im_file'].rsplit(".")[0] + self.suffix

            with open(sample['tmp_file'], 'rb') as f:
                # print(f"sample['tmp_file']:{sample['tmp_file']}")
                sample['tmp_image'] = f.read()
            sample.pop('im_file')
            sample.pop('tmp_file')

        im = sample['image']
        tmp_im = sample['tmp_image']
        data = np.frombuffer(im, dtype='uint8')
        tmp_data = np.frombuffer(tmp_im, dtype='uint8')
        im = cv2.imdecode(data, 1)  # BGR mode, but need RGB mode
        tmp_im = cv2.imdecode(tmp_data, 1)

        if 'keep_ori_im' in sample and sample['keep_ori_im']:
            sample['ori_image'] = im
            sample['ori_tmp_image'] = tmp_im
        im = cv2.cvtColor(im, cv2.COLOR_BGR2RGB)
        tmp_im = cv2.cvtColor(tmp_im, cv2.COLOR_BGR2RGB)

        sample['image'] = im
        sample['tmp_image'] = tmp_im
        if 'h' not in sample:
            sample['h'] = im.shape[0]
        elif sample['h'] != im.shape[0]:
            # logger.warning(
        ...
        # print(f"sample['tmp_image'].shape:{sample['tmp_image'].shape}")
        return sample
...
@register_op
class PermutePair(BaseOperator):
    def __init__(self):
        """
        Change the channel to be (C, H, W)
        """
        super(PermutePair, self).__init__()
...
@register_op
class NormalizeImagePair(BaseOperator):
    def __init__(self,
                 mean=[0.485, 0.456, 0.406],
                 std=[0.229, 0.224, 0.225],
                 is_scale=True,
        ...
        Args:
            ...
            norm_type (str): type in ['mean_std', 'none']
        """
        super(NormalizeImagePair, self).__init__()
        self.mean = mean
        self.std = std
        self.is_scale = is_scale
        self.norm_type = norm_type
    ...
    def apply(self, sample, context=None):
        ...
        tmp_im = sample['tmp_image']
        im = im.astype(np.float32, copy=False)
        tmp_im = tmp_im.astype(np.float32, copy=False)
        if self.is_scale:
            scale = 1.0 / 255.0
            im *= scale
            tmp_im *= scale
...
@register_op
class RandomDistortPair(BaseOperator):
    """Random color distortion.
    Args:
        hue (list): hue settings. in [lower, upper, probability] format.
        saturation (list): saturation settings. in [lower, upper, probability] format.
    ...
    def __init__(self,
                 ...
                 count=4,
                 random_channel=False):
        super(RandomDistortPair, self).__init__()
        self.hue = hue
        self.saturation = saturation
        self.contrast = contrast
        self.brightness = brightness
    ...
    def apply_hue(self, img, tmp_img):
        low, high, prob = self.hue
        if np.random.uniform(0., 1.) < prob:
            return img, tmp_img
        ...
        t = np.dot(np.dot(ityiq, bt), tyiq).T
        img = np.dot(img, t)
        tmp_img = np.dot(tmp_img, t)
        return img, tmp_img

    def apply_saturation(self, img, tmp_img):
        low, high, prob = self.saturation
        if np.random.uniform(0., 1.) < prob:
            return img, tmp_img
        delta = np.random.uniform(low, high)
        ...
        tmp_gray = tmp_img * np.array([[[0.299, 0.587, 0.114]]], dtype=np.float32)
        tmp_gray = tmp_gray.sum(axis=2, keepdims=True)
        tmp_gray *= (1.0 - delta)
        tmp_img *= delta
        tmp_img += tmp_gray
        return img, tmp_img

    def apply_contrast(self, img, tmp_img):
        low, high, prob = self.contrast
        if np.random.uniform(0., 1.) < prob:
            return img, tmp_img
        delta = np.random.uniform(low, high)
    ...
    def apply_brightness(self, img, tmp_img):
        low, high, prob = self.brightness
        if np.random.uniform(0., 1.) < prob:
            return img, tmp_img
        delta = np.random.uniform(low, high)
    ...
    def apply(self, sample, context=None):
        ...
        if self.random_apply:
            ...
            distortions = np.random.permutation(functions)[:self.count]
            for func in distortions:
                img, tmp_img = func(img, tmp_img)
                # tmp_img = func(tmp_img)
            sample['image'] = img
            sample['tmp_image'] = tmp_img
            return sample

        img, tmp_img = self.apply_brightness(img, tmp_img)
        mode = np.random.randint(0, 2)

        if mode:
            img, tmp_img = self.apply_contrast(img, tmp_img)

        img, tmp_img = self.apply_saturation(img, tmp_img)
        img, tmp_img = self.apply_hue(img, tmp_img)
        ...
        if self.random_channel:
            if np.random.randint(0, 2):
                img = img[..., np.random.permutation(3)]
                tmp_img = tmp_img[..., np.random.permutation(3)]
        sample['image'] = img
        sample['tmp_image'] = tmp_img
        return sample
...
@register_op
class RandomFlipPair(BaseOperator):
    def __init__(self, prob=0.5):
        """
        Args:
            prob (float): the probability of flipping image
        """
        super(RandomFlipPair, self).__init__()
        self.prob = prob
        if not (isinstance(self.prob, float)):
            raise TypeError("{}: input type is invalid.".format(self))
    ...
    def apply_image(self, image, tmp_image):
        return image[:, ::-1, :], tmp_image[:, ::-1, :]
    ...
    def apply(self, sample, context=None):
        ...
        if np.random.uniform(0, 1) < self.prob:
            ...
            tmp_im = sample['tmp_image']
            height, width = im.shape[:2]
            im , tmp_im = self.apply_image(im, tmp_im)
            if 'gt_bbox' in sample and len(sample['gt_bbox']) > 0:
                sample['gt_bbox'] = self.apply_bbox(sample['gt_bbox'], width)
            if 'gt_poly' in sample and len(sample['gt_poly']) > 0:
                sample['gt_poly'] = self.apply_segm(sample['gt_poly'], height,
            ...
            sample['image'] = im
            sample['tmp_image'] = tmp_im
            # print(f"random_flip_im:{sample['image'].shape}")
            # print(f"random_flip_tmp_im:{sample['tmp_image'].shape}")
            # print(f"--------------")
        return sample
...
class ResizePair(BaseOperator):
    def __init__(self, target_size, keep_ratio, interp=cv2.INTER_LINEAR):
        ...
        Args:
            ...
            interp (int): the interpolation method
        """
        super(ResizePair, self).__init__()
        self.keep_ratio = keep_ratio
        self.interp = interp
        if not isinstance(target_size, (Integral, Sequence)):
            raise TypeError(
    ...
    def apply(self, sample, context=None):
        ...
        if not isinstance(im, np.ndarray):
            raise TypeError("{}: image type is not numpy.".format(self))
        if not isinstance(tmp_im, np.ndarray):
            raise TypeError("{}: tmp_image type is not numpy.".format(self))
        if len(im.shape) != 3:
            raise ImageError('{}: image is not 3-dimensional.'.format(self))
        if len(tmp_im.shape) != 3:
            raise ImageError('{}: tmp_image is not 3-dimensional.'.format(self))
        ...
        # print(f"sample['image'].shape:{sample['image'].shape}")
        # print(f"sample['tmp_image'].shape:{sample['tmp_image'].shape}")
        tmp_im = self.apply_image(sample['tmp_image'], [im_scale_x, im_scale_y])
        sample['image'] = im.astype(np.float32)
        sample['tmp_image'] = tmp_im.astype(np.float32)
        sample['im_shape'] = np.asarray([resize_h, resize_w], dtype=np.float32)
        if 'scale_factor' in sample:
            scale_factor = sample['scale_factor']
            sample['scale_factor'] = np.asarray(
...
@register_op
class RandomExpandPair(BaseOperator):
    """Random expand the canvas.
    Args:
        ratio (float): maximum expansion ratio.
        prob (float): probability to expand.
    ...
    def __init__(self, ratio=4., prob=0.5, fill_value=(127.5, 127.5, 127.5)):
        super(RandomExpandPair, self).__init__()
        assert ratio > 1.01, "expand ratio must be larger than 1.01"
        self.ratio = ratio
        self.prob = prob
        assert isinstance(fill_value, (Number, Sequence)), \
...
@register_op
class RandomCropPair(BaseOperator):
    """Random crop image and bboxes.
    Args:
        aspect_ratio (list): aspect ratio of cropped region.
            in [min, max] format.
    ...
    def __init__(self,
                 ...
                 cover_all_box=False,
                 is_mask_crop=False):
        super(RandomCropPair, self).__init__()
        self.aspect_ratio = aspect_ratio
        self.thresholds = thresholds
        self.scaling = scaling
        self.num_attempts = num_attempts
    ...
    def apply(self, sample, context=None):
        # print(f"random_expand_im:{sample['image'].shape}")
        # print(f"random_expand_tmp_im:{sample['tmp_image'].shape}")
        # print(f"--------------")
        ...
        for thresh in thresholds:
            ...
            if found:
                ...
                sample['image'] = self._crop_image(sample['image'], crop_box)
                sample['tmp_image'] = self._crop_image(sample['tmp_image'], crop_box)
                sample['gt_bbox'] = np.take(cropped_box, valid_ids, axis=0)
                sample['gt_class'] = np.take(
                    sample['gt_class'], valid_ids, axis=0)
                if 'gt_score' in sample:
                ...
                # print(f"random_crop_im:{sample['image'].shape}")
                # print(f"random_crop_tmp_im:{sample['tmp_image'].shape}")
                # print(f"------------------")
                return sample
...
class PadPair(BaseOperator):
    ...
    def apply(self, sample, context=None):
        ...
        sample['image'] = self.apply_image(im, offsets, im_size, size)
        sample['tmp_image'] = self.apply_image(tmp_im, offsets, im_size, size)
