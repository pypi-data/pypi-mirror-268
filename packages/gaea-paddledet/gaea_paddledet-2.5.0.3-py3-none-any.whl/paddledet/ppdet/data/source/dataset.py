...
class ImageFolder(DetDataset):
    ...
    def _parse_pair(self):
        ...
        pair_images = []
        for im_dir in image_dir:
            if os.path.isdir(im_dir[0]):
                im_dir = os.path.join(self.dataset_dir, im_dir[0])
                images.extend(_make_dataset(im_dir[0]))
            elif os.path.isfile(im_dir[0]) and _is_valid_file(im_dir[0]):
                images.append(im_dir[0])

            if os.path.isdir(im_dir[1]):
                im_dir = os.path.join(self.dataset_dir, im_dir[1])
                images_tmp.extend(_make_dataset(im_dir[1]))
            elif os.path.isfile(im_dir[1]) and _is_valid_file(im_dir[1]):
                images_tmp.append(im_dir[1])
        for i in range(len(images)):
            pair_images.append([images[i], images_tmp[i]])
        return pair_images
    ...
    def _load_images_pair(self):
        images = self._parse_pair()
        ct = 0
        records = []
        for image in images:
            assert image[0] != '' and os.path.isfile(image[0]), \
                    "Image {} not found".format(image[0])
            if self.sample_num > 0 and ct >= self.sample_num:
                break
            rec = {'im_id': np.array([ct]), 'im_file': image[0], 'tmp_file': image[1]}
            self._imid2path[ct] = image[0]
            ct += 1
            records.append(rec)
        assert len(records) > 0, "No image file found"
        return records
    ...
    def set_images_pair(self, images):
        self.image_dir = images
        self.roidbs = self._load_images_pair()
