...
class Trainer(object):
    ...
    def _init_callbacks(self):
        self._metrics_tracker_callback = None
        if self.mode == 'train':
            ...
            self._metrics_tracker_callback = MetricsTracker(self)
            self._compose_callback = ComposeCallback(self._callbacks)
        elif self.mode == 'eval':
            self._callbacks = [LogPrinter(self)]
            if self.cfg.metric == 'WiderFace':
                self._callbacks.append(WiferFaceEval(self))
            self._callbacks.append(MetricsTracker(self))
            self._compose_callback = ComposeCallback(self._callbacks)
    ...
    def train(self, validate=False):
        ...
        for epoch_id in range(self.start_epoch, self.cfg.epoch):
            ...
            for step_id, data in enumerate(self.loader):
                ...
                global_step = self.status['step_id'] + epoch_id * self.status['steps_per_epoch']
                if self._metrics_tracker_callback is not None \
                    and 0 == global_step % min(20, self.status['steps_per_epoch']):
                    self._metrics_tracker_callback.on_step_end(self.status)
                if self.use_ema:
                    self.ema.update()
                iter_tic = time.time()
        ...
        if self._metrics_tracker_callback is not None:
            self._metrics_tracker_callback.on_train_end(self.status)
    ...
    def _eval_with_loader(self, loader):
        ...
        if self._metrics_tracker_callback is not None:
            self.status['mode'] = 'train'
            self._metrics_tracker_callback.on_epoch_end(self.status)
            self.status['mode'] = 'eval'
    ...
    def predict(self,
        ...
        # self.dataset.set_images(images)
        if pair:
            self.dataset.set_images_pair(images)
        else:
           self.dataset.set_images(images) 
        loader = create('TestReader')(self.dataset, 0)
    ...
    def _get_infer_cfg_and_input_spec_pair(self,
                                      save_dir,
                                      prune_input=True,
                                      kl_quant=False):
        image_shape = None
        ...
        input_spec = [{
            "image": InputSpec(
                shape=image_shape, name='image'),
            "tmp_image": InputSpec(
                shape=image_shape, name='tmp_image'),
            "im_shape": InputSpec(
                shape=im_shape, name='im_shape'),
            "scale_factor": InputSpec(
                shape=scale_factor, name='scale_factor')
    ...
    def export_pair(self, save_dir='output_inference'):
        self.model.eval()
        ...
        static_model, pruned_input_spec = self._get_infer_cfg_and_input_spec_pair(
            save_dir)
    ...
    def export(self, save_dir='output_inference'):
        self.model.eval()
