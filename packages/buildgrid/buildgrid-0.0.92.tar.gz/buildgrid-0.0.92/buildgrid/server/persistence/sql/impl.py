...
from buildgrid.server.metrics_names import (
    ...
    DATA_STORE_QUEUE_TIMEOUT_NUM_METRIC_NAME,
    DATA_STORE_QUEUE_TIMEOUT_TIME_METRIC_NAME,
...
from buildgrid.server.metrics_utils import Counter, DurationMetric, publish_timer_metric
from buildgrid.server.operations.filtering import DEFAULT_SORT_KEYS, OperationFilter, SortKey
from buildgrid.server.persistence.sql.models import (
    BotEntry,
    ClientIdentityEntry,
    JobEntry,
    LeaseEntry,
...
from buildgrid.server.persistence.sql.notifier import OperationsNotifier
...
from buildgrid.utils import BrowserURL, convert_values_to_sorted_lists, create_digest, hash_from_dict
...
class DataStoreMetrics(TypedDict, total=False):
...
T = TypeVar("T", bound="SQLDataStore")
...
class SQLDataStore:
    ...
    def __init__(
        ...
        *,
        ...
        self._instance_name: str = None  # type: ignore # This should be set during initialization
    ...
    def _bot_in_instance(self) -> ClauseElement:
        return or_(BotEntry.instance_name == self._instance_name, BotEntry.instance_name.is_(None))
    ...
    def _create_operation(
        ...
        *,
        ...
        operation = OperationEntry(
            ...
            invocation_id=request_metadata.tool_invocation_id or None,
            correlated_invocations_id=request_metadata.correlated_invocations_id or None,
            tool_name=request_metadata.tool_details.tool_name or None,
            tool_version=request_metadata.tool_details.tool_version or None,
        )
    ...
    def _load_operation(self, op: OperationEntry) -> Operation:
        ...
        if execute_response:
            if self.action_browser_url:
                url = BrowserURL(self.action_browser_url, self._instance_name)
    ...
    def get_operation_request_metadata_by_name(self, operation_name: str) -> Optional[Dict[str, Any]]:
        with self._sql_ro.session(exceptions_to_not_raise_on=[Exception]) as session:
            ...
            return {
                "tool-name": operation.tool_name or "",
    ...
    def _batch_timeout_jobs(self, job_select_stmt: Select, status_code: int, message: str) -> int:
        ...
        with self._sql.session(sqlite_lock_immediately=True, exceptions_to_not_raise_on=[Exception]) as session:
            ...
            if jobs:
                ...
                stmt_timeout_jobs = (
                    update(JobEntry)
                    .where(JobEntry.name.in_(jobs), self._job_in_instance())
                    .values(
    ...
    def _do_execution_timeout(self, shutdown_requested: threading.Event) -> None:
    ...
    def list_operations(
        ...
        with self._sql_ro.session(exceptions_to_not_raise_on=[Exception]) as session:
            statement = select(OperationEntry).join(JobEntry, OperationEntry.job_name == JobEntry.name)
    ...
    def get_metrics(self) -> Optional[DataStoreMetrics]:
    ...
    def _do_queue_timeout(self, shutdown_requested: threading.Event) -> None:
        ...
        while not shutdown_requested.is_set():
            ...
            try:
                with DurationMetric(
                    DATA_STORE_QUEUE_TIMEOUT_TIME_METRIC_NAME, instance_name=self._instance_name, instanced=True
                ):
                ...
                if num_timeout > 0:
                    with Counter(
                        metric_name=DATA_STORE_QUEUE_TIMEOUT_NUM_METRIC_NAME,
                        instance_name=self._instance_name,
                    ) as num_dequeued_metric:
                        num_dequeued_metric.increment(num_timeout)
    ...
    def _do_prune(self, shutdown_requested: threading.Event) -> None:
        ...
        while not shutdown_requested.is_set():
            ...
            try:
                with DurationMetric(
                    DATA_STORE_PRUNER_DELETE_TIME_METRIC_NAME, instance_name=self._instance_name, instanced=True
                ):
                ...
                if num_rows > 0:
                    with Counter(
                        metric_name=DATA_STORE_PRUNER_NUM_ROWS_DELETED_METRIC_NAME, instance_name=self._instance_name
                    ) as num_rows_deleted:
    ...
    def _delete_jobs_prior_to(self, delete_before_datetime: datetime, limit: int) -> int:
        ...
        delete_stmt = delete(JobEntry).where(
            JobEntry.name.in_(
                ...
                .where(
                    self._job_in_instance(),
                    JobEntry.worker_completed_timestamp <= delete_before_datetime,
                )
                .limit(limit)
            ),
            self._job_in_instance(),
    ...
    def add_bot_entry(self, *, bot_session_id: str, bot_session_status: int) -> str:
        with self._sql.session() as session:
            ...
            bot_name = f"{self._instance_name}/{str(uuid.uuid4())}"
            session.add(
                BotEntry(
                    name=bot_name,
    ...
    def reap_expired_sessions(self) -> bool:
        ...
        with self._sql.session() as session:
            locate_bot_stmt = (
                select(BotEntry)
                .where(BotEntry.expiry_time < datetime.utcnow(), self._bot_in_instance())
                .order_by(BotEntry.expiry_time.desc())
                .with_for_update(skip_locked=True)
                .limit(5)
            )
            if bots := session.execute(locate_bot_stmt).scalars().all():
                for bot in bots:
                    LOGGER.warning(
                        f"BotSession name=[{bot.name}] for bot_id=[{bot.bot_id}] "
    ...
    def synchronize_bot_lease(
        ...
        log_tags = f"instance_name=[{self._instance_name}]"
    ...
    def _publish_execution_stats(
        ...
        context_metadata = {"instance-name": self._instance_name} if self._instance_name else None
        ...
        publish_timer_metric(INPUTS_FETCHING_TIME_METRIC_NAME, input_fetch_time, metadata=context_metadata)

        # Emit build execution time record:
        execution_time = execution_completed - execution_start
        publish_timer_metric(EXECUTION_TIME_METRIC_NAME, execution_time, metadata=context_metadata)

        # Emit build outputs uploading time record:
        output_upload_time = upload_completed - upload_start
        publish_timer_metric(OUTPUTS_UPLOADING_TIME_METRIC_NAME, output_upload_time, metadata=context_metadata)
        ...
        publish_timer_metric(QUEUED_TIME_METRIC_NAME, total_queued_time, metadata=context_metadata)

        # Emit total time spent in worker
        total_worker_time = worker_completed - worker_start
        publish_timer_metric(WORKER_HANDLING_TIME_METRIC_NAME, total_worker_time, metadata=context_metadata)

        # Emit total build handling time record:
        total_handling_time = worker_completed - queued
        publish_timer_metric(TOTAL_HANDLING_TIME_METRIC_NAME, total_handling_time, metadata=context_metadata)
