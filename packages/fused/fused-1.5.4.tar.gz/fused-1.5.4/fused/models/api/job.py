...
from fused._cache import get_or_cache_sample
...
def _common_validate_for_run(
    *,
    output: Optional[DatasetOutputBase],
    udf: Optional[BaseUdf],
...
def _assert_udf_has_parameters(udf: BaseUdf):
    assert hasattr(
        udf, "set_parameters"
...
class JobStepConfig(FusedBaseModel):
    ...
    _registered_subtypes: Dict[StrictStr, Type["JobStepConfig"]] = dict()
    ...
    def _resolve_type(cls, data):
        ...
        if isinstance(data, dict):
            ...
            if data_type is None:
                raise ValueError("Missing 'type' in JobStepConfig")
...
class GeospatialPartitionJobStepConfig(
    PartitionJobStepConfig, type="partition_geospatial"
):
    type = "partition_geospatial"
    ...
    partitioning_force_utm: Optional[Literal["file", "chunk"]]
...
class NonGeospatialPartitionJobStepConfig(
    PartitionJobStepConfig, type="partition_nongeospatial"
):
    type = "partition_nongeospatial"

    partition_col: Optional[StrictStr] = None


class UdfJobStepConfig(JobStepConfig, type="udf"):
...
class MapJobStepConfig(JobStepConfig, type="map"):
    type = "map"

    udf: BaseUdf
    """The UDF to run on this operation."""

    input: DatasetInputBase
    """The dataset to map over."""

    output: DatasetOutputBase
    ...
    @property
    def input_metadata(self) -> DatasetInputBase:
...
class JoinJobStepConfig(JobStepConfig, type="join"):
    type = "join"
    udf: BaseUdf
    input_left: DatasetInputBase
    input_right: DatasetInputBase
    output: DatasetOutputBase
...
class JoinSinglefileJobStepConfig(JobStepConfig, type="join_singlefile"):
    type = "join_singlefile"
    udf: BaseUdf
    input_left: DatasetInputBase
    input_right: StrictStr
    output: DatasetOutputBase
    experimental_read_once: Optional[StrictBool] = None
    experimental_read_udf: Optional[BaseUdf] = None
...
def _validate_headers_for_remote_exec(udf: BaseUdf) -> bool:
