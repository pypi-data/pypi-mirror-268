...
def crop_array(
    ...
    # crop based on the give n contour and factor_sigma
        ...
        # z axis
        zs, ze = max(0, list_indices[0][0] - ext[2]), min(
            arr.shape[0] - list_indices[0][1] + ext[2], arr.shape[0]
        )
        # y axis
        ys, ye = max(0, list_indices[2][0] - ext[1]), min(
            arr.shape[1] - list_indices[2][1] + ext[1], arr.shape[1]
        )
        # x axis
        xs, xe = max(0, list_indices[1][0] - ext[0]), min(
            arr.shape[2] - list_indices[1][1] + ext[0], arr.shape[2]
        )
