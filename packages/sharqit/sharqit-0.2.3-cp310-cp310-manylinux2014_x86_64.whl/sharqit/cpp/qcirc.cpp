...
{
  Sharqit::QCirc qc = *this;
  qc.decomp_tof();
  std::map<std::string, uint32_t> sts;
  sts["id_count"] = qc.id_count();
  sts["x_count"] = qc.x_count();
  sts["z_count"] = qc.z_count();
  sts["h_count"] = qc.h_count();
  sts["s_count"] = qc.s_count();
  sts["t_count"] = qc.t_count();
  sts["cx_count"] = qc.cx_count();
  sts["2q_count"] = qc.twoq_count();
  sts["rz_count"] = qc.rz_count();
  sts["gate_count"] = qc.gate_count();
  sts["depth"] = qc.depth();
  sts["qubit_num"] = qc.qubit_num();
  return sts;
}
...
uint32_t Sharqit::QCirc::ccx_count() const
{
  uint32_t cnt = 0;
  for (auto& qgate:qgates_) if (qgate.is_CCX_gate()) ++cnt;
  return cnt;
}

uint32_t Sharqit::QCirc::ccz_count() const
{
  uint32_t cnt = 0;
  for (auto& qgate:qgates_) if (qgate.is_CCZ_gate()) ++cnt;
  return cnt;
}
...
std::string Sharqit::QCirc::to_string(const uint32_t width) const
{
  ...
  for (auto qgate:qgates_) {
    ...
    else if ((qgate.kind() == Sharqit::QGateKind::CX) || (qgate.kind() == Sharqit::QGateKind::CZ)) {
    ...
    else if ((qgate.kind() == Sharqit::QGateKind::CCX) || (qgate.kind() == Sharqit::QGateKind::CCZ)) {
      for (auto& str:str_list) {
	if (str.size() < max_strlen) {
	  str.insert(str.size(), max_strlen - str.size(), '-');
	}
					   ...
					   const nlohmann::json& probs)
{
  ...
  for (auto it = probs.begin(); it != probs.end(); ++it) {
    ...
    if (std::get<0>(kp) == Sharqit::QGateKind::CCX && qubit_num <= 2) {
      throw std::runtime_error("not allowed CCX gate for qubit_num <= 2.");
    }
    if (std::get<0>(kp) == Sharqit::QGateKind::CCZ && qubit_num <= 2) {
      throw std::runtime_error("not allowed CCZ gate for qubit_num <= 2.");
    }
  }
  uint32_t prob_list_num = prob_list.size();
  if (prob_list[prob_list_num - 1] != 1.0) prob_list[prob_list_num - 1] = 1.0;
  ...
  for (uint32_t i = 0; i < qgate_num; ++i) {
    /* prob_idx: selected randomly from the indexes of prob_list */
    double r = uni(engine);
    uint32_t prob_idx = 0;
    ...
    /* kind, qid, phase of the gate related to the prob_idx */
    ...
    else if (kind_list[prob_idx] == Sharqit::QGateKind::CCX || kind_list[prob_idx] == Sharqit::QGateKind::CCZ) {
      /* make qubit icndexes list except the index qid[0] */
	...
	q.push_back(i);
      ...
      /* shuffle the list */
      std::shuffle(q.begin(), q.end(), engine);
      /* top two of the list as a controlled qubit and a target qubit index */
      qid.push_back(q[0]);
      qid.push_back(q[1]);
    }
...
void Sharqit::QCirc::to_dot_file(const std::string& file_name) const
{
  ...
  for (uint32_t i = 0; i < qgates_.size(); ++i) {
	...
	fontcolor = "black";
      ...
      nodes.push_back("node_" + std::to_string(cnt + offset));
      edges.push_back({lasts[qgate.qid()[2]], "node_"+ std::to_string(cnt + offset)});
      lasts[qgate.qid()[2]] = "node_"+ std::to_string(cnt + offset);
      ...
      uint32_t node_0 = cnt + offset - 2;
      uint32_t node_1 = cnt + offset - 1;
      uint32_t node_2 = cnt + offset;
      
      if (qgate.qid()[0] < qgate.qid()[1] && qgate.qid()[1] < qgate.qid()[2]) {
	edges.push_back({"node_"+ std::to_string(node_0), "node_"+ std::to_string(node_1)});
	edges.push_back({"node_"+ std::to_string(node_1), "node_"+ std::to_string(node_2)});
      }
      else if (qgate.qid()[0] < qgate.qid()[2] && qgate.qid()[2] < qgate.qid()[1]) {
	edges.push_back({"node_"+ std::to_string(node_0), "node_"+ std::to_string(node_2)});
	edges.push_back({"node_"+ std::to_string(node_2), "node_"+ std::to_string(node_1)});
      }
      else if (qgate.qid()[1] < qgate.qid()[0] && qgate.qid()[0] < qgate.qid()[2]) {
	edges.push_back({"node_"+ std::to_string(node_1), "node_"+ std::to_string(node_0)});
	edges.push_back({"node_"+ std::to_string(node_0), "node_"+ std::to_string(node_2)});
      }
      else if (qgate.qid()[1] < qgate.qid()[2] && qgate.qid()[2] < qgate.qid()[0]) {
	edges.push_back({"node_"+ std::to_string(node_1), "node_"+ std::to_string(node_2)});
	edges.push_back({"node_"+ std::to_string(node_2), "node_"+ std::to_string(node_0)});
      }
      else if (qgate.qid()[2] < qgate.qid()[0] && qgate.qid()[0] < qgate.qid()[1]) {
	edges.push_back({"node_"+ std::to_string(node_2), "node_"+ std::to_string(node_0)});
	edges.push_back({"node_"+ std::to_string(node_0), "node_"+ std::to_string(node_1)});
      }
      else if (qgate.qid()[2] < qgate.qid()[1] && qgate.qid()[1] < qgate.qid()[0]) {
	edges.push_back({"node_"+ std::to_string(node_2), "node_"+ std::to_string(node_1)});
	edges.push_back({"node_"+ std::to_string(node_1), "node_"+ std::to_string(node_0)});
      }
      same_ranks.push_back({"node_"+ std::to_string(node_0), "node_"+ std::to_string(node_1)});
      same_ranks.push_back({"node_"+ std::to_string(node_1), "node_"+ std::to_string(node_2)});
    }
