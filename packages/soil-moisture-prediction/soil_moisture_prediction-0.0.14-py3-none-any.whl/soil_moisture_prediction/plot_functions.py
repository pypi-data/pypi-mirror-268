...
def draw_error_band_path(x, y, error):
    ...
    Except the first point which uses a forward difference and the last point which uses
...
def mc_iterations_histogram(rfo_model, time_step, point_x, point_y):
    ...
    point_x : point coordinate on the x axis
    point_y : point coordinate on the y axis
    """
    plt.figure()
    plt.hist(rfo_model[:, time_step, point_x, point_y], bins=30, range=(0.15, 0.6))
