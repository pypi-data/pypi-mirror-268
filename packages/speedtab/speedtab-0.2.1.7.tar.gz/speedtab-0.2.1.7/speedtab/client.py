...
class SpreadSheet:
    ...
    def __init__(self, spreadsheet_id, token_path, credentials, connect_v4, connect_v3, user):
...
class Client:
    ...
    def copy_spreadsheet(self,
        ...
        ss = SpreadSheet(self.connect_v3.files().copy(fileId=spreadsheet, body={'name': new_title},
    ...
    def create_spreadsheet(self, title, sheets: list = None, hide_grid_lines: Union[list, bool] = False):
        ...
        ss = SpreadSheet(self.connect_v4.spreadsheets()
    ...
    def delete_file(self, file_id: Union[str, SpreadSheet]):
        ...
        self.connect_v3.files().delete(fileId=file_id, supportsAllDrives=True).execute()
    ...
    def get_spreadsheet(self, spreadsheet_id: str):
        ...
        The id will be: ABCDEFGHIJKLMNOPQRSTUVWXYZ123456789abcdefghi
        """
        ss = SpreadSheet(spreadsheet_id=parse_url(spreadsheet_id), token_path=self.token_path,
                         credentials=self.credentials, connect_v4=self.connect_v4,
                         connect_v3=self.connect_v3, user=self)
        self.list_of_spreadsheets.append(ss)
        return ss
    ...
    def move_to_folder(self, file_id: Union[str, SpreadSheet], folder_id: str):
        ...
        previous_parents = ','.join(
            self.connect_v3.files().get(fileId=file_id, fields='parents').execute().get('parents'))
        self.connect_v3.files().update(fileId=file_id, addParents=parse_url(folder_id),
    ...
    def move_to_trash(self, file_id: Union[str, SpreadSheet], trashed: bool = True):
        ...
        self.connect_v3.files().update(fileId=file_id, body={'trashed': trashed}, supportsAllDrives=True).execute()
