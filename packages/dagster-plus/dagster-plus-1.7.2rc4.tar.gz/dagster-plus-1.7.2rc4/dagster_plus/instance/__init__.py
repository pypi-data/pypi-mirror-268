...
from dagster_plus_cli.core.graphql_client import (
    create_agent_graphql_client,
    create_agent_http_client,
    create_graphql_requests_session,
    get_agent_headers,
)
from dagster_plus_cli.core.headers.auth import DagsterCloudInstanceScope
from requests import Session
from requests.adapters import HTTPAdapter
from urllib3 import Retry

from dagster_plus.agent import AgentQueuesConfig
...
if TYPE_CHECKING:
    from dagster_plus.workspace.user_code_launcher.user_code_launcher import (
...
class DagsterCloudAgentInstance(DagsterCloudInstance):
    ...
    def user_code_launcher(self) -> "DagsterCloudUserCodeLauncher":
        ...
        if not self._user_code_launcher:
            module_elems = self._user_code_launcher_data.module_name.split(".")  # type: ignore  # (possible none)
            # When attempting to load the user code launcher, attempt to load from dagster_plus
            ...
            if (
                ...
                try:
                    dagster_plus_user_code_launcher_data = ConfigurableClassData(
                        module_name=".".join(["dagster_plus", *module_elems[1:]]),
                        class_name=self._user_code_launcher_data.class_name,  # type: ignore  # (possible none)
                        config_yaml=self._user_code_launcher_data.config_yaml,  # type: ignore  # (possible none)
                    )
                    self._user_code_launcher = self._exit_stack.enter_context(
                        dagster_plus_user_code_launcher_data.rehydrate(
            ...
            # If the user code launcher wasn't loadable from dagster_plus,
...
"""
    ...
    def config_defaults(base_dir):
        ...
        # Use dagster_cloud module names for backcompat with older agent / user code versions.
        # Relies on OSS logic (https://github.com/dagster-io/dagster/pull/21036) to retry
        # with dagster_plus module names if the dagster_cloud module names fail.
...
def _cached_inject_deployment(
    ...
    return ConfigurableClassData(
        custom_instance_class_data.module_name,
