...
def connect(host=None,
    ...
    """
    业务Pymysql连接通用参数封装
    :return pymysql.Connection or dbutils.pooled_db.PooledDB
    """
    if pool:
...
class _PyMysqlTransactionCursor:
    """
    pymysql 事务 cursor 拓展
    仅执行 SQL 不提交不回滚, 由 PyMysqlHelper 处理提交回滚连接
    """
    ...
    def execute(self,
        ...
        :param sql: 执行的sql
        :param args: 执行的sql中的参数
        :param modelclass: pydantic model 类型, 传入该值会将结果转为 pydantic model
        """
