...
class FeishuRobotPostMsgContentBuilder:
    ...
    def add_text(self, text: str, un_escape: Optional[bool] = None):
        ...
        添加一个text文本标签
        :param text: 文本内容
        :param un_escape: 表示是否 unescape 解码。默认值为 false，未用到 unescape 时可以不填
        """
    ...
    def add_a(self, text: str, href: str):
        ...
        添加一个a超链接标签
        :param text: 超链接的文本内容
        :param href: 默认的链接地址，你需要确保链接地址的合法性，否则消息会发送失败
        """
        self._data[self._index].append(
...
class FeishuRobotCardMsgTemplate(BaseModel):
    ...
    def gen_msg(self, elements_map: Union[Dict[str, Any], pydantic.BaseModel, None] = None,
        ...
        """
        通过模板生成消息配置
        :param elements_map: 卡片消息模板名称与值的映射
        :param header_map: 卡片消息标题模板名称与值的映射
        """
...
class FeishuRobotNotice:
    ...
    def __init__(self, webhook: str, secret: Optional[str] = None):
        """
        :param webhook: 飞书webhook地址
        :param secret: 签名校验秘钥, 非必填
        """
    ...
    def send_text_msg(self, text: str, timeout: Union[float, tuple] = None):
        ...
        发送飞书机器人纯文本消息
        :param text: 文本消息
        :param timeout: 可选, 发送消息超时时间, 支持float或者tuple2 (connection timeout, read timeout)
        """
    ...
    def send_post_msg(self,
        ...
        发送飞书机器人富文本消息
        :param title: 富文本标题
        :param content_builder: 富文本内容建造器, 通过 FeishuRobotPostMsg.content_builder() 可获取Builder实例
        :param timeout: 可选, 发送消息超时时间, 支持float或者tuple2 (connection timeout, read timeout)
        """
        content = []
        if content_builder:
