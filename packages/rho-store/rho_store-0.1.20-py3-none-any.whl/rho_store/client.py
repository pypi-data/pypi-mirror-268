...
class RhoClient:
    def __init__(self, api_key: str):
        ...
        self._api_port = RhoApiGraphqlAdapter(base_url=self._config.GRAPHQL_URL, api_key=api_key)
        self._file_upload_port = UploadFileHttpAdapter()
        self.data_transport_port = DataTransportRestAdapter(base_url=self._config.API_URL, api_key=api_key)
    ...
    def store_df(
        ...
        table = self._api_port.process_file(file_id, table_id, strategy, run_async=run_async)
    ...
    def get_table(self, table_id: str, version: Optional[int] = None) -> Table:
        ...
        return Table(table_id=table_id, workspace_id=table_data.workspace_id, data=df)
    ...
    def _get_table_data(self, table_id: str, version: Optional[int] = None) -> TableDataResult:
        result = self.data_transport_port.get_table_data(table_id, version)
        return result
