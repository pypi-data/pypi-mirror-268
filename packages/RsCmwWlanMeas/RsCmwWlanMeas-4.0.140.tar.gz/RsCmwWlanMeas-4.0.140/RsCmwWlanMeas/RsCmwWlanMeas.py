...
class RsCmwWlanMeas:
	...
	_driver_options = "SupportedInstrModels = CMW500/CMW100/CMW270/CMW280/CMP, SupportedIdnPatterns = CMW, SimulationIdnString = 'Rohde&Schwarz,CMW500,100001,4.0.140.0060'"
	...
	def __init__(self, resource_name: str, id_query: bool = True, reset: bool = False, options: str = None, direct_session: object = None):
		...
		self._core = Core(resource_name, id_query, reset, RsCmwWlanMeas._driver_options, options, direct_session)
		self._core.driver_version = '4.0.140.0060'
	...
	@classmethod
	def from_existing_session(cls, session: object, options: str = None) -> 'RsCmwWlanMeas':
	...
	def __str__(self) -> str:
		if self._core.io:
			return f"RsCmwWlanMeas session '{self._core.io.resource_name}'"
		else:
			return f"RsCmwWlanMeas with session closed"
	...
	def assert_minimum_version(min_version: str) -> None:
		...
		curr_version_list = '4.0.140.0060'.split('.')
		...
		for i in range(count):
			...
			if curr > minimum:
				break
			if curr < minimum:
				raise RsInstrException(f"Assertion for minimum RsCmwWlanMeas version failed. Current version: '4.0.140.0060', minimum required version: '{min_version}'")
	...
	def _sync_to_custom_properties(self, cloned: 'RsCmwWlanMeas') -> None:
	...
	@property
	def route(self):
		"""route commands group. 2 Sub-classes, 2 commands."""
	...
	@property
	def configure(self):
		"""configure commands group. 6 Sub-classes, 1 commands."""
	...
	def tmode(self):
		...
		if not hasattr(self, '_tmode'):
			from .Implementations.Tmode import TmodeCls
	...
	def clone(self) -> 'RsCmwWlanMeas':
		"""Creates a deep copy of the RsCmwWlanMeas object. Also copies:
		...
		cloned = RsCmwWlanMeas.from_existing_session(self.get_session_handle(), self._options)
