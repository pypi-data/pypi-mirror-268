...
class BCls:
	...
	def set(self, tsm_lim_yrel_lev_b: float, bandwidthB=repcap.BandwidthB.Bw5) -> None:
		...
		Snippet: driver.configure.multiEval.limit.tsMask.pofdm.bw.cb.y.b.set(tsm_lim_yrel_lev_b = 1.0, bandwidthB = repcap.BandwidthB.Bw5) \n
		Defines the Y-value of point B (Δf = 1.5 <bandwidth>) on the 802.11p spectrum mask for power class B and the specified
	...
	def get(self, bandwidthB=repcap.BandwidthB.Bw5) -> float:
		"""SCPI: CONFigure:WLAN:MEASurement<Instance>:MEValuation:LIMit:TSMask:POFDm:BW<bandwidth>:CB:Y:B \n
		Snippet: value: float = driver.configure.multiEval.limit.tsMask.pofdm.bw.cb.y.b.get(bandwidthB = repcap.BandwidthB.Bw5) \n
		Defines the Y-value of point B (Δf = 1.5 <bandwidth>) on the 802.11p spectrum mask for power class B and the specified
		<bandwidth>. For background information, see 'Transmit spectrum mask OFDM, by regulation'. \n
			:param bandwidthB: optional repeated capability selector. Default value: Bw5
