...
class CurrentCls:
	...
	def read(self, mimo=repcap.Mimo.Default) -> ResultData:
		...
		Snippet: value: ResultData = driver.multiEval.tsMask.mimo.segments.frequency.current.read(mimo = repcap.Mimo.Default) \n
	...
	def fetch(self, mimo=repcap.Mimo.Default) -> ResultData:
		...
		Snippet: value: ResultData = driver.multiEval.tsMask.mimo.segments.frequency.current.fetch(mimo = repcap.Mimo.Default) \n
		Return the X-positions of the limit line margins of the transmit spectrum mask, for MIMO measurements, antenna number <n>,
		bandwidths with two segments (80+80) . Positions for the current, average, minimum and maximum traces are returned. The
		values described below are returned by FETCh and READ commands. CALCulate commands return limit check results instead,
		one value for each result listed below. \n
	...
	def calculate(self, mimo=repcap.Mimo.Default) -> CalculateStruct:
		...
		Snippet: value: CalculateStruct = driver.multiEval.tsMask.mimo.segments.frequency.current.calculate(mimo = repcap.Mimo.Default) \n
		Return the X-positions of the limit line margins of the transmit spectrum mask, for MIMO measurements, antenna number <n>,
		bandwidths with two segments (80+80) . Positions for the current, average, minimum and maximum traces are returned. The
		values described below are returned by FETCh and READ commands. CALCulate commands return limit check results instead,
		one value for each result listed below. \n
