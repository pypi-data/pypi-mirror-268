...
class AverageCls:
	...
	def read(self) -> ResultData:
		...
		Snippet: value: ResultData = driver.multiEval.tsMask.frequency.average.read() \n
		...
		return self._core.io.query_struct(f'READ:WLAN:MEASurement<Instance>:MEValuation:TSMask:FREQuency:AVERage?', self.__class__.ResultData())

	def fetch(self) -> ResultData:
		"""SCPI: FETCh:WLAN:MEASurement<Instance>:MEValuation:TSMask:FREQuency:AVERage \n
		Snippet: value: ResultData = driver.multiEval.tsMask.frequency.average.fetch() \n
		Return the X-positions of the limit line margins of the transmit spectrum mask. For MIMO measurements, antenna/stream
		number <n>, bandwidths with one segment. Positions for the current, average, minimum and maximum traces are returned. The
		values described below are returned by FETCh and READ commands. CALCulate commands return limit check results instead,
		one value for each result listed below. \n
			:return: structure: for return value, see the help for ResultData structure arguments."""
		return self._core.io.query_struct(f'FETCh:WLAN:MEASurement<Instance>:MEValuation:TSMask:FREQuency:AVERage?', self.__class__.ResultData())
	...
	def calculate(self) -> CalculateStruct:
		"""SCPI: CALCulate:WLAN:MEASurement<Instance>:MEValuation:TSMask:FREQuency:AVERage \n
		Snippet: value: CalculateStruct = driver.multiEval.tsMask.frequency.average.calculate() \n
		Return the X-positions of the limit line margins of the transmit spectrum mask. For MIMO measurements, antenna/stream
		number <n>, bandwidths with one segment. Positions for the current, average, minimum and maximum traces are returned. The
		values described below are returned by FETCh and READ commands. CALCulate commands return limit check results instead,
		one value for each result listed below. \n
			:return: structure: for return value, see the help for CalculateStruct structure arguments."""
		return self._core.io.query_struct(f'CALCulate:WLAN:MEASurement<Instance>:MEValuation:TSMask:FREQuency:AVERage?', self.__class__.CalculateStruct())
