...
class StandardDevCls:
	...
	def fetch(self) -> List[float]:
		"""SCPI: FETCh:WLAN:MEASurement<Instance>:MEValuation:LIST:MODulation:DSSS:EVMPeak:SDEViation \n
		Snippet: value: List[float] = driver.multiEval.listPy.modulation.dsss.evmPeak.standardDev.fetch() \n
		...
		Use RsCmwWlanMeas.reliability.last_value to read the updated reliability indicator. \n
			:return: evm_peak: No help available"""
		suppressed = ArgSingleSuppressed(0, DataType.Integer, False, 1, 'Reliability')
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'FETCh:WLAN:MEASurement<Instance>:MEValuation:LIST:MODulation:DSSS:EVMPeak:SDEViation?', suppressed)
		return response
