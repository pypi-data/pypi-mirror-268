...
class MaximumCls:
	...
	def fetch(self, resourceUnit=repcap.ResourceUnit.Default) -> List[float]:
		...
		Snippet: value: List[float] = driver.multiEval.power.runit.maximum.fetch(resourceUnit = repcap.ResourceUnit.Default) \n
		...
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'FETCh:WLAN:MEASurement<Instance>:MEValuation:POWer:RUNit{resourceUnit_cmd_val}:MAXimum?', suppressed)
		return response
