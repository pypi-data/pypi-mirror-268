...
class MaximumCls:
	...
	def read(self, start: float = None, count: float = None, decimation: float = None) -> List[float]:
		...
		Snippet: value: List[float] = driver.multiEval.trace.evMagnitude.symbol.maximum.read(start = 1.0, count = 1.0, decimation = 1.0) \n
	...
	def fetch(self, start: float = None, count: float = None, decimation: float = None) -> List[float]:
		...
		Snippet: value: List[float] = driver.multiEval.trace.evMagnitude.symbol.maximum.fetch(start = 1.0, count = 1.0, decimation = 1.0) \n
		Return the values of the EVM vs symbol traces for OFDM signals according to standard 802.11a, g, n, ac, ax, or p.
		The results of the current, average and maximum traces can be retrieved. \n
		Use RsCmwWlanMeas.reliability.last_value to read the updated reliability indicator. \n
			:param start: numeric For the optional query parameters start, count and decimation, see 'Trace sub-arrays'.
