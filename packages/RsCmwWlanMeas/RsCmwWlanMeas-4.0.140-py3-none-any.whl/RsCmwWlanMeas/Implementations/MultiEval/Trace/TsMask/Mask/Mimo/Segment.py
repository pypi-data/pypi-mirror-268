...
class SegmentCls:
	...
	def read(self, start: float = None, count: float = None, decimation: float = None, mimo=repcap.Mimo.Default, segment=repcap.Segment.Default) -> List[float]:
		"""SCPI: READ:WLAN:MEASurement<Instance>:MEValuation:TRACe:TSMask:MASK:MIMO<n>:SEGMent<seg> \n
		Snippet: value: List[float] = driver.multiEval.trace.tsMask.mask.mimo.segment.read(start = 1.0, count = 1.0, decimation = 1.0, mimo = repcap.Mimo.Default, segment = repcap.Segment.Default) \n
		Return the power values (Y-values) of the transmit spectrum mask limit line trace for segment <seg> and antenna <n>, for
		MIMO measurements and bandwidths with two segments. \n
		...
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'READ:WLAN:MEASurement<Instance>:MEValuation:TRACe:TSMask:MASK:MIMO{mimo_cmd_val}:SEGMent{segment_cmd_val}? {param}'.rstrip(), suppressed)
		return response
	...
	def fetch(self, start: float = None, count: float = None, decimation: float = None, mimo=repcap.Mimo.Default, segment=repcap.Segment.Default) -> List[float]:
		"""SCPI: FETCh:WLAN:MEASurement<Instance>:MEValuation:TRACe:TSMask:MASK:MIMO<n>:SEGMent<seg> \n
		Snippet: value: List[float] = driver.multiEval.trace.tsMask.mask.mimo.segment.fetch(start = 1.0, count = 1.0, decimation = 1.0, mimo = repcap.Mimo.Default, segment = repcap.Segment.Default) \n
		Return the power values (Y-values) of the transmit spectrum mask limit line trace for segment <seg> and antenna <n>, for
		MIMO measurements and bandwidths with two segments. \n
		Use RsCmwWlanMeas.reliability.last_value to read the updated reliability indicator. \n
			:param start: numeric For the optional query parameters start, count and decimation, see 'Trace sub-arrays'.
		...
		segment_cmd_val = self._cmd_group.get_repcap_cmd_value(segment, repcap.Segment)
		suppressed = ArgSingleSuppressed(0, DataType.Integer, False, 1, 'Reliability')
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'FETCh:WLAN:MEASurement<Instance>:MEValuation:TRACe:TSMask:MASK:MIMO{mimo_cmd_val}:SEGMent{segment_cmd_val}? {param}'.rstrip(), suppressed)
		return response
