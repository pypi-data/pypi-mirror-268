...
class TimeCls:
	...
	def read(self, start: float = None, count: float = None, decimation: float = None) -> List[float]:
		...
		Snippet: value: List[float] = driver.multiEval.trace.powerVsTime.fallingEdge.time.read(start = 1.0, count = 1.0, decimation = 1.0) \n
		Return the time indices corresponding to the power vs time ramp traces, falling edge (FEDGe) and rising edge (REDGe) .
		Refer to method RsCmwWlanMeas.MultiEval.Trace.PowerVsTime.FallingEdge.Current.fetch etc. \n
	...
	def fetch(self, start: float = None, count: float = None, decimation: float = None) -> List[float]:
		...
		Snippet: value: List[float] = driver.multiEval.trace.powerVsTime.fallingEdge.time.fetch(start = 1.0, count = 1.0, decimation = 1.0) \n
		Return the time indices corresponding to the power vs time ramp traces, falling edge (FEDGe) and rising edge (REDGe) .
		Refer to method RsCmwWlanMeas.MultiEval.Trace.PowerVsTime.FallingEdge.Current.fetch etc. \n
		Use RsCmwWlanMeas.reliability.last_value to read the updated reliability indicator. \n
			:param start: numeric For the optional query parameters start, count and decimation, see 'Trace sub-arrays'.
