...
class MinimumCls:
	...
	def fetch(self, segmentB=repcap.SegmentB.Default) -> FetchStruct:
		"""SCPI: FETCh:WLAN:MEASurement<Instance>:MEValuation:LIST:SEGMent<segment>:TSMask:MINimum \n
		Snippet: value: FetchStruct = driver.multiEval.listPy.segment.tsMask.minimum.fetch(segmentB = repcap.SegmentB.Default) \n
		...
		return self._core.io.query_struct(f'FETCh:WLAN:MEASurement<Instance>:MEValuation:LIST:SEGMent{segmentB_cmd_val}:TSMask:MINimum?', self.__class__.FetchStruct())
