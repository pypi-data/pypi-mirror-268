...
class BwCls:
	...
	def set(self, offset_value_rel: float or bool, offset_value_abs: float or bool = None, bandwidthE=repcap.BandwidthE.Default) -> None:
		...
		Snippet: driver.configure.multiEval.limit.modulation.vhtOfdm.iqOffset.bw.set(offset_value_rel = 1.0, offset_value_abs = 1.0, bandwidthE = repcap.BandwidthE.Default) \n
		Defines and activates upper limits for the I/Q origin offset, for 802.11ac and channel bandwidth <BW>. \n
			...
			:param offset_value_abs: (float or boolean) numeric | ON | OFF Absolute limit, only for BW=8080 Range: -100 dBm to 0 dBm, Unit: dBm Additional parameters: OFF | ON (disables | enables the limit check)
		...
		self._core.io.write(f'CONFigure:WLAN:MEASurement<Instance>:MEValuation:LIMit:MODulation:VHTofdm:IQOFfset:BW{bandwidthE_cmd_val} {param}'.rstrip())
	...
	def get(self, bandwidthE=repcap.BandwidthE.Default) -> BwStruct:
		"""SCPI: CONFigure:WLAN:MEASurement<Instance>:MEValuation:LIMit:MODulation:VHTofdm:IQOFfset:BW<BW> \n
		Snippet: value: BwStruct = driver.configure.multiEval.limit.modulation.vhtOfdm.iqOffset.bw.get(bandwidthE = repcap.BandwidthE.Default) \n
		Defines and activates upper limits for the I/Q origin offset, for 802.11ac and channel bandwidth <BW>. \n
			:param bandwidthE: optional repeated capability selector. Default value: Bw5 (settable in the interface 'Bw')
			:return: structure: for return value, see the help for BwStruct structure arguments."""
		bandwidthE_cmd_val = self._cmd_group.get_repcap_cmd_value(bandwidthE, repcap.BandwidthE)
		return self._core.io.query_struct(f'CONFigure:WLAN:MEASurement<Instance>:MEValuation:LIMit:MODulation:VHTofdm:IQOFfset:BW{bandwidthE_cmd_val}?', self.__class__.BwStruct())
