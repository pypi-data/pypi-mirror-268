...
2024-03-28新接口文档：
https://doc.weixin.qq.com/doc/w3_AIgARAadAMMV2g61jwjSba2zEekCJ?scode=AFoA2wfSABEfFC3J8oAGgAigZTABE
"""
...
class Basics:
    ...
    def get_app_list(
            self,
            start_time: int = 1546272000,  # 2019-01-01 00:00:00
            end_time: int = None,
            page: int = 1,
            coop_type: int = None
    ):
        ...
        url = 'https://fxapi.yuewen.com/wechat/GetAppList'
        data = {
            'email': self.email,
    ...
    def query_charge_log(
        ...
        :param end_time: 查询结束时间戳
        :param page: 分页，默认为 1
        :param app_flag: 产品标识（可从后台>>合作产品管理 >> 业务管理获取），每次请求仅能传一个。
        ...
        if int(coop_type) == 1:  # 微信分销
            url = 'https://fxapi.yuewen.com/wechat/Recharge/GetRechargeLog'
        elif int(coop_type) == 9:  # 陌香快应用（共享包）
            url = 'https://fxapi.yuewen.com/moxiang/Recharge/GetRechargeLog'
        elif int(coop_type) == 11:  # 快应用（独立包）
            url = 'https://fxapi.yuewen.com/quickapp/Recharge/GetRechargeLog'
        else:
            return
...
def query_charge_log(
    ...
    :param end_time: 查询结束时间戳
    :param page: 分页，默认为 1
    :param app_flag: 产品标识（可从后台获取），不传时查询商户帐号下有权限的所有产品订单；一次传1个
    :param openid: 用户 ID，传此字段时必传对应的 appflags
    :param guid: 阅文用户 ID，传此字段时必传对应的 appflags
    :param order_id: 微信或支付宝订单号
    :param order_status: 订单状态：1 待支付、2 已支付
