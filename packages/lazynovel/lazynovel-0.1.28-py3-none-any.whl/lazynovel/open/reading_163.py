...
"""
文鼎分销API
文档：https://docs.qq.com/doc/DVHJFSnVFaFBVT21h
...
class MpWeiXin:
    ...
    def make_sign(
        ...
        if not data:
            ...
            pass
        data['timestamp'] = int(time.time() * 1000)  # 增加必填字段
        data['consumerkey'] = self.consumer_key  # 增加必填字段
        temp_data = copy.deepcopy(data)
        temp_data['secretkey'] = self.secret_key
        sorted_keys = sorted(temp_data.keys())
        key_values = ''
        for key in sorted_keys:
            key_value = '%s=%s' % (key, temp_data[key])
            key_values += key_value
        data['sign'] = copy.deepcopy(lazymd5.md5_str(content=key_values, encoding='UTF-8'))  # 增加sign
    ...
    def recharge_list(
        ...
        if pay_status is not None:
            request_data['paystatus'] = pay_status

        return lazyrequests.lazy_requests(
            method='GET',
            url='https://bi.reading.163.com/dist-api/rechargeList',
            params=self.make_sign(data=request_data),
            return_json=True
        )
    ...
    def get_user_data(
        ...
        if user_openid:
            request_data['useropenid'] = user_openid
...
class QuickApp:
    ...
    def make_sign(
        ...
        if not data:
            ...
            pass
        data['timestamp'] = int(time.time() * 1000)  # 增加必填字段
        data['appkey'] = self.app_key  # 增加必填字段
        temp_data = copy.deepcopy(data)
        temp_data['appsecret'] = self.app_secret
        sorted_keys = sorted(temp_data.keys())
        key_values = ''
        for key in sorted_keys:
            key_value = '%s=%s' % (key, temp_data[key])
            key_values += key_value
        data['sign'] = lazymd5.md5_str(content=key_values, encoding='UTF-8')  # 增加sign
    ...
    def recharge_list(
        ...
        return lazyrequests.lazy_requests(
            ...
            params=self.make_sign(data=local_data),
            return_json=True
        )
