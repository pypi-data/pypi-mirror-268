...
def groupby_shift(table, groupkeys, selector, with_selector, periods):
    return _build_op(
        Op_groupby_shift,
        [TableType],
        [
...
def take(table, indices, axis, ignore_index):
    op = Op_take_rows if axis == 0 else Op_take_cols
    return _build_op(
        op,
        [TableType],
        [table, indices, create_i1_attr("ignore_index", ignore_index)],
        chaining=True,
    ).outs[0]
