...
class AlignmentPopulator(object):
    ...
    def _merge_aligns(self, pas: List[PathAlignment]) -> List[PathAlignment]:
        """Merges nodes in ``pas`` with the same triples in to alignments with
        multiple indices.
    ...
    def __call__(self) -> Tuple[PathAlignment, ...]:
        """Add the alignments to the graph using the ISI formatted alignemnt
        string.
    ...
    def get_missing_alignments(self) -> Tuple[PathAlignment, ...]:
        """Find all path alignments not in the graph.  This is done by matching
        against the epi mapping.  This is only useful for testing.
