...
class Opinum(object):
    ...
    @token_required
    def get_tss(self, ssvs, interval=None):
        if isinstance(ssvs[0], dict):
            return {
                tuple(ssv.values()): self.get_ts(**ssv, interval=interval) 
                for ssv in ssvs
            }
        return {
            tuple(ssv): self.get_ts(*ssv, interval=interval) 
            for ssv in ssvs
        }
