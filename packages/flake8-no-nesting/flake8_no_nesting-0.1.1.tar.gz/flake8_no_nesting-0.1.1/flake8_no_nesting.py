from ast import AST, NodeVisitor, For, If, While, With, Try
...
class Visitor(NodeVisitor):
    ...
    def find_nested_nodes(self, node):
        ...
        with_nodes: List[With] = self.find_child_nodes(node=node, node_type_obj=With)
        try_nodes: List[Try] = self.find_child_nodes(node=node, node_type_obj=Try)
        ...
        self.append_error(with_nodes, 'FNN103 nested with found')
        self.append_error(try_nodes, 'FNN104 nested try found')
