    ...
    return new Promise(((resolve, reject) => {
        axios.post('', params).then(res => {
            if (res.status != 200) {
                ...
                if (res.data.type && res.data.type == 'redirect') {
...
window.init = function () {
    window.app = new Vue({
        ...
        computed: {
            ...
            topDynamicContent() {
                if (this.dynamicDisplay && this.dynamicDisplay.top) {
                    return this.dynamicDisplay.top;
                }

                return null;
            },
            bottomDynamicContent() {
                if (this.dynamicDisplay && this.dynamicDisplay.bottom) {
                    return this.dynamicDisplay.bottom;
                }
        ...
        methods: {
            ...
            onSubmit() {
                ...
                post(requestData, self).then(function (res) {
                    ...
                    self.dynamicDisplay = res.dynamic_display;
        ...
        mounted: function () {
              //校验完整性
            if (typeof (showManifest) === "undefined") {
                document.write("程序不完整，请重新下载安装！")
                alert("程序不完整，请重新下载安装！")
                return
            }
            //调用sdk
...
function onCheck(res) {
    console.log(res.o.a)
    if (res.o.a.code === "error") {
        alert("请检查您的网络连接，无法联网可能会影响使用体验！")
    }
}