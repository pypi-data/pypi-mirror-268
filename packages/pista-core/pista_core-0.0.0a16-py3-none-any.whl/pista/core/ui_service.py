...
from pista.core._root_manager import PROJECT_ROOT_DIR, SCREENSHOT_DIR
...
class UIService:
    ...
    _ALL_DRIVERS = WebBrowser.ALL_DRIVERS
    MAX_WAITTIME_FOR_PAGELOAD = int(PISTA_CONFIG.get('ui', 'pageload_waittime_in_sec'))
    MAX_WAITTIME_FOR_ELEMENT = int(PISTA_CONFIG.get('ui', 'element_waittime_in_sec'))
    ...
    def __init__(self, driver: _ALL_DRIVERS = None, page_title_xpath: str = None, is_for_screenshot: bool = False):
        if is_for_screenshot:
            ...
            if driver is None:
                _browser = WebBrowser.TEST_BROWSER
                self.driver = WebBrowser.open_browser(_browser)
            else:
                self.driver = driver
    ...
    def wait_for_elementload(self, xpath):
        ...
        for i in range(0, self.ITERATIONS_FOR_ELEMENT):
            try:
                webelements = self.get_webelements(xpath)
    ...
    def wait_for_elementvisible(self, xpath):
        if type(xpath) is tuple:
            xpathKey, xpathVal = xpath[0], xpath[1]
            self.elementwait.until(EC.visibility_of_element_located((xpathKey, xpathVal)))
        else:
    ...
    def get_webelements(self, xpath) -> list[WebElement]:
        ...
        if type(xpath) is tuple:
            xpathKey, xpathVal = xpath[0], xpath[1]
            webelements = self.driver.find_elements(xpathKey, xpathVal)
        else:
    ...
    def get_webelement(self, xpath) -> WebElement:
        ...
        for i in range(0, self.ITERATIONS_FOR_ELEMENT):
            try:
                if type(xpath) is tuple:
                    xpathKey, xpathVal = xpath[0], xpath[1]
                    webelement = self.driver.find_element(xpathKey, xpathVal)
                else:
                    ...
                    if webelement is not None:
                        for j in range(0, self.ITERATIONS_FOR_ELEMENT):
                            ...
                            if webelement.is_displayed():
                                if type(xpath) is tuple:
                                    xpathKey, xpathVal = xpath[0], xpath[1]
                                    self.elementwait.until(EC.element_to_be_clickable((xpathKey, xpathVal)))
                                else:
    ...
    def zoom_screen(self, zoomPercent: int):
        self.driver.execute_script(f"document.body.style.zoom='{zoomPercent}%'")
        self.wait_for(2)
        
    def capture_screen(self, refFileName: str = ''):
        SCREEN_FILEPATH = os.path.join(SCREENSHOT_DIR, 'screen_{}_{}_{}.png')
        thread_id = threading.current_thread().native_id
        file_suffix = Commons.build_date_for_filename()

        screenshot_file = SCREEN_FILEPATH.format(refFileName, thread_id, file_suffix)
        self.logger.info('Screenshot: ' + screenshot_file.replace(PROJECT_ROOT_DIR, ''))
        try:
