...
from pista.core.data_service import DataGeneric
...
class JsonService:
    ...
    @staticmethod
    def get_json_from_excel(xlfilepath: str, sheet: str, column: str, isChildJson:bool=None):
        worksheet = ExcelUtil.read_excel_sheet(xlfilepath, sheet)
            ...
            for pp, cpp in zip(parentPath_colIdList_dict, calc_parentPath_colIdList_dict):
                if 'json(' in str(jsonDict):
                    ...
                    for c in reqCol_list:
                        temp_jsonDict, temp_jsonStr = JsonService.get_json_from_excel(xlfilepath, colSheet, c, isChildJson=True)
    ...
    def _json_replace_with_rowsList(jsonDict, xlfilepath: str):
        if 'rows(' not in str(jsonDict):
            ...
            for pp, cpp in zip(parentPath_set, calc_parentPath_set):
                ...
                colHdrRange_dict, rowHdrRange_dict, maxRow = ExcelUtil._get_all_excel_headers_numrange_dict(xlfilepath, rowSheet)
                ...
                rowItems = []
                df = ExcelDFUtil.fetch_excel_df_data(xlfilepath, rowSheet)
                for r in range(start, end + 1):  # iterate for each rows
                    rowItem = {}
                    for c in v_list:  # iterate for each cols
                        val = ExcelDFUtil.get_excel_df_val_by_rowIndex_colName(df, r - 2, c)
                        # print(pp, r, c, val)
                        rowItem[c] = val
    ...
    # json_service file
    def read_excel_validationSheet_by_rowName(self, xlfilepath, rowName):
        sheet = 'Validation'
        validation_dict = {}

        colHdrRange_dict, rowHdrRange_dict, maxRow = ExcelUtil._get_all_excel_headers_numrange_dict(xlfilepath, sheet)
        start, end = rowHdrRange_dict[rowName]

        df = ExcelDFUtil.fetch_excel_df_data(xlfilepath, sheet)
        maxcol = len(df.columns)
        for c in range(1, maxcol):
            colName = df.columns[c]
            val = ExcelDFUtil.get_excel_df_val_by_rowIndex_colName(df, start - 2, colName)
            validation_dict[colName] = val

        return validation_dict

    @staticmethod
    def get_excel_validationData_by_rowName(xlfilepath, rowName):
        """Sheet = Validation
        """
        sheet = 'Validation'
        validation_dict = {}
...
class ExcelDFUtil:
    ...
    @staticmethod
    def fetch_excel_df_data(xl_file_path, sheet: str = None):
    ...
    @staticmethod
    def get_excel_df_val_by_rowIndex_colName(df, rowIndex, colName):
