...
class Commons:
    ...
    @staticmethod
    def get_tuplestr_by_replace(from_data: Union[str, int, float, list],
    ...
    @staticmethod
    def build_date_for_filename() -> str:
    ...
    def update_dict(curr_dict: dict, new_dict: dict, skip_existing_key: bool = True):
        ...
        if skip_existing_key:
            ...
            curr_dict = {**curr_dict, **{k: v for k, v in new_dict.items()}}
        return curr_dict
    ...
    @staticmethod
    def check_if_file_exist(dir_path, filename):
        file_path = os.path.join(dir_path, filename)
        return os.path.exists(file_path)

    @staticmethod
    def _get_root_path(test_path):
        """Get root path using pytest.ini from bottom to top from the provided path"""
        root_path = ''

        parts = str(test_path).split(os.path.sep)
        index = parts.index('tests') if 'tests' in parts else -1
        if index >= 0:
            for i in range(index):
                curr_path = os.path.sep.join(parts[:index - i])
                is_root_py_found = Commons.check_if_file_exist(curr_path, 'pytest.ini')
                if is_root_py_found:
                    root_path = curr_path
                    break
    ...
    @staticmethod
    def get_project_root_from_sys_args(test_dir=None):
        """Get project root using sys.argv"""
        _is_a_project = False
        project_root_path = ''

        # all_args = [test_dir] if test_dir else sys.argv
        all_args = sys.argv 
        for i in range(len(all_args)):
            test_path = os.path.abspath(all_args[i])

            parts = str(test_path).split(os.path.sep)
            if 'tests' in parts:
                consider_path = test_path
                parts = str(consider_path).split(os.path.sep)
                if 'pista' not in parts:
                    _is_a_project = True
                    project_root_path = Commons._get_root_path(consider_path)
                # else:
                #     pista_root_path = Commons._get_root_path(consider_path)
                break
                
        # print('SysArgv:', sys.argv)
        # print('ProjectRoot:', _is_a_project, project_root_path)

        return _is_a_project, project_root_path

    @staticmethod
    def get_project_root_from_ini_path(ini_path):
        """Get project root using pytest.ini path"""
        _is_a_project = False
        project_root_path = ''

        ini_path = os.path.abspath(ini_path)
        parts = str(ini_path).split(os.path.sep)

        assert parts[len(parts) - 1] == 'pytest.ini', 'pytest.ini path is wrong. Debug and fix.'

        if 'pista' not in parts:
            _is_a_project = True
            project_root_path = os.path.sep.join(parts[:len(parts) - 1])
        # else:
        #     pista_root_path = Commons._get_root_path(consider_path)

        # print('PytestIni:', ini_path)
        # print('ProjectRoot:', _is_a_project, project_root_path)

        return _is_a_project, project_root_path

    @staticmethod
    def get_line_in_file_startswith(file_path, target_text):
        try:
            with open(file_path, 'r') as file:
                for line in file:
                    if line.startswith(target_text):
                        return line  # Return the line without leading/trailing whitespaces
            return ''
