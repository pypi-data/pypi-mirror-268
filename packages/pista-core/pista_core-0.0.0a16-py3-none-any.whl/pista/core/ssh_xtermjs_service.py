...
from pista.core._root_manager import PROJECT_ROOT_DIR, OUTPUT_DIR, PISTA_CONFIG
...
class SshXtermService(UIService):
    ...
    KEY_DOWN = Keys.DOWN
    KEY_UP = Keys.UP
    KEY_CTRL_A = Keys.CONTROL + 'a'
    KEY_CTRL_B = Keys.CONTROL + 'b'
    KEY_CTRL_C = Keys.CONTROL + 'c'
    KEY_CTRL_D = Keys.CONTROL + 'd'
    KEY_CTRL_E = Keys.CONTROL + 'e'
    KEY_CTRL_F = Keys.CONTROL + 'f'
    KEY_CTRL_G = Keys.CONTROL + 'g'
    KEY_CTRL_H = Keys.CONTROL + 'h'
    KEY_CTRL_I = Keys.CONTROL + 'i'
    KEY_CTRL_J = Keys.CONTROL + 'j'
    KEY_CTRL_K = Keys.CONTROL + 'k'
    KEY_CTRL_L = Keys.CONTROL + 'l'
    KEY_CTRL_M = Keys.CONTROL + 'm'
    KEY_CTRL_N = Keys.CONTROL + 'n'
    KEY_CTRL_O = Keys.CONTROL + 'o'
    KEY_CTRL_P = Keys.CONTROL + 'p'
    KEY_CTRL_Q = Keys.CONTROL + 'q'
    KEY_CTRL_R = Keys.CONTROL + 'r'
    KEY_CTRL_S = Keys.CONTROL + 's'
    KEY_CTRL_T = Keys.CONTROL + 't'
    KEY_CTRL_U = Keys.CONTROL + 'u'
    KEY_CTRL_V = Keys.CONTROL + 'v'
    KEY_CTRL_W = Keys.CONTROL + 'w'
    KEY_CTRL_X = Keys.CONTROL + 'x'
    KEY_CTRL_Y = Keys.CONTROL + 'y'
    KEY_CTRL_Z = Keys.CONTROL + 'z'
    ...
    XTERMJS_CODE_PATH = PISTA_CONFIG.get('ssh', 'xtermjs_code_path')
    XTERMJS_CODE_PATH = XTERMJS_CODE_PATH if XTERMJS_CODE_PATH.strip()[1] == ':' else os.path.join(PROJECT_ROOT_DIR, XTERMJS_CODE_PATH)
    XTERMJS_LOG_FILE = os.path.join(OUTPUT_DIR, PISTA_CONFIG.get('ssh', 'xtermjs_logfile'))
    XTERM_URL = PISTA_CONFIG.get('ssh', 'xtermjs_terminal_url')

    def __init__(self, driver, sshHost=None, sshUser=None, sshPwd_en=None, isPageOpen: bool = False):
        ...
        if not isPageOpen:
            ...
            # self._connect_ssh(sshHost, sshUser, sshPwd)  # Called from apps/
    ...
    def _connect_ssh(self, sshHost, sshUser, sshPwd_en) -> str:
        print('Opening SSH: ', sshHost)
        # self.sendData('cls')
        ...
        if output.endswith('password:'):
            sshPwd = DataHandler.decrypt_it(sshPwd_en)
    ...
    def _assertWaitUntilNoBlankScreen(self):
        ...
        assert isBlankScreenGone, '<SSH> Ssh blank screen didnt go within max waittime'
        return all_texts
    ...
    def assertScreenTextExist(self, expectedTxts: Union[str, list]):
        ...
        allTxtsFound = True
        missing_texts = []
        expTextsList = expectedTxts if type(expectedTxts) is list else [expectedTxts]
        ...
        assert allTxtsFound, f"<SSH> Terminal texts not found, expected: {missing_texts}, actual: {screentxt}"
    ...
    def acceptMsgIfExist(self, textsIn1Msg: list[str], acceptKey=KEY_CTRL_A):
