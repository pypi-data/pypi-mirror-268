...
class SshParamikoService:
    ...
    def __init__(self, sshHost, sshUser, sshPwd_en):
        self.output = None
        self.channel = self._connect_host(sshHost=sshHost, sshUser=sshUser, sshPwd_en=sshPwd_en)
    ...
    def _connect_host(self, sshHost, sshUser, sshPwd_en):
        try:
            client = paramiko.SSHClient()  # Create an SSH client
            client.set_missing_host_key_policy(paramiko.AutoAddPolicy())

            sshPwd = DataHandler.decrypt_it(sshPwd_en)
            client.connect(sshHost, username=sshUser, password=sshPwd_en)  # Connect to the host
