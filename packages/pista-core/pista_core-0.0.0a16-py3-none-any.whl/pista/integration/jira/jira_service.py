...
from pista.core.json_util import JsonUtil
from pista.root import PISTA_DIR

# from configparser import ConfigParser

JIRA_CONFIG = PISTA_CONFIG

zephyr_baseUrl = JIRA_CONFIG.get('jira', 'zephyr_url')
zephyr_execution_endpoint = JIRA_CONFIG.get('jira', 'zephyr_test_execution_endpoint')
zephyr_token = JIRA_CONFIG.get('jira', 'zephyr_token')
projectId = JIRA_CONFIG.get('jira', 'zephyr_project_key')
zephyr_cycle_key = JIRA_CONFIG.get('jira', 'zephyr_cycle_key')
...
def _update_result_to_jira(testCaseId, statusName):
    ...
    try:
        if testCaseId == "":
            ...
            if testCycleId:
                jira_json_filepath = os.path.join(PISTA_DIR, 'integration', 'jira', 'jira_json.json')
