...
class MolecularDynamic_builder(PartitionManager, ):
    ...
    def get_displacement(self, reference:str=None):
        ...
        for container_i, container in enumerate(self.containers):
            ...
            pos[:,:,container_i] =  container.AtomPositionManager.atomPositions - self.containers[0].AtomPositionManager.atomPositions
        
        #data_displacement = np.linalg.norm( pos[:,:,:] - pos[:,:,:-1] , axis=1).T 
        data_displacement = np.linalg.norm( pos, axis=1).T 
        ...
        for container_i in range( len(self.containers)-1 ):
            ...
            data_displacement[container_i, :] += self._calculate_reference_values(self.containers[0], 'z')
    ...
    def plot_displacement(self, data_displacement, output_path:str=None, save:bool=True, verbose:bool=True):
        ...
        for u in self.containers[0].AtomPositionManager.uniqueAtomLabels:
            ...
            var_displacement = np.mean(data_displacement[:, mask], axis=0) - self._calculate_reference_values(self.containers[0], 'z')[mask]
            avg_displacement = self._calculate_reference_values(self.containers[0], 'z')[mask]
            ...
            plt.tight_layout()
            if save:
                plt.savefig(f'{output_path}/displacements_{u}.png', dpi=1000, transparent=True)
    ...
    def handleMDAnalysis(self, values:list ):
        ...
        """
        MDA_data = {}
        self.containers = self.containers[400:]
        for plot in values:
            ...
            if plot.upper() == 'BOND_DISTANCE_TRACKING':
                ...
                for container_i, container in enumerate(self.containers):
                    ...
                    total, total_i = 0, 0
                    for n in np.array(range(container.AtomPositionManager.atomCount))[container.AtomPositionManager.atomLabelsList == 'Ni']:
                        pos = container.AtomPositionManager.atomPositions[n] 
                        fn = container.AtomPositionManager.find_all_neighbors_radius( pos, 2.5 )
                        for n2 in fn:
                            pos2 = container.AtomPositionManager.atomPositions[n2] 
                            dist = container.AtomPositionManager.distance( pos, pos2, True )
                            if container.AtomPositionManager.atomLabelsList[n2] == 'O' and dist<2.5:
                                total += dist
                                total_i += 1

                    datax.append( float(total)/total_i  )   
