...
@typechecked
def mIoU(labels: Union[np.ndarray, torch.Tensor],
    ...
    miou = _measure.miou(outputs=outputs, labels=labels)
    return torch.mean(miou)


@typechecked
def IoU(true_onoff: Union[np.ndarray, torch.Tensor],
...
@typechecked
def GIoU(true_onoff: Union[np.ndarray, torch.Tensor],
         pred_onoff: Union[np.ndarray, torch.Tensor]):
    """
    ...
    giou = _measure.giou(true_onoff=true_onoff, pred_onoff=pred_onoff)
...
def mse(true: torch.Tensor, predict: torch.Tensor):
    return _measure.mse(true, predict)
