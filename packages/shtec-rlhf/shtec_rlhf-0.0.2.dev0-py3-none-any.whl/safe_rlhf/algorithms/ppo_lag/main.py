...
# 
# *******************************************************************************************************/
"""The main training script to train Safe-RLHF using PPO-Lagrange algorithm."""
...
from shtec_rlhf.algorithms.ppo_lag.trainer import PPOLagTrainer
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        prog='deepspeed --module shtec_rlhf.algorithms.ppo_reward_shaping',
    ...
    training_parser.add_argument(
        ...
        type=float,
        default=0.0,
        help='The threshold for the expect episode cost.',
    )
    training_parser.add_argument(
        '--lambda_init',
        type=float,
        default=1.0,
        help='The initial value for the lambda (Lagrangian multiplier).',
    )
    training_parser.add_argument(
        '--lambda_lr',
        ...
        type=float,
        default=0.1,
        help='The learning rate for the lambda (Lagrangian multiplier).',
    )
    training_parser.add_argument(
        '--lambda_max',
        type=float,
        default=None,
        help='The maximum value for the lambda (Lagrangian multiplier).',
    )
    training_parser.add_argument(
        '--lambda_update_delay_steps',
        type=int,
        default=0,
        help='The number of steps to delay the lambda (Lagrangian multiplier) update.',
    )
    training_parser.add_argument(
        '--episode_cost_window_size',
        type=int,
        default=128,
        help='The window size to compute the expected episode cost.',
    )
    training_parser.add_argument(
        '--kl_coeff',
        type=float,
    ...
    training_parser.add_argument(
        ...
        type=str2bool,
        default=False,
        help='Whether to normalize the cost during RL training.',
    )
    training_parser.add_argument(
        '--seed',
        type=int,
...
def main() -> None:
    ...
    trainer = PPOLagTrainer(args, ds_train_config, ds_eval_config)
    trainer.train()
    trainer.save()
