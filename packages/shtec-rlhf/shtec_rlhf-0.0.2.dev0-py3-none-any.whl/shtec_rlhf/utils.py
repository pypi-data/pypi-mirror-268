...
from shtec_rlhf.configs.constants import PROMPT_ASSISTANT
...
def __initialize_pytree_registry_once() -> None:
    ...
    from shtec_rlhf.models.score_model import ScoreModelOutput  # noqa: F401
...
def to_device(batch: TensorTree, device: torch.device | str | int | None) -> TensorTree:
    ...
    if device is None:
        return batch
    return optree.tree_map(lambda x: x.to(device), batch, namespace='shtec_rlhf')
