...
class DbModel(Model):
    ...
    @classmethod
    def sheet_name(cls, suffix: Union[str, int, float, datetime, date] = None):
        """
        数据表名 
        ...
        if cls._split_type:
            ...
            if cls._split_type == 1:
                ...
                tb_name = f"{tb_name}_{tool_dt.dt_str(suffix, fmt=fmt)}"
        return tb_name
    ...
    def get_meta_db(cls, db_key: str = None):
        db = connections.get(db_key or 'default')
        if db:
            return db
        raise Exception(f'Error: {db_key} database configuration does not exist')
    ...
    def __fetch_generator_class(cls):
        ...
        if dialect == 'postgres':
            from msanic.orm.pgsql_generator import PgSqlGenerator
            GeneratorClass = PgSqlGenerator
        elif dialect == 'mysql':
            from msanic.orm.mysql_generator import MySqlGenerator
            GeneratorClass = MySqlGenerator
        elif dialect == 'mssql':
            from msanic.orm.mssql_generator import MsSqlGenerator
    ...
    @classmethod
    def get_split_suffix(cls, split: Union[str, int, float, datetime, date], param: dict):
        """
        获取字典参数的分表后缀
        ...
        return f'_{tool_dt.dt_str(kval, fmt=cls._SPLIT_SUFFIX.get(cls._split_type))}'
    ...
    def gen_simple_query(
            ...
            db_key: str = None):
        suffix = cls.get_split_suffix(split, param)
        db = cls.get_meta_db(db_key)
        cls._meta.basetable._table_name = cls.get_tb_name(suffix)
    ...
    async def check_table(cls, tb_suffix: str, db_key: str = None):
        ...
        if tb_name not in cls._SPLIT_TABLES:
            ...
            if not _creator:
                raise Exception(f'未定义分表的创建模式：{cls._meta.db.schema_generator.DIALECT}')
            cls._meta.db_table = tb_name
            sql = _creator(db).gen_create_sql(cls)
    ...
    async def fetch_on_page(
        ...
        q_set, out_field = cls.gen_simple_query(
            param=param, field=field, exclude=exclude, groups=groups, orders=orders, split=split, db_key=db_key)
        if page < 1:
            page = 1
        offset = (page - 1) * size
        total = 0
    ...
    async def get_by_pk(
        ...
        query_model, f_list = cls.gen_simple_query(
            {cls.pk_name(): val}, field=field, exclude=exclude, split=split, limit=1, db_key=db_key)
    ...
    async def update_by_pk(
        ...
        if not old_data:
            ...
            suffix = cls.get_split_suffix(split, split_param)
        else:
    ...
    async def update_by_cond(
        ...
        if new_dict:
            ('updated' in cls._meta.db_fields) and new_dict.update({'updated': tool_dt.cur_time()})
            query_model, _ = cls.gen_simple_query(param, limit=limit, split=split, db_key=db_key)
    ...
    async def del_by_pk(
        ...
        if (not hasattr(cls, 'sta_del')) or force:
            db = cls.get_meta_db(db_key)
            suffix = cls.get_split_suffix(split, {cls._split_field: split})
            cls._meta.basetable._table_name = cls.get_tb_name(suffix)
    ...
    async def del_by_cond(
            ...
            db_key: str = None,
            fun_success=None,
            fun_ags: (list, tuple) = None):
        if fun_ags is None:
            fun_ags = ()
        query_model, _ = cls.gen_simple_query(param, limit=limit, with_del=force, split=split, db_key=db_key)
        if (not hasattr(cls, 'sta_del')) or force:
    ...
    async def split_bulk_insert(
        ...
        for item in data:
            if isinstance(item, dict):
                ...
                suffix = cls.get_split_suffix(split or cls._split_field, item)
                ...
                suffix = cls.get_split_suffix(
                    split or cls._split_field, {cls._split_field: getattr(item, cls._split_field, None)})
        ...
        async with transactions.in_transaction(db_key or 'default'):
            for k, v in model_map.items():
                cls._meta.basetable._table_name = cls.get_tb_name(k)
    ...
    async def split_add_one(
        ...
        suffix = cls.get_split_suffix(split or cls._split_field, data)
        await cls.check_table(suffix, db_key)
        db = cls.get_meta_db(db_key)
        cls._meta.basetable._table_name = cls.get_tb_name(suffix)
