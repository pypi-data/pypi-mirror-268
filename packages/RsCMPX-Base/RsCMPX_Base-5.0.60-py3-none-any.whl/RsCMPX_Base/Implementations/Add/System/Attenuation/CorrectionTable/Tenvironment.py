...
class TenvironmentCls:
	...
	def set(self, name: str, frequency: List[float] = None, attenuation: List[float] = None) -> None:
		...
		Snippet: driver.add.system.attenuation.correctionTable.tenvironment.set(name = 'abc', frequency = [1.1, 2.2, 3.3], attenuation = [1.1, 2.2, 3.3]) \n
		Adds entries to an existing channel-specific correction table. Specify at least one parameter pair <Frequency>,
		<Attenuation>. \n
		...
		self._core.io.write(f'ADD:SYSTem:ATTenuation:CTABle:TENVironment {param}'.rstrip())
