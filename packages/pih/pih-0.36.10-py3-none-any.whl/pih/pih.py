    ...
    class DATA(
        ...
        class IOTDevices:
            ...
            def status_value(device_id: str, status_value_code: str) -> Any:
                status: IOTDeviceStatus | None = PIH.RESULT.IOTDevices.status(device_id).data
                if n(status):
                    return None
                status_properties: list[IOTDeviceStatusProperty] | None = PIH.RESULT.IOTDevices.status_properties(device_id).data
                        ...
                        for status_property_item in status_properties:
                            if status_property_item.code == status_value_code:
                                if status_property_item.type == "Integer":
                                    status_property_item_integer: IOTDeviceStatusIntegerProperty = status_property_item
                                    return float(status_item.value) / pow(10, status_property_item_integer.scale)
                return None
            ...
            def status(id: str) -> Result[IOTDeviceStatus]:
                def mapper(value: strdict) -> IOTDeviceStatus:
                    ...
                    for status_item in value["result"]:
                        values.append(DataTool.fill_data_from_source(IOTDeviceStatusValue(), status_item))
                    return IOTDeviceStatus(value["t"]/1000, tuple(values))
                return DataTool.to_result(
                    PIH.RESULT.IOTDevices._call(IOT.Commands.device_status, (id,)), mapper
                )
                ...
                def mapper(value: strdict) -> list[IOTDeviceStatusProperty]:
                    ...
                    for status_item in value["result"]["status"]:
                        if lw(status_item["type"]) == "integer":
                            values.append(DataTool.fill_data_from_source(IOTDeviceStatusIntegerProperty(status_item["code"], status_item["name"], status_item["type"]), DataTool.rpc_decode(status_item["values"]), skip_not_none=True))
                    return values
                return DataTool.to_result(
                    PIH.RESULT.IOTDevices._call(IOT.Commands.device_status_properties, (id,)), mapper
                )
