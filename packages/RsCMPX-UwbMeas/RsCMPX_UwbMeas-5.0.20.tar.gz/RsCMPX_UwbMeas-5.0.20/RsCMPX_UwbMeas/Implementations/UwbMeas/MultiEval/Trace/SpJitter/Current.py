...
class CurrentCls:
	...
	def fetch(self, ppdu=repcap.Ppdu.Default) -> List[float]:
		"""SCPI: FETCh:UWB:MEASurement<Instance>:MEValuation:TRACe:SPJitter:CURRent<PPDU> \n
		Snippet: value: List[float] = driver.uwbMeas.multiEval.trace.spJitter.current.fetch(ppdu = repcap.Ppdu.Default) \n
		Returns the y-values of the current symbol phase jitter trace. See also 'Symbol Jitter square'. \n
		...
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'FETCh:UWB:MEASurement<Instance>:MEValuation:TRACe:SPJitter:CURRent{ppdu_cmd_val}?', suppressed)
		return response

	def read(self, ppdu=repcap.Ppdu.Default) -> List[float]:
		"""SCPI: READ:UWB:MEASurement<Instance>:MEValuation:TRACe:SPJitter:CURRent<PPDU> \n
		Snippet: value: List[float] = driver.uwbMeas.multiEval.trace.spJitter.current.read(ppdu = repcap.Ppdu.Default) \n
		Returns the y-values of the current symbol phase jitter trace. See also 'Symbol Jitter square'. \n
		Suppressed linked return values: reliability \n
			:param ppdu: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Current')
			:return: jitter: Comma-separated list of symbol jitter values."""
		ppdu_cmd_val = self._cmd_group.get_repcap_cmd_value(ppdu, repcap.Ppdu)
		suppressed = ArgSingleSuppressed(0, DataType.Integer, False, 1, 'Reliability')
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'READ:UWB:MEASurement<Instance>:MEValuation:TRACe:SPJitter:CURRent{ppdu_cmd_val}?', suppressed)
		return response
