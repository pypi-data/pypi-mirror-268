...
class StateCls:
	...
	def fetch(self, timeout: float = None, target_main_state: enums.TargetMainState = None, target_sync_state: enums.TargetSyncState = None) -> enums.ResourceState:
		"""SCPI: FETCh:UWB:MEASurement<Instance>:MEValuation:STATe \n
		Snippet: value: enums.ResourceState = driver.uwbMeas.multiEval.state.fetch(timeout = 1.0, target_main_state = enums.TargetMainState.OFF, target_sync_state = enums.TargetSyncState.ADJusted) \n
		...
		the state is returned when the <TargetMainState> and the <TargetSyncState> are reached or when the <Timeout> expires. \n
			...
			:return: meas_state: Current state or target state of ongoing state transition OFF: measurement off RUN: measurement running RDY: measurement completed"""
		...
		response = self._core.io.query_str(f'FETCh:UWB:MEASurement<Instance>:MEValuation:STATe? {param}'.rstrip())
