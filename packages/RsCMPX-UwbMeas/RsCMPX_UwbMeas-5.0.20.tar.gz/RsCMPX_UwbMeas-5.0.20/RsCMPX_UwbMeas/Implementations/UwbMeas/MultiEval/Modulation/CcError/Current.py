...
class CurrentCls:
	...
	def fetch(self, ppdu=repcap.Ppdu.Default) -> float:
		"""SCPI: FETCh:UWB:MEASurement<Instance>:MEValuation:MODulation:CCERor:CURRent<PPDU> \n
		Snippet: value: float = driver.uwbMeas.multiEval.modulation.ccError.current.fetch(ppdu = repcap.Ppdu.Default) \n
		...
		response = self._core.io.query_str_suppressed(f'FETCh:UWB:MEASurement<Instance>:MEValuation:MODulation:CCERor:CURRent{ppdu_cmd_val}?', suppressed)
		return Conversions.str_to_float(response)

	def read(self, ppdu=repcap.Ppdu.Default) -> float:
		"""SCPI: READ:UWB:MEASurement<Instance>:MEValuation:MODulation:CCERor:CURRent<PPDU> \n
		Snippet: value: float = driver.uwbMeas.multiEval.modulation.ccError.current.read(ppdu = repcap.Ppdu.Default) \n
		Returns the positive or negative chip clock error. \n
		Suppressed linked return values: reliability \n
			:param ppdu: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Current')
			:return: error: No help available"""
		ppdu_cmd_val = self._cmd_group.get_repcap_cmd_value(ppdu, repcap.Ppdu)
		suppressed = ArgSingleSuppressed(0, DataType.Integer, False, 1, 'Reliability')
		response = self._core.io.query_str_suppressed(f'READ:UWB:MEASurement<Instance>:MEValuation:MODulation:CCERor:CURRent{ppdu_cmd_val}?', suppressed)
		return Conversions.str_to_float(response)

	# noinspection PyTypeChecker
	def calculate(self, ppdu=repcap.Ppdu.Default) -> enums.ResultStatus2:
		"""SCPI: CALCulate:UWB:MEASurement<Instance>:MEValuation:MODulation:CCERor:CURRent<PPDU> \n
		Snippet: value: enums.ResultStatus2 = driver.uwbMeas.multiEval.modulation.ccError.current.calculate(ppdu = repcap.Ppdu.Default) \n
		Returns the positive or negative chip clock error. \n
		Suppressed linked return values: reliability \n
			:param ppdu: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Current')
			:return: error: No help available"""
		ppdu_cmd_val = self._cmd_group.get_repcap_cmd_value(ppdu, repcap.Ppdu)
		suppressed = ArgSingleSuppressed(0, DataType.Integer, False, 1, 'Reliability')
		response = self._core.io.query_str_suppressed(f'CALCulate:UWB:MEASurement<Instance>:MEValuation:MODulation:CCERor:CURRent{ppdu_cmd_val}?', suppressed)
