...
class CurrentCls:
	...
	def fetch(self, ppdu=repcap.Ppdu.Default) -> int:
		"""SCPI: FETCh:UWB:MEASurement<Instance>:MEValuation:MODulation:IQOFfset:CURRent<PPDU> \n
		Snippet: value: int = driver.uwbMeas.multiEval.modulation.iqOffset.current.fetch(ppdu = repcap.Ppdu.Default) \n
		...
		response = self._core.io.query_str_suppressed(f'FETCh:UWB:MEASurement<Instance>:MEValuation:MODulation:IQOFfset:CURRent{ppdu_cmd_val}?', suppressed)
		return Conversions.str_to_int(response)

	def read(self, ppdu=repcap.Ppdu.Default) -> int:
		"""SCPI: READ:UWB:MEASurement<Instance>:MEValuation:MODulation:IQOFfset:CURRent<PPDU> \n
		Snippet: value: int = driver.uwbMeas.multiEval.modulation.iqOffset.current.read(ppdu = repcap.Ppdu.Default) \n
		Returns the I/Q offset. \n
		Suppressed linked return values: reliability \n
			:param ppdu: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Current')
			:return: iq_offset: No help available"""
		ppdu_cmd_val = self._cmd_group.get_repcap_cmd_value(ppdu, repcap.Ppdu)
		suppressed = ArgSingleSuppressed(0, DataType.Integer, False, 1, 'Reliability')
		response = self._core.io.query_str_suppressed(f'READ:UWB:MEASurement<Instance>:MEValuation:MODulation:IQOFfset:CURRent{ppdu_cmd_val}?', suppressed)
		return Conversions.str_to_int(response)
