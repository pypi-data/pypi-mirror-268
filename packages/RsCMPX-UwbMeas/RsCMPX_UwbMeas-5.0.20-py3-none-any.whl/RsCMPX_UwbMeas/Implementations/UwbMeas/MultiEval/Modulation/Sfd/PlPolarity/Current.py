...
class CurrentCls:
	...
	def fetch(self, ppdu=repcap.Ppdu.Default) -> enums.Result:
		"""SCPI: FETCh:UWB:MEASurement<Instance>:MEValuation:MODulation:SFD:PLPolarity:CURRent<PPDU> \n
		Snippet: value: enums.Result = driver.uwbMeas.multiEval.modulation.sfd.plPolarity.current.fetch(ppdu = repcap.Ppdu.Default) \n
		...
		response = self._core.io.query_str_suppressed(f'FETCh:UWB:MEASurement<Instance>:MEValuation:MODulation:SFD:PLPolarity:CURRent{ppdu_cmd_val}?', suppressed)
		return Conversions.str_to_scalar_enum(response, enums.Result)

	# noinspection PyTypeChecker
	def read(self, ppdu=repcap.Ppdu.Default) -> enums.Result:
		"""SCPI: READ:UWB:MEASurement<Instance>:MEValuation:MODulation:SFD:PLPolarity:CURRent<PPDU> \n
		Snippet: value: enums.Result = driver.uwbMeas.multiEval.modulation.sfd.plPolarity.current.read(ppdu = repcap.Ppdu.Default) \n
		Returns the result of the check for correct pulse location and polarity, for SFD. \n
		Suppressed linked return values: reliability \n
			:param ppdu: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Current')
			:return: sfd_polarity: No help available"""
		ppdu_cmd_val = self._cmd_group.get_repcap_cmd_value(ppdu, repcap.Ppdu)
		suppressed = ArgSingleSuppressed(0, DataType.Integer, False, 1, 'Reliability')
		response = self._core.io.query_str_suppressed(f'READ:UWB:MEASurement<Instance>:MEValuation:MODulation:SFD:PLPolarity:CURRent{ppdu_cmd_val}?', suppressed)
		return Conversions.str_to_scalar_enum(response, enums.Result)
