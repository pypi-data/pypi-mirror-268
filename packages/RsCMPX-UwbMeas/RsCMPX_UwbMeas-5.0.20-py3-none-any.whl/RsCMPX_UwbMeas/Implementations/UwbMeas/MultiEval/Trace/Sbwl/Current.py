...
class CurrentCls:
	...
	def fetch(self, ppdu=repcap.Ppdu.Default) -> List[float]:
		"""SCPI: FETCh:UWB:MEASurement<Instance>:MEValuation:TRACe:SBWL:CURRent<PPDU> \n
		Snippet: value: List[float] = driver.uwbMeas.multiEval.trace.sbwl.current.fetch(ppdu = repcap.Ppdu.Default) \n
		Returns the y-values of the current 50-MHz spectrum trace. See also '1MHz / 50MHz Spectrum square'. \n
		...
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'FETCh:UWB:MEASurement<Instance>:MEValuation:TRACe:SBWL:CURRent{ppdu_cmd_val}?', suppressed)
		return response

	def read(self, ppdu=repcap.Ppdu.Default) -> List[float]:
		"""SCPI: READ:UWB:MEASurement<Instance>:MEValuation:TRACe:SBWL:CURRent<PPDU> \n
		Snippet: value: List[float] = driver.uwbMeas.multiEval.trace.sbwl.current.read(ppdu = repcap.Ppdu.Default) \n
		Returns the y-values of the current 50-MHz spectrum trace. See also '1MHz / 50MHz Spectrum square'. \n
		Suppressed linked return values: reliability \n
			:param ppdu: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Current')
			:return: power: Comma-separated list of power values."""
		ppdu_cmd_val = self._cmd_group.get_repcap_cmd_value(ppdu, repcap.Ppdu)
		suppressed = ArgSingleSuppressed(0, DataType.Integer, False, 1, 'Reliability')
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'READ:UWB:MEASurement<Instance>:MEValuation:TRACe:SBWL:CURRent{ppdu_cmd_val}?', suppressed)
		return response
