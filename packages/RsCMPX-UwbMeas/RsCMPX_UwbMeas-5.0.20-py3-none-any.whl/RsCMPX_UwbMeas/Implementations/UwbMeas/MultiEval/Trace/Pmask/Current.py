...
class CurrentCls:
	...
	def fetch(self, ppdu=repcap.Ppdu.Default) -> List[float]:
		"""SCPI: FETCh:UWB:MEASurement<Instance>:MEValuation:TRACe:PMASk:CURRent<PPDU> \n
		Snippet: value: List[float] = driver.uwbMeas.multiEval.trace.pmask.current.fetch(ppdu = repcap.Ppdu.Default) \n
		Returns the y-values of the pulse mask trace. See also 'Pulse Mask square'. \n
		Suppressed linked return values: reliability \n
			:param ppdu: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Current')
			:return: values: Comma-separated list of normalized magnitude values."""
		ppdu_cmd_val = self._cmd_group.get_repcap_cmd_value(ppdu, repcap.Ppdu)
		suppressed = ArgSingleSuppressed(0, DataType.Integer, False, 1, 'Reliability')
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'FETCh:UWB:MEASurement<Instance>:MEValuation:TRACe:PMASk:CURRent{ppdu_cmd_val}?', suppressed)
		return response

	def read(self, ppdu=repcap.Ppdu.Default) -> List[float]:
		"""SCPI: READ:UWB:MEASurement<Instance>:MEValuation:TRACe:PMASk:CURRent<PPDU> \n
		Snippet: value: List[float] = driver.uwbMeas.multiEval.trace.pmask.current.read(ppdu = repcap.Ppdu.Default) \n
		Returns the y-values of the pulse mask trace. See also 'Pulse Mask square'. \n
		Suppressed linked return values: reliability \n
			:param ppdu: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Current')
			:return: values: Comma-separated list of normalized magnitude values."""
		ppdu_cmd_val = self._cmd_group.get_repcap_cmd_value(ppdu, repcap.Ppdu)
		suppressed = ArgSingleSuppressed(0, DataType.Integer, False, 1, 'Reliability')
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'READ:UWB:MEASurement<Instance>:MEValuation:TRACe:PMASk:CURRent{ppdu_cmd_val}?', suppressed)
		return response
