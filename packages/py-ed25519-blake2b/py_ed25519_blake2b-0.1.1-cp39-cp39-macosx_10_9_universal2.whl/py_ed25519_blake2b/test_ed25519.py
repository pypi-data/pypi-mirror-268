...
from py_ed25519_blake2b import _ed25519 as raw
...
class Basic(unittest.TestCase):
    ...
    def test_version(self):
        ...
        ver = py_ed25519_blake2b.__version__
    ...
    def test_keypair(self):
        sk, vk = py_ed25519_blake2b.create_keypair()
        self.failUnless(isinstance(sk, py_ed25519_blake2b.SigningKey), sk)
        self.failUnless(isinstance(vk, py_ed25519_blake2b.VerifyingKey), vk)
        sk2, vk2 = py_ed25519_blake2b.create_keypair()
        self.failIfEqual(hexlify(sk.to_bytes()), hexlify(sk2.to_bytes()))

        # you can control the entropy source
        def not_so_random(length):
            return b"4"*length
        sk1, vk1 = py_ed25519_blake2b.create_keypair(entropy=not_so_random)
        ...
        sk2, vk2 = py_ed25519_blake2b.create_keypair(entropy=not_so_random)
        self.failUnlessEqual(sk1.to_ascii(encoding="base64"),
    ...
    def test_publickey(self):
        ...
        sk = py_ed25519_blake2b.SigningKey(seed)
        ...
        self.failUnlessRaises(ValueError,
                              py_ed25519_blake2b.SigningKey, b"wrong length")
        sk2 = py_ed25519_blake2b.SigningKey(seed)
    ...
    def test_OOP(self):
        ...
        sk_s = unhexlify(b"4ba96b0b5303328c7405220598a587c4"
                         b"acb06ed9a9601d149f85400195f1ec3d")
        sk = py_ed25519_blake2b.SigningKey(sk_s)
        ...
        self.failUnlessEqual(
            py_ed25519_blake2b.VerifyingKey(vk.to_bytes()), vk)
        msg = b"hello world"
        sig = sk.sign(msg)
        self.failUnlessEqual(len(sig), 64)
        ...
        self.failUnlessRaises(py_ed25519_blake2b.BadSignatureError,
    ...
    def test_object_identity(self):
        ...
        sk1a = py_ed25519_blake2b.SigningKey(sk1_s)
        sk1b = py_ed25519_blake2b.SigningKey(sk1_s)
        vk1a = sk1a.get_verifying_key()
        vk1b = sk1b.get_verifying_key()
        sk2 = py_ed25519_blake2b.SigningKey(sk2_s)
    ...
    def test_prefix(self):
        ...
        vk2 = py_ed25519_blake2b.VerifyingKey(p, prefix=PREFIX)
        self.failUnlessEqual(vk1, vk2)
        self.failUnlessEqual(repr(vk1.to_bytes()), repr(vk2.to_bytes()))
        self.failUnlessRaises(py_ed25519_blake2b.BadPrefixError,
                              py_ed25519_blake2b.VerifyingKey, p, prefix=b"WRONG-")
    ...
    def test_ascii(self):
        b2a = py_ed25519_blake2b.to_ascii
        a2b = py_ed25519_blake2b.from_ascii
    ...
    def test_encoding(self):
        ...
        sk1 = py_ed25519_blake2b.SigningKey(sk_s)
        ...
        def check1(encoding, expected):
            ...
            sk2 = py_ed25519_blake2b.SigningKey(
                p, prefix=PREFIX, encoding=encoding)
            self.failUnlessEqual(repr(sk1.to_bytes()), repr(sk2.to_bytes()))
            self.failUnlessEqual(sk1, sk2)
        ...
        check1(
            "hex", b"private0-8888888888888888888888888888888888888888888888888888888888888888")

        def check2(encoding, expected):
            PREFIX = "public0-"
            p = vk1.to_ascii(PREFIX, encoding)
            self.failUnlessEqual(p, expected)
            vk2 = py_ed25519_blake2b.VerifyingKey(
                p, prefix=PREFIX, encoding=encoding)
            self.failUnlessEqual(repr(vk1.to_bytes()), repr(vk2.to_bytes()))
            self.failUnlessEqual(vk1, vk2)
        ...
        check2(
            "hex", b"public0-2f28eaa728e9b0a7be812590d2a3ea70bc9ceebfb2ad990ee7813e46a0f3a46f")
