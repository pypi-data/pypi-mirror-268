...
def get_attr_value(context: CertoraContext, attr: Attr.ContextAttribute) -> Any:
    conf_key = attr.get_conf_key()
    return getattr(context, conf_key, None)


def collect_args_with_jar_flags(context: CertoraContext) -> List[Attr.ContextAttribute]:
    return [arg for arg in Attr.ContextAttribute if get_attr_value(context, arg) and arg.value.jar_flag]


def collect_args_build_cache_affecting(context: CertoraContext) -> List[Attr.ContextAttribute]:
    return [arg for arg in Attr.ContextAttribute if get_attr_value(context, arg) and arg.value.affects_build_cache_key]


def collect_args_build_cache_disabling(context: CertoraContext) -> List[Attr.ContextAttribute]:
    return [arg for arg in Attr.ContextAttribute if get_attr_value(context, arg) and arg.value.disables_build_cache]
...
def collect_jar_args(context: CertoraContext) -> List[str]:
    ...
    """
    return_list = []
    for arg in collect_args_with_jar_flags(context):
        attr = get_attr_value(context, arg)
        jar_flag = arg.value.jar_flag
...
class ProverParser(AttrUtil.CertoraArgumentParser):
    ...
    def format_help(self) -> str:
        ...
        Attr.ContextAttribute.print_attr_help()
