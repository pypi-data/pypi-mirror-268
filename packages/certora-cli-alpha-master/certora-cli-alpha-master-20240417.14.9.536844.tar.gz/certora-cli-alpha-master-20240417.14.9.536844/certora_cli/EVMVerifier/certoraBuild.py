...
from typing import Any, Dict, List, Tuple, Optional, Set, BinaryIO, Iterator, NoReturn
from Crypto.Hash import keccak

from EVMVerifier.certoraBuildCacheManager import CertoraBuildCacheManager, CachedFiles
from EVMVerifier.certoraCompilerParameters import CompilerParameters, SolcParameters
from EVMVerifier.certoraVerifyGenerator import CertoraVerifyGenerator
from EVMVerifier.certoraContractFuncs import Func, InternalFunc, STATEMUT
...
from EVMVerifier.Compiler.CompilerCollectorYul import CompilerLangYul
...
class ContractInSDC:
    def __init__(self, name: str, source_file: str, lang: str,
                 ...
                 compiler_parameters: Optional[CompilerParameters] = None,
                 ):
        self.name = name
...
def generate_compressed_finder(f: Func, internal_id: int, sym: int,
    ...
    for ty in f.fullArgs:
        ...
        # https://certora.atlassian.net/wiki/spaces/CER/pages/281411612/Certora+Autofinder+Binary+Format
        if finder_gen.is_decomposed(ty):
...
def generate_modifier_finder(f: Func, internal_id: int, sym: int,
                             compiler_collector: CompilerCollectorSol, compress: bool) -> Optional[Tuple[str, str]]:
    ...
    if len(compressed[0]) == 0:
        ...
        loggable_types = map(lambda e: f.fullArgs[e], compressed[0])
        ...
        for (logged_ty, logged_name) in zip(loggable_types, loggable_names):
            ...
            formal_strings.append(f"{logged_ty.get_source_and_location_str()} {logged_name}")
...
class CertoraBuildGenerator:
    ...
    def __init__(self, input_config: InputConfig, context: CertoraContext) -> None:
        ...
        self.compiler_coll_factory = CompilerCollectorFactory(self.context,
                                                              self.context.solc,
                                                              self.context.solc_map)  # depends only on input
        ...
        for c_file, c_name, c_is_lib in base_contract_files:
            ...
            for containing_file, func_def in [(c_file, f) for f in
                ...
                if body_node is not None and body_node["nodeType"] == "Block":
                    ...
                    where_tuple = (containing_file, body_node["src"])
    ...
    def _handle_optimize(self, contract_file_path: str, settings_dict: Dict[str, Any],
        ...
        if self.context.solc_optimize_map:
            ...
            if not self.context.disable_finder_friendly_optimizer and (
               self.context.solc_via_ir or self.context.solc_experimental_via_ir):
            ...
            elif not self.context.disable_finder_friendly_optimizer:
                compiler_version = compiler_collector.compiler_version
                major, minor, patch = compiler_version
                if major != 0:
    ...
    def _fill_codegen_related_options(self, contract_file_path: str, settings_dict: Dict[str, Any],
        ...
        @param settings_dict: data structure for sending to the solc compiler
        """
        if self.context.solc_via_ir or self.context.solc_experimental_via_ir:
        ...
        if "errors" in data:
            ...
            if len(severe_errors) > 0:
                for i, e in enumerate(severe_errors):
                    ...
                    if self.context.contract_compiler_skip_severe_warning_as_error or not fail_on_compilation_error:
    ...
    def collect_for_file(self,
        ...
        if not is_vyper:
            ...
            compiler_ver_to_run = get_relevant_compiler(Path(build_arg_contract_file), self.context.solc,
                                                        self.context.solc_map)
            """
            ...
            if reroute_main_path:
                ...
                case anyway.
                """
                main_path = compile_wd / self.context.solc_allow_path
            else:
        ...
        # Standard JSON
        input_for_solc = self.standard_json(Path(file_abs_path), build_arg_contract_file, self.context.remappings,
                                            compiler_collector)
    ...
    def build(self, certora_verify_generator: CertoraVerifyGenerator) -> None:
        ...
        for i, build_arg_contract_file in enumerate(sorted(self.input_config.sorted_files)):
            ...
            try:
                self.cwd_rel_in_sources = self.build_source_tree(sources, context)
            ...
            if compiler_lang == CompilerLangSol():
                added_finders_to_sdc, success = \
                    self.instrument_auto_finders(build_arg_contract_file, i,
                                                 sdc_pre_finders,
                                                 path_for_compiler_collector_file)  # type: Tuple[List[Tuple[Dict, SDC]],bool]
    ...
    def instrument_auto_finders(self, build_arg_contract_file: str, i: int,
                                ...
                                path_for_compiler_collector_file: str) -> Tuple[List[Tuple[Dict[str, InternalFunc],
                                                                                SDC]], bool]:
        ...
        self.context.file_to_contract[new_file] = self.context.file_to_contract[
            build_arg_contract_file]
        if self.context.solc_map and build_arg_contract_file in self.context.solc_map:
            self.context.solc_map[new_file] = self.context.solc_map[build_arg_contract_file]
    ...
    def resolve_slot(self, primary_contract: str, slot_name: str) -> str:
        ...
        solc_ver_to_run = get_relevant_compiler(Path(file_of_primary_contract), self.context.solc,
                                                self.context.solc_map)
...
def build_from_scratch(certora_build_generator: CertoraBuildGenerator,
    ...
    for sdc in certora_build_generator.SDCs.values():
        ...
        absolute_sources_dir = Path(os.path.normpath(Util.get_certora_sources_dir().absolute()))
    ...
    if saw_paths_not_in_sources:
        if build_cache_enabled:
            build_cache_logger.warning(f"Saw some paths that were not rooted in a common directory, " +
                                       f"thus disabling build cache: {paths_not_in_sources}")
...
def build_from_cache_or_scratch(context: CertoraContext,
                                ...
                                certora_verify_generator: CertoraVerifyGenerator,
                                certora_build_cache_manager: CertoraBuildCacheManager) -> Tuple[bool, bool, CachedFiles]:
    """
...
def build(context: CertoraContext, ignore_spec_syntax_check: bool = False) -> None:
    ...
    try:
        ...
        if Cv.mode_has_spec_file(context) and not context.build_only and not ignore_spec_syntax_check:
            ...
            if attr:
                ...
                spec_check_exit_code = Util.run_local_spec_check(with_typechecking=False, rules_to_check=context.rule,
                                                                 rules_to_exclude=context.exclude_rule)
        ...
        with cached_files.build_output_props_file.open() as build_output_props_handle:
            ...
            auto_finders_failed = "auto_finders_failed" in build_output_props and build_output_props["auto_finders_failed"]
