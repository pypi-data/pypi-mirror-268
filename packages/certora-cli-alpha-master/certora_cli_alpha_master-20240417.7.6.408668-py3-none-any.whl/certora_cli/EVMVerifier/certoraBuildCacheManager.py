...
from EVMVerifier.certoraContext import collect_args_build_cache_affecting, get_attr_value, \
...
class CachedFiles:
    ...
    def all_exist(self, main_cache_entry_dir: Path, sub_cache_key: str) -> bool:
        exist = self.certora_build_file.exists() and self.certora_build_file == main_cache_entry_dir / f"{sub_cache_key}.{CachedFiles.certora_build_suffix}"
        exist = exist and (main_cache_entry_dir / f"{sub_cache_key}.{CachedFiles.certora_build_suffix}").exists()
        exist = exist and self.build_output_props_file.exists() and self.build_output_props_file == main_cache_entry_dir / f"{sub_cache_key}.{CachedFiles.build_output_props_suffix}"
...
class CertoraBuildCacheManager:
    ...
    def build_from_cache(context: CertoraContext) -> Optional[CachedFiles]:
        ...
        for file_list_file in file_list_files:
            ...
            with open(file_list_file, 'r') as file_list_handle:
                ...
                if sub_cache_key_current_for_list is None:
                    ...
                    build_cache_logger.info(f"We have a match on build cache key {main_cache_key} and {sub_cache_key_current_for_list}")
        ...
        if sub_cache_key is None:
            build_cache_logger.info(f"All sub-cache-key file list files missed, cache miss on build cache key {main_cache_key}")
            return None
        ...
        if not certora_build_file.exists():
            build_cache_logger.warning(f"Got a cache-hit on build cache key {main_cache_key} and {sub_cache_key} "
                  f"but .certora_build.json file does not exist")
            return None

        if all_contract_files is None:  # should not be feasible
            build_cache_logger.warning(f"Got a cache-hit on build cache key {main_cache_key} and {sub_cache_key} "
                  f"but file list file does not exist")
            return None
    ...
    def save_build_cache(context: CertoraContext, cached_files: CachedFiles) -> None:
        ...
        if sub_cache_key is None:
            build_cache_logger.warning(f"Cannot save cache for main build cache key {main_cache_key} as sub-cache-key could not be computed")
            return

        if main_cache_entry_dir.exists():
            build_cache_logger.info(f"main build cache key already exists {main_cache_key}, saving sub build cache key {sub_cache_key}")
            if cached_files.all_exist(main_cache_entry_dir, sub_cache_key):
                ...
                build_cache_logger.debug(f"cache was corrupted, need to re-save build cache key {main_cache_key} and sub cache key {sub_cache_key}")
