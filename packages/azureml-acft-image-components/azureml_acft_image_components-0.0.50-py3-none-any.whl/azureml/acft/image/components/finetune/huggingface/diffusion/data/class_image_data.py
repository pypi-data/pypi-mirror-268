...
class ClassImageGenerator:
    ...
    def _load_pipeline(self) -> DiffusionPipeline:
        ...
        if self.azml_sd_pipeline is None:
            self.azml_sd_pipeline = AzuremlStableDiffusionPipeline.from_pretrained(
                self.pretrained_model_name_or_path, **self.kwargs
            )
            self.azml_sd_pipeline.unet.to(self.torch_dtype)
            self.azml_sd_pipeline.text_encoder.to(self.torch_dtype)
            # If pipeline is created in this method, then delete it after use.
            self._del_azml_pipeline = True
    ...
    def run_pipeline(self, dataloader: DataLoader, class_image_dir: str, cur_class_images: int = 0) -> None:
        """Run pipeline to generate class images.
        ...
        try:
            ...
            if self._del_azml_pipeline:
                del pipeline
                del self.azml_sd_pipeline
