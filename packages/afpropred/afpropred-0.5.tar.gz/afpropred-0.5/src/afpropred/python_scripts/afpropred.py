...
BLAST_BINARIES = {
    ...
    "Darwin": "/../blast_binaries/mac/psiblast",
    "Windows": "/../blast_binaries/windows/psiblast.exe",
}

def get_psiblast_path(def_file_path):
    operating_system = platform.system()
    psiblast_path = def_file_path + BLAST_BINARIES.get(operating_system)
...
blast = get_psiblast_path(nf_path)
...
def gen_pssm(fasta_path, pssm_path):
    ...
    for i in listdir:
        ...
        cmd = blast + " -out "+pssm_path+"/pssm_raw1/"+filename+".homologs -outfmt 7 -query "+fasta_path+"/"+filename+".fasta -db "+nf_path+"/../swissprot/swissprot -evalue 0.001 -word_size 3 -max_target_seqs 6000 -num_threads 10 -gapopen 11 -gapextend 1 -matrix BLOSUM62 -comp_based_stats T -num_iterations 3 -out_pssm "+pssm_path+"/pssm_raw1/"+filename+".cptpssm -out_ascii_pssm "+pssm_path+"/pssm_raw/"+filename+".pssm"
        os.system(cmd)
