...
def get_hate(table, column, enum, description, verbose, dot, client, gpt4):
    ...
    data_file_path = pkg_resources.resource_filename('autopipeline', 'data/6/hate.csv')
...
def get_hate_class(table, column, enum, description, verbose, dot, client, gpt4):
    ...
    data_file_path = pkg_resources.resource_filename('autopipeline', 'data/7/hate.csv')
    df = pd.read_csv(data_file_path)
...
def tree_path(user_query, columns, description, verbose, client, gpt4, udf=""):
    ...
    if gpt4:
        ...
        if verbose:
            ...
            print("******Number of prompt tokens for 'tree_path': ", response.usage.prompt_tokens)
            print("******Number of answer tokens for 'tree_path': ", response.usage.completion_tokens)
            print("******Number of total tokens for 'tree_path': ", response.usage.total_tokens)
    else:
        response = client.chat.completions.create(
...
def schema_gpt_tree(user_query, column_description, description, function_chain, function_list, verbose, client, gpt4, udf = None):
    ...
    if gpt4:
        ...
        if verbose:
            ...
            print("******Number of prompt tokens for 'schema_gpt_tree': ", response.usage.prompt_tokens)
            print("******Number of answer tokens for 'schema_gpt_tree': ", response.usage.completion_tokens)
            print("******Number of total tokens for 'schema_gpt_tree': ", response.usage.total_tokens)
    else:
        response = client.chat.completions.create(
            model="gpt-3.5-turbo-0125",
            messages=messages,
    ...
    while True:
        ...
        if f in added_functions:
            ...
            if f == "get_trigger":
                ...
                table, enum, description, dot = globals()[f](user_query, table, col, enum, description, verbose, dot, client, gpt4)
        ...
        if is_notebook() or is_notebook_colab():
            ...
            output_path = dot.render(filename=output_filename, format='png', cleanup=True)
