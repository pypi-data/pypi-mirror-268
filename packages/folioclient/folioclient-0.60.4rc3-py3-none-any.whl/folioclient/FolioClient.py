...
class FolioClient:
    ...
    def login(self):
        ...
        # Poppy and later
        retry_factor = float(os.environ.get("FOLIOCLIENT_AUTH_TIMEOUT_RETRY_FACTOR", "3"))
        max_retries = int(os.environ.get("FOLIOCLIENT_MAX_AUTH_TIMEOUT_RETRIES", "0"))
        retry_delay = 10
        ...
        try:
            for retry in range(max_retries + 1):
                try:
                    ...
                    if req.status_code in [501, 502, 503, 504]:
                        if retry == max_retries:
                            logging.exception(
                                "Requesting new auth token timed out or experienced a network "
                                "error, maximum number of retries reached, giving up."
                            )
                            raise
                        logging.info(
                            f"Requesting new auth token timed out or experienced a network error. Retrying"
                            f" again in {retry_delay} seconds. Retry {retry + 1} of {max_retries}"
                        )
                    ...
                    try:
                        ...
                        if req.status_code in [501, 502, 503, 504]:
                            if retry == max_retries:
                                ...
                                raise
                            logging.info(
                                f"Requesting new auth token timed out or experienced a network error."
                                f" Retrying again in {retry_delay} seconds. Retry {retry + 1}"
                                f" of {max_retries}"
                            )
                            time.sleep(retry_delay)
                            retry_delay *= retry_factor
