...
class Client(object):
    ...
    def _gen_new_put_sql(self, outcome_obj):
        ...
        local_file_list = self._get_real_local_file_list(outcome_obj['request']['localPaths'], prefix)
        if len(local_file_list) == 0:
            raise Exception('No local file to put into volume')
        sql = 'put '
        for i in range(len(local_file_list)):
            sql += "'" + local_file_list[i] + "'"
            if i != len(local_file_list) - 1:
                sql += ", "
        sql += ' TO VOLUME ' + outcome_obj['request']['volume']
    ...
    def _get_real_local_file_list(self, local_path_list: [], prefix: str):
        if len(local_path_list) == 0:
            raise Exception('No local file to put into volume')
        file_names = []
        for local_path in local_path_list:
            if local_path[-1] == '*' or local_path[-1] == '/':
                local_dir = local_path if local_path[-1] == '/' else local_path[:-1]
                file_names += [f for f in os.listdir(local_dir)
                               if os.path.isfile(os.path.join(local_dir, f)) and f.startswith(prefix)]
            else:
                file_names.append(local_path)
        return file_names
