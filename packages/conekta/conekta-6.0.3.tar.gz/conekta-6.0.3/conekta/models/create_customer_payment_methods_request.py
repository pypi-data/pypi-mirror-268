...
class CreateCustomerPaymentMethodsRequest(BaseModel):
    ...
    def actual_instance_must_validate_oneof(cls, v):
        ...
        if not isinstance(v, PaymentMethodCashRequest):
        ...
        if not isinstance(v, PaymentMethodSpeiRequest):
            error_messages.append(f"Error! Input type `{type(v)}` is not `PaymentMethodSpeiRequest`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in CreateCustomerPaymentMethodsRequest with oneOf schemas: PaymentMethodCardRequest, PaymentMethodCashRequest, PaymentMethodSpeiRequest. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into CreateCustomerPaymentMethodsRequest with oneOf schemas: PaymentMethodCardRequest, PaymentMethodCashRequest, PaymentMethodSpeiRequest. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into CreateCustomerPaymentMethodsRequest with oneOf schemas: PaymentMethodCardRequest, PaymentMethodCashRequest, PaymentMethodSpeiRequest. Details: " + ", ".join(error_messages))
        else:
            return instance
