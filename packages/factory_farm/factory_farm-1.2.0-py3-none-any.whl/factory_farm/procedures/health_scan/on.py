...
def turn_on_health_check (
	...
	env ["factory_farm___status_path"] = the_path
		
	env ["factory_farm___harbor_host"] = "0.0.0.0"
	env ["factory_farm___harbor_port"] = str (implicit_procedure ["port"])	
	
	
	if (type (relative_path) == str and len (relative_path) >= 1):
		name = os.path.relpath (the_path, relative_path)
		env ["factory_farm___status_relative_path"] = relative_path
	else:
		name = "scan " + the_path
		env ["factory_farm___status_relative_path"] = ''
