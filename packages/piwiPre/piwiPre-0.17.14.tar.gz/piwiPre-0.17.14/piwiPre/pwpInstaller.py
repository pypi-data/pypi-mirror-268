...
class PwpUi:
    def __init__(self, args, installer):
        ...
        if installer.args.mode == "test":
            ...
            self.root.after(4 * 1000, lambda: self.root.quit())
...
class Installer:
    ...
    def install_piwipre(self):
        ...
        self.download(f"{base}/{target}/pwpConfigurator-{target}.exe",
                      f"{self.piwipre_path}/pwpConfigurator.exe")
        shutil.copy2(f"{self.piwipre_path}/pwpConfigurator.exe",
                     f"{self.piwipre_path}/pwpConfigurator-{target}.exe")

        src = f"{base}/{target}/pwpInstaller-{target}.exe"
        dst1 = os.path.abspath(f"{self.piwipre_path}/pwpInstaller.exe")
        dst2 = os.path.abspath(f"{self.piwipre_path}/pwpInstaller-{target}.exe")
        myself = os.path.abspath(sys.orig_argv[0])

        if myself == dst1:
            self.download(src, dst2)
            self.warning(f"Latest package version '{target}' installed as '{dst2}'")
            self.warning(f"You can copy it: copy  {dst2}  {dst1}")
        elif myself == dst2:
            self.download(src, dst1)
            self.warning(f"Latest package version '{target}' installed as '{dst1}'")
            self.warning(f"You can copy it: copy  {dst1}  {dst2}")
        else:
            self.download(src, dst1)
            shutil.copy2(dst1, dst2)
            self.warning(f"Latest package version '{target}' installed as '{dst1}' ")
            self.warning(f"and also copied as '{dst2}' ")
    ...
    def run_min_test(self, system_dir=True):
        ...
        test_dir = (os.environ['PROGRAMFILES(X86)'] if system_dir else '.') + '\\piwiPreTmp'   # noqa
    ...
    def run(self):
        ...
        if self.args.user:
            ...
            self.msg("No elevation, due to cmdline arguments")
        else:
            self.error("user should have been passed on cmdline")

        if not self.args.piwipre and not self.args.ffmpeg and not self.args.mariadb and self.args.test:
            self.run_min_test()
