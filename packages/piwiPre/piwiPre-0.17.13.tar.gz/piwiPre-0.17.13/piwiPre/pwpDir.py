...
class PwpDirEntry(ItemEntry):
    ...
    def __init__(self, local: str, config: PwpConfig, context: str):
        ...
        :param config: config
        :param context: either local, web or album.
        """
        ...
        elif context == 'web' and ACTOR.ssh_connection and config['remote-web'] and config['enable-remote-web']:
            if not local.startswith(config['web']):
                raise PwpInternalError(f"Illegal dir path {local} does not start with web '{config['web']}'")
            self.remote = local.replace(config['web'], config['remote-web'], 1)
...
class PwpFileEntry(ItemEntry):
    def __init__(self, local: str, context: str, config: PwpConfig, father: PwpDirEntry or None = None):
        ...
        self.remote = None
        if ACTOR.ssh_connection and self.config['enable-remote-web'] and self.local.startswith(self.config['web']):
            self.remote = self.local.replace(self.config['web'], self.config['remote-web'])
