...
class PwpActor:
    ...
    def connect_ssh(self, config: dict):
        ...
        self.remote_port = int(config['remote-port']) if config['remote-port'] else None
        ...
        # CAVEAT: here, we want to connect to SSH even if enable-remote-album or enable-remote-web are False,
        # because we will turn one of them True in the course of tests.

        remote = (config['remote-album'] or config['remote-web']) and self.remote_host and self.remote_port
    ...
    def connect_sql(self, config: dict):
        ...
        """
        if (config['sql-host'] is None or config['sql-port'] is None or config['sql-user'] is None
                or config['sql-pwd'] is None) or not config['sql-database'] or not config['enable-database'] \
                or config['dryrun']:
        ...
        if self.sql_first_album is None:
