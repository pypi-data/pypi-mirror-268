...
def update_epub(path_to_epub, config_file_path, debug):
    try:
        ...
        compress_images_config: bool = ficimage_config.get(
            "compress_images", True)
        default_image_format_config: str = ficimage_config.get(
            "default_image_format", "JPEG")
        if str(default_image_format_config).lower() not in ("jpg", "jpeg", "png"):
            default_image_format_config = "JPEG"
        max_image_size_config: int = ficimage_config.get(
            "max_image_size", 1_000_000)

        # if sys.platform.startswith('win32') or sys.platform.startswith('cygwin'):
        #     file_name: str = path_to_epub.split('\\')[-1].split('.epub')[0]
        # else:
        #     file_name: str = path_to_epub.split('/')[-1].split('.epub')[0]
                    ...
                    for image in images:
                        if image is None:
                            ...
                            try:
                                ...
                                image_path = f"images/" \
                                             f"{item_file_name}_image_{images.index(image)}.{
                                ...
                                book.add_item(img)
                                image.replace_with(
                                    BeautifulSoup(new_image, 'lxml-xml'))
                            except Exception as e:
                                print(f"Error with image {images.index(
                                    image) + 1}: {e}, skipping ...")
            ...
            if number_of_all_images_found != 0:
                try:
                    ...
                    new_filename = os.path.join(
                        epub_dir, f"[FicImage]{file_name}.epub")
                    if debug:
                ...
                for k, v in images_downloaded.items():
                    print(f"{k}{' ' * (18 - len(k))
                    }\t{v[0]} out of {v[1]} images downloaded")
                print("=" * 54)
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Update a FicHub epub file with images.")
    parser.add_argument("-p", "--path_to_epub",
                        help="The path to the FicHub epub file.")
