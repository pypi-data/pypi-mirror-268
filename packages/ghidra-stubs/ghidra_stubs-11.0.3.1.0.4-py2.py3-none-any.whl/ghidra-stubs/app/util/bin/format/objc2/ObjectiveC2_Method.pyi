import ghidra.app.util.bin.format.objectiveC
import ghidra.program.model.data
import ghidra.program.model.symbol
import java.lang


class ObjectiveC2_Method(ghidra.app.util.bin.format.objectiveC.ObjectiveC_Method):




    def __init__(self, state: ghidra.app.util.bin.format.objc2.ObjectiveC2_State, reader: ghidra.app.util.bin.BinaryReader, methodType: ghidra.app.util.bin.format.objectiveC.ObjectiveC_MethodType, isSmallList: bool): ...



    def applyTo(self, namespace: ghidra.program.model.symbol.Namespace) -> None: ...

    def equals(self, __a0: object) -> bool: ...

    def getClass(self) -> java.lang.Class: ...

    def getImplementation(self) -> long: ...

    def getIndex(self) -> long: ...

    def getMethodType(self) -> ghidra.app.util.bin.format.objectiveC.ObjectiveC_MethodType: ...

    def getName(self) -> unicode: ...

    def getTypes(self) -> unicode: ...

    def hashCode(self) -> int: ...

    def notify(self) -> None: ...

    def notifyAll(self) -> None: ...

    def toDataType(self) -> ghidra.program.model.data.DataType: ...

    def toString(self) -> unicode: ...

    @overload
    def wait(self) -> None: ...

    @overload
    def wait(self, __a0: long) -> None: ...

    @overload
    def wait(self, __a0: long, __a1: int) -> None: ...

    @property
    def implementation(self) -> long: ...

    @property
    def name(self) -> unicode: ...

    @property
    def types(self) -> unicode: ...