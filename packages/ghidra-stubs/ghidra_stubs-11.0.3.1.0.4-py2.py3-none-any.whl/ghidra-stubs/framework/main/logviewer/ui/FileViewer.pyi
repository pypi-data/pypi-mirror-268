...
class FileViewer(javax.swing.JPanel, java.util.Observer):
    ...
    UI for viewing the contents of very large files efficiently. Pieces of a file are read in using
     the ChunkReader, which are then displayed line-by-line in FVTable.  As users
     scroll up/down, new sections of the file are swapped in as appropriate.
 
     Notes:
 
     The viewer consists of a simple JTable and a custom JSlider. The table displays lines of
     ...
     Because only part of the file is loaded into the viewable table at any given time, the
        ...
        we use our own slider implementation (FVSlider) and manage the size/position
     ...
     Communication between modules (the table, the slider, the viewport utility, etc...) is done
