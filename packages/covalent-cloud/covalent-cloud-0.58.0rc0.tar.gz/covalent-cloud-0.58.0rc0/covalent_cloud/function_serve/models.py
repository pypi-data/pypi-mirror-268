...
from covalent_cloud.function_serve.common import ServiceStatus, SupportedMethods
...
class DeploymentInfo(BaseModel):
    ...
    @staticmethod
    def from_function_record(data: Dict[str, Any]) -> "DeploymentInfo":
        """
        Create a DeploymentInfo object from a function's database record.
        """
    ...
    def attach_route_methods(self, overwrite: bool = False):
        """
        Attach methods to the DeploymentInfo object for each route.
        """
    ...
    def reload(self):
        ...
        response = deployment_client.get(
            f"/functions/{self.function_id}",
        )

        new_deployment_info = DeploymentInfo.from_function_record(response.json())
        self.__dict__.update(new_deployment_info.__dict__)

        # Attach route methods for ease of use
        self.attach_route_methods(overwrite=True)

    def _stringify_route(self, route: RouteInfo) -> Table:
        """Get rich Table representation of route info."""

        table = Table(
            title=f"{route.method.value} {route.route}",
            box=None,
            show_header=False,
            padding=(0, 1),
        )
        table.add_row("Streaming", "Yes" if route.streaming else "No")
        table.add_row("Description", route.description)
    ...
    def __str__(self) -> str:
        ...
        info_panel = Panel(info_table, title="Deployment Information", expand=False)

        # Prepare route info as Tables inside Panels
        route_panels = [Panel(self._stringify_route(route), expand=False) for route in self.routes]
        ...
        with console.capture() as capture:
            ...
            for panel in route_panels:
                console.print(panel)
            if self.token:
                console.print(f"Authorization token: {self.token}")
