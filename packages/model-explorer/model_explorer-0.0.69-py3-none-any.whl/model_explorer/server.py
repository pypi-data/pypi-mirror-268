...
def start(host='localhost',
  ...
  Args:
    ...
    model_paths: A list of file paths to the models. Default to empty.
    graphs: The object returned by the convert method from an adapter.
        Default to None.
    ...
    colab: Whether to use in colab. When set to true, the UI will be shown in a
        embedded iFrame in a colab cell.
    colab_height: The height of the embedded iFrame.
  ...
  if colab:
    print('ℹ️ Please re-run `model_explorer.start(...)` in each new session')
    print()
  ...
  if len(model_paths) > 0:
    abs_model_paths = [os.path.abspath(
        os.path.expanduser(x)) for x in model_paths]
  ...
  def start_server():
    ...
    if graphs is not None:
      url_params.append('load_graphs_json=1')
    if len(abs_model_paths) > 0:
      url_params.append(f'model_paths={",".join(abs_model_paths)}')
  ...
  def embed_in_colab():
    ...
    shell = """
      (async () => {
          ...
          if ('%MODEL_PATHS%' !== '') {
            url += ('&model_paths=' + '%MODEL_PATHS%');
          }
          if ('%LOAD_GRAPHS_JSON%' === 'True') {
            url += '&load_graphs_json=1';
          }
    ...
    """
    is_hosted_runtime = 'google.colab' in sys.modules
    replacements = [
        ("%PORT%", f"{colab_port}"),
        ("%HOSTED_RUNTIME%", f"{is_hosted_runtime}"),
        ("%HEIGHT%", f"{colab_height}"),
        ("%MODEL_PATHS%", f"{','.join(abs_model_paths)}"),
        ("%LOAD_GRAPHS_JSON%", f"{graphs is not None}"),
    ]
