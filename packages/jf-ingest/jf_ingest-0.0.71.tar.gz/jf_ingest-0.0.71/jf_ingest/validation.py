...
from jf_ingest.jf_git.adapters import GitAdapter, StandardizedRepository
...
from jf_ingest.utils import retry_for_429s, test_jira_or_git_object_access
...
def validate_jira(
    ...
    projects = {p.key for p in retry_for_429s(jira_connection.projects)}

    # Marry the discovered project data to anything that we were told to in/exclude
    if config.include_projects:
        projects = projects.intersection(set(config.include_projects))
    if config.exclude_projects:
        projects = projects.difference(set(config.exclude_projects))
    ...
    if config.include_fields:
        healthcheck_result.included_inaccessible_fields = [
            f
            for f in config.include_fields
            not in [field.name for field in retry_for_429s(jira_connection.fields)]
        ]
    ...
    if healthcheck_result.boards.access_ok:
        boards = retry_for_429s(jira_connection.boards)
    ...
    healthcheck_result.successful &= all(
        ...
        + [par.issues for par in healthcheck_result.project_data.values()]
        + [par.versions for par in healthcheck_result.project_data.values()]
        + [par.components for par in healthcheck_result.project_data.values()]
...
def validate_git(git_configs: list[GitConfig]) -> list[GitConnectionHealthCheckResult]:
    ...
    for i, git_config in enumerate(git_configs, start=1):
        ...
        for org in sorted(git_adapter.get_organizations(), key=lambda org: org.login):
            ...
            if git_adapter.client.uses_jwt:
            ...
            try:
                ...
                repos: list[StandardizedRepository] = git_adapter.get_repos(org)
