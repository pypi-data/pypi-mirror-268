...
def test_ci_init(monkeypatch, mocker, empty_config) -> None:
    ...
    with tempfile.TemporaryDirectory() as statedir:
        ...
        with with_dagster_yaml(DAGSTER_CLOUD_YAML) as project_dir:
            ...
            mocker.patch(
                "dagster_plus_cli.commands.ci.get_deployment_from_context",
...
def test_ci_build_docker(
    ...
    mocker.patch(
        "dagster_plus_cli.commands.ci.utils.get_registry_info",
        return_value={"registry_url": "example.com/image-registry"},
    )
    build_image = mocker.patch("dagster_plus_cli.docker_utils.build_image", return_value=0)
    upload_image = mocker.patch("dagster_plus_cli.docker_utils.upload_image", return_value=0)
    mark_cli_event = mocker.patch("dagster_plus_cli.gql.mark_cli_event", return_value=True)
...
def test_ci_deploy_docker(
    ...
    mocker.patch("dagster_plus_cli.docker_utils.build_image", return_value=0)
    mocker.patch("dagster_plus_cli.docker_utils.upload_image", return_value=0)
    update_code_location = mocker.patch("dagster_plus_cli.gql.add_or_update_code_location")
    wait_for_load = mocker.patch("dagster_plus_cli.commands.ci.wait_for_load")
    mark_cli_event = mocker.patch("dagster_plus_cli.gql.mark_cli_event", return_value=True)
...
def test_ci_deploy_pex(
    ...
    build_upload_pex = mocker.patch(
        "dagster_plus_cli.pex_utils.build_upload_pex",
...
def test_ci_branch_deployment(
    ...
    monkeypatch.setenv("DAGSTER_CLOUD_API_TOKEN", "fake-token")
    mocker.patch(
        "dagster_plus_cli.commands.ci.get_deployment_from_context", return_value="branch-dep-name"
    )
    with with_dagster_yaml(DAGSTER_CLOUD_YAML) as project_dir:
        runner = CliRunner()
        result = runner.invoke(
