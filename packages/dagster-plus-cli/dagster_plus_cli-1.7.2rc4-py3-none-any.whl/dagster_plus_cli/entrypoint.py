...
from dagster_plus_cli.utils import create_stub_app
...
has_dagster_cloud = (
    importlib.util.find_spec("dagster_cloud") is not None
    or importlib.util.find_spec("dagster_plus") is not None
)


if has_dagster_cloud:
    from dagster_plus.agent.cli import app as agent_app
    from dagster_plus.pex.grpc.server.cli import app as pex_app
else:
    agent_app = create_stub_app("dagster-plus")
    pex_app = create_stub_app("dagster-plus")
...
def version_callback(value: bool):
    if value:
        typer.echo(f"Dagster Plus version: {__version__}")
        raise typer.Exit()
