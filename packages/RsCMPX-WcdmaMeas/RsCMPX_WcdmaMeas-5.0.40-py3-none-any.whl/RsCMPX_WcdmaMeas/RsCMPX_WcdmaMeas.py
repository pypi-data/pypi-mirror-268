...
class RsCMPX_WcdmaMeas:
	...
	@classmethod
	def from_existing_session(cls, session: object, options: str = None) -> 'RsCMPX_WcdmaMeas':
		"""Creates a new RsCMPX_WcdmaMeas object with the entered 'session' reused. \n
	...
	def __str__(self) -> str:
		if self._core.io:
			return f"RsCMPX_WcdmaMeas session '{self._core.io.resource_name}'"
		else:
			return f"RsCMPX_WcdmaMeas with session closed"
	...
	def close(self) -> None:
		"""Closes the active RsCMPX_WcdmaMeas session."""
		self._core.io.close()
	...
	def _sync_to_custom_properties(self, cloned: 'RsCMPX_WcdmaMeas') -> None:
	...
	@property
	def wcdmaMeas(self):
		"""wcdmaMeas commands group. 5 Sub-classes, 0 commands."""
		if not hasattr(self, '_wcdmaMeas'):
			from .Implementations.WcdmaMeas import WcdmaMeasCls
			self._wcdmaMeas = WcdmaMeasCls(self._core, self._cmd_group)
		return self._wcdmaMeas

	def clone(self) -> 'RsCMPX_WcdmaMeas':
		"""Creates a deep copy of the RsCMPX_WcdmaMeas object. Also copies:
		...
		cloned = RsCMPX_WcdmaMeas.from_existing_session(self.get_session_handle(), self._options)
