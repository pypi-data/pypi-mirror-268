...
class CurrentCls:
	...
	def fetch(self) -> List[float]:
		...
		Snippet: value: List[float] = driver.wcdmaMeas.prach.trace.perror.chip.current.fetch() \n
		Return the values of the phase error vs chip diagram. \n
		Suppressed linked return values: reliability \n
			:return: phase_error_chip: Comma-separated list of 4096 values, one per chip of the preselected preamble"""
	...
	def read(self) -> List[float]:
		...
		Snippet: value: List[float] = driver.wcdmaMeas.prach.trace.perror.chip.current.read() \n
		Return the values of the phase error vs chip diagram. \n
		Suppressed linked return values: reliability \n
			:return: phase_error_chip: Comma-separated list of 4096 values, one per chip of the preselected preamble"""
		suppressed = ArgSingleSuppressed(0, DataType.Integer, False, 1, 'Reliability')
