...
class CurrentCls:
	...
	def fetch(self) -> List[float]:
		...
		Snippet: value: List[float] = driver.wcdmaMeas.prach.trace.evMagnitude.peak.current.fetch() \n
	...
	def read(self) -> List[float]:
		...
		Snippet: value: List[float] = driver.wcdmaMeas.prach.trace.evMagnitude.peak.current.read() \n
		Return the EVM RMS and peak values for each measured preamble. \n
		Suppressed linked return values: reliability \n
			:return: evm: Comma-separated list of values, one result per measured preamble (see method RsCMPX_WcdmaMeas.Configure.WcdmaMeas.Prach.mpreamble)"""
		suppressed = ArgSingleSuppressed(0, DataType.Integer, False, 1, 'Reliability')
