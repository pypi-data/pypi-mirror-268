...
class StandardDevCls:
	...
	def fetch(self, carrier=repcap.Carrier.Default) -> List[float]:
		...
		Snippet: value: List[float] = driver.wcdmaMeas.multiEval.carrier.trace.merror.peak.standardDev.fetch(carrier = repcap.Carrier.Default) \n
		...
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'FETCh:WCDMa:MEASurement<Instance>:MEValuation:CARRier{carrier_cmd_val}:TRACe:MERRor:PEAK:SDEViation?', suppressed)
		return response
	...
	def read(self, carrier=repcap.Carrier.Default) -> List[float]:
		...
		Snippet: value: List[float] = driver.wcdmaMeas.multiEval.carrier.trace.merror.peak.standardDev.read(carrier = repcap.Carrier.Default) \n
		Returns the values of the peak magnitude error traces for up to 120 slots. Each current value is determined for a
		half-slot or a full-slot, depending on the measurement period (see method RsCMPX_WcdmaMeas.Configure.WcdmaMeas.MultiEval.
		Mperiod.modulation) . The number of results depends on the measurement length (see method RsCMPX_WcdmaMeas.Configure.
		WcdmaMeas.MultiEval.msCount) . The results of the current, average, maximum and standard deviation traces can be
		...
		carrier_cmd_val = self._cmd_group.get_repcap_cmd_value(carrier, repcap.Carrier)
		suppressed = ArgSingleSuppressed(0, DataType.Integer, False, 1, 'Reliability')
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'READ:WCDMa:MEASurement<Instance>:MEValuation:CARRier{carrier_cmd_val}:TRACe:MERRor:PEAK:SDEViation?', suppressed)
		return response
