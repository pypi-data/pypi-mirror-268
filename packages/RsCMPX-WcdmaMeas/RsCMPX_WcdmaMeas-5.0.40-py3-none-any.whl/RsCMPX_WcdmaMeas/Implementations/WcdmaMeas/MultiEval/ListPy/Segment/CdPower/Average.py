...
class AverageCls:
	...
	def fetch(self, segment=repcap.Segment.Default) -> FetchStruct:
		...
		Snippet: value: FetchStruct = driver.wcdmaMeas.multiEval.listPy.segment.cdPower.average.fetch(segment = repcap.Segment.Default) \n
		Returns the RMS CDP vs slot results for segment <no> in list mode. \n
			:param segment: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Segment')
			:return: structure: for return value, see the help for FetchStruct structure arguments."""
		segment_cmd_val = self._cmd_group.get_repcap_cmd_value(segment, repcap.Segment)
		return self._core.io.query_struct(f'FETCh:WCDMa:MEASurement<Instance>:MEValuation:LIST:SEGMent{segment_cmd_val}:CDPower:AVERage?', self.__class__.FetchStruct())
