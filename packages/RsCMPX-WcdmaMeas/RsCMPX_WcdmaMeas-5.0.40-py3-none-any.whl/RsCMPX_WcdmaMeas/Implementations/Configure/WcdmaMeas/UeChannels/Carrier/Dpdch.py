...
class DpdchCls:
	...
	def set(self, enable: bool, beta_factor: int, spreading_factor: int, carrier=repcap.Carrier.Default) -> None:
		...
		Snippet: driver.configure.wcdmaMeas.ueChannels.carrier.dpdch.set(enable = False, beta_factor = 1, spreading_factor = 1, carrier = repcap.Carrier.Default) \n
		...
		self._core.io.write(f'CONFigure:WCDMa:MEASurement<Instance>:UECHannels:CARRier{carrier_cmd_val}:DPDCh {param}'.rstrip())

	# noinspection PyTypeChecker
	class DpdchStruct(StructBase):
	...
	def get(self, carrier=repcap.Carrier.Default) -> DpdchStruct:
		"""SCPI: CONFigure:WCDMa:MEASurement<instance>:UECHannels:CARRier<carrier>:DPDCh \n
		Snippet: value: DpdchStruct = driver.configure.wcdmaMeas.ueChannels.carrier.dpdch.get(carrier = repcap.Carrier.Default) \n
		Specifies the presence of a DPDCH in the uplink signal and the beta factor and spreading factor of the channel. \n
			:param carrier: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Carrier')
			:return: structure: for return value, see the help for DpdchStruct structure arguments."""
		carrier_cmd_val = self._cmd_group.get_repcap_cmd_value(carrier, repcap.Carrier)
		return self._core.io.query_struct(f'CONFigure:WCDMa:MEASurement<Instance>:UECHannels:CARRier{carrier_cmd_val}:DPDCh?', self.__class__.DpdchStruct())
