...
class UserDefinedCls:
	...
	def set(self, nominal_max_power: float, upper_limit: float, lower_limit: float) -> None:
		...
		Snippet: driver.configure.wcdmaMeas.tpc.limit.ilpControl.maxPower.userDefined.set(nominal_max_power = 1.0, upper_limit = 1.0, lower_limit = 1.0) \n
		Sets the user-defined maximum output power limits for the Inner Loop Power Control mode. To activate the usage of this
		limit set, see method RsCMPX_WcdmaMeas.Configure.WcdmaMeas.Tpc.Limit.IlpControl.MaxPower.set. \n
		...
		self._core.io.write(f'CONFigure:WCDMa:MEASurement<Instance>:TPC:LIMit:ILPControl:MAXPower:UDEFined {param}'.rstrip())
	...
	def get(self) -> UserDefinedStruct:
		"""SCPI: CONFigure:WCDMa:MEASurement<instance>:TPC:LIMit:ILPControl:MAXPower:UDEFined \n
		Snippet: value: UserDefinedStruct = driver.configure.wcdmaMeas.tpc.limit.ilpControl.maxPower.userDefined.get() \n
		Sets the user-defined maximum output power limits for the Inner Loop Power Control mode. To activate the usage of this
		limit set, see method RsCMPX_WcdmaMeas.Configure.WcdmaMeas.Tpc.Limit.IlpControl.MaxPower.set. \n
			:return: structure: for return value, see the help for UserDefinedStruct structure arguments."""
		return self._core.io.query_struct(f'CONFigure:WCDMa:MEASurement<Instance>:TPC:LIMit:ILPControl:MAXPower:UDEFined?', self.__class__.UserDefinedStruct())
