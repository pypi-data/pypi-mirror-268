...
class MerrorCls:
	...
	def set(self, rms: float or bool, peak: float or bool) -> None:
		...
		Snippet: driver.configure.wcdmaMeas.prach.limit.merror.set(rms = 1.0, peak = 1.0) \n
		...
		self._core.io.write(f'CONFigure:WCDMa:MEASurement<Instance>:PRACh:LIMit:MERRor {param}'.rstrip())
	...
	def get(self) -> MerrorStruct:
		"""SCPI: CONFigure:WCDMa:MEASurement<instance>:PRACh:LIMit:MERRor \n
		Snippet: value: MerrorStruct = driver.configure.wcdmaMeas.prach.limit.merror.get() \n
		Defines upper limits for the RMS and peak values of the magnitude error. \n
			:return: structure: for return value, see the help for MerrorStruct structure arguments."""
		return self._core.io.query_struct(f'CONFigure:WCDMa:MEASurement<Instance>:PRACh:LIMit:MERRor?', self.__class__.MerrorStruct())
