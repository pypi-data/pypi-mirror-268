...
class StateCls:
	...
	def fetch(self, timeout: float = None, target_main_state: enums.TargetMainState = None, target_sync_state: enums.TargetSyncState = None) -> enums.ResourceState:
		...
		Snippet: value: enums.ResourceState = driver.wcdmaMeas.tpc.state.fetch(timeout = 1.0, target_main_state = enums.TargetMainState.OFF, target_sync_state = enums.TargetSyncState.ADJusted) \n
		...
		response = self._core.io.query_str(f'FETCh:WCDMa:MEASurement<Instance>:TPC:STATe? {param}'.rstrip())
