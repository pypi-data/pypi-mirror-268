...
class CurrentCls:
	...
	def fetch(self) -> List[float]:
		...
		Snippet: value: List[float] = driver.wcdmaMeas.prach.trace.merror.peak.current.fetch() \n
	...
	def read(self) -> List[float]:
		...
		Snippet: value: List[float] = driver.wcdmaMeas.prach.trace.merror.peak.current.read() \n
		Return the magnitude error RMS and peak values for each measured preamble. \n
		Suppressed linked return values: reliability \n
			:return: magnitude_error: Comma-separated list of values, one result per measured preamble (see method RsCMPX_WcdmaMeas.Configure.WcdmaMeas.Prach.mpreamble)"""
		suppressed = ArgSingleSuppressed(0, DataType.Integer, False, 1, 'Reliability')
