...
class CurrentCls:
	...
	class FetchStruct(StructBase):
		"""Response structure. Fields: \n
			...
			- Dpcch: List[float]: RMS CDE values for the indicated channels
			- Dpdch: List[float]: RMS CDE values for the indicated channels
			- Hsdpcch: List[float]: RMS CDE values for the indicated channels
			- Edpcch: List[float]: RMS CDE values for the indicated channels
			- Edpdch_1: List[float]: RMS CDE values for the indicated channels
			- Edpdch_2: List[float]: RMS CDE values for the indicated channels
			- Edpdch_3: List[float]: RMS CDE values for the indicated channels
			- Edpdch_4: List[float]: RMS CDE values for the indicated channels"""
		__meta_args_list = [
	...
	def fetch(self) -> FetchStruct:
		...
		Snippet: value: FetchStruct = driver.wcdmaMeas.multiEval.listPy.cdError.current.fetch() \n
		Return the RMS CDE vs. slot results in list mode. The values listed below in curly brackets {} are returned for the
		...
		return self._core.io.query_struct(f'FETCh:WCDMa:MEASurement<Instance>:MEValuation:LIST:CDERror:CURRent?', self.__class__.FetchStruct())
