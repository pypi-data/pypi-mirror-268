...
class AllCls:
	...
	def fetch(self, timeout: float = None, target_main_state: enums.TargetMainState = None, target_sync_state: enums.TargetSyncState = None) -> List[enums.ResourceState]:
		...
		Snippet: value: List[enums.ResourceState] = driver.wcdmaMeas.olpControl.state.all.fetch(timeout = 1.0, target_main_state = enums.TargetMainState.OFF, target_sync_state = enums.TargetSyncState.ADJusted) \n
		...
		response = self._core.io.query_str(f'FETCh:WCDMa:MEASurement<Instance>:OLPControl:STATe:ALL? {param}'.rstrip())
