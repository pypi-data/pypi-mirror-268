...
class ApPowerCls:
	...
	def get_pathloss(self) -> float:
		...
		Snippet: value: float = driver.sense.uplink.setb.apPower.get_pathloss() \n
		Queries the pathloss resulting from the advanced UL power settings. \n
			:return: pathloss: float Unit: dB
		"""
		response = self._core.io.query_str('SENSe:LTE:SIGNaling<Instance>:UL:SETB:APPower:PATHloss?')
		return Conversions.str_to_float(response)

	def get_epp_power(self) -> float:
		"""SCPI: SENSe:LTE:SIGNaling<instance>:UL:SETB:APPower:EPPPower \n
		Snippet: value: float = driver.sense.uplink.setb.apPower.get_epp_power() \n
		...
		"""
		response = self._core.io.query_str('SENSe:LTE:SIGNaling<Instance>:UL:SETB:APPower:EPPPower?')
		return Conversions.str_to_float(response)

	def get_eo_power(self) -> float:
		"""SCPI: SENSe:LTE:SIGNaling<instance>:UL:SETB:APPower:EOPower \n
		Snippet: value: float = driver.sense.uplink.setb.apPower.get_eo_power() \n
		...
		"""
		response = self._core.io.query_str('SENSe:LTE:SIGNaling<Instance>:UL:SETB:APPower:EOPower?')
		return Conversions.str_to_float(response)
