...
class ChrpdCls:
	...
	def __init__(self, core: Core, parent):
		self._core = core
		self._cmd_group = CommandsGroup("chrpd", core, parent)

	def get(self, index: enums.Cdma2kBand = None) -> List[bool]:
		"""SCPI: SENSe:LTE:SIGNaling<instance>:UECapability:MEAS:IRNGaps:CHRPd \n
		Snippet: value: List[bool] = driver.sense.ueCapability.meas.irnGaps.chrpd.get(index = enums.Cdma2kBand.BC0) \n
		Returns a list of values indicating the need for downlink measurement gaps when operating on a specific E-UTRA band and
		measuring on a specific CDMA2000 HRPD band. The full list contains 18 times 256 values. Each block of 18 values
		...
		param = ArgSingleList().compose_cmd_string(ArgSingle('index', index, DataType.Enum, enums.Cdma2kBand, is_optional=True))
		response = self._core.io.query_str(f'SENSe:LTE:SIGNaling<Instance>:UECapability:MEAS:IRNGaps:CHRPd? {param}'.rstrip())
