...
class DeterminedCls:
	...
	def get(self, table_name: enums.Table = None, secondaryCompCarrier=repcap.SecondaryCompCarrier.Default) -> List[int]:
		"""SCPI: SENSe:LTE:SIGNaling<instance>:CONNection:SCC<Carrier>:FWBCqi:DL:MCSTable:SSUBframe:DETermined \n
		Snippet: value: List[int] = driver.sense.connection.scc.fwbcqi.downlink.mcsTable.ssubframe.determined.get(table_name = enums.Table.ANY, secondaryCompCarrier = repcap.SecondaryCompCarrier.Default) \n
		Queries an automatically determined mapping table for special subframes. For the scheduling type 'Follow WB CQI' in the
		table mode DETermined. \n
			:param table_name: ANY | CW1 | CW2 | OTLC1 | OTLC2 | TFLC1 | TFLC2 Selects which mapping table is queried. To check which tables are available, use method RsCmwLteSig.Sense.Connection.Scc.Fwbcqi.Downlink.Mcs.Atable.ListPy.get_. ANY: table used for all code word / layer constellations CW1: codeword mapped to 1 layer CW2: codeword mapped to 2 layers OTLC1: codeword mapped to 1 layer / 1 or 2 layers in total OTLC2: codeword mapped to 2 layers / 2 layers in total TFLC1: codeword mapped to 1 layer / 3 or 4 layers in total TFLC2: codeword mapped to 2 layers / 3 or 4 layers in total If the Tablename is omitted, ANY is used.
		...
		response = self._core.io.query_bin_or_ascii_int_list(f'SENSe:LTE:SIGNaling<Instance>:CONNection:SCC{secondaryCompCarrier_cmd_val}:FWBCqi:DL:MCSTable:SSUBframe:DETermined? {param}'.rstrip())
		return response
