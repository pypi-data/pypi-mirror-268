...
class StreamCls:
	...
	def get(self, stream=repcap.Stream.Default) -> float:
		...
		Snippet: value: float = driver.sense.connection.ethroughput.downlink.pcc.stream.get(stream = repcap.Stream.Default) \n
		...
		response = self._core.io.query_str(f'SENSe:LTE:SIGNaling<Instance>:CONNection:ETHRoughput:DL:PCC:STReam{stream_cmd_val}?')
