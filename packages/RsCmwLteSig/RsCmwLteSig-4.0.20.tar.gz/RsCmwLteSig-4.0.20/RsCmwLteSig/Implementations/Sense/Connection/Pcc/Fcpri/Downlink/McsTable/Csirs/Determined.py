...
class DeterminedCls:
	...
	def get(self, table_name: enums.Table = None) -> List[int]:
		"""SCPI: SENSe:LTE:SIGNaling<instance>:CONNection[:PCC]:FCPRi:DL:MCSTable:CSIRs:DETermined \n
		Snippet: value: List[int] = driver.sense.connection.pcc.fcpri.downlink.mcsTable.csirs.determined.get(table_name = enums.Table.ANY) \n
		...
		response = self._core.io.query_bin_or_ascii_int_list(f'SENSe:LTE:SIGNaling<Instance>:CONNection:PCC:FCPRi:DL:MCSTable:CSIRs:DETermined? {param}'.rstrip())
		return response
