...
class CfCls:
	...
	def get_flexible(self) -> FlexibleStruct:
		...
		Snippet: value: FlexibleStruct = driver.route.scenario.cf.get_flexible() \n
		Activates the scenario '3CC - nx2 nx2 nx2' and selects the signal paths. For possible parameter values, see 'Values for
		signal path selection'. \n
			:return: structure: for return value, see the help for FlexibleStruct structure arguments.
		"""
		return self._core.io.query_struct('ROUTe:LTE:SIGNaling<Instance>:SCENario:CF:FLEXible?', self.__class__.FlexibleStruct())
	...
	def set_flexible(self, value: FlexibleStruct) -> None:
		...
		"""
		self._core.io.write_struct('ROUTe:LTE:SIGNaling<Instance>:SCENario:CF:FLEXible', value)
