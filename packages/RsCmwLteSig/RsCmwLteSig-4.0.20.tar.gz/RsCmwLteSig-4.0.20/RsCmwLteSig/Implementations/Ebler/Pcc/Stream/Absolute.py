...
class AbsoluteCls:
	...
	def fetch(self, stream=repcap.Stream.Default) -> FetchStruct:
		...
		Snippet: value: FetchStruct = driver.ebler.pcc.stream.absolute.fetch(stream = repcap.Stream.Default) \n
		...
		return self._core.io.query_struct(f'FETCh:LTE:SIGNaling<Instance>:EBLer:PCC:STReam{stream_cmd_val}:ABSolute?', self.__class__.FetchStruct())
