...
class DateCls:
	...
	def set(self, day: int, month: int, year: int) -> None:
		...
		Snippet: driver.configure.sms.outgoing.sctStamp.date.set(day = 1, month = 1, year = 1) \n
		...
		self._core.io.write(f'CONFigure:LTE:SIGNaling<Instance>:SMS:OUTGoing:SCTStamp:DATE {param}'.rstrip())
	...
	def get(self) -> DateStruct:
		"""SCPI: CONFigure:LTE:SIGNaling<Instance>:SMS:OUTGoing:SCTStamp:DATE \n
		Snippet: value: DateStruct = driver.configure.sms.outgoing.sctStamp.date.get() \n
		Specifies the date of the service center time stamp for the time source DATE (see method RsCmwLteSig.Configure.Sms.
		Outgoing.SctStamp.tsource) . \n
			:return: structure: for return value, see the help for DateStruct structure arguments."""
		return self._core.io.query_struct(f'CONFigure:LTE:SIGNaling<Instance>:SMS:OUTGoing:SCTStamp:DATE?', self.__class__.DateStruct())
