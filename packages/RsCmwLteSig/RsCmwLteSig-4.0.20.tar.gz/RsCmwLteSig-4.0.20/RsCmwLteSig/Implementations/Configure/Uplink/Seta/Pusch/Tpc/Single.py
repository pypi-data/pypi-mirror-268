...
class SingleCls:
	...
	def set(self, no_of_steps: int, step_direction: enums.UpDownDirection) -> None:
		...
		Snippet: driver.configure.uplink.seta.pusch.tpc.single.set(no_of_steps = 1, step_direction = enums.UpDownDirection.DOWN) \n
		...
		self._core.io.write(f'CONFigure:LTE:SIGNaling<Instance>:UL:SETA:PUSCh:TPC:SINGle {param}'.rstrip())
	...
	def get(self) -> SingleStruct:
		"""SCPI: CONFigure:LTE:SIGNaling<instance>:UL:SETA:PUSCh:TPC:SINGle \n
		Snippet: value: SingleStruct = driver.configure.uplink.seta.pusch.tpc.single.get() \n
		Defines a pattern for power control of the PUSCH with the TPC setup SINGle. The pattern consists of 1 to 35 up (+1 dB) or
		down (-1 dB) commands, followed by 'constant power' commands (0 dB) . \n
			:return: structure: for return value, see the help for SingleStruct structure arguments."""
		return self._core.io.query_struct(f'CONFigure:LTE:SIGNaling<Instance>:UL:SETA:PUSCh:TPC:SINGle?', self.__class__.SingleStruct())
