...
class PuschCls:
	...
	@property
	def tpc(self):
		"""tpc commands group. 3 Sub-classes, 4 commands."""
	...
	def get_oln_power(self) -> float:
		...
		Snippet: value: float = driver.configure.uplink.seta.pusch.get_oln_power() \n
		...
		"""
		response = self._core.io.query_str('CONFigure:LTE:SIGNaling<Instance>:UL:SETA:PUSCh:OLNPower?')
		return Conversions.str_to_float(response)

	def set_oln_power(self, power: float) -> None:
		"""SCPI: CONFigure:LTE:SIGNaling<instance>:UL:SETA:PUSCh:OLNPower \n
		Snippet: driver.configure.uplink.seta.pusch.set_oln_power(power = 1.0) \n
		Defines a cell-specific nominal power value for full resource block allocation in the UL (entire cell bandwidth used) .
		From this value, the cell-specific nominal power value PO_NOMINAL_PUSCH related to one resource block is determined and
		sent to all UEs via broadcast. This command is only relevant for basic configuration and rejected if advanced
		configuration is active. \n
		...
		self._core.io.write(f'CONFigure:LTE:SIGNaling<Instance>:UL:SETA:PUSCh:OLNPower {param}')
