...
class AnbCls:
	...
	def set(self, additional_nb: bool, anb=repcap.Anb.Default) -> None:
		...
		Snippet: driver.configure.connection.pcc.emamode.a.downlink.anb.set(additional_nb = False, anb = repcap.Anb.Default) \n
		...
		self._core.io.write(f'CONFigure:LTE:SIGNaling<Instance>:CONNection:PCC:EMAMode:A:DL:ANB{anb_cmd_val} {param}')

	def get(self, anb=repcap.Anb.Default) -> bool:
		"""SCPI: CONFigure:LTE:SIGNaling<instance>:CONNection[:PCC]:EMAMode:A:DL:ANB<Number> \n
		Snippet: value: bool = driver.configure.connection.pcc.emamode.a.downlink.anb.get(anb = repcap.Anb.Default) \n
		Enables or disables additional DL narrowbands for the eMTC auto mode, for a maximum eMTC bandwidth of 5 MHz. \n
			:param anb: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Anb')
			:return: additional_nb: OFF | ON ON: Use the additional narrowband. OFF: Do not use the additional narrowband."""
		anb_cmd_val = self._cmd_group.get_repcap_cmd_value(anb, repcap.Anb)
		response = self._core.io.query_str(f'CONFigure:LTE:SIGNaling<Instance>:CONNection:PCC:EMAMode:A:DL:ANB{anb_cmd_val}?')
