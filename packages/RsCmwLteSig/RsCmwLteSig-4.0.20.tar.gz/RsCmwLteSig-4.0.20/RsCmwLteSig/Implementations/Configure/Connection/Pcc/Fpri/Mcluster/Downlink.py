...
class DownlinkCls:
	...
	def set(self, cluster: str, modulation: enums.Modulation, trans_block_size_idx: int) -> None:
		...
		Snippet: driver.configure.connection.pcc.fpri.mcluster.downlink.set(cluster = rawAbc, modulation = enums.Modulation.Q1024, trans_block_size_idx = 1) \n
		Configures the downlink for the scheduling type 'Follow WB PMI-RI', with multi-cluster allocation. The allowed input
		...
		self._core.io.write(f'CONFigure:LTE:SIGNaling<Instance>:CONNection:PCC:FPRI:MCLuster:DL {param}'.rstrip())
	...
	def get(self) -> DownlinkStruct:
		"""SCPI: CONFigure:LTE:SIGNaling<instance>:CONNection[:PCC]:FPRI:MCLuster:DL \n
		Snippet: value: DownlinkStruct = driver.configure.connection.pcc.fpri.mcluster.downlink.get() \n
		Configures the downlink for the scheduling type 'Follow WB PMI-RI', with multi-cluster allocation. The allowed input
		ranges have dependencies and are described in the background information, see 'CQI channels' and especially Table 'RBG
		parameters'. \n
			:return: structure: for return value, see the help for DownlinkStruct structure arguments."""
		return self._core.io.query_struct(f'CONFigure:LTE:SIGNaling<Instance>:CONNection:PCC:FPRI:MCLuster:DL?', self.__class__.DownlinkStruct())
