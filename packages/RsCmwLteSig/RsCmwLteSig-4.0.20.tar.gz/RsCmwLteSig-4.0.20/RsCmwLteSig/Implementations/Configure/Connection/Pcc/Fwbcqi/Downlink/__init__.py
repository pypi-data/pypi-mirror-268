...
class DownlinkCls:
	...
	def get_stti(self) -> List[bool]:
		...
		Snippet: value: List[bool] = driver.configure.connection.pcc.fwbcqi.downlink.get_stti() \n
		Configures which subframes are scheduled for the DL of the scheduling type 'Follow WB CQI'. For most subframes, the
		...
		"""
		response = self._core.io.query_str('CONFigure:LTE:SIGNaling<Instance>:CONNection:PCC:FWBCqi:DL:STTI?')
		return Conversions.str_to_bool_list(response)

	def set_stti(self, scheduled: List[bool]) -> None:
		"""SCPI: CONFigure:LTE:SIGNaling<instance>:CONNection[:PCC]:FWBCqi:DL:STTI \n
		Snippet: driver.configure.connection.pcc.fwbcqi.downlink.set_stti(scheduled = [True, False, True]) \n
		Configures which subframes are scheduled for the DL of the scheduling type 'Follow WB CQI'. For most subframes, the
		setting is fixed, depending on the duplex mode and the UL-DL configuration. For these subframes, your setting is ignored. \n
			:param scheduled: OFF | ON Comma-separated list of 10 values, for subframe 0 to 9
		"""
		param = Conversions.list_to_csv_str(scheduled)
		self._core.io.write(f'CONFigure:LTE:SIGNaling<Instance>:CONNection:PCC:FWBCqi:DL:STTI {param}')
	...
	def set(self, number_rb: int, start_rb: int, table: enums.MultiClusterDlTable) -> None:
		...
		Snippet: driver.configure.connection.pcc.fwbcqi.downlink.set(number_rb = 1, start_rb = 1, table = enums.MultiClusterDlTable.DETermined) \n
		...
		self._core.io.write(f'CONFigure:LTE:SIGNaling<Instance>:CONNection:PCC:FWBCqi:DL {param}'.rstrip())
	...
	def get(self) -> DownlinkStruct:
		"""SCPI: CONFigure:LTE:SIGNaling<instance>:CONNection[:PCC]:FWBCqi:DL \n
		Snippet: value: DownlinkStruct = driver.configure.connection.pcc.fwbcqi.downlink.get() \n
		Configures the downlink for the scheduling type 'Follow WB CQI', with contiguous RB allocation. The allowed input ranges
		have dependencies and are described in the background information, see 'CQI channels'. \n
			:return: structure: for return value, see the help for DownlinkStruct structure arguments."""
		return self._core.io.query_struct(f'CONFigure:LTE:SIGNaling<Instance>:CONNection:PCC:FWBCqi:DL?', self.__class__.DownlinkStruct())
