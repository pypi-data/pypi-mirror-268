...
class DownlinkCls:
	...
	def set(self, tti: float, number_rb: int or bool, start_rb: int or bool, modulation: enums.Modulation, trans_block_size_idx: int or bool, secondaryCompCarrier=repcap.SecondaryCompCarrier.Default, stream=repcap.Stream.Default) -> None:
		"""SCPI: CONFigure:LTE:SIGNaling<instance>:CONNection:SCC<Carrier>:UDTTibased:DL<Stream> \n
		Snippet: driver.configure.connection.scc.udttiBased.downlink.set(tti = 1.0, number_rb = 1, start_rb = 1, modulation = enums.Modulation.Q1024, trans_block_size_idx = 1, secondaryCompCarrier = repcap.SecondaryCompCarrier.Default, stream = repcap.Stream.Default) \n
		...
		return OFF,OFF,OFF,OFF, indicating that the queried subframe is no DL subframe. \n
			...
			:param number_rb: (integer or boolean) numeric | OFF Number of allocated resource blocks. The same value must be configured for all streams.
		...
		self._core.io.write(f'CONFigure:LTE:SIGNaling<Instance>:CONNection:SCC{secondaryCompCarrier_cmd_val}:UDTTibased:DL{stream_cmd_val} {param}'.rstrip())

	# noinspection PyTypeChecker
	class GetStruct(StructBase):
		"""Response structure. Fields: \n
			- Number_Rb: int or bool: numeric | OFF Number of allocated resource blocks. The same value must be configured for all streams.
	...
	def get(self, tti: float, secondaryCompCarrier=repcap.SecondaryCompCarrier.Default, stream=repcap.Stream.Default) -> GetStruct:
		"""SCPI: CONFigure:LTE:SIGNaling<instance>:CONNection:SCC<Carrier>:UDTTibased:DL<Stream> \n
		Snippet: value: GetStruct = driver.configure.connection.scc.udttiBased.downlink.get(tti = 1.0, secondaryCompCarrier = repcap.SecondaryCompCarrier.Default, stream = repcap.Stream.Default) \n
		Configures a selected downlink subframe for the scheduling type 'User-defined TTI-Based'. The allowed input ranges have
		dependencies and are described in the background information, see 'User-defined channels'. A query for TDD can also
		return OFF,OFF,OFF,OFF, indicating that the queried subframe is no DL subframe. \n
			:param tti: numeric Number of the subframe to be configured/queried. Range: 0 to 9
		...
		secondaryCompCarrier_cmd_val = self._cmd_group.get_repcap_cmd_value(secondaryCompCarrier, repcap.SecondaryCompCarrier)
		stream_cmd_val = self._cmd_group.get_repcap_cmd_value(stream, repcap.Stream)
		return self._core.io.query_struct(f'CONFigure:LTE:SIGNaling<Instance>:CONNection:SCC{secondaryCompCarrier_cmd_val}:UDTTibased:DL{stream_cmd_val}? {param}', self.__class__.GetStruct())
