...
class DownlinkCls:
	...
	def set(self, number_rb: int, start_rb: int, table: enums.MultiClusterDlTable, secondaryCompCarrier=repcap.SecondaryCompCarrier.Default) -> None:
		...
		Snippet: driver.configure.connection.scc.fcri.downlink.set(number_rb = 1, start_rb = 1, table = enums.MultiClusterDlTable.DETermined, secondaryCompCarrier = repcap.SecondaryCompCarrier.Default) \n
		...
		self._core.io.write(f'CONFigure:LTE:SIGNaling<Instance>:CONNection:SCC{secondaryCompCarrier_cmd_val}:FCRI:DL {param}'.rstrip())
	...
	def get(self, secondaryCompCarrier=repcap.SecondaryCompCarrier.Default) -> DownlinkStruct:
		"""SCPI: CONFigure:LTE:SIGNaling<instance>:CONNection:SCC<Carrier>:FCRI:DL \n
		Snippet: value: DownlinkStruct = driver.configure.connection.scc.fcri.downlink.get(secondaryCompCarrier = repcap.SecondaryCompCarrier.Default) \n
		Configures the downlink for the scheduling type 'Follow WB CQI-RI', with contiguous allocation. The allowed input ranges
		have dependencies and are described in the background information, see 'CQI channels'. \n
			:param secondaryCompCarrier: optional repeated capability selector. Default value: CC1 (settable in the interface 'Scc')
			:return: structure: for return value, see the help for DownlinkStruct structure arguments."""
		secondaryCompCarrier_cmd_val = self._cmd_group.get_repcap_cmd_value(secondaryCompCarrier, repcap.SecondaryCompCarrier)
		return self._core.io.query_struct(f'CONFigure:LTE:SIGNaling<Instance>:CONNection:SCC{secondaryCompCarrier_cmd_val}:FCRI:DL?', self.__class__.DownlinkStruct())
