...
class DownlinkCls:
	...
	def set(self, number_rb: int, start_rb: int, modulation: enums.Modulation, trans_block_size_idx: int, stream=repcap.Stream.Default) -> None:
		...
		Snippet: driver.configure.connection.pcc.udChannels.downlink.set(number_rb = 1, start_rb = 1, modulation = enums.Modulation.Q1024, trans_block_size_idx = 1, stream = repcap.Stream.Default) \n
		Configures a user-defined downlink channel with contiguous allocation (no LAA, no eMTC) . The <NumberRB> and <StartRB>
		settings apply to all DL streams. The other settings apply to DL stream <s>. The allowed input ranges have dependencies
		and are described in the background information, see 'User-defined channels'. \n
		...
		self._core.io.write(f'CONFigure:LTE:SIGNaling<Instance>:CONNection:PCC:UDCHannels:DL{stream_cmd_val} {param}'.rstrip())
	...
	def get(self, stream=repcap.Stream.Default) -> DownlinkStruct:
		"""SCPI: CONFigure:LTE:SIGNaling<instance>:CONNection[:PCC]:UDCHannels:DL<Stream> \n
		Snippet: value: DownlinkStruct = driver.configure.connection.pcc.udChannels.downlink.get(stream = repcap.Stream.Default) \n
		Configures a user-defined downlink channel with contiguous allocation (no LAA, no eMTC) . The <NumberRB> and <StartRB>
		settings apply to all DL streams. The other settings apply to DL stream <s>. The allowed input ranges have dependencies
		and are described in the background information, see 'User-defined channels'. \n
			:param stream: optional repeated capability selector. Default value: S1 (settable in the interface 'Downlink')
		...
		stream_cmd_val = self._cmd_group.get_repcap_cmd_value(stream, repcap.Stream)
		return self._core.io.query_struct(f'CONFigure:LTE:SIGNaling<Instance>:CONNection:PCC:UDCHannels:DL{stream_cmd_val}?', self.__class__.DownlinkStruct())
