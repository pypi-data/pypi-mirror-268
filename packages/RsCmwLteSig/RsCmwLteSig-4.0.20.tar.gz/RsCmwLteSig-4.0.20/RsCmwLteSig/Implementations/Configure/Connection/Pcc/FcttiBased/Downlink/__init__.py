...
class DownlinkCls:
	...
	def set(self, tti: float, number_rb: int or bool, start_rb: int or bool, cqi_idx: int or bool, stream=repcap.Stream.Default) -> None:
		"""SCPI: CONFigure:LTE:SIGNaling<instance>:CONNection[:PCC]:FCTTibased:DL<Stream> \n
		Snippet: driver.configure.connection.pcc.fcttiBased.downlink.set(tti = 1.0, number_rb = 1, start_rb = 1, cqi_idx = 1, stream = repcap.Stream.Default) \n
		...
		self._core.io.write(f'CONFigure:LTE:SIGNaling<Instance>:CONNection:PCC:FCTTibased:DL{stream_cmd_val} {param}'.rstrip())
	...
	def get(self, tti: float, stream=repcap.Stream.Default) -> GetStruct:
		"""SCPI: CONFigure:LTE:SIGNaling<instance>:CONNection[:PCC]:FCTTibased:DL<Stream> \n
		Snippet: value: GetStruct = driver.configure.connection.pcc.fcttiBased.downlink.get(tti = 1.0, stream = repcap.Stream.Default) \n
		Configures a selected downlink subframe for the scheduling type 'Fixed CQI'. The allowed input ranges have dependencies
		and are described in the background information, see 'CQI channels'. A query for TDD can also return OFF,OFF,OFF,OFF,
		indicating that the queried subframe is no DL subframe. \n
			:param tti: numeric Number of the subframe to be configured/queried Range: 0 to 9
		...
		stream_cmd_val = self._cmd_group.get_repcap_cmd_value(stream, repcap.Stream)
		return self._core.io.query_struct(f'CONFigure:LTE:SIGNaling<Instance>:CONNection:PCC:FCTTibased:DL{stream_cmd_val}? {param}', self.__class__.GetStruct())
