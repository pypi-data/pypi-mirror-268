...
class WcdmaCls:
	...
	def set(self, band: enums.OperatingBandB, dl_channel: int) -> None:
		...
		Snippet: driver.configure.connection.csfb.wcdma.set(band = enums.OperatingBandB.OB1, dl_channel = 1) \n
		Configures the WCDMA target for MO CSFB. \n
			...
			:param dl_channel: decimal Downlink channel number Range: 412 to 11000, depending on operating band, see table below
		"""
		...
		self._core.io.write(f'CONFigure:LTE:SIGNaling<Instance>:CONNection:CSFB:WCDMa {param}'.rstrip())
	...
	def get(self) -> WcdmaStruct:
		"""SCPI: CONFigure:LTE:SIGNaling<instance>:CONNection:CSFB:WCDMa \n
		Snippet: value: WcdmaStruct = driver.configure.connection.csfb.wcdma.get() \n
		Configures the WCDMA target for MO CSFB. \n
			:return: structure: for return value, see the help for WcdmaStruct structure arguments."""
		return self._core.io.query_struct(f'CONFigure:LTE:SIGNaling<Instance>:CONNection:CSFB:WCDMa?', self.__class__.WcdmaStruct())
