...
class DownlinkCls:
	...
	def set(self, symbols: enums.Symbols, number_rb: int, start_rb: int, modulation: enums.Modulation, trans_block_size_idx: int, secondaryCompCarrier=repcap.SecondaryCompCarrier.Default, stream=repcap.Stream.Default) -> None:
		"""SCPI: CONFigure:LTE:SIGNaling<instance>:CONNection:SCC<Carrier>:UDCHannels:LAA:RBURst:PEPSubframes:DL<Stream> \n
		Snippet: driver.configure.connection.scc.udChannels.laa.rburst.pepSubFrames.downlink.set(symbols = enums.Symbols.S0, number_rb = 1, start_rb = 1, modulation = enums.Modulation.Q1024, trans_block_size_idx = 1, secondaryCompCarrier = repcap.SecondaryCompCarrier.Default, stream = repcap.Stream.Default) \n
		Configures a user-defined downlink channel with contiguous allocation, for LAA, random bursts. The <NumberRB> and
		<StartRB> settings apply to all subframes of the burst and to all DL streams. The <Modulation> and <TransBlockSizeIdx>
		settings apply to partial ending subframes with the specified number of allocated OFDM <Symbols> and DL stream <s>. The
		...
		"""
		param = ArgSingleList().compose_cmd_string(ArgSingle('symbols', symbols, DataType.Enum, enums.Symbols), ArgSingle('number_rb', number_rb, DataType.Integer), ArgSingle('start_rb', start_rb, DataType.Integer), ArgSingle('modulation', modulation, DataType.Enum, enums.Modulation), ArgSingle('trans_block_size_idx', trans_block_size_idx, DataType.Integer))
		...
		self._core.io.write(f'CONFigure:LTE:SIGNaling<Instance>:CONNection:SCC{secondaryCompCarrier_cmd_val}:UDCHannels:LAA:RBURst:PEPSubframes:DL{stream_cmd_val} {param}'.rstrip())
	...
	def get(self, symbols: enums.Symbols, secondaryCompCarrier=repcap.SecondaryCompCarrier.Default, stream=repcap.Stream.Default) -> GetStruct:
		"""SCPI: CONFigure:LTE:SIGNaling<instance>:CONNection:SCC<Carrier>:UDCHannels:LAA:RBURst:PEPSubframes:DL<Stream> \n
		Snippet: value: GetStruct = driver.configure.connection.scc.udChannels.laa.rburst.pepSubFrames.downlink.get(symbols = enums.Symbols.S0, secondaryCompCarrier = repcap.SecondaryCompCarrier.Default, stream = repcap.Stream.Default) \n
		Configures a user-defined downlink channel with contiguous allocation, for LAA, random bursts. The <NumberRB> and
		<StartRB> settings apply to all subframes of the burst and to all DL streams. The <Modulation> and <TransBlockSizeIdx>
		settings apply to partial ending subframes with the specified number of allocated OFDM <Symbols> and DL stream <s>. The
		allowed input ranges have dependencies and are described in the background information, see 'User-defined channels for
		...
		secondaryCompCarrier_cmd_val = self._cmd_group.get_repcap_cmd_value(secondaryCompCarrier, repcap.SecondaryCompCarrier)
		stream_cmd_val = self._cmd_group.get_repcap_cmd_value(stream, repcap.Stream)
		return self._core.io.query_struct(f'CONFigure:LTE:SIGNaling<Instance>:CONNection:SCC{secondaryCompCarrier_cmd_val}:UDCHannels:LAA:RBURst:PEPSubframes:DL{stream_cmd_val}? {param}', self.__class__.GetStruct())
