...
class ThresholdsCls:
	...
	def set(self, high: int, low: int) -> None:
		...
		Snippet: driver.configure.ncell.cdma.thresholds.set(high = 1, low = 1) \n
		...
		self._core.io.write(f'CONFigure:LTE:SIGNaling<Instance>:NCELl:CDMA:THResholds {param}'.rstrip())
	...
	def get(self) -> ThresholdsStruct:
		"""SCPI: CONFigure:LTE:SIGNaling<instance>:NCELl:CDMA:THResholds \n
		Snippet: value: ThresholdsStruct = driver.configure.ncell.cdma.thresholds.get() \n
		No command help available \n
			:return: structure: for return value, see the help for ThresholdsStruct structure arguments."""
		return self._core.io.query_struct(f'CONFigure:LTE:SIGNaling<Instance>:NCELl:CDMA:THResholds?', self.__class__.ThresholdsStruct())
