...
class CdmaCls:
	...
	def set(self, band_class: enums.BandClass, dl_channel: int) -> None:
		...
		Snippet: driver.prepare.handover.external.cdma.set(band_class = enums.BandClass.AWS, dl_channel = 1) \n
		...
		self._core.io.write(f'PREPare:LTE:SIGNaling<Instance>:HANDover:EXTernal:CDMA {param}'.rstrip())
	...
	def get(self) -> CdmaStruct:
		"""SCPI: PREPare:LTE:SIGNaling<instance>:HANDover:EXTernal:CDMA \n
		Snippet: value: CdmaStruct = driver.prepare.handover.external.cdma.get() \n
		Configures the destination parameters for handover to a CDMA2000 or 1xEV-DO destination at another instrument. \n
			:return: structure: for return value, see the help for CdmaStruct structure arguments."""
		return self._core.io.query_struct(f'PREPare:LTE:SIGNaling<Instance>:HANDover:EXTernal:CDMA?', self.__class__.CdmaStruct())
