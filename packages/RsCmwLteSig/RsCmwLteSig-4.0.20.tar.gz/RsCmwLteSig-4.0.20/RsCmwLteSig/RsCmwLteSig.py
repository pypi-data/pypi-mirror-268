...
class RsCmwLteSig:
	...
	_driver_options = "SupportedInstrModels = CMW500/CMW270/CMW280/CMP, SupportedIdnPatterns = CMW, SimulationIdnString = 'Rohde&Schwarz,CMW500,100001,4.0.20.0028'"
	...
	def __init__(self, resource_name: str, id_query: bool = True, reset: bool = False, options: str = None, direct_session: object = None):
		...
		self._core.driver_version = '4.0.20.0028'
	...
	def assert_minimum_version(min_version: str) -> None:
		...
		curr_version_list = '4.0.20.0028'.split('.')
		...
		for i in range(count):
			...
			if curr > minimum:
				break
			if curr < minimum:
				raise RsInstrException(f"Assertion for minimum RsCmwLteSig version failed. Current version: '4.0.20.0028', minimum required version: '{min_version}'")
	...
	def _sync_to_custom_properties(self, cloned: 'RsCmwLteSig') -> None:
	...
	def prepare(self):
		...
		if not hasattr(self, '_prepare'):
			from .Implementations.Prepare import PrepareCls
	...
	def a(self):
		...
		if not hasattr(self, '_a'):
			from .Implementations.A import ACls
	...
	def b(self):
		...
		if not hasattr(self, '_b'):
			from .Implementations.B import BCls
	...
	def scc(self):
		...
		if not hasattr(self, '_scc'):
			from .Implementations.Scc import SccCls
	...
	def ebler(self):
		...
		if not hasattr(self, '_ebler'):
			from .Implementations.Ebler import EblerCls
	...
	def intermediate(self):
		...
		if not hasattr(self, '_intermediate'):
			from .Implementations.Intermediate import IntermediateCls
			self._intermediate = IntermediateCls(self._core, self._cmd_group)
	...
	def throughput(self):
		...
		if not hasattr(self, '_throughput'):
			from .Implementations.Throughput import ThroughputCls
