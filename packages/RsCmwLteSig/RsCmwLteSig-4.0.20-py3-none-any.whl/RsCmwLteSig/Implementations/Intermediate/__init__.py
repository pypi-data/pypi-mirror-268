...
class IntermediateCls:
	...
	def __init__(self, core: Core, parent):
		self._core = core
		self._cmd_group = CommandsGroup("intermediate", core, parent)

	@property
	def ebler(self):
		"""ebler commands group. 3 Sub-classes, 0 commands."""
		if not hasattr(self, '_ebler'):
			from .Ebler import EblerCls
			self._ebler = EblerCls(self._core, self._cmd_group)
		return self._ebler

	def clone(self) -> 'IntermediateCls':
		"""Clones the group by creating new object from it and its whole existing subgroups
		Also copies all the existing default Repeated Capabilities setting,
		which you can change independently without affecting the original group"""
		new_group = IntermediateCls(self._core, self._cmd_group.parent)
