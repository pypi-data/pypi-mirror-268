...
class StreamCls:
	...
	def fetch(self, stream=repcap.Stream.Default) -> List[float]:
		"""SCPI: FETCh:LTE:SIGNaling<instance>:EBLer:TRACe:CQIReporting[:PCC]:STReam<Stream> \n
		Snippet: value: List[float] = driver.ebler.trace.cqiReporting.pcc.stream.fetch(stream = repcap.Stream.Default) \n
		...
		Use RsCmwLteSig.reliability.last_value to read the updated reliability indicator. \n
			...
			:return: yvalue: float Comma-separated list of 16 Y-values, for CQI index 0 to 15"""
		stream_cmd_val = self._cmd_group.get_repcap_cmd_value(stream, repcap.Stream)
		suppressed = ArgSingleSuppressed(0, DataType.Integer, False, 1, 'Reliability')
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'FETCh:LTE:SIGNaling<Instance>:EBLer:TRACe:CQIReporting:PCC:STReam{stream_cmd_val}?', suppressed)
		return response
