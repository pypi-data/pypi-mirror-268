...
class StreamCls:
	...
	def fetch(self, stream=repcap.Stream.Default) -> FetchStruct:
		"""SCPI: FETCh:LTE:SIGNaling<instance>:EBLer:TRACe:THRoughput[:PCC]:MCQI:STReam<Stream> \n
		Snippet: value: FetchStruct = driver.ebler.trace.throughput.pcc.mcqi.stream.fetch(stream = repcap.Stream.Default) \n
		...
		return self._core.io.query_struct(f'FETCh:LTE:SIGNaling<Instance>:EBLer:TRACe:THRoughput:PCC:MCQI:STReam{stream_cmd_val}?', self.__class__.FetchStruct())
