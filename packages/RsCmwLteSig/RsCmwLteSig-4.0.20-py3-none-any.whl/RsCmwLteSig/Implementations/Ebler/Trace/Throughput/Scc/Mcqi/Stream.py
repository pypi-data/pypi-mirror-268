...
class StreamCls:
	...
	def fetch(self, secondaryCompCarrier=repcap.SecondaryCompCarrier.Default, stream=repcap.Stream.Default) -> FetchStruct:
		"""SCPI: FETCh:LTE:SIGNaling<instance>:EBLer:TRACe:THRoughput:SCC<Carrier>:MCQI:STReam<Stream> \n
		Snippet: value: FetchStruct = driver.ebler.trace.throughput.scc.mcqi.stream.fetch(secondaryCompCarrier = repcap.SecondaryCompCarrier.Default, stream = repcap.Stream.Default) \n
		Returns the median CQI trace for one downlink stream. Each value is returned as a pair of X-value and Y-value. The number
		of result pairs n equals the number of subframes to be processed per measurement cycle, divided by 200. Returned results:
		<Reliability>, <XValue>1, <YValue>1, ..., <XValue>n, <YValue>n \n
		...
		return self._core.io.query_struct(f'FETCh:LTE:SIGNaling<Instance>:EBLer:TRACe:THRoughput:SCC{secondaryCompCarrier_cmd_val}:MCQI:STReam{stream_cmd_val}?', self.__class__.FetchStruct())
