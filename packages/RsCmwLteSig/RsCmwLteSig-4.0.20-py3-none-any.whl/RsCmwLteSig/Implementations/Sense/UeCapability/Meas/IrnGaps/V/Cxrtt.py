...
class CxrttCls:
	...
	def get(self, index: enums.Cdma2kBand = None) -> List[bool]:
		...
		Snippet: value: List[bool] = driver.sense.ueCapability.meas.irnGaps.v.cxrtt.get(index = enums.Cdma2kBand.BC0) \n
		Returns a list of values indicating the need for downlink measurement gaps when operating on a specific E-UTRA band
		combination and measuring on a specific CDMA2000 1xRTT band class. The full list contains 18 times n+1 values. Each block
		...
		response = self._core.io.query_str(f'SENSe:LTE:SIGNaling<Instance>:UECapability:MEAS:IRNGaps:V1020:CXRTt? {param}'.rstrip())
