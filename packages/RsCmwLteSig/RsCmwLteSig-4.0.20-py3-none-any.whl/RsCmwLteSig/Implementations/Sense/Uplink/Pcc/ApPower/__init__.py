...
class ApPowerCls:
	...
	def get_pathloss(self) -> float:
		...
		Snippet: value: float = driver.sense.uplink.pcc.apPower.get_pathloss() \n
		Queries the pathloss resulting from the advanced UL power settings. \n
			:return: pathloss: float Unit: dB
		"""
		response = self._core.io.query_str('SENSe:LTE:SIGNaling<Instance>:UL:PCC:APPower:PATHloss?')
		return Conversions.str_to_float(response)

	def get_epp_power(self) -> float:
		"""SCPI: SENSe:LTE:SIGNaling<instance>:UL[:PCC]:APPower:EPPPower \n
		Snippet: value: float = driver.sense.uplink.pcc.apPower.get_epp_power() \n
		...
		"""
		response = self._core.io.query_str('SENSe:LTE:SIGNaling<Instance>:UL:PCC:APPower:EPPPower?')
		return Conversions.str_to_float(response)

	def get_eo_power(self) -> float:
		"""SCPI: SENSe:LTE:SIGNaling<instance>:UL[:PCC]:APPower:EOPower \n
		Snippet: value: float = driver.sense.uplink.pcc.apPower.get_eo_power() \n
		...
		"""
		response = self._core.io.query_str('SENSe:LTE:SIGNaling<Instance>:UL:PCC:APPower:EOPower?')
		return Conversions.str_to_float(response)
