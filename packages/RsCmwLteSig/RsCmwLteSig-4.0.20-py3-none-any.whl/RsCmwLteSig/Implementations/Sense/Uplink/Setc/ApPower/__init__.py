...
class ApPowerCls:
	...
	def get_pathloss(self) -> float:
		...
		Snippet: value: float = driver.sense.uplink.setc.apPower.get_pathloss() \n
		Queries the pathloss resulting from the advanced UL power settings. \n
			:return: pathloss: float Unit: dB
		"""
		response = self._core.io.query_str('SENSe:LTE:SIGNaling<Instance>:UL:SETC:APPower:PATHloss?')
		return Conversions.str_to_float(response)

	def get_epp_power(self) -> float:
		"""SCPI: SENSe:LTE:SIGNaling<instance>:UL:SETC:APPower:EPPPower \n
		Snippet: value: float = driver.sense.uplink.setc.apPower.get_epp_power() \n
		...
		"""
		response = self._core.io.query_str('SENSe:LTE:SIGNaling<Instance>:UL:SETC:APPower:EPPPower?')
		return Conversions.str_to_float(response)

	def get_eo_power(self) -> float:
		"""SCPI: SENSe:LTE:SIGNaling<instance>:UL:SETC:APPower:EOPower \n
		Snippet: value: float = driver.sense.uplink.setc.apPower.get_eo_power() \n
		...
		"""
		response = self._core.io.query_str('SENSe:LTE:SIGNaling<Instance>:UL:SETC:APPower:EOPower?')
		return Conversions.str_to_float(response)
