...
class GsmCls:
	...
	def set(self, band: enums.GsmBand, dl_channel: int, band_indicator: enums.BandIndicator) -> None:
		...
		Snippet: driver.prepare.handover.external.gsm.set(band = enums.GsmBand.G04, dl_channel = 1, band_indicator = enums.BandIndicator.G18) \n
		Configures the destination parameters for handover to a GSM destination at another instrument. \n
			...
			:param dl_channel: decimal Channel number used for the BCCH Range: depends on GSM band, see table below
		...
		self._core.io.write(f'PREPare:LTE:SIGNaling<Instance>:HANDover:EXTernal:GSM {param}'.rstrip())
	...
	def get(self) -> GsmStruct:
		"""SCPI: PREPare:LTE:SIGNaling<instance>:HANDover:EXTernal:GSM \n
		Snippet: value: GsmStruct = driver.prepare.handover.external.gsm.get() \n
		Configures the destination parameters for handover to a GSM destination at another instrument. \n
			:return: structure: for return value, see the help for GsmStruct structure arguments."""
		return self._core.io.query_struct(f'PREPare:LTE:SIGNaling<Instance>:HANDover:EXTernal:GSM?', self.__class__.GsmStruct())
