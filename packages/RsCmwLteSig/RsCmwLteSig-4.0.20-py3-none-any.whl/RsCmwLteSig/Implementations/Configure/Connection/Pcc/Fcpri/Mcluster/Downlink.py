...
class DownlinkCls:
	...
	def set(self, cluster: str, table: enums.MultiClusterDlTable) -> None:
		...
		Snippet: driver.configure.connection.pcc.fcpri.mcluster.downlink.set(cluster = rawAbc, table = enums.MultiClusterDlTable.DETermined) \n
		...
		self._core.io.write(f'CONFigure:LTE:SIGNaling<Instance>:CONNection:PCC:FCPRi:MCLuster:DL {param}'.rstrip())
	...
	def get(self) -> DownlinkStruct:
		"""SCPI: CONFigure:LTE:SIGNaling<instance>:CONNection[:PCC]:FCPRi:MCLuster:DL \n
		Snippet: value: DownlinkStruct = driver.configure.connection.pcc.fcpri.mcluster.downlink.get() \n
		Configures the downlink for the scheduling type 'Follow WB CQI-PMI-RI', with multi-cluster allocation. The allowed input
		ranges have dependencies and are described in the background information, see 'CQI channels' and especially Table 'RBG
		parameters'. \n
			:return: structure: for return value, see the help for DownlinkStruct structure arguments."""
		return self._core.io.query_struct(f'CONFigure:LTE:SIGNaling<Instance>:CONNection:PCC:FCPRi:MCLuster:DL?', self.__class__.DownlinkStruct())
