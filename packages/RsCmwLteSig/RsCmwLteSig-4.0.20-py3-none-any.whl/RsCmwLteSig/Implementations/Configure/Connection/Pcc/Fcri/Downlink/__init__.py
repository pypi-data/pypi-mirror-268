...
class DownlinkCls:
	...
	def set(self, number_rb: int, start_rb: int, table: enums.MultiClusterDlTable) -> None:
		...
		Snippet: driver.configure.connection.pcc.fcri.downlink.set(number_rb = 1, start_rb = 1, table = enums.MultiClusterDlTable.DETermined) \n
		...
		self._core.io.write(f'CONFigure:LTE:SIGNaling<Instance>:CONNection:PCC:FCRI:DL {param}'.rstrip())
	...
	def get(self) -> DownlinkStruct:
		"""SCPI: CONFigure:LTE:SIGNaling<instance>:CONNection[:PCC]:FCRI:DL \n
		Snippet: value: DownlinkStruct = driver.configure.connection.pcc.fcri.downlink.get() \n
		Configures the downlink for the scheduling type 'Follow WB CQI-RI', with contiguous allocation. The allowed input ranges
		have dependencies and are described in the background information, see 'CQI channels'. \n
			:return: structure: for return value, see the help for DownlinkStruct structure arguments."""
		return self._core.io.query_struct(f'CONFigure:LTE:SIGNaling<Instance>:CONNection:PCC:FCRI:DL?', self.__class__.DownlinkStruct())
