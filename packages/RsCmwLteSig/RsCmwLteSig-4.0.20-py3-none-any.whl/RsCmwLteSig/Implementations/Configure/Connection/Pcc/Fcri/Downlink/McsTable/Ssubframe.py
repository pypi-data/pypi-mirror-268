...
class SsubframeCls:
	...
	def get_user_defined(self) -> List[int]:
		"""SCPI: CONFigure:LTE:SIGNaling<instance>:CONNection[:PCC]:FCRI:DL:MCSTable:SSUBframe:UDEFined \n
		Snippet: value: List[int] = driver.configure.connection.pcc.fcri.downlink.mcsTable.ssubframe.get_user_defined() \n
		...
		"""
		response = self._core.io.query_bin_or_ascii_int_list('CONFigure:LTE:SIGNaling<Instance>:CONNection:PCC:FCRI:DL:MCSTable:SSUBframe:UDEFined?')
		return response

	def set_user_defined(self, mcs: List[int]) -> None:
		"""SCPI: CONFigure:LTE:SIGNaling<instance>:CONNection[:PCC]:FCRI:DL:MCSTable:SSUBframe:UDEFined \n
		Snippet: driver.configure.connection.pcc.fcri.downlink.mcsTable.ssubframe.set_user_defined(mcs = [1, 2, 3]) \n
		Configures a user-defined mapping table for special subframes that assigns an MCS index value to each possible reported
		wideband CQI index value. The table is used for the scheduling type 'Follow WB CQI-RI' if the table mode is set to
		UDEFined. \n
		...
		self._core.io.write(f'CONFigure:LTE:SIGNaling<Instance>:CONNection:PCC:FCRI:DL:MCSTable:SSUBframe:UDEFined {param}')
