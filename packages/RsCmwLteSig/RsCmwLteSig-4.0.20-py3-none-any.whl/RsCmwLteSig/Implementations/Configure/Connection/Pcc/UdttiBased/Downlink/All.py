...
class AllCls:
	...
	def set(self, number_rb: List[int or bool], start_rb: List[int or bool], modulation: List[enums.Modulation], trans_block_size_idx: List[int or bool], stream=repcap.Stream.Default) -> None:
		"""SCPI: CONFigure:LTE:SIGNaling<instance>:CONNection[:PCC]:UDTTibased:DL<Stream>:ALL \n
		Snippet: driver.configure.connection.pcc.udttiBased.downlink.all.set(number_rb = [1, True, 2, False, 3], start_rb = [1, True, 2, False, 3], modulation = [Modulation.Q1024, Modulation.QPSK], trans_block_size_idx = [1, True, 2, False, 3], stream = repcap.Stream.Default) \n
		...
		self._core.io.write(f'CONFigure:LTE:SIGNaling<Instance>:CONNection:PCC:UDTTibased:DL{stream_cmd_val}:ALL {param}'.rstrip())
