...
class AllCls:
	...
	def set(self, number_rb: float, start_rb: int, narrow_band: int, modulation: enums.Modulation, transp_block_size_idx: int) -> None:
		"""SCPI: CONFigure:LTE:SIGNaling<instance>:CONNection[:PCC]:EMAMode:B:DL:ALL \n
		Snippet: driver.configure.connection.pcc.emamode.b.downlink.all.set(number_rb = 1.0, start_rb = 1, narrow_band = 1, modulation = enums.Modulation.Q1024, transp_block_size_idx = 1) \n
		Configures the eMTC auto mode, downlink, for CE mode B. The indicated input ranges list all possible values. The ranges
		have dependencies described in the background information, see 'eMTC auto mode'. \n
			:param number_rb: numeric | ZERO | N4 | N6 Number of allocated resource blocks
			:param start_rb: numeric Position of first resource block Range: 0 to 0
			:param narrow_band: numeric Narrowband for the first transmission Range: 0 to 15
			:param modulation: QPSK Modulation type QPSK
			:param transp_block_size_idx: numeric Transport block size index Range: 0 to 9
		"""
		param = ArgSingleList().compose_cmd_string(ArgSingle('number_rb', number_rb, DataType.Float), ArgSingle('start_rb', start_rb, DataType.Integer), ArgSingle('narrow_band', narrow_band, DataType.Integer), ArgSingle('modulation', modulation, DataType.Enum, enums.Modulation), ArgSingle('transp_block_size_idx', transp_block_size_idx, DataType.Integer))
		self._core.io.write(f'CONFigure:LTE:SIGNaling<Instance>:CONNection:PCC:EMAMode:B:DL:ALL {param}'.rstrip())
	...
	class AllStruct(StructBase):
		...
		__meta_args_list = [
			ArgStruct.scalar_float('Number_Rb'),
	...
	def get(self) -> AllStruct:
		"""SCPI: CONFigure:LTE:SIGNaling<instance>:CONNection[:PCC]:EMAMode:B:DL:ALL \n
		Snippet: value: AllStruct = driver.configure.connection.pcc.emamode.b.downlink.all.get() \n
		Configures the eMTC auto mode, downlink, for CE mode B. The indicated input ranges list all possible values. The ranges
		have dependencies described in the background information, see 'eMTC auto mode'. \n
			:return: structure: for return value, see the help for AllStruct structure arguments."""
		return self._core.io.query_struct(f'CONFigure:LTE:SIGNaling<Instance>:CONNection:PCC:EMAMode:B:DL:ALL?', self.__class__.AllStruct())
