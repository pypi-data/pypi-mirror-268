...
class UplinkCls:
	...
	def set(self, number_rb: int, start_rb: int, modulation: enums.Modulation, trans_block_size_idx: int) -> None:
		...
		Snippet: driver.configure.connection.pcc.udChannels.emtc.b.uplink.set(number_rb = 1, start_rb = 1, modulation = enums.Modulation.Q1024, trans_block_size_idx = 1) \n
		...
		self._core.io.write(f'CONFigure:LTE:SIGNaling<Instance>:CONNection:PCC:UDCHannels:EMTC:B:UL {param}'.rstrip())
	...
	def get(self) -> UplinkStruct:
		"""SCPI: CONFigure:LTE:SIGNaling<instance>:CONNection[:PCC]:UDCHannels:EMTC:B:UL \n
		Snippet: value: UplinkStruct = driver.configure.connection.pcc.udChannels.emtc.b.uplink.get() \n
		No command help available \n
			:return: structure: for return value, see the help for UplinkStruct structure arguments."""
		return self._core.io.query_struct(f'CONFigure:LTE:SIGNaling<Instance>:CONNection:PCC:UDCHannels:EMTC:B:UL?', self.__class__.UplinkStruct())
