...
class EightCls:
	"""Eight commands group definition. 1 total commands, 0 Subgroups, 1 group commands
	Repeated Capability: MatrixEightLine, default value after init: MatrixEightLine.Nr1"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._cmd_group = CommandsGroup("eight", core, parent)
		self._cmd_group.rep_cap = RepeatedCapability(self._cmd_group.group_name, 'repcap_matrixEightLine_get', 'repcap_matrixEightLine_set', repcap.MatrixEightLine.Nr1)

	def repcap_matrixEightLine_set(self, matrixEightLine: repcap.MatrixEightLine) -> None:
		...
		self._cmd_group.set_repcap_enum_value(matrixEightLine)
	...
	def set(self, structure: SetStruct, secondaryCompCarrier=repcap.SecondaryCompCarrier.Default, matrixEightLine=repcap.MatrixEightLine.Default) -> None:
		"""SCPI: CONFigure:LTE:SIGNaling<instance>:CONNection:SCC<Carrier>:TM<nr>:CMATrix:EIGHt<line> \n
		...
		driver.configure.connection.scc.tm.cmatrix.eight.set(structure, secondaryCompCarrier = repcap.SecondaryCompCarrier.Default, matrixEightLine = repcap.MatrixEightLine.Default) \n
		...
		matrixEightLine_cmd_val = self._cmd_group.get_repcap_cmd_value(matrixEightLine, repcap.MatrixEightLine)
		self._core.io.write_struct(f'CONFigure:LTE:SIGNaling<Instance>:CONNection:SCC{secondaryCompCarrier_cmd_val}:TM9:CMATrix:EIGHt{matrixEightLine_cmd_val}', structure)
	...
	def get(self, secondaryCompCarrier=repcap.SecondaryCompCarrier.Default, matrixEightLine=repcap.MatrixEightLine.Default) -> GetStruct:
		"""SCPI: CONFigure:LTE:SIGNaling<instance>:CONNection:SCC<Carrier>:TM<nr>:CMATrix:EIGHt<line> \n
		Snippet: value: GetStruct = driver.configure.connection.scc.tm.cmatrix.eight.get(secondaryCompCarrier = repcap.SecondaryCompCarrier.Default, matrixEightLine = repcap.MatrixEightLine.Default) \n
		Configures the 8x2 channel coefficients for TM 9.
			INTRO_CMD_HELP: There are two types of parameters: \n
			- <hnmabs> defines the square of the magnitude of the channel coefficient nm: <hnmabs> = (hnm) 2 The sum of all values in one matrix line must not be greater than 1. <h8xabs> is calculated automatically, so that the sum equals 1.
			- <hnmphi> defines the phase of the channel coefficient nm: <hnmphi> = φ(hnm)
		...
		secondaryCompCarrier_cmd_val = self._cmd_group.get_repcap_cmd_value(secondaryCompCarrier, repcap.SecondaryCompCarrier)
		matrixEightLine_cmd_val = self._cmd_group.get_repcap_cmd_value(matrixEightLine, repcap.MatrixEightLine)
		return self._core.io.query_struct(f'CONFigure:LTE:SIGNaling<Instance>:CONNection:SCC{secondaryCompCarrier_cmd_val}:TM9:CMATrix:EIGHt{matrixEightLine_cmd_val}?', self.__class__.GetStruct())

	def clone(self) -> 'EightCls':
		"""Clones the group by creating new object from it and its whole existing subgroups
		Also copies all the existing default Repeated Capabilities setting,
		which you can change independently without affecting the original group"""
		new_group = EightCls(self._core, self._cmd_group.parent)
