...
class DownlinkCls:
	...
	def set(self, cluster: str, modulation: enums.Modulation, trans_block_size_idx: int, secondaryCompCarrier=repcap.SecondaryCompCarrier.Default, stream=repcap.Stream.Default) -> None:
		...
		Snippet: driver.configure.connection.scc.udChannels.laa.fburst.fullSubFrames.mcluster.downlink.set(cluster = rawAbc, modulation = enums.Modulation.Q1024, trans_block_size_idx = 1, secondaryCompCarrier = repcap.SecondaryCompCarrier.Default, stream = repcap.Stream.Default) \n
		...
		self._core.io.write(f'CONFigure:LTE:SIGNaling<Instance>:CONNection:SCC{secondaryCompCarrier_cmd_val}:UDCHannels:LAA:FBURst:FSUBframes:MCLuster:DL{stream_cmd_val} {param}'.rstrip())
