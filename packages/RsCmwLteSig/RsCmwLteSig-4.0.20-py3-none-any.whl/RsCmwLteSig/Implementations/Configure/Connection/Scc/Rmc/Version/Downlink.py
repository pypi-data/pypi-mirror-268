...
class DownlinkCls:
	...
	def set(self, version: int, secondaryCompCarrier=repcap.SecondaryCompCarrier.Default, stream=repcap.Stream.Default) -> None:
		"""SCPI: CONFigure:LTE:SIGNaling<instance>:CONNection:SCC<Carrier>:RMC:VERSion:DL<Stream> \n
		Snippet: driver.configure.connection.scc.rmc.version.downlink.set(version = 1, secondaryCompCarrier = repcap.SecondaryCompCarrier.Default, stream = repcap.Stream.Default) \n
		...
		multiple antenna configurations, see 'DL RMCs, multiple TX antennas (TM 2 to 6) '. \n
			:param version: integer Range: 0 to 1
		...
		self._core.io.write(f'CONFigure:LTE:SIGNaling<Instance>:CONNection:SCC{secondaryCompCarrier_cmd_val}:RMC:VERSion:DL{stream_cmd_val} {param}')

	def get(self, secondaryCompCarrier=repcap.SecondaryCompCarrier.Default, stream=repcap.Stream.Default) -> int:
		"""SCPI: CONFigure:LTE:SIGNaling<instance>:CONNection:SCC<Carrier>:RMC:VERSion:DL<Stream> \n
		Snippet: value: int = driver.configure.connection.scc.rmc.version.downlink.get(secondaryCompCarrier = repcap.SecondaryCompCarrier.Default, stream = repcap.Stream.Default) \n
		Selects the version to distinguish ambiguous RMCs. This command is only relevant for certain downlink RMCs for TDD
		multiple antenna configurations, see 'DL RMCs, multiple TX antennas (TM 2 to 6) '. \n
			:param secondaryCompCarrier: optional repeated capability selector. Default value: CC1 (settable in the interface 'Scc')
			:param stream: optional repeated capability selector. Default value: S1 (settable in the interface 'Downlink')
			:return: version: integer Range: 0 to 1"""
		secondaryCompCarrier_cmd_val = self._cmd_group.get_repcap_cmd_value(secondaryCompCarrier, repcap.SecondaryCompCarrier)
		stream_cmd_val = self._cmd_group.get_repcap_cmd_value(stream, repcap.Stream)
		response = self._core.io.query_str(f'CONFigure:LTE:SIGNaling<Instance>:CONNection:SCC{secondaryCompCarrier_cmd_val}:RMC:VERSion:DL{stream_cmd_val}?')
