...
class FourCls:
	...
	def set(self, structure: SetStruct, secondaryCompCarrier=repcap.SecondaryCompCarrier.Default, matrixFourLine=repcap.MatrixFourLine.Default) -> None:
		...
		self._core.io.write_struct(f'CONFigure:LTE:SIGNaling<Instance>:CONNection:SCC{secondaryCompCarrier_cmd_val}:TM9:CMATrix:FOUR{matrixFourLine_cmd_val}', structure)
	...
	def get(self, secondaryCompCarrier=repcap.SecondaryCompCarrier.Default, matrixFourLine=repcap.MatrixFourLine.Default) -> GetStruct:
		"""SCPI: CONFigure:LTE:SIGNaling<instance>:CONNection:SCC<carrier>:TM<nr>:CMATrix:FOUR<line> \n
		Snippet: value: GetStruct = driver.configure.connection.scc.tm.cmatrix.four.get(secondaryCompCarrier = repcap.SecondaryCompCarrier.Default, matrixFourLine = repcap.MatrixFourLine.Default) \n
		Configures the 4x2 channel coefficients for TM 9.
			INTRO_CMD_HELP: There are two types of parameters: \n
			- <hnmabs> defines the square of the magnitude of the channel coefficient nm: <hnmabs> = (hnm) 2 The sum of all values in one matrix line must not be greater than 1. <h4xabs> is calculated automatically, so that the sum equals 1.
			- <hnmphi> defines the phase of the channel coefficient nm: <hnmphi> = φ(hnm)
		...
		secondaryCompCarrier_cmd_val = self._cmd_group.get_repcap_cmd_value(secondaryCompCarrier, repcap.SecondaryCompCarrier)
		matrixFourLine_cmd_val = self._cmd_group.get_repcap_cmd_value(matrixFourLine, repcap.MatrixFourLine)
		return self._core.io.query_struct(f'CONFigure:LTE:SIGNaling<Instance>:CONNection:SCC{secondaryCompCarrier_cmd_val}:TM9:CMATrix:FOUR{matrixFourLine_cmd_val}?', self.__class__.GetStruct())
