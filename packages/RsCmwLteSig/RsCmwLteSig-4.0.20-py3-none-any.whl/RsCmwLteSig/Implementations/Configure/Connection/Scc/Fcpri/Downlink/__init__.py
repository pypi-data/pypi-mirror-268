...
class DownlinkCls:
	...
	def set(self, number_rb: int, start_rb: int, table: enums.MultiClusterDlTable, secondaryCompCarrier=repcap.SecondaryCompCarrier.Default) -> None:
		...
		Snippet: driver.configure.connection.scc.fcpri.downlink.set(number_rb = 1, start_rb = 1, table = enums.MultiClusterDlTable.DETermined, secondaryCompCarrier = repcap.SecondaryCompCarrier.Default) \n
		...
		self._core.io.write(f'CONFigure:LTE:SIGNaling<Instance>:CONNection:SCC{secondaryCompCarrier_cmd_val}:FCPRi:DL {param}'.rstrip())
