...
class DownlinkCls:
	...
	def set(self, cluster: str, modulation: enums.Modulation, trans_block_size_idx: int, secondaryCompCarrier=repcap.SecondaryCompCarrier.Default, stream=repcap.Stream.Default) -> None:
		"""SCPI: CONFigure:LTE:SIGNaling<instance>:CONNection:SCC<Carrier>:UDCHannels:LAA:RBURst:PIPSubframes:MCLuster:DL<Stream> \n
		Snippet: driver.configure.connection.scc.udChannels.laa.rburst.pipSubFrames.mcluster.downlink.set(cluster = rawAbc, modulation = enums.Modulation.Q1024, trans_block_size_idx = 1, secondaryCompCarrier = repcap.SecondaryCompCarrier.Default, stream = repcap.Stream.Default) \n
		Configures a user-defined downlink channel with multi-cluster allocation, for LAA, random bursts. The <Cluster> setting
		applies to all subframes of the burst and to all DL streams. The other settings apply to initial subframes with partial
		...
		self._core.io.write(f'CONFigure:LTE:SIGNaling<Instance>:CONNection:SCC{secondaryCompCarrier_cmd_val}:UDCHannels:LAA:RBURst:PIPSubframes:MCLuster:DL{stream_cmd_val} {param}'.rstrip())
	...
	def get(self, secondaryCompCarrier=repcap.SecondaryCompCarrier.Default, stream=repcap.Stream.Default) -> DownlinkStruct:
		"""SCPI: CONFigure:LTE:SIGNaling<instance>:CONNection:SCC<Carrier>:UDCHannels:LAA:RBURst:PIPSubframes:MCLuster:DL<Stream> \n
		Snippet: value: DownlinkStruct = driver.configure.connection.scc.udChannels.laa.rburst.pipSubFrames.mcluster.downlink.get(secondaryCompCarrier = repcap.SecondaryCompCarrier.Default, stream = repcap.Stream.Default) \n
		Configures a user-defined downlink channel with multi-cluster allocation, for LAA, random bursts. The <Cluster> setting
		applies to all subframes of the burst and to all DL streams. The other settings apply to initial subframes with partial
		allocation and DL stream <s>. The allowed input ranges have dependencies and are described in the background information,
		see 'User-defined channels for LAA' and especially Table 'RBG parameters'. \n
		...
		secondaryCompCarrier_cmd_val = self._cmd_group.get_repcap_cmd_value(secondaryCompCarrier, repcap.SecondaryCompCarrier)
		stream_cmd_val = self._cmd_group.get_repcap_cmd_value(stream, repcap.Stream)
		return self._core.io.query_struct(f'CONFigure:LTE:SIGNaling<Instance>:CONNection:SCC{secondaryCompCarrier_cmd_val}:UDCHannels:LAA:RBURst:PIPSubframes:MCLuster:DL{stream_cmd_val}?', self.__class__.DownlinkStruct())
