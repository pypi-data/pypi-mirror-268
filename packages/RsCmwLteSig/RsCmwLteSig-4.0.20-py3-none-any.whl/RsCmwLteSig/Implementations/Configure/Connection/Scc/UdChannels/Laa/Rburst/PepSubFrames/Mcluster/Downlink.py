...
class DownlinkCls:
	...
	def set(self, symbols: enums.Symbols, cluster: float, modulation: enums.Modulation, trans_block_size_idx: int, secondaryCompCarrier=repcap.SecondaryCompCarrier.Default, stream=repcap.Stream.Default) -> None:
		...
		ranges have dependencies and are described in the background information, see 'User-defined channels for LAA'. \n
		...
		"""
		param = ArgSingleList().compose_cmd_string(ArgSingle('symbols', symbols, DataType.Enum, enums.Symbols), ArgSingle('cluster', cluster, DataType.Float), ArgSingle('modulation', modulation, DataType.Enum, enums.Modulation), ArgSingle('trans_block_size_idx', trans_block_size_idx, DataType.Integer))
