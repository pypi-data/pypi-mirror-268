...
class AllCls:
	...
	def set(self, number_rb: List[int or bool], start_rb: List[int or bool], modulation: List[enums.Modulation], trans_block_size_idx: List[int or bool], secondaryCompCarrier=repcap.SecondaryCompCarrier.Default) -> None:
		"""SCPI: CONFigure:LTE:SIGNaling<instance>:CONNection:SCC<Carrier>:UDTTibased:UL:ALL \n
		Snippet: driver.configure.connection.scc.udttiBased.uplink.all.set(number_rb = [1, True, 2, False, 3], start_rb = [1, True, 2, False, 3], modulation = [Modulation.Q1024, Modulation.QPSK], trans_block_size_idx = [1, True, 2, False, 3], secondaryCompCarrier = repcap.SecondaryCompCarrier.Default) \n
		...
		self._core.io.write(f'CONFigure:LTE:SIGNaling<Instance>:CONNection:SCC{secondaryCompCarrier_cmd_val}:UDTTibased:UL:ALL {param}'.rstrip())
