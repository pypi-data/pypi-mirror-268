...
class AllCls:
	...
	def set(self, number_rb: List[int or bool], start_rb: List[int or bool], cqi_idx: List[int or bool], secondaryCompCarrier=repcap.SecondaryCompCarrier.Default, stream=repcap.Stream.Default) -> None:
		"""SCPI: CONFigure:LTE:SIGNaling<instance>:CONNection:SCC<Carrier>:FCTTibased:DL<Stream>:ALL \n
		Snippet: driver.configure.connection.scc.fcttiBased.downlink.all.set(number_rb = [1, True, 2, False, 3], start_rb = [1, True, 2, False, 3], cqi_idx = [1, True, 2, False, 3], secondaryCompCarrier = repcap.SecondaryCompCarrier.Default, stream = repcap.Stream.Default) \n
		...
		self._core.io.write(f'CONFigure:LTE:SIGNaling<Instance>:CONNection:SCC{secondaryCompCarrier_cmd_val}:FCTTibased:DL{stream_cmd_val}:ALL {param}'.rstrip())
	...
	def get(self, secondaryCompCarrier=repcap.SecondaryCompCarrier.Default, stream=repcap.Stream.Default) -> AllStruct:
		"""SCPI: CONFigure:LTE:SIGNaling<instance>:CONNection:SCC<Carrier>:FCTTibased:DL<Stream>:ALL \n
		Snippet: value: AllStruct = driver.configure.connection.scc.fcttiBased.downlink.all.get(secondaryCompCarrier = repcap.SecondaryCompCarrier.Default, stream = repcap.Stream.Default) \n
		Configures the downlink channel for the scheduling type 'Fixed CQI'. The parameters are entered 10 times, so that all
		subframes are configured by a single command (index = subframe number 0 to 9) : <NumberRB>0, ..., <NumberRB>9, <StartRB>0,
		..., <StartRB>9, <CQIIdx>0, ..., <CQIIdx>9 The allowed input ranges have dependencies and are described in the background
		information, see 'CQI channels'. For TDD UL and special subframes, you can set OFF or specify a number from the allowed
		...
		secondaryCompCarrier_cmd_val = self._cmd_group.get_repcap_cmd_value(secondaryCompCarrier, repcap.SecondaryCompCarrier)
		stream_cmd_val = self._cmd_group.get_repcap_cmd_value(stream, repcap.Stream)
		return self._core.io.query_struct(f'CONFigure:LTE:SIGNaling<Instance>:CONNection:SCC{secondaryCompCarrier_cmd_val}:FCTTibased:DL{stream_cmd_val}:ALL?', self.__class__.AllStruct())
