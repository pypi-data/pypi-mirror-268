...
class TimeCls:
	...
	def set(self, hour: int, minute: int, second: int) -> None:
		...
		Snippet: driver.configure.cell.time.time.set(hour = 1, minute = 1, second = 1) \n
		...
		self._core.io.write(f'CONFigure:LTE:SIGNaling<Instance>:CELL:TIME:TIME {param}'.rstrip())
	...
	def get(self) -> TimeStruct:
		"""SCPI: CONFigure:LTE:SIGNaling<instance>:CELL:TIME:TIME \n
		Snippet: value: TimeStruct = driver.configure.cell.time.time.get() \n
		Specifies the UTC time for the time source DATE (see method RsCmwLteSig.Configure.Cell.Time.tsource) . \n
			:return: structure: for return value, see the help for TimeStruct structure arguments."""
		return self._core.io.query_struct(f'CONFigure:LTE:SIGNaling<Instance>:CELL:TIME:TIME?', self.__class__.TimeStruct())
