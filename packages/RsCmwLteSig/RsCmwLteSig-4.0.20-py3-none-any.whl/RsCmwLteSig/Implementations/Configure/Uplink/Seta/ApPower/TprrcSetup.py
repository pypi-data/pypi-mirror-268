...
class TprrcSetupCls:
	...
	def get_advanced(self) -> bool:
		...
		Snippet: value: bool = driver.configure.uplink.seta.apPower.tprrcSetup.get_advanced() \n
		...
		"""
		response = self._core.io.query_str('CONFigure:LTE:SIGNaling<Instance>:UL:SETA:APPower:TPRRcsetup:ADVanced?')
		return Conversions.str_to_bool(response)

	def set_advanced(self, enable: bool) -> None:
		"""SCPI: CONFigure:LTE:SIGNaling<instance>:UL:SETA:APPower:TPRRcsetup:ADVanced \n
		Snippet: driver.configure.uplink.seta.apPower.tprrcSetup.set_advanced(enable = False) \n
		Enables or disables P0-UE-PUSCH toggling and thus determines the P0-UE-PUSCH values signaled to the UE during RRC
		connection setup if advanced UL power configuration applies. \n
			:param enable: OFF | ON
		"""
		param = Conversions.bool_to_str(enable)
		self._core.io.write(f'CONFigure:LTE:SIGNaling<Instance>:UL:SETA:APPower:TPRRcsetup:ADVanced {param}')
