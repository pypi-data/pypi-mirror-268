...
class PucchCls:
	...
	def get_clt_power(self) -> int:
		...
		Snippet: value: int = driver.configure.uplink.pcc.pucch.get_clt_power() \n
		No command help available \n
			:return: power: No help available
		"""
		response = self._core.io.query_str('CONFigure:LTE:SIGNaling<Instance>:UL:PCC:PUCCh:CLTPower?')
		return Conversions.str_to_int(response)

	def set_clt_power(self, power: int) -> None:
		"""SCPI: CONFigure:LTE:SIGNaling<instance>:UL[:PCC]:PUCCh:CLTPower \n
		Snippet: driver.configure.uplink.pcc.pucch.set_clt_power(power = 1) \n
		No command help available \n
			:param power: No help available
		"""
		param = Conversions.decimal_value_to_str(power)
		self._core.io.write(f'CONFigure:LTE:SIGNaling<Instance>:UL:PCC:PUCCh:CLTPower {param}')
