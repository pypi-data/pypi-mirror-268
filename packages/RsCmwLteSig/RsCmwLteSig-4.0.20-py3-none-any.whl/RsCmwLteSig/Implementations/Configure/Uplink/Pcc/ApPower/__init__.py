...
class ApPowerCls:
	...
	def get_ea_settings(self) -> bool:
		...
		Snippet: value: bool = driver.configure.uplink.pcc.apPower.get_ea_settings() \n
		...
		"""
		response = self._core.io.query_str('CONFigure:LTE:SIGNaling<Instance>:UL:PCC:APPower:EASettings?')
		return Conversions.str_to_bool(response)

	def set_ea_settings(self, enable: bool) -> None:
		"""SCPI: CONFigure:LTE:SIGNaling<instance>:UL[:PCC]:APPower:EASettings \n
		Snippet: driver.configure.uplink.pcc.apPower.set_ea_settings(enable = False) \n
		Enables or disables advanced configuration of the PRACH and open loop power settings via the other
		CONFigure:LTE:SIGN:UL:PCC/SCC<c>:APPower:... commands. \n
			:param enable: OFF | ON
		"""
		param = Conversions.bool_to_str(enable)
		self._core.io.write(f'CONFigure:LTE:SIGNaling<Instance>:UL:PCC:APPower:EASettings {param}')
