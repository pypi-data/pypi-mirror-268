...
class PathCls:
	...
	def set(self, pep: float, level: float, secondaryCompCarrier=repcap.SecondaryCompCarrier.Default, path=repcap.Path.Default) -> None:
		"""SCPI: CONFigure:LTE:SIGNaling<instance>:IQIN:SCC<Carrier>:PATH<n> \n
		Snippet: driver.configure.iqIn.scc.path.set(pep = 1.0, level = 1.0, secondaryCompCarrier = repcap.SecondaryCompCarrier.Default, path = repcap.Path.Default) \n
		...
		self._core.io.write(f'CONFigure:LTE:SIGNaling<Instance>:IQIN:SCC{secondaryCompCarrier_cmd_val}:PATH{path_cmd_val} {param}'.rstrip())
