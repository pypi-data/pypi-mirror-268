...
class ThresholdsCls:
	...
	def set(self, high: int, low: int) -> None:
		...
		Snippet: driver.configure.ncell.tdscdma.thresholds.set(high = 1, low = 1) \n
		...
		self._core.io.write(f'CONFigure:LTE:SIGNaling<Instance>:NCELl:TDSCdma:THResholds {param}'.rstrip())
	...
	def get(self) -> ThresholdsStruct:
		"""SCPI: CONFigure:LTE:SIGNaling<instance>:NCELl:TDSCdma:THResholds \n
		Snippet: value: ThresholdsStruct = driver.configure.ncell.tdscdma.thresholds.get() \n
		No command help available \n
			:return: structure: for return value, see the help for ThresholdsStruct structure arguments."""
		return self._core.io.query_struct(f'CONFigure:LTE:SIGNaling<Instance>:NCELl:TDSCdma:THResholds?', self.__class__.ThresholdsStruct())

	def get_low(self) -> int:
		"""SCPI: CONFigure:LTE:SIGNaling<instance>:NCELl:TDSCdma:THResholds:LOW \n
		Snippet: value: int = driver.configure.ncell.tdscdma.thresholds.get_low() \n
		Configures the reselection threshold value 'threshX-Low' for TD-SCDMA neighbor cells. \n
			:return: low: numeric Range: 0 to 31
		"""
		response = self._core.io.query_str('CONFigure:LTE:SIGNaling<Instance>:NCELl:TDSCdma:THResholds:LOW?')
		return Conversions.str_to_int(response)

	def set_low(self, low: int) -> None:
		"""SCPI: CONFigure:LTE:SIGNaling<instance>:NCELl:TDSCdma:THResholds:LOW \n
		Snippet: driver.configure.ncell.tdscdma.thresholds.set_low(low = 1) \n
		Configures the reselection threshold value 'threshX-Low' for TD-SCDMA neighbor cells. \n
			:param low: numeric Range: 0 to 31
		"""
		param = Conversions.decimal_value_to_str(low)
		self._core.io.write(f'CONFigure:LTE:SIGNaling<Instance>:NCELl:TDSCdma:THResholds:LOW {param}')
