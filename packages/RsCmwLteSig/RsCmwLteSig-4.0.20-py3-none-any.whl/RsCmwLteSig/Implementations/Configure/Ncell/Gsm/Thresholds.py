...
class ThresholdsCls:
	...
	def set(self, high: int, low: int) -> None:
		...
		Snippet: driver.configure.ncell.gsm.thresholds.set(high = 1, low = 1) \n
		...
		self._core.io.write(f'CONFigure:LTE:SIGNaling<Instance>:NCELl:GSM:THResholds {param}'.rstrip())
	...
	def get(self) -> ThresholdsStruct:
		"""SCPI: CONFigure:LTE:SIGNaling<instance>:NCELl:GSM:THResholds \n
		Snippet: value: ThresholdsStruct = driver.configure.ncell.gsm.thresholds.get() \n
		No command help available \n
			:return: structure: for return value, see the help for ThresholdsStruct structure arguments."""
		return self._core.io.query_struct(f'CONFigure:LTE:SIGNaling<Instance>:NCELl:GSM:THResholds?', self.__class__.ThresholdsStruct())

	def get_low(self) -> int:
		"""SCPI: CONFigure:LTE:SIGNaling<instance>:NCELl:GSM:THResholds:LOW \n
		Snippet: value: int = driver.configure.ncell.gsm.thresholds.get_low() \n
		Configures the reselection threshold value 'threshX-Low' for GSM neighbor cells. \n
			:return: low: numeric Range: 0 to 31
		"""
		response = self._core.io.query_str('CONFigure:LTE:SIGNaling<Instance>:NCELl:GSM:THResholds:LOW?')
		return Conversions.str_to_int(response)

	def set_low(self, low: int) -> None:
		"""SCPI: CONFigure:LTE:SIGNaling<instance>:NCELl:GSM:THResholds:LOW \n
		Snippet: driver.configure.ncell.gsm.thresholds.set_low(low = 1) \n
		Configures the reselection threshold value 'threshX-Low' for GSM neighbor cells. \n
			:param low: numeric Range: 0 to 31
		"""
		param = Conversions.decimal_value_to_str(low)
		self._core.io.write(f'CONFigure:LTE:SIGNaling<Instance>:NCELl:GSM:THResholds:LOW {param}')
