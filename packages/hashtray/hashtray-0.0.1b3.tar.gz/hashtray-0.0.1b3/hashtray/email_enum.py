...
class EmailEnum:
    ...
    def combinator(self):
        ...
        self.bar.write(
            f"Chunks: {self.show_chunks(elements)} / domains : {self.len_domains} / combinations : {self.n_combs}.\n"
        )
    ...
    def find(self):
        ...
        if result:
            self.c.print(
                f"\nEmail found with hash enumeration after {self.n} hashes in {self.bar.format_interval(self.elapsed)} at {self.bar.format_sizeof(self.n / self.elapsed)} hashes/s:",
                style="#1e8cbe",
            )
