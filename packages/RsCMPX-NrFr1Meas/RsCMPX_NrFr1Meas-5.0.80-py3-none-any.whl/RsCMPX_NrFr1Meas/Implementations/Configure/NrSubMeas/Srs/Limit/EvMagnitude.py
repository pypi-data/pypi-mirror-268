...
class EvMagnitudeCls:
	...
	def set(self, rms: float or bool, peak: float or bool) -> None:
		...
		Snippet: driver.configure.nrSubMeas.srs.limit.evMagnitude.set(rms = 1.0, peak = 1.0) \n
		...
		self._core.io.write(f'CONFigure:NRSub:MEASurement<Instance>:SRS:LIMit:EVMagnitude {param}'.rstrip())
	...
	def get(self) -> EvMagnitudeStruct:
		"""SCPI: CONFigure:NRSub:MEASurement<Instance>:SRS:LIMit:EVMagnitude \n
		Snippet: value: EvMagnitudeStruct = driver.configure.nrSubMeas.srs.limit.evMagnitude.get() \n
		No command help available \n
			:return: structure: for return value, see the help for EvMagnitudeStruct structure arguments."""
		return self._core.io.query_struct(f'CONFigure:NRSub:MEASurement<Instance>:SRS:LIMit:EVMagnitude?', self.__class__.EvMagnitudeStruct())
