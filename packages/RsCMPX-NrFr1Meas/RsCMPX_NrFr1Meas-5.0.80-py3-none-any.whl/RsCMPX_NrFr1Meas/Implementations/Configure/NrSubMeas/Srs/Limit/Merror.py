...
class MerrorCls:
	...
	def set(self, rms: float or bool, peak: float or bool) -> None:
		...
		Snippet: driver.configure.nrSubMeas.srs.limit.merror.set(rms = 1.0, peak = 1.0) \n
		...
		self._core.io.write(f'CONFigure:NRSub:MEASurement<Instance>:SRS:LIMit:MERRor {param}'.rstrip())
	...
	def get(self) -> MerrorStruct:
		"""SCPI: CONFigure:NRSub:MEASurement<Instance>:SRS:LIMit:MERRor \n
		Snippet: value: MerrorStruct = driver.configure.nrSubMeas.srs.limit.merror.get() \n
		No command help available \n
			:return: structure: for return value, see the help for MerrorStruct structure arguments."""
		return self._core.io.query_struct(f'CONFigure:NRSub:MEASurement<Instance>:SRS:LIMit:MERRor?', self.__class__.MerrorStruct())
