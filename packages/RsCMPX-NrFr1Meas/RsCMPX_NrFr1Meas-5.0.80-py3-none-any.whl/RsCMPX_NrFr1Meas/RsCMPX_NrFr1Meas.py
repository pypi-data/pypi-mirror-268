...
class RsCMPX_NrFr1Meas:
	"""1055 total commands, 4 Subgroups, 0 group commands"""
	_driver_options = "SupportedInstrModels = CMX/CMP/CMW/PVT, SupportedIdnPatterns = CMX/CMP/CMW/PVT, SimulationIdnString = 'Rohde&Schwarz,CMP180,100001,5.0.80.0017'"
	...
	def __init__(self, resource_name: str, id_query: bool = True, reset: bool = False, options: str = None, direct_session: object = None):
		"""Initializes new RsCMPX_NrFr1Meas session. \n
		...
		self._core = Core(resource_name, id_query, reset, RsCMPX_NrFr1Meas._driver_options, options, direct_session)
		self._core.driver_version = '5.0.80.0017'
	...
	@classmethod
	def from_existing_session(cls, session: object, options: str = None) -> 'RsCMPX_NrFr1Meas':
	...
	def __str__(self) -> str:
		if self._core.io:
			return f"RsCMPX_NrFr1Meas session '{self._core.io.resource_name}'"
		else:
			return f"RsCMPX_NrFr1Meas with session closed"
	...
	def assert_minimum_version(min_version: str) -> None:
		...
		curr_version_list = '5.0.80.0017'.split('.')
		...
		for i in range(count):
			...
			if curr > minimum:
				break
			if curr < minimum:
				raise RsInstrException(f"Assertion for minimum RsCMPX_NrFr1Meas version failed. Current version: '5.0.80.0017', minimum required version: '{min_version}'")
	...
	def _sync_to_custom_properties(self, cloned: 'RsCMPX_NrFr1Meas') -> None:
	...
	@property
	def nrSubMeas(self):
		"""nrSubMeas commands group. 3 Sub-classes, 0 commands."""
		if not hasattr(self, '_nrSubMeas'):
			from .Implementations.NrSubMeas import NrSubMeasCls
	...
	def clone(self) -> 'RsCMPX_NrFr1Meas':
		"""Creates a deep copy of the RsCMPX_NrFr1Meas object. Also copies:
		...
		cloned = RsCMPX_NrFr1Meas.from_existing_session(self.get_session_handle(), self._options)
