...
async def download_asset_stream(data: AssetData, chunks: collections.abc.AsyncGenerator[bytes, None] | None,
    ...
    if accepted_request:
        # await logger.debug(f"download asset stream accept message {format_msg(accepted_request)}")
...
async def upload_asset_stream(creator: collections.abc.AsyncGenerator[int, bytes | None], stream: Stream, *,
    ...
    try:
        ...
        while not done:
            ...
            # await logger.debug(f"upload asset stream chunk message {format_msg(chunk_request, suppress=['chunk'])}")
            if chunk_request:
