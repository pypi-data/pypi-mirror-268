...
class DataConnection(BaseDataConnection):
    ...
    def __init__(self,
        ...
        elif isinstance(location, (AssetLocation, CloudAssetLocation,
                                    DeploymentOutputAssetLocation)):
    ...
    def _recreate_holdout(
        ...
        if self.auto_pipeline_params.get('prediction_columns') is not None:
            ...
            if self.auto_pipeline_params.get('lookback_window') == -1 or self.auto_pipeline_params.get('lookback_window') is None:
                ts_metrics = self._api_client.training.get_details(self.auto_pipeline_params.get('run_id'), _internal=True)['entity']['status']['metrics']
        ...
        **Examples**
        ...
        try:
            use_flight = kwargs.get(
                'use_flight',
                bool((self._api_client is not None or 'USER_ACCESS_TOKEN' in os.environ or 'RUNTIME_ENV_ACCESS_TOKEN_FILE' in os.environ) and 
                        self._api_client.CPD_version))
        except:
            use_flight = False
        ...
        if self.type == DataConnectionTypes.S3:
            ...
            if use_flight:
                ...
                try:
                    ...
                    if self._api_client and ((self._is_data_asset_normal() and self._is_size_acceptable()) or self._is_data_asset_nfs()) :
                        import warnings
    ...
    def write(self, data: Union[str, 'DataFrame'], remote_name: str = None, **kwargs) -> None:
        ...
        if self._api_client is None:
            token = self._get_token_from_environment()
            if token is None:
                raise ConnectionError("API client missing. Please initialize API client and pass it to "
...
class S3Connection(BaseConnection):
    ...
    def __init__(self, endpoint_url: str, access_key_id: str = None, secret_access_key: str = None,
        ...
        if not _internal_use:
            warn(message="S3 DataConnection is not supported. Please use data_asset_id instead.")
...
class FSLocation(BaseLocation):
    ...
    def _get_file_size(self, workspace: 'WorkSpace') -> 'int':
        ...
        try:
            ...
            url = workspace.api_client.service_instance._href_definitions.get_wsd_model_attachment_href() \
                  + f"/{self.path.split('/assets/')[-1]}"
            path_info_response = requests.head(url, headers=workspace.api_client._get_headers(),
...
class AssetLocation(BaseLocation):
    ...
    def _get_file_size(self, workspace: 'WorkSpace', *args) -> 'int':
        asset_info_response = requests.get(
            workspace.api_client.service_instance._href_definitions.get_data_asset_href(self.id),
            params=workspace.api_client._params(),
    ...
    def api_client(self, var):
        ...
        if self.__api_client:
            self.href = self.__api_client.service_instance._href_definitions.get_base_asset_href(self._initial_asset_id)
        else:
        ...
        if self.__api_client:
            if self.__api_client.default_space_id:
                ...
                self.href = f'{self.href}?project_id={self.__api_client.default_project_id}'
...
class NFSLocation(BaseLocation):
    ...
    def _get_file_size(self, workspace: 'Workspace', *args) -> 'int':
        ...
        href = workspace.api_client.connections._href_definitions.get_connection_by_id_href(self.id) + '/assets'
        asset_info_response = requests.get(href,
                                           params=params, headers=workspace.api_client._get_headers(None))
        if asset_info_response.status_code != 200:
