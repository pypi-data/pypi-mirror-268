...
class TestDataAsset(unittest.TestCase):
    ...
    def setUp(cls) -> None:
        ...
        cls.flight_service_location = cls.credentials.get('flight_service_location')
        cls.flight_service_port = cls.credentials.get('flight_service_port', 443)
    ...
    def test_00__prepare_normal_data_asset(self):
        ...
        TestDataAsset.data_asset_id = self.api_client.data_assets.get_id(asset_details)
    ...
    def test_02__connect_wml_client_to_data_connection(self):
        try:
            ...
            self.data_connection._api_client = self.api_client

        self.assertIsNotNone(self.data_connection._api_client)
        self.assertIsInstance(self.data_connection._api_client, APIClient)

    def test_03_read_data_asset_with_data_asset_get_content_client_core(self):
        content = self.api_client.data_assets.get_content(self.data_asset_id)
        pprint(content)
    ...
    def test_10b__prepare_connected_data_asset(self):
        asset_details = self.api_client.data_assets.store({
            ...
            self.api_client.data_assets.ConfigurationMetaNames.NAME: "Connected data asset unit test",
            self.api_client.data_assets.ConfigurationMetaNames.DATA_CONTENT_NAME: os.path.join(self.bucket_name,
                                                                                               self.data_cos_path)
        })

        TestDataAsset.connected_data_asset_id = self.api_client.data_assets.get_id(asset_details)
    ...
    def test_12__connect_wml_client_to_data_connection_cda(self):
        try:
            self.data_connection_cda.set_client(self.api_client)
        except AttributeError:
            self.data_connection_cda._api_client = self.api_client

        self.assertIsNotNone(self.data_connection_cda._api_client)
        self.assertIsInstance(self.data_connection_cda._api_client, APIClient)
    ...
    def test_99_delete_data_asset_and_connection(self):
        self.api_client.data_assets.delete(self.data_asset_id)
        self.api_client.data_assets.delete(self.connected_data_asset_id)
