...
class TestAutoAIRemote(unittest.TestCase):
    ...
    def test_01_wml_credentials_development_False(self):
        if self.DEV_FIELD_NAME in self.credentials.keys():
            self.credentials.pop(self.DEV_FIELD_NAME)
        self.assertNotIn(self.DEV_FIELD_NAME, self.credentials.keys())

    def test_02_initialize_AutoAI_experiment(self):
        TestAutoAIRemote.experiment = AutoAI(credentials=self.credentials)
    ...
    def test_06_wml_credentials_add_development_True(self):
        ...
        self.credentials[self.DEV_FIELD_NAME] = True
        self.assertIn(self.DEV_FIELD_NAME, self.credentials.keys())
