...
class TestAutoAIRemote(unittest.TestCase):
    ...
    def test_00a_space_cleanup(self):
        space_cleanup(self.api_client,
                      ...
                      days_old=7)
        TestAutoAIRemote.space_id = get_space_id(self.api_client, self.space_name,
                                                 cos_resource_instance_id=self.cos_resource_instance_id)
    ...
    def test_01_initialize_AutoAI_experiment__pass_credentials__object_initialized(self):
        TestAutoAIRemote.experiment = AutoAI(credentials=self.credentials.copy(),
    ...
    def test_21_deployment_setup_and_preparation(self):
        TestAutoAIRemote.service = WebService(source_instance_credentials=self.credentials.copy(),
    ...
    def test_30_batch_deployment_setup_and_preparation(self):
        TestAutoAIRemote.service_batch = Batch(source_instance_credentials=self.credentials.copy(),
                                               source_project_id=self.project_id,
                                               target_instance_credentials=self.credentials,
                                               target_space_id=self.space_id)
