...
class TestAutoAIRemote(AbstractTestAutoAIRemote, unittest.TestCase):
    ...
    def test_00c_prepare_connection_to_postgres(self):
        ...
        properties['jar_uris'] = self.api_client.connections.sign_db_driver_url(driver_file_name)
    ...
    def test_00d_prepare_connected_data_asset(self):
        asset_details = self.api_client.data_assets.store({
            ...
            self.api_client.data_assets.ConfigurationMetaNames.DATA_CONTENT_NAME: f"/{self.connection_asset_details['schema_name']}/{self.connection_asset_details['table_name']}",
