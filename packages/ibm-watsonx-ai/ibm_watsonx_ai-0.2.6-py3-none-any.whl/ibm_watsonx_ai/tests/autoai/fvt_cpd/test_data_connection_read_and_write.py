...
class TestDataConnection(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.FLIGHT_SERVICE_LOCATION = cls.credentials.get('FLIGHT_SERVICE_LOCATION', os.environ.get('FLIGHT_SERVICE_LOCATION'))
        cls.FLIGHT_SERVICE_PORT = cls.credentials.get('FLIGHT_SERVICE_PORT', os.environ.get('FLIGHT_SERVICE_PORT'))
    ...
    #                   days_old=7)
    #     TestDataConnection.space_id = get_space_id(self.api_client, self.space_name,
    #                                                cos_resource_instance_id=self.cos_resource_instance_id)
    ...
    def test_00e_prepare_connected_data_asset_COS(self):
        asset_details = self.api_client.data_assets.store({
            ...
            self.api_client.data_assets.ConfigurationMetaNames.DATA_CONTENT_NAME: '/' + self.bucket_name + '/' + self.csv_data_location.split('/')[-1]
        })
    ...
    def test_00g_prepare_connected_data_asset_json(self):
        asset_details = self.api_client.data_assets.store({
            ...
            self.api_client.data_assets.ConfigurationMetaNames.DATA_CONTENT_NAME: '/' + self.bucket_name + '/' + self.json_data_location.split('/')[-1]
        })
    ...
    def tearDownClass(cls) -> None:
        response = cls.api_client.data_assets.delete(cls.fs_csv_data_asset_id)
        print(response)

        response = cls.api_client.connections.delete(cls.cos_csv_connection_id)
        print(response)

        response = cls.api_client.data_assets.delete(cls.connected_data_asset_cos_id)
        print(response)

        response = cls.api_client.data_assets.delete(cls.write_connected_data_asset_cos_id)
        print(response)

        response = cls.api_client.data_assets.delete(cls.write_connected_data_asset_cos_id_json)
        print(response)
