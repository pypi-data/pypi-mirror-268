...
class TestAutoAIRemote(unittest.TestCase):
    ...
    def test_02a_read_saved_remote_data_before_fit(self):
        with self.assertRaises(ConnectionError, msg="Should fail with adequate error - reading without api_client"):
            _ = self.data_connection.read()

        self.data_connection.set_client(api_client=self.api_client)
    ...
    def test_30_batch_deployment_setup_and_preparation(self):
        if self.api_client.ICP:
            ...
            TestAutoAIRemote.service_batch = Batch(self.credentials,
                                                   source_space_id=self.space_id)
