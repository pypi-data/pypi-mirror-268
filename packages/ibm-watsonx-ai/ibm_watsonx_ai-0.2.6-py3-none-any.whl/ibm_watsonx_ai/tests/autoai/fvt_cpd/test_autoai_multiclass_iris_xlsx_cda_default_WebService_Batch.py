...
class TestAutoAIRemote(AbstractTestAutoAIRemote, unittest.TestCase):
    ...
    def test_00c_prepare_connection_to_COS(self):
        connection_details = self.api_client.connections.create({
            'datasource_type': self.api_client.connections.get_datasource_type_uid_by_name('bluemixcloudobjectstorage'), #'bluemixcloudobjectstorage',
        ...
        TestAutoAIRemote.connection_id = self.api_client.connections.get_uid(connection_details)
    ...
    def test_00d_prepare_connected_data_asset_train_data(self):
        ...
        #     self.api_client.data_assets.ConfigurationMetaNames.DATA_CONTENT_NAME: self.data_cos_path
        # })
        asset_details = self.api_client.data_assets.store({
            self.api_client.data_assets.ConfigurationMetaNames.CONNECTION_ID: self.connection_id,
            self.api_client.data_assets.ConfigurationMetaNames.NAME: "Iris - training asset",
    ...
    def test_00e_prepare_connected_data_asset_holdout_data(self):
        ...
        asset_details = self.api_client.data_assets.store({
            self.api_client.data_assets.ConfigurationMetaNames.CONNECTION_ID: self.connection_id,
            self.api_client.data_assets.ConfigurationMetaNames.NAME: "Iris - holdout/test asset",
            self.api_client.data_assets.ConfigurationMetaNames.DATA_CONTENT_NAME: f"/{self.bucket_name}/{self.test_data_cos_path}/{self.sheet_name}"
        })

        TestAutoAIRemote.test_asset_id = self.api_client.data_assets.get_id(asset_details)
        self.assertIsInstance(self.asset_id, str)
    ...
    def test_29_delete_connection_and_connected_data_asset(self):
        ...
        self.api_client.data_assets.delete(self.test_asset_id)
