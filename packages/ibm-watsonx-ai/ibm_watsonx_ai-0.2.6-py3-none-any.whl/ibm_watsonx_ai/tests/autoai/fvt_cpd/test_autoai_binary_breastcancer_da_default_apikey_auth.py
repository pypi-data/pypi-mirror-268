...
class TestAutoAIRemote(AbstractTestAutoAIAsync, unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        try:
            del cls.credentials['password']
        except:
            pass
        try:
            del cls.credentials['bedrock_url']
        except:
            pass

        assert(cls.credentials.get('username') is not None)
        assert(cls.credentials.get('password') is None)
        assert (cls.credentials.get('bedrock_url') is None)
        assert(cls.credentials.get('apikey') is not None)
        cls.api_client = APIClient(wml_credentials=cls.credentials)
