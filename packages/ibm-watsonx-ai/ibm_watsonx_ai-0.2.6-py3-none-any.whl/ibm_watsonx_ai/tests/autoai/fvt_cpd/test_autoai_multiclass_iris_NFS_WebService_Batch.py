...
class TestAutoAIRemote(unittest.TestCase):
    ...
    def test_20_deployment_setup_and_preparation(self):
        ...
        if self.api_client.ICP:
            ...
            TestAutoAIRemote.service = WebService(source_instance_credentials=self.credentials,
                                                  source_space_id=self.space_id)

        delete_model_deployment(self.api_client, deployment_name=self.DEPLOYMENT_NAME)

        self.api_client.set.default_project(self.project_id) if self.api_client.ICP else None
    ...
    def test_25_deployment_setup_and_preparation_different_wml(self):
        if self.api_client.ICP:
            TestAutoAIRemote.service = WebService(source_instance_credentials=self.credentials.copy(),
                                                  source_project_id=self.project_id,
                                                  source_space_id=self.space_id,
                                                  target_instance_credentials=self.credentials.copy(),
