...
class AbstractTestAutoAIAsync(abc.ABC):
    ...
    def test_01_initialize_AutoAI_experiment__pass_credentials__object_initialized(self):
        ...
        if self.SPACE_ONLY:
            AbstractTestAutoAIAsync.experiment = AutoAI(credentials=self.credentials.copy(),
        ...
        AbstractTestAutoAIAsync.batched = ('include_batched_ensemble_estimators' not in self.experiment_info and self.api_client.ICP) or \
    ...
    def test_08_get_run_details(self):
        ...
        import json
        training_details = self.api_client.training.get_details(training_uid=parameters['metadata']['id'])
    ...
    def test_93_delete_experiment(self):
        ...
        self.api_client.pipelines.delete(pipeline_id)
        self.api_client.training.cancel(self.run_id, hard_delete=True)

        with self.assertRaises(WMLClientError):
            self.api_client.training.get_details(self.run_id)
            self.api_client.pipelines.get_details(pipeline_id)
...
class AbstractTestAutoAISync(abc.ABC):
    ...
    def test_01_initialize_AutoAI_experiment__pass_credentials__object_initialized(self):
        if self.SPACE_ONLY:
            AbstractTestAutoAISync.experiment = AutoAI(credentials=self.credentials.copy(),
                                                       space_id=self.space_id)
        else:
            AbstractTestAutoAISync.experiment = AutoAI(credentials=self.credentials.copy(),
                                                       project_id=self.project_id)
    ...
    def test_07_get_summary(self):
        ...
        metrics = self.api_client.training.get_details(self.run_id)['entity']['status'].get('metrics', [])
    ...
    #     AbstractTestAutoAISync.service = WebService(source_instance_credentials=self.credentials.copy(),
