...
class AbstractTestAutoAIRemote(abc.ABC):
    ...
    def test_00a_space_cleanup(self):
        space_cleanup(self.api_client,
                      ...
                      days_old=7)
        AbstractTestAutoAIRemote.space_id = get_space_id(self.api_client, self.space_name,
                                                         cos_resource_instance_id=self.cos_resource_instance_id)
    ...
    def test_01_initialize_AutoAI_experiment__pass_credentials__object_initialized(self):
        AbstractTestAutoAIRemote.experiment = AutoAI(credentials=self.credentials.copy(),
    ...
    def test_03_initialize_optimizer(self):
        AbstractTestAutoAIRemote.remote_auto_pipelines = self.experiment.optimizer(
            ...
            t_shirt_size=TShirtSize.S if self.api_client.ICP else TShirtSize.L,
    ...
    def test_08b_get_metrics(self):
        self.api_client.training.get_metrics(self.run_id)
    ...
    def test_19_deployment_setup_and_preparation(self):
        AbstractTestAutoAIRemote.service = WebService(source_instance_credentials=self.credentials.copy(),
        ...
        self.api_client.set.default_project(self.project_id) if self.project_id else None
    ...
    def test_23_delete_deployment(self):
        ...
        self.service.delete()
        self.api_client.set.default_space(self.space_id) if not self.api_client.default_space_id else None
        self.api_client.repository.delete(self.service.asset_id)
        self.api_client.set.default_project(self.project_id) if is_cp4d() else None
    ...
    def test_24_batch_deployment_setup_and_preparation(self):
        AbstractTestAutoAIRemote.service_batch = Batch(source_instance_credentials=self.credentials.copy(),
                                                       source_project_id=self.project_id,
                                                       target_instance_credentials=self.credentials,
                                                       target_space_id=self.space_id)
    ...
    def test_28_delete_deployment_batch(self):
        ...
        self.api_client.set.default_space(self.space_id) if not self.api_client.default_space_id else None
        self.api_client.repository.delete(self.service_batch.asset_id)
        self.api_client.set.default_project(self.project_id)
