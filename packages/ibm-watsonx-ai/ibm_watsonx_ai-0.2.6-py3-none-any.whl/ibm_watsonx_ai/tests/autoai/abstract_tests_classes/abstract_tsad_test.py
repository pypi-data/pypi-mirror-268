...
class AbstractTestTSADAsync(abc.ABC):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.api_client = APIClient(wml_credentials=cls.credentials.copy())

        cls.cos_credentials = get_cos_credentials()
        cls.cos_endpoint = cls.cos_credentials.get('endpoint_url')
        cls.cos_resource_instance_id = cls.cos_credentials.get('resource_instance_id')

        cls.project_id = cls.credentials.get('project_id')
    ...
    def test_00a_space_cleanup(self):
        ...
        while not space_checked:
            space_cleanup(self.api_client,
                          get_space_id(self.api_client, self.space_name,
                                       cos_resource_instance_id=self.cos_resource_instance_id),
                          days_old=7)
            space_id = get_space_id(self.api_client, self.space_name,
                                    cos_resource_instance_id=self.cos_resource_instance_id)
            try:
    ...
    def test_01_initialize_autoai_experiment__pass_credentials__object_initialized(self):
        if self.SPACE_ONLY:
            AbstractTestTSADAsync.experiment = AutoAI(credentials=self.credentials.copy(),
    ...
    def test_08_get_run_details(self):
        ...
        import json
        print(json.dumps(self.api_client.training.get_details(training_uid=parameters['metadata']['id']), indent=4))
        print(parameters)
        self.assertIsNotNone(parameters)

    def test_08b_get_metrics(self):
        metrics = self.api_client.training.get_metrics(self.run_id)
    ...
    def test_08c_get_pipeline_notebook(self):
        ...
        details = self.api_client.training.get_details(training_uid=self.remote_auto_pipelines.get_run_details()['metadata']['id'])
    ...
    def test_31_deployment_online_setup_and_preparation(self):
        ...
        self.api_client.set.default_space(AbstractTestTSADAsync.target_space_id)
        delete_model_deployment(self.api_client, deployment_name=self.DEPLOYMENT_NAME)

        if self.SPACE_ONLY:
            AbstractTestTSADAsync.service = WebService(source_instance_credentials=self.credentials,
    ...
    def test_36_delete_deployment(self):
        ...
        self.api_client.repository.delete(AbstractTestTSADAsync.service.asset_id)
    ...
    def test_41_batch_deployment_setup_and_preparation(self):
        ...
        if self.SPACE_ONLY:
            AbstractTestTSADAsync.service_batch = Batch(source_instance_credentials=self.credentials,
                                                      source_space_id=self.space_id,
                                                      target_instance_credentials=self.credentials,
                                                      target_space_id=AbstractTestTSADAsync.target_space_id)
        else:
    ...
    def test_45_run_job_batch_deployed_model_with_data_assets(self):
        ...
        # self.api_client.set.default_space(self.target_space_id)
        ...
        if not self.BATCH_DEPLOYMENT_WITH_DA:
            ...
            asset_id = self.api_client.data_assets.get_id(asset_details)
        ...
        deployment_job_id = self.api_client.deployments.get_job_uid(scoring_params)

        self.api_client.data_assets.list()

        data_asset_details = self.api_client.data_assets.get_details()
    ...
    def test_46_run_job_batch_deployed_model_with_data_assets_with_cos_connection(self):
        ...
        AbstractTestTSADAsync.connection_id = self.api_client.connections.get_uid(connection_details)

        self.assertIsNotNone(self.batch_payload_location,
                             "Test configuration failure: Batch payload location is missing")

        asset_details = self.api_client.data_assets.store({
            self.api_client.data_assets.ConfigurationMetaNames.CONNECTION_ID: AbstractTestTSADAsync.connection_id,
            self.api_client.data_assets.ConfigurationMetaNames.NAME: "Batch deployment asset",
            self.api_client.data_assets.ConfigurationMetaNames.DATA_CONTENT_NAME: f"{self.bucket_name}/{self.batch_payload_cos_location}"
        })
    ...
    def test_47_run_job_batch_deployed_model_with_connected_data_asset(self):
        ...
        self.assertIsNotNone(self.api_client.default_space_id, "TEST Error: default space was not set correctly")
    ...
    def test_48_run_job_batch_deployed_model_with_data_connection_container(self):
        ...
        if self.api_client.ICP or self.api_client.WSD:
    ...
    def test_49_delete_deployment_batch(self):
        ...
        self.api_client.set.default_space(self.target_space_id) if not self.api_client.default_space_id else None
        self.api_client.repository.delete(AbstractTestTSADAsync.service_batch.asset_id)
        ...
        for asset_id in AbstractTestTSADAsync.data_assets_to_delete:
            self.api_client.data_assets.delete(asset_id)
