...
from ibm_watsonx_ai.utils.autoai.enums import DataConnectionTypes, PredictionType
...
class BaseTestStoreModel(abc.ABC):
    ...
    def test_30a_setup_experiment_metadata(self):
        run_details = self.api_client.training.get_details(self.run_id)
    ...
    def test_30b_store_model_from_object_with_experiment_metadata(self):
        ...
        model_metadata = {
            self.api_client.repository.ModelMetaNames.NAME: 'AutoAI api_client.repository.store_model() with experiment_metadata'
        }

        stored_model_details = self.api_client.repository.store_model(model=BaseTestStoreModel.pipeline_model,
        ...
        model_id = self.api_client.repository.get_model_id(stored_model_details)
        model_details = self.api_client.repository.get_model_details(model_id)
    ...
    def test_30c_store_model_from_pipeline_name_with_experiment_metadata(self):
        ...
        stored_model_details = self.api_client.repository.store_model(model=self.pipeline_to_deploy,
                                                                      meta_props=model_metadata,
                                                                      experiment_metadata=BaseTestStoreModel.experiment_metadata)
    ...
    def test_31a_store_model_from_object__with_training_id(self):
        ...
        model_metadata = {
            self.api_client.repository.ModelMetaNames.NAME: 'AutoAI api_client.repository.store_model() with training_id'
        }
        ...
        if self.SPACE_ONLY:
            ...
            for model_id in BaseTestStoreModel.project_models_ids:
                ...
                BaseTestStoreModel.space_models_ids.append(
                    self.api_client.spaces.promote(asset_id=model_id,
                                                   source_project_id=self.project_id,
                                                   target_space_id=self.space_id))
        self.api_client.set.default_space(self.space_id)
        for model_id in BaseTestStoreModel.space_models_ids:
            promoted_model_details = self.api_client.repository.get_model_details(model_id)
    ...
    def test_33a_deploy_model_web_service_with_experiment_metadata(self):
        ...
        if self.SPACE_ONLY:
            service = WebService(
                source_instance_credentials=self.credentials,
                target_instance_credentials=self.credentials,
        ...
        with self.assertRaises(WMLClientError, msg="It should throw an exception due to missing deployment ID"):
            self.api_client.deployments.get_details(deployment_id)
    ...
    def test_33b_deploy_online_models(self):
        for model_id in self.space_models_ids:
            ...
            self.assertIn(model_id, str(model_details))
            deploy_meta = {
                self.api_client.deployments.ConfigurationMetaNames.NAME: "Tests: AutoAI model stored with repository.store_model()",
                self.api_client.deployments.ConfigurationMetaNames.ONLINE: {},
            }

            deployment_details = self.api_client.deployments.create(artifact_uid=model_id, meta_props=deploy_meta)
            BaseTestStoreModel.online_deployments_ids.append(self.api_client.deployments.get_id(deployment_details))
    ...
    def test_34_score_online_deployments(self):
        ...
        for deployment_id in self.online_deployments_ids:
            ...
            predictions = self.api_client.deployments.score(deployment_id, scoring_payload)
            print(predictions)
    ...
    def test_35_deploy_batch_models(self):
        ...
        for model_id in self.space_models_ids:
            deployment_props = {
                self.api_client.deployments.ConfigurationMetaNames.NAME: "Tests: AutoAI model stored with repository.store_model()",
                self.api_client.deployments.ConfigurationMetaNames.BATCH: {},
                self.api_client.deployments.ConfigurationMetaNames.HYBRID_PIPELINE_HARDWARE_SPECS: [
                    {
            ...
            BaseTestStoreModel.batch_deployments_ids.append(self.api_client.deployments.get_id(deployment_details))
    ...
    def test_36_score_batch_deployments(self):
        ...
        for deployment_id in self.batch_deployments_ids:
            print(f"Scoring deployment: {deployment_id}...")
            scoring_payload = {
                self.api_client.deployments.ScoringMetaNames.INPUT_DATA: [{'values': self.scoring_df}]
            }
            job_details = self.api_client.deployments.create_job(deployment_id, scoring_payload)
            deployment_job_id = self.api_client.deployments.get_job_uid(job_details)
            ...
            while status_state not in ['failed', 'error', 'completed', 'canceled']:
                ...
                print(".", end=" ")
                status_state = \
                    self.api_client.deployments.get_job_details(deployment_job_id)['entity']['scoring']['status'][
                        'state']
            print(f"Batch deployment job status state: {status_state}")
            self.assertEqual(status_state, 'completed')
            predictions = self.api_client.deployments.get_job_details(deployment_job_id)
            print(predictions)
    ...
    def test_91_delete_deployments(self):
        ...
        for deployment_id in self.batch_deployments_ids:
            self.api_client.deployments.delete(deployment_id)
    ...
    def test_92_delete_models(self):
        def delete_models(models_ids):
            for model_id in models_ids:
                self.api_client.repository.delete(model_id)
                with self.assertRaises(WMLClientError):
                    self.api_client.repository.get_model_details(model_id)

        if not self.api_client.default_space_id:
            self.api_client.set.default_space(self.space_id)
