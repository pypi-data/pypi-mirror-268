...
class AbstractTestTSAsyncWML(abc.ABC):
    """
    ...
    def test_00a_space_cleanup(self):
        ...
        AbstractTestTSAsyncWML.space_id = space_id
    ...
    def test_01_initialize_AutoAI_experiment__pass_credentials__object_initialized(self):
        ...
        if self.SPACE_ONLY:
            AbstractTestTSAsyncWML.experiment = AutoAI(wml_credentials=self.wml_credentials.copy(),
    ...
    def test_03_initialize_optimizer(self):
        AbstractTestTSAsyncWML.experiment_info = validate_autoai_timeseries_experiment(self.experiment_info,
                                                                                    self.pod_version)

        AbstractTestTSAsyncWML.remote_auto_pipelines = self.experiment.optimizer(**AbstractTestTSAsyncWML.experiment_info)
    ...
    def test_05_fit_run_training_of_auto_ai_in_wml(self):
        AbstractTestTSAsyncWML.trained_pipeline_details = self.remote_auto_pipelines.fit(
            training_data_reference=[self.data_connection],
            training_results_reference=self.results_connection,
            background_mode=False)

        AbstractTestTSAsyncWML.run_id = self.trained_pipeline_details['metadata']['id']
    ...
    def test_06_get_train_data(self):
        # AbstractTestTSAsyncWML.train_data = self.remote_auto_pipelines.get_data_connections()[0].read()
        #
        # print("train data sample:")
        # print(self.train_data.head())
        # self.assertGreater(len(self.train_data), 0)

        AbstractTestTSAsyncWML.train_X, AbstractTestTSAsyncWML.test_X, AbstractTestTSAsyncWML.train_y, AbstractTestTSAsyncWML.test_y = \
            self.remote_auto_pipelines.get_data_connections()[0].read(with_holdout_split=True)

        AbstractTestTSAsyncWML.scoring_df = self.train_X[:10]
    ...
    def test_10_summary_listing_all_pipelines_from_wml(self):
        ...
        AbstractTestTSAsyncWML.winning_pipelines_summary = pipelines_summary[pipelines_summary['Winner']]
        AbstractTestTSAsyncWML.discarded_pipelines_summary = pipelines_summary[pipelines_summary['Winner'] == False]

        self.assertGreater(len(AbstractTestTSAsyncWML.winning_pipelines_summary), 0,
        ...
        if 'max_number_of_estimators' in self.experiment_info and self.experiment_info:
            ...
            self.assertEqual(len(AbstractTestTSAsyncWML.winning_pipelines_summary), nb_gen_pipelines,
    ...
    def test_13_get_pipeline__load_lale_pipeline__pipeline_loaded(self):
        AbstractTestTSAsyncWML.lale_pipeline = self.remote_auto_pipelines.get_pipeline()
    ...
    def test_15_list_historical_runs_and_get_run_ids(self):
        ...
        if len(runs_completed_df) > 1:
            AbstractTestTSAsyncWML.prev_run_id = runs_completed_df.run_id.iloc[1]  # prev run_id
            print("Random historical run_id: {}".format(AbstractTestTSAsyncWML.prev_run_id))
            self.assertIsNotNone(AbstractTestTSAsyncWML.prev_run_id)
    ...
    def test_16_get_params_of_last_historical_run(self):
        ...
        AbstractTestTSAsyncWML.historical_opt = self.experiment.runs.get_optimizer(self.run_id)
    ...
    def test_31_deployment_online_setup_and_preparation(self):
        ...
        if self.target_space_id is None:
            AbstractTestTSAsyncWML.target_space_id = self.space_id
        else:
            AbstractTestTSAsyncWML.target_space_id = self.target_space_id
        # end note

        self.wml_client.set.default_space(AbstractTestTSAsyncWML.target_space_id)
        delete_model_deployment(self.wml_client, deployment_name=self.DEPLOYMENT_NAME)

        if self.SPACE_ONLY:
            AbstractTestTSAsyncWML.service = WebService(source_wml_credentials=self.wml_credentials,
                                                     source_space_id=self.space_id,
                                                     target_wml_credentials=self.wml_credentials,
                                                     target_space_id=AbstractTestTSAsyncWML.target_space_id)
        else:
            AbstractTestTSAsyncWML.service = WebService(source_wml_credentials=self.wml_credentials,
        ...
        self.assertIsInstance(AbstractTestTSAsyncWML.service, WebService, msg="Deployment is not of WebService type.")
        self.assertIsInstance(AbstractTestTSAsyncWML.service._source_workspace, WorkSpace,
                              msg="Workspace set incorrectly.")
        self.assertEqual(AbstractTestTSAsyncWML.service.id, None, msg="Deployment ID initialized incorrectly")
        self.assertEqual(AbstractTestTSAsyncWML.service.name, None, msg="Deployment name initialized incorrectly")
    ...
    def test_32__deploy__deploy_best_computed_pipeline_from_autoai_on_wml(self):
        ...
        AbstractTestTSAsyncWML.service.create(
            experiment_run_id=self.remote_auto_pipelines._engine._current_run_id,
            model=pipeline_name,
            deployment_name=self.DEPLOYMENT_NAME)

        self.assertIsNotNone(AbstractTestTSAsyncWML.service.id, msg="Online Deployment creation - missing id")
        self.assertIsNotNone(AbstractTestTSAsyncWML.service.name, msg="Online Deployment creation - name not set")
        self.assertIsNotNone(AbstractTestTSAsyncWML.service.scoring_url,
                             msg="Online Deployment creation - mscoring url  missing")

    def test_33_score_deployed_model_with_empty_payload(self):
        predictions = AbstractTestTSAsyncWML.service.score()
        print(predictions)
    ...
    def test_34_score_deployed_model_with_df(self):
        predictions = AbstractTestTSAsyncWML.service.score(payload=self.scoring_df)
        print(predictions)
        self.assertIsNotNone(predictions)
    ...
    def test_35_list_deployments(self):
        AbstractTestTSAsyncWML.service.list()
        params = AbstractTestTSAsyncWML.service.get_params()
        print(params)
        self.assertIsNotNone(params)

        self.assertIn(AbstractTestTSAsyncWML.service.deployment_id, str(params))

    def test_36_delete_deployment(self):
        print("Delete current deployment: {}".format(AbstractTestTSAsyncWML.service.deployment_id))
        AbstractTestTSAsyncWML.service.delete()
        self.wml_client.repository.delete(AbstractTestTSAsyncWML.service.asset_id)
        self.assertEqual(AbstractTestTSAsyncWML.service.id, None, msg="Deployment ID deleted incorrectly")
        self.assertEqual(AbstractTestTSAsyncWML.service.name, None, msg="Deployment name deleted incorrectly")
        self.assertEqual(AbstractTestTSAsyncWML.service.scoring_url, None,
    ...
    def test_41_batch_deployment_setup_and_preparation(self):
        ...
        self.assertIsNotNone(AbstractTestTSAsyncWML.target_space_id, "Test issue: target space not set.")

        if self.SPACE_ONLY:
            AbstractTestTSAsyncWML.service_batch = Batch(source_wml_credentials=self.wml_credentials,
                                                      source_space_id=self.space_id,
                                                      target_wml_credentials=self.wml_credentials,
                                                      target_space_id=AbstractTestTSAsyncWML.target_space_id)
        else:
        ...
        self.assertIsInstance(AbstractTestTSAsyncWML.service_batch, Batch, msg="Deployment is not of Batch type.")
        self.assertIsInstance(AbstractTestTSAsyncWML.service_batch._source_workspace, WorkSpace,
                              msg="Workspace set incorrectly.")
        self.assertEqual(AbstractTestTSAsyncWML.service_batch.id, None, msg="Deployment ID initialized incorrectly")
        self.assertEqual(AbstractTestTSAsyncWML.service_batch.name, None, msg="Deployment name initialized incorrectly")
    ...
    def test_42_deploy__batch_deploy_best_computed_pipeline_from_autoai_on_wml(self):
        ...
        pipeline_name = self.winning_pipelines_summary.reset_index()['Pipeline Name'][0]
        print('Deploying', pipeline_name)
        AbstractTestTSAsyncWML.service_batch.create(
            experiment_run_id=self.remote_auto_pipelines._engine._current_run_id,
            model=pipeline_name,
            deployment_name=self.DEPLOYMENT_NAME + ' BATCH')

        self.assertIsNotNone(AbstractTestTSAsyncWML.service_batch.id, msg="Batch Deployment creation - missing id")
        self.assertIsNotNone(AbstractTestTSAsyncWML.service_batch.id, msg="Batch Deployment creation - name not set")
        self.assertIsNotNone(AbstractTestTSAsyncWML.service_batch.asset_id,
    ...
    def test_43_list_batch_deployments(self):
        ...
        deployments = AbstractTestTSAsyncWML.service_batch.list()
        print(deployments)
        self.assertIn('created_at', str(deployments).lower())
        self.assertIn('status', str(deployments).lower())

        params = AbstractTestTSAsyncWML.service_batch.get_params()
        print(params)
        self.assertIsNotNone(params)
    ...
    def test_44_run_job__batch_deployed_model_with_data_frame(self):
        ...
        if self.BATCH_DEPLOYMENT_WITH_DF:
            scoring_params = AbstractTestTSAsyncWML.service_batch.run_job(
                payload=self.scoring_df,
                background_mode=False)
            print(scoring_params)
            print(AbstractTestTSAsyncWML.service_batch.get_job_result(scoring_params['metadata']['id']))
    ...
    def test_45_run_job_batch_deployed_model_with_data_assets(self):
        ...
        if not self.BATCH_DEPLOYMENT_WITH_DA:
            ...
            AbstractTestTSAsyncWML.data_assets_to_delete.append(asset_id)
        ...
        predictions = AbstractTestTSAsyncWML.service_batch.get_job_result(deployment_job_id)
        print(predictions)
    ...
    def test_46_run_job_batch_deployed_model_with_data_assets_with_cos_connection(self):
        ...
        AbstractTestTSAsyncWML.bucket_name = save_data_to_cos_bucket(self.batch_payload_location,
        ...
        AbstractTestTSAsyncWML.connection_id = self.wml_client.connections.get_uid(connection_details)

        self.assertIsNotNone(self.batch_payload_location,
                             "Test configuration failure: Batch payload location is missing")

        asset_details = self.wml_client.data_assets.store({
            self.wml_client.data_assets.ConfigurationMetaNames.CONNECTION_ID: AbstractTestTSAsyncWML.connection_id,
    ...
    def test_47_run_job_batch_deployed_model_with_connected_data_asset(self):
        ...
        # prepare connection
        if not AbstractTestTSAsyncWML.connection_id:
            connection_details = self.wml_client.connections.create({
                'datasource_type': self.wml_client.connections.get_datasource_type_uid_by_name(
                    'bluemixcloudobjectstorage'),
                'name': 'Connection to COS for tests',
        ...
        conn_space = DataConnection(
            connection_asset_id=AbstractTestTSAsyncWML.connection_id,
            location=S3Location(
                bucket=self.bucket_name,
    ...
    def test_49_delete_deployment_batch(self):
        ...
        print("Delete current deployment: {}".format(AbstractTestTSAsyncWML.service_batch.deployment_id))
        AbstractTestTSAsyncWML.service_batch.delete()
        self.wml_client.set.default_space(self.target_space_id) if not self.wml_client.default_space_id else None
        self.wml_client.repository.delete(AbstractTestTSAsyncWML.service_batch.asset_id)

        print(f"Delete all created assets in Batch deployment tests. {AbstractTestTSAsyncWML.data_assets_to_delete}")
        for asset_id in AbstractTestTSAsyncWML.data_assets_to_delete:
            self.wml_client.data_assets.delete(asset_id)

        self.assertEqual(AbstractTestTSAsyncWML.service_batch.id, None, msg="Deployment ID deleted incorrectly")
        self.assertEqual(AbstractTestTSAsyncWML.service_batch.name, None, msg="Deployment name deleted incorrectly")
        self.assertEqual(AbstractTestTSAsyncWML.service_batch.scoring_url, None,
                         msg="Deployment scoring_url deleted incorrectly")
