...
class AbstractTestTSAsync(abc.ABC):
    ...
    def test_01_initialize_AutoAI_experiment__pass_credentials__object_initialized(self):
        ...
        if self.SPACE_ONLY:
            AbstractTestTSAsync.experiment = AutoAI(credentials=self.credentials.copy(),
    ...
    def test_31_deployment_online_setup_and_preparation(self):
        ...
        self.api_client.set.default_space(AbstractTestTSAsync.target_space_id)
        delete_model_deployment(self.api_client, deployment_name=self.DEPLOYMENT_NAME)

        if self.SPACE_ONLY:
            AbstractTestTSAsync.service = WebService(source_instance_credentials=self.credentials,
    ...
    def test_36_delete_deployment(self):
        ...
        self.api_client.repository.delete(AbstractTestTSAsync.service.asset_id)
    ...
    def test_41_batch_deployment_setup_and_preparation(self):
        ...
        if self.SPACE_ONLY:
            AbstractTestTSAsync.service_batch = Batch(source_instance_credentials=self.credentials,
                                                      source_space_id=self.space_id,
                                                      target_instance_credentials=self.credentials,
                                                      target_space_id=AbstractTestTSAsync.target_space_id)
        else:
    ...
    def test_46_run_job_batch_deployed_model_with_data_assets_with_cos_connection(self):
        ...
        AbstractTestTSAsync.connection_id = self.api_client.connections.get_uid(connection_details)

        self.assertIsNotNone(self.batch_payload_location,
                             "Test configuration failure: Batch payload location is missing")

        asset_details = self.api_client.data_assets.store({
            self.api_client.data_assets.ConfigurationMetaNames.CONNECTION_ID: AbstractTestTSAsync.connection_id,
    ...
    def test_49_delete_deployment_batch(self):
        ...
        self.api_client.repository.delete(AbstractTestTSAsync.service_batch.asset_id)
