...
class TestDataConnection(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.api_client.set.default_project(cls.project_id)
    ...
    def test_00b_create_normal_data_asset(self):
        ...
        TestDataConnection.fs_csv_data_asset_id = self.api_client.data_assets.get_id(asset_details)
    ...
    def test_00d_prepare_cos_csv_connection_and_upload_data(self):
        ...
        TestDataConnection.cos_csv_connection_id = self.api_client.connections.get_uid(connection_details)
    ...
    def test_00e_prepare_connected_data_asset_COS(self):
        asset_details = self.api_client.data_assets.store({
            self.api_client.data_assets.ConfigurationMetaNames.CONNECTION_ID: self.cos_csv_connection_id,
            self.api_client.data_assets.ConfigurationMetaNames.NAME: "Iris - training asset",
            self.api_client.data_assets.ConfigurationMetaNames.DATA_CONTENT_NAME: '/' + self.bucket_name + '/' +
                                                                                  self.csv_data_location.split('/')[-1]
        })

        TestDataConnection.connected_data_asset_cos_id = self.api_client.data_assets.get_id(asset_details)
    ...
    def test_00f_prepare_cos_json_connection_and_upload_data(self):
        ...
        TestDataConnection.cos_json_connection_id = self.api_client.connections.get_uid(connection_details)
        self.assertIsInstance(self.cos_json_connection_id, str)

    def test_00g_prepare_connected_data_asset_json(self):
        asset_details = self.api_client.data_assets.store({
            self.api_client.data_assets.ConfigurationMetaNames.CONNECTION_ID: self.cos_json_connection_id,
            self.api_client.data_assets.ConfigurationMetaNames.NAME: "pipeline json",
            self.api_client.data_assets.ConfigurationMetaNames.DATA_CONTENT_NAME: '/' + self.bucket_name + '/' +
                                                                                  self.json_data_location.split('/')[-1]
        })

        TestDataConnection.cos_json_connected_data_asset_cos_id = self.api_client.data_assets.get_id(asset_details)
    ...
    def test_00h_prepare_cos_zip_connection_and_upload_data(self):
        ...
        TestDataConnection.cos_zip_connection_id = self.api_client.connections.get_uid(connection_details)
        self.assertIsInstance(self.cos_zip_connection_id, str)

    def test_00i_prepare_connected_data_asset_zip(self):
        asset_details = self.api_client.data_assets.store({
            self.api_client.data_assets.ConfigurationMetaNames.CONNECTION_ID: self.cos_zip_connection_id,
            self.api_client.data_assets.ConfigurationMetaNames.NAME: "zip asset",
            self.api_client.data_assets.ConfigurationMetaNames.DATA_CONTENT_NAME: '/' + self.bucket_name + '/' +
                                                                                  self.zip_data_location.split('/')[-1]
        })

        TestDataConnection.cos_zip_connected_data_asset_cos_id = self.api_client.data_assets.get_id(asset_details)
    ...
    #     connection.set_client(self.api_client)
    #
    #     try:
    ...
    def test_08a_upload_data_asset_csv_to_COS_from_file(self):
        ...
        TestDataConnection.write_connected_data_asset_cos_id = self.api_client.data_assets.get_id(asset_details)
    ...
    def test_10_upload_data_asset_json_to_COS_and_download_it(self):
        ...
        asset_details = self.api_client.data_assets.store({
            self.api_client.data_assets.ConfigurationMetaNames.CONNECTION_ID: self.cos_json_connection_id,
            self.api_client.data_assets.ConfigurationMetaNames.NAME: "json test",
            self.api_client.data_assets.ConfigurationMetaNames.DATA_CONTENT_NAME: '/' + self.bucket_name + '/' +
                                                                                  self.json_data_location.split(
                                                                                      '/')[-1]
        })

        TestDataConnection.write_connected_data_asset_cos_id_json = self.api_client.data_assets.get_id(asset_details)
    ...
    def tearDownClass(cls) -> None:
        ...
        for asset_id in [cls.fs_csv_data_asset_id, cls.cos_csv_connection_id, cls.connected_data_asset_cos_id,
            ...
            if asset_id:
                response = cls.api_client.data_assets.delete(asset_id)
                print(response)
        try:
