...
available_models = ['ibm-mistralai/mixtral-8x7b-instruct-v01-q']
    ...
    def test_10_generate_stream_guardrails_disabled(self, model):
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            assert len(['Potentially harmful text detected' in str(warning.message) for warning in w]) == 0 or \
                   len(['Personally identifiable information detected' in str(warning.message) for warning in w]) == 0, "Warning with HAP/PII detection raised"
