...
class TestPromptTemplate(unittest.TestCase):
    ...
    def test_02d_store_langchain_with_metadata(self):
        template = ("Generate a random question"
                    " about {topic}: Question: ")
        name = "LangChain PT name"
        task_ids = ["extraction"]
        ...
        stored_prompt_template = self.prompt_mgr.store_prompt(langchain_prompt)
        self.assertEqual(stored_prompt_template.name, name)
        self.assertEqual(stored_prompt_template.input_text, template)
        self.assertEqual(stored_prompt_template.task_ids, task_ids)
        self.assertEqual(stored_prompt_template.description, description)
        self.assertEqual(stored_prompt_template.examples, examples)
        self.assertListEqual(list(stored_prompt_template.input_variables.keys()), ["topic"])

    def test_02e_missing_name(self):
