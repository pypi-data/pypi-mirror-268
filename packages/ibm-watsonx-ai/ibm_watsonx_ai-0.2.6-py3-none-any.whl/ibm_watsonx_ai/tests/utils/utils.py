...
def get_wml_credentials(env=environment):
    wml_credentials = json.loads(config.get(env, "wx_credentials"))
    if "flight_service_location" in wml_credentials and not os.environ.get(
        "FLIGHT_SERVICE_LOCATION"
    ):
        os.environ["FLIGHT_SERVICE_LOCATION"] = wml_credentials[
            "flight_service_location"
        ]
    if "flight_service_port" in wml_credentials and not os.environ.get(
        "FLIGHT_SERVICE_PORT"
    ):
...
def create_bucket(cos_resource, bucket_name):
    ...
    try:
        ...
        print(
            f"Bucket cleanup with prefix {bucket_name}- failed due to:\n{e}\n skipped"
        )
...
def get_space_id(wml_client, space_name, cos_resource_instance_id=None):
    ...
    """
    spaces_details = []
    for entry in wml_client.spaces.get_details(
        limit=200, asynchronous=True, get_all=True
    ):
    ...
    if not space_id:
        ...
        if wml_client.CLOUD_PLATFORM_SPACES:
            ...
            metadata = {wml_client.spaces.ConfigurationMetaNames.NAME: space_name}
...
def resources_discovery(wml_client):
    ...
    def check(parent_el, parent_name):
        for r in parent_el.__dict__:
            try:
                if parent_el.__dict__[r].__class__.__module__.startswith(
                    "ibm_watsonx_ai."
                ):
...
def setup_nfs_env(wml_credentials, dataset_file_name, remote_directory="datasets"):
    ...
    if not (ssh_host and ssh_username and ssh_password):
        print(
            "NFS setup omitted. Required credentials missing: ssh_host, ssh_username, ssh_password"
        )
        return
    ...
    def run_ssh(cmd, format="plain"):
        ...
        if err and not (
            "200 OK" in err or "409 Conflict" in err or "201 Created" in err
        ):
            raise Exception(err)
    ...
    res = run_ssh(
        f"oc login -u {kube_username} -p {kube_password} --kubeconfig /root/auth/kubeconfig"
    )
    ...
    if storage_class_name in res:
        ...
        res = run_ssh(
            f"oc create -f ./{storage_config_file_name} --kubeconfig /root/auth/kubeconfig"
        )
    ...
    if pvc_name in res:
        ...
        res = run_ssh(
            f"oc create -f ./{pvc_config_file_name} --kubeconfig /root/auth/kubeconfig"
        )
    ...
    for e in res["service_instances"]:
        if (
            e["display_name"] == volume_name
            and e["metadata"]["zenControlPlaneNamespace"] == namespace
    ...
    if (
        directory_exists
        and dataset_file_name in res["responseObject"]["directoryContents"]
    ):
...
def save_data_to_container(data_path, data_cos_path, wml_client, data_filenames=None):
    ...
    if wml_client.default_space_id:
        ...
        response = requests.get(project_url, headers=wml_client._get_headers())
    ...
    properties.update(
        details["entity"]["storage"]["properties"]["credentials"]["editor"]
    )
    properties.update(
        details["entity"]["storage"]["properties"]["credentials"]["editor"]
    )
...
def create_connection_to_cos(
    ...
    if save_data and data_path and data_cos_path:
        bucket_name = save_data_to_cos_bucket(
            data_path,
            data_cos_path,
    ...
    if auth_endpoint is None:
        auth_endpoint = (
            wml_client.service_instance._href_definitions.get_iam_token_url()
        )
