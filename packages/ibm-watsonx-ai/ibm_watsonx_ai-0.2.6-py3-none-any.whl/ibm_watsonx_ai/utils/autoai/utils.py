...
def fetch_pipelines(run_params: dict,
                    ...
                    **kwargs) -> Tuple[Union[None, Dict[str, 'Pipeline']], bool]:
    ...
    def check_pipeline_nodes(pipeline: dict, request_json: dict, api_client) -> bool:
        ...
        if estimator_name in xgboost_estimators:
            check_lale = check_dependencies_versions(request_json, api_client, 'xgboost')

        elif estimator_name in lightgbm_estimators:
            check_lale = check_dependencies_versions(request_json, api_client, 'lightgbm')
        ...
        for pipeline in run_params['entity']['status'].get('metrics', []):
            ...
            if pipeline['context']['phase'] == model_phase:
                ...
                if pipeline_name is None:
                    ...
                    # note: check dependencies for estimators and other packages
                    request_json = download_request_json(run_params, pipelines_names[-1], api_client,
                                                         auto_pipelines_parameters=auto_pipelines_parameters)
                    check_lale = check_pipeline_nodes(pipeline, request_json, api_client)
        ...
        if load_pipelines:
            ...
            with redirect_stdout(open(os.devnull, "w")):
                for model_path, pipeline_name in zip(model_paths, pipelines_names):
                    pipelines[pipeline_name] = joblib.load(load_file_from_file_system(api_client=api_client,
            ...
            if pipeline_name == f"Pipeline_{model_number}":
                ...
                if pipeline['context']['phase'] == model_phase:
                    ...
                    with open(filename, 'wb') as file:
                        file.write(load_file_from_file_system(api_client=api_client, file_path=notebook_path).getvalue())
...
def load_file_from_file_system(api_client: Optional['APIClient'] = None,
    ...
    api_client.deployments._handle_response(200, u'get asset file', response_with_model, json_response=False)
...
def load_file_from_file_system_nonautoai(api_client: Optional['APIClient'] = None,
                                         file_path: Optional[str] = None,
                                         stream: bool = True,
                                         **kwargs) -> 'io.BytesIO':
    """Load file into memory from the file system.
...
def prepare_auto_ai_model_to_publish_normal_scenario(
    ...
    # --- end note
    try:
        request_json: Dict[str, dict] = download_request_json(run_params, pipeline_model, api_client, result_reference,
                                                              auto_pipelines_parameters=auto_pipelines_parameters)
    except:
        raise MissingPipeline(
            pipeline_model,
...
def prepare_auto_ai_model_to_publish(
    ...
    if api_client.ICP:
        ...
        if run_params['entity']['results_reference']['type'] == DataConnectionTypes.CA:
            connection_details = api_client.connections.get_details(run_params['entity']['results_reference']['connection']['id'])['entity']
        ...
        cos_client.meta.client.download_file(Bucket=bucket, Filename=pipeline_definition_name,
...
def check_dependencies_versions(request_json: dict,
                                api_client: Optional['APIClient'] = None,
...
def download_request_json(run_params: dict, model_name: str, api_client=None, results_reference: 'DataConnection' = None,
                          auto_pipelines_parameters=None, **kwargs) -> dict:
    ...
    if api_client.ICP:
        ...
        if results_reference['type'] == DataConnectionTypes.CA:
            connection_details = api_client.connections.get_details(results_reference['connection']['id'])['entity']
            cos_client = init_cos_client(connection_details)
        else:
