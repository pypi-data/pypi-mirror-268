...
from ibm_watsonx_ai.utils.autoai.enums import (
    ...
    ImputationStrategy)
from ibm_watsonx_ai.utils.autoai.errors import PipelineNotLoaded, FitNeeded, InvalidPredictionType, \
...
class ServiceEngine(BaseEngine):
    ...
    def __init__(self, workspace: 'WorkSpace') -> None:
        ...
        self._api_client = workspace.api_client
    ...
    def _initialize_pipeline_metadata(self, **kwargs) -> None:
        """Initialization of Pipeline Document (API client Meta Parameter)
            with provided parameters and default ones."""

        self._pipeline_metadata = {
            self._api_client.pipelines.ConfigurationMetaNames.NAME:
                self._auto_pipelines_parameters.get('name', 'Default name.'),
            self._api_client.pipelines.ConfigurationMetaNames.DESCRIPTION:
        ...
        # New parameters for autoai-core which are supported in versions CPD >= 4.0
        pred_type = self._auto_pipelines_parameters.get('prediction_type')
        if ((self._api_client.CLOUD_PLATFORM_SPACES or self._api_client.CPD_version) and
                pred_type != PredictionType.FORECASTING and
                pred_type != PredictionType.TIMESERIES_ANOMALY_PREDICTION):
            ...
            if self._auto_pipelines_parameters.get('word2vec_feature_number') is not None:
                self._pipeline_metadata[self._api_client.pipelines.ConfigurationMetaNames.DOCUMENT][
                    ...
                    'text_processing_options'] = {'word2vec':
                                                      {'output_dim': self._auto_pipelines_parameters.get(
                                                          'word2vec_feature_number')}}
            ...
            if self._api_client.CLOUD_PLATFORM_SPACES or self._api_client.CPD_version >= 4.5:
            ...
            if self._api_client.CLOUD_PLATFORM_SPACES or self._api_client.CPD_version >= 4.8:
                if self._auto_pipelines_parameters.get('time_ordered_data') is not None:
                    self._pipeline_metadata[self._api_client.pipelines.ConfigurationMetaNames.DOCUMENT][
                        'pipelines'][0]['nodes'][0]['parameters']['optimization'][
                        'time_ordered_data'] = self._auto_pipelines_parameters.get('time_ordered_data')

                if self._auto_pipelines_parameters.get('feature_selector_mode') is not None:
                    self._pipeline_metadata[self._api_client.pipelines.ConfigurationMetaNames.DOCUMENT][
                        'pipelines'][0]['nodes'][0]['parameters']['optimization'][
                        'feature_selector_mode'] = self._auto_pipelines_parameters.get('feature_selector_mode')
        ...
        if self._20_class_limit_removal_test:
            self._pipeline_metadata[self._api_client.pipelines.ConfigurationMetaNames.DOCUMENT]['pipelines'][0]['nodes'][0]['parameters'].update({
        ...
        if 0 < len(t_size) <= 2:
            self._pipeline_metadata[self._api_client.pipelines.ConfigurationMetaNames.DOCUMENT]['runtimes'][0][
        ...
        if (self._auto_pipelines_parameters.get('positive_label') is not None
        ...
        if self._auto_pipelines_parameters.get('daub_give_priority_to_runtime') is not None:
            self._pipeline_metadata[self._api_client.pipelines.ConfigurationMetaNames.DOCUMENT][
                'pipelines'][0]['nodes'][0]['parameters']['optimization'][
    ...
    def _initialize_training_metadata(self,
        ...
        """Initialization of training metadata (API client Meta Parameter).
        ...
        self._training_metadata = {
            self._api_client.training.ConfigurationMetaNames.TAGS:
                ['autoai'],

            self._api_client.training.ConfigurationMetaNames.TRAINING_DATA_REFERENCES:
                [connection._to_dict() for connection in training_data_reference],

            self._api_client.training.ConfigurationMetaNames.TRAINING_RESULTS_REFERENCE:
                training_results_reference._to_dict(),

            self._api_client.training.ConfigurationMetaNames.PIPELINE:
                {'id': self._stored_pipeline_details['metadata']['id']},

            self._api_client.training.ConfigurationMetaNames.NAME:
                f"{self._auto_pipelines_parameters.get('name', 'Default name.')[:40]} - "
                f"pipeline: {self._stored_pipeline_details['metadata']['id']}",

            self._api_client.training.ConfigurationMetaNames.DESCRIPTION:
        ...
        if test_data_references is not None:
            self._training_metadata[
                self._api_client.training.ConfigurationMetaNames.TEST_DATA_REFERENCES
        ...
        for connection in \
                self._training_metadata[self._api_client.training.ConfigurationMetaNames.TRAINING_DATA_REFERENCES] +\
                [self._training_metadata[self._api_client.training.ConfigurationMetaNames.TRAINING_RESULTS_REFERENCE]]:
    ...
    ##########################################################
    #   Pipeline Part / AutoPipelineOptimizer Init Part  #
    ##########################################################
    def initiate_remote_resources(self, params: dict, **kwargs) -> None:
        ...
        self._initialize_pipeline_metadata(**kwargs)
        ...
        self._stored_pipeline_details = self._api_client.pipelines.store(meta_props=self._pipeline_metadata, **kwargs)
    ...
    ##########################################################
    #   Training Part / AutoPipelineOptimizer Run Part   #
    ##########################################################
    def fit(self,
            training_data_reference: List['DataConnection'],
            training_results_reference: 'DataConnection',
            background_mode: bool = True,
            test_data_references: List['DataConnection'] = None) -> dict:
        ...
        :rtype: dict
        """
        self._initialize_training_metadata(training_data_reference=training_data_reference,
        ...
        if background_mode:
            ...
            if self._api_client.default_project_id:
                url = f"{url}?project_id={self._api_client.default_project_id}&version={params.get('version')}"
            elif self._api_client.default_space_id:
                url = f"{url}?space_id={self._api_client.default_space_id}&version={params.get('version')}"
            ...
            while not end:
                ...
                websocket.add_header(bytes("Authorization", "utf-8"),
                                     bytes("Bearer " + self._api_client.service_instance._get_token(), "utf-8"))
    ...
    def _get_data_from_property_or_file(self, property_name: str, details: Dict, results_reference: Dict, default_value: List) -> List:
        if f'{property_name}_location' in details:
            ...
            conn._api_client = self._api_client
