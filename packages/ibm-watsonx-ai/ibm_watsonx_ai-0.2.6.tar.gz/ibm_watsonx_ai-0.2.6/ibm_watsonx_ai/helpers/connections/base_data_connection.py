...
class BaseDataConnection(ABC):
    ...
    def _download_csv_file(self, path) -> dict:
        ...
        if self.type == DataConnectionTypes.FS:
            csv_url = self._api_client.service_instance._href_definitions.get_wsd_model_attachment_href() + f"/auto_ml/{path.split('/auto_ml/')[-1]}"

            with requests.get(csv_url, params=self._api_client._params(), headers=self._api_client._get_headers(),
        ...
        if self.type == DataConnectionTypes.S3:
            ...
            with requests.get(json_url, params=self._api_client._params(), headers=self._api_client._get_headers(),
                ...
                try:
                    ...
                    json_content = [json.loads(jline) for jline in buffer.getvalue().decode('utf-8').splitlines()]
            ...
            if self._check_if_connection_asset_is_s3():
                ...
                try:
                    ...
                    try:
                        ...
                        json_content = [json.loads(jline) for jline in content.decode('utf-8').splitlines()]
    ...
    def _get_attachemnt_details(self, data_asset_id: str):
        response = requests.get(self._api_client.service_instance._href_definitions.get_data_asset_href(data_asset_id),
                                params=self._api_client._params(),
                                headers=self._api_client._get_headers())
        data_asset_details = self._api_client.data_assets._handle_response(200, u'Cannot get data asset details', response)
        ...
        if attachments_data_asset_details and attachments_data_asset_details.get('connection_id') is None:
            ...
            self._api_client.data_assets._handle_response(200, u'Cannot get attachment details', response)
    ...
    def _check_if_connection_asset_is_s3(self) -> bool:
        try:
            ...
            if self.type == 'data_asset':
                ...
                if self._api_client is not None:
                    connection_details = self._api_client.connections.get_details(self.connection.id)
    ...
    def _download_training_data_from_data_asset_storage(self) -> 'DataFrame':
        ...
        if self._api_client is not None:
            ...
            asset_response = requests.get(
                self._api_client.service_instance._href_definitions.get_data_asset_href(asset_id),
                params=self._api_client._params(),
                headers=self._api_client._get_headers())

            asset_details = self._api_client.data_assets._handle_response(200, u'get assets', asset_response)

            attachment_id = asset_details['attachments'][0]['id']
            response = requests.get(
                self._api_client.service_instance._href_definitions.get_attachment_href(asset_id, attachment_id),
    ...
    def _download_training_data_from_file_system(self, binary: bool = False) -> Tuple['DataFrame', bytes]:
        ...
        try:
            url = self._api_client.service_instance._href_definitions.get_wsd_model_attachment_href() + f"/{self.location.path.split('/assets/')[-1]}"
            # note: stream the whole CSV file
    ...
    def _download_indices_from_file_system(self, location_path: str) -> 'DataFrame':
        ...
        try:
            url = self._api_client.service_instance._href_definitions.get_wsd_model_attachment_href() + f"/{location_path.split('/assets/')[-1]}"
            # note: stream the whole CSV file
            csv_response = requests.get(url,
                                        params=self._api_client._params(),
                                        headers=self._api_client._get_headers(),
    ...
    def _download_data_from_cos(self, cos_client: 'resource', binary: bool = False) -> Union['DataFrame', bytes]:
    ...
    def _create_conn_details_for_container(self) -> dict:
        if self._api_client is not None:
            if self._api_client.default_space_id is not None:
                details = self._api_client.spaces.get_details(self._api_client.default_space_id)

            else:
                project_url = self._api_client.service_instance._href_definitions.get_project_href(
                    self._api_client.default_project_id)

                response = requests.get(
                    project_url,
                    headers=self._api_client._get_headers(projects_token=True)
                )
    ...
    def _download_data_from_flight_service(self,
        ...
        from ibm_watsonx_ai.data_loaders.datasets.experiment import ExperimentIterableDataset
        ...
        if data is not None:
            ...
            if isinstance(self.location, DatabaseLocation) and not binary:
                if '.csv' in file_path:
                    ...
                    raise ValueError(f"Cannot upload file {file_path} to database. Please provide binary=True "
    ...
    def _is_size_acceptable(self):
        ...
        if self._api_client and self.type == 'data_asset':
            ...
            asset_size = self._api_client.data_assets.get_details(data_asset_id).get('metadata', {}).get("size", 0)
