...
class occSDK:
    ...
    async def stock_loans(self, report_date: str = None, as_dataframe:bool=True) -> Optional[StockLoans]:
        """Retrieve stock loan data for a specific report date and type.
    ...
    async def stock_info(self, symbol:str, has_options:bool=True, as_dataframe:bool=True):
        stockId = await self.get_ticker_id(symbol)


        async with httpx.AsyncClient() as client:
            response = await client.get(f"https://private-authorization.ivolatility.com/lookup/stockInformation?stockId={stockId}&region=1&rtMode=RT&hasOptions={has_options}", headers=await self.get_headers())

            data = response.json()
            if as_dataframe == True:
                df = StockInfo(data, ticker=symbol).as_dataframe
                df = format_large_numbers_in_dataframe(df)
                return data, df
    ...
    async def expiry_dates(self, symbol:str='AAPL'):
        headers = await self.get_headers()
        id = await self.get_ticker_id(symbol)
