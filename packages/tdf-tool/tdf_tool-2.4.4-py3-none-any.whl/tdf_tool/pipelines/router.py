...
class Router:
        ...
        for packageItem in self.__packagesList:
            ...
            if self.__hasRouterDartFile(packageItem.packagePath):
                ...
                self.__defaultImportCreator(
                    f, "'package:tdf_router/tdf_router.dart';")

                ShellDir.goInShellDir()
                os.chdir(packageItem.packagePath)

                nowaPath = os.popen(PlatformTools.pwd_cmd()
                                    ).read().split("\n")[0]
    ...
    def __generate_router_init_files(self):
        ...
        self.__defaultImportCreator(
            initF, "'package:tdf_router/tdf_router.dart';")
        # 添加flutter业务入口页面import
        self.__defaultImportCreator(
            initF, "'entry/tz_router_entry_page.dart';")
        mapList = []
        for root, _, files in os.walk(nowaPath):
            for file in files:
                src_file = os.path.join(root, file)
                ...
                itemFileAlias = (
                    "router_map_" + md5(itemFilePath.encode()
                                        ).hexdigest()[8:-8]
                )
...
class _{2}State extends State<{2}> {{
    ...
    """
        mFileWriter.write(content.format(
            moduleName, self.__mapAddListCreator(list)))
