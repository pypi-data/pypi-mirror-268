...
# 检查箱型箱量是否和集装箱列表匹配
def check_ctntype_id(ctntype_id, ctn_list):
    ...
    if set2 - set1 and len(set1) == 1 and len(set2) == 1:
        ...
        return ResponseData(code=RespType.task_success, data=ctntype_id)
    elif set2 - set1:
        return ResponseData(code=RespType.task_failed, msg='放舱列表中未找到对应箱型，请联系一代')
    else:
        return ResponseData(code=RespType.task_success, data=ctntype_id)
