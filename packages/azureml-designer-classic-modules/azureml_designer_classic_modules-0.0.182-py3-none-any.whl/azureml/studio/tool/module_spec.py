...
from azureml.studio.tool.checkers import is_conda_dependencies_same
...
class ModuleSpec:
    ...
    def env(self, value: Environment):
        if is_conda_dependencies_same(value.python.conda_dependencies._conda_dependencies,
                                      self.meta.conda_dependencies.conda_dependency_dict):
            self._env = value
        else:
            raise ValueError(
                f"Failed to set {value.name} with version {value.version} to the module spec with name {self.name}")
    ...
    def yaml_spec_dict_v2(self):
        ...
        if self.env is None:
            ...
            aml_env = {"name": self.env.name, "version": self.env.version}
