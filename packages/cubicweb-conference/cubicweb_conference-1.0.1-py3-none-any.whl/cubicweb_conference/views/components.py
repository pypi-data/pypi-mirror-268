...
class TalkStateCtxComponent(component.EntityCtxComponent):
    ...
    def render_body(self, w):
        w(f'<div style="background-color: {self.color}">')
        w(self.msg)
        w("</div>")
...
class TalkStateCtxComponent2(TalkStateCtxComponent):
    ...
    def init_rendering(self):
        ...
        if state == "inreview":
            self.msg = _cw._(
                'This talk is in state "%s", which means you have '
            ...
            self.msg = _cw._(
                'This talk is in state "%s", which means the author '
