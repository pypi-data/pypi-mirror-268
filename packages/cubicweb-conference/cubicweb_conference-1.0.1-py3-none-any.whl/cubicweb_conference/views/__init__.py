...
from cubicweb_web.views import primary, uicfg
from cubicweb_conference.views.forms import (
    subject_in_track_vocabulary,
    subject_call_open_conf_vocabulary,
)

from cubicweb_web.views.basetemplates import HTMLPageHeader

from cubicweb_web.views.basecomponents import ApplicationName
...
class CWUserPrimaryView(primary.PrimaryView):
    ...
    def render_entity_title(self, entity):
        ...
        self.w(f"<h1>{title}</h1>")

    def render_entity_attributes(self, entity):
        if entity.representing:
            self.w(f"{self._cw._('Representing')} {entity.representing}")
...
class ConferenceTabRewrite(SimpleReqRewriter):
    rules = [
        (
            ...
            dict(
                rql='Any C WHERE C is Conference, C url_id "\\1"',
                selected_tab=r"\2",
                tab=r"\2",
            ),
        ),
