...
# body param
SchemaForRequestBodyApplicationJson = BasePushOperationRequest


request_body_base_push_operation_request = api_client.RequestBody(
    content={
...
class BaseApi(api_client.Api):
    ...
    def _de_isolate_computer_oapg(
        ...
        """
        De-Isolates the computers matching the given query. <p>De-isolating a computer restores its access to network resources. Affects only isolated computers</p>
    
        API Parameters:
         - x-mgmt-run-as-job: RunAsJobONOn - Mandatory placed in "header_params" param
         - body: BasePushOperationRequest - Mandatory 
        """
        """
        De-isolate computers matching a given query
        ...
        serialized_data = parameter.serialize('de_isolate_computer', prefix_separator_iterator)
        ...
        _fields = None
        _body = None
        serialized_data = request_body_base_push_operation_request.serialize(body, content_type)
        ...
        logger(f'Sending operation "de_isolate_computer" url "{final_url}" ...')
        ...
        try:
            ...
            error_logger(f'Receiving operation "de_isolate_computer" communication error for url "{final_url}" with status, reason "{e.reason}"')
        ...
        logger(f'Receiving operation "de_isolate_computer" url "{final_url}" with status code "{response.status}"')
        network_logger(f'Receiving "{final_url}" with options: "{response_options}"')
    
        if not 200 <= response.status <= 299:
            error_logger(f'Receiving error operation "de_isolate_computer" url "{final_url}" with "{response_options}"')
...
class DeIsolateComputer(BaseApi):
    # this class is used by api classes that refer to endpoints with operationId fn names




    def de_isolate_computer(
        self,
        # GTO-EA-1-2
        body: SchemaForRequestBodyApplicationJson,
        ...
        return self._de_isolate_computer_oapg(
            body=body,
