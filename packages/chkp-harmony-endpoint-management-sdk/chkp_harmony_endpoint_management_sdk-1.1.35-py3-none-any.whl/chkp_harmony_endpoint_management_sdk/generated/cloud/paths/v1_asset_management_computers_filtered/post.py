...
# body param
SchemaForRequestBodyApplicationJson = ComputersByFilterArgs


request_body_computers_by_filter_args = api_client.RequestBody(
    content={
...
SchemaFor200ResponseBodyApplicationJson = JobOrResultIComputerList
...
SchemaFor401ResponseBodyApplicationJson = MissingApiTokenErrorExample
...
class HarmonyResponse:
    def __init__(self):
        self.payload: JobOrResultIComputerList = None
...
class BaseApi(api_client.Api):
    ...
    def _computers_by_filter_oapg(
        ...
        serialized_data = parameter.serialize('computers_by_filter', prefix_separator_iterator)
        ...
        _fields = None
        _body = None
        serialized_data = request_body_computers_by_filter_args.serialize(body, content_type)
        ...
        logger(f'Sending operation "computers_by_filter" url "{final_url}" ...')
        ...
        try:
            ...
            error_logger(f'Receiving operation "computers_by_filter" communication error for url "{final_url}" with status, reason "{e.reason}"')
        ...
        logger(f'Receiving operation "computers_by_filter" url "{final_url}" with status code "{response.status}"')
        network_logger(f'Receiving "{final_url}" with options: "{response_options}"')
    
        if not 200 <= response.status <= 299:
            error_logger(f'Receiving error operation "computers_by_filter" url "{final_url}" with "{response_options}"')
...
class ComputersByFilter(BaseApi):
    ...
    def computers_by_filter(
        ...
        return self._computers_by_filter_oapg(
            body=body,
