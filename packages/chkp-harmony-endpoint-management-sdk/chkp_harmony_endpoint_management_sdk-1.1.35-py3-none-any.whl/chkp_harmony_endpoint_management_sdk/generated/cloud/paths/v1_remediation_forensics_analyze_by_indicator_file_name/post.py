...
class BaseApi(api_client.Api):
    @typing.overload
    def _analyze_by_file_name_oapg(
        self,
        # GTO-EA-2-2
    ...
    def _analyze_by_file_name_oapg(
        ...
        """
        Collects forensics data whenever a computer that matches the given query accesses or executes the given file
        ...
        serialized_data = parameter.serialize('analyze_by_file_name', prefix_separator_iterator)
        ...
        logger(f'Sending operation "analyze_by_file_name" url "{final_url}" ...')
        ...
        try:
            ...
            error_logger(f'Receiving operation "analyze_by_file_name" communication error for url "{final_url}" with status, reason "{e.reason}"')
        ...
        logger(f'Receiving operation "analyze_by_file_name" url "{final_url}" with status code "{response.status}"')
        network_logger(f'Receiving "{final_url}" with options: "{response_options}"')
    
        if not 200 <= response.status <= 299:
            error_logger(f'Receiving error operation "analyze_by_file_name" url "{final_url}" with "{response_options}"')
...
class AnalyzeByFileName(BaseApi):
    # this class is used by api classes that refer to endpoints with operationId fn names




    def analyze_by_file_name(
        self,
        # GTO-EA-1-2
        body: SchemaForRequestBodyApplicationJson,
        ...
        return self._analyze_by_file_name_oapg(
            body=body,
