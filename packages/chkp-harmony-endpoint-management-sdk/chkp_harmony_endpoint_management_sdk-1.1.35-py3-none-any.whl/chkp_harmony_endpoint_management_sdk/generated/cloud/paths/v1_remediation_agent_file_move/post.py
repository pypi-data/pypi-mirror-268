...
# body param
SchemaForRequestBodyApplicationJson = PushOperationRequestWithMandatoryParamsBaseFileRelocationParams


request_body_push_operation_request_with_mandatory_params_base_file_relocation_params = api_client.RequestBody(
    content={
...
class BaseApi(api_client.Api):
    ...
    def _agent_move_file_oapg(
        ...
        API Parameters:
         ...
         - body: PushOperationRequestWithMandatoryParamsBaseFileRelocationParams - Mandatory 
        """
        """
        Move a given file on computers matching the given query
        ...
        serialized_data = parameter.serialize('agent_move_file', prefix_separator_iterator)
        ...
        _fields = None
        _body = None
        serialized_data = request_body_push_operation_request_with_mandatory_params_base_file_relocation_params.serialize(body, content_type)
        ...
        logger(f'Sending operation "agent_move_file" url "{final_url}" ...')
        ...
        try:
            ...
            error_logger(f'Receiving operation "agent_move_file" communication error for url "{final_url}" with status, reason "{e.reason}"')
        ...
        logger(f'Receiving operation "agent_move_file" url "{final_url}" with status code "{response.status}"')
        network_logger(f'Receiving "{final_url}" with options: "{response_options}"')
    
        if not 200 <= response.status <= 299:
            error_logger(f'Receiving error operation "agent_move_file" url "{final_url}" with "{response_options}"')
...
class AgentMoveFile(BaseApi):
    ...
    def agent_move_file(
        ...
        return self._agent_move_file_oapg(
            body=body,
