...
class BaseApi(api_client.Api):
    ...
    def _agent_copy_file_oapg(
        ...
        """
        """
        Copy a given file on computers matching the given query
        ...
        serialized_data = parameter.serialize('agent_copy_file', prefix_separator_iterator)
        ...
        logger(f'Sending operation "agent_copy_file" url "{final_url}" ...')
        ...
        try:
            ...
            error_logger(f'Receiving operation "agent_copy_file" communication error for url "{final_url}" with status, reason "{e.reason}"')
        ...
        logger(f'Receiving operation "agent_copy_file" url "{final_url}" with status code "{response.status}"')
        network_logger(f'Receiving "{final_url}" with options: "{response_options}"')
    
        if not 200 <= response.status <= 299:
            error_logger(f'Receiving error operation "agent_copy_file" url "{final_url}" with "{response_options}"')
...
class AgentCopyFile(BaseApi):
    ...
    def agent_copy_file(
        ...
        return self._agent_copy_file_oapg(
            body=body,
