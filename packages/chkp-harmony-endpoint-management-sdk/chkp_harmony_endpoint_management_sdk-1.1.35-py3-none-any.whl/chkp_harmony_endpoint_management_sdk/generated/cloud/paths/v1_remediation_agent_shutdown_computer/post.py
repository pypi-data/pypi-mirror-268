...
# body param
SchemaForRequestBodyApplicationJson = PushOperationRequestAgentRebootOrShutdownParameters


request_body_push_operation_request_agent_reboot_or_shutdown_parameters = api_client.RequestBody(
    content={
...
class BaseApi(api_client.Api):
    ...
    def _shutdown_computer_oapg(
        ...
        """
        Shuts-down computers matching the given query
    
        API Parameters:
         - x-mgmt-run-as-job: RunAsJobONOn - Mandatory placed in "header_params" param
         - body: PushOperationRequestAgentRebootOrShutdownParameters - Mandatory 
        """
        """
        Shut-down computers matching a given query
        ...
        serialized_data = parameter.serialize('shutdown_computer', prefix_separator_iterator)
        ...
        _fields = None
        _body = None
        serialized_data = request_body_push_operation_request_agent_reboot_or_shutdown_parameters.serialize(body, content_type)
        ...
        logger(f'Sending operation "shutdown_computer" url "{final_url}" ...')
        ...
        try:
            ...
            error_logger(f'Receiving operation "shutdown_computer" communication error for url "{final_url}" with status, reason "{e.reason}"')
        ...
        logger(f'Receiving operation "shutdown_computer" url "{final_url}" with status code "{response.status}"')
        network_logger(f'Receiving "{final_url}" with options: "{response_options}"')
    
        if not 200 <= response.status <= 299:
            error_logger(f'Receiving error operation "shutdown_computer" url "{final_url}" with "{response_options}"')
...
class ShutdownComputer(BaseApi):
    ...
    def shutdown_computer(
        ...
        return self._shutdown_computer_oapg(
            body=body,
