...
class BaseApi(api_client.Api):
    ...
    def _delete_ioc_by_ids_oapg(
        ...
        API Parameters:
         ...
         - ids: [str] - Mandatory placed in "query_params" param
        """
        """
        Delete Indicators of Compromise by ID
        ...
        serialized_data = parameter.serialize('delete_ioc_by_ids', prefix_separator_iterator)
        for serialized_value in serialized_data.values():
            used_path += serialized_value
        ...
        logger(f'Sending operation "delete_ioc_by_ids" url "{final_url}" ...')
        ...
        try:
            ...
            error_logger(f'Receiving operation "delete_ioc_by_ids" communication error for url "{final_url}" with status, reason "{e.reason}"')
        ...
        logger(f'Receiving operation "delete_ioc_by_ids" url "{final_url}" with status code "{response.status}"')
        network_logger(f'Receiving "{final_url}" with options: "{response_options}"')
    
        if not 200 <= response.status <= 299:
            error_logger(f'Receiving error operation "delete_ioc_by_ids" url "{final_url}" with "{response_options}"')
...
class DeleteIocByIds(BaseApi):
    # this class is used by api classes that refer to endpoints with operationId fn names



    def delete_ioc_by_ids(
        self,
        query_params: RequestQueryParams = frozendict.frozendict(),
        header_params: RequestHeaderParams = frozendict.frozendict(),
        ...
        return self._delete_ioc_by_ids_oapg(
