...
# Path params
VirtualGroupIdSchema = schemas.StrSchema
RequestRequiredPathParams = typing_extensions.TypedDict(
    'RequestRequiredPathParams',
    {
        'virtualGroupId': typing.Union[VirtualGroupIdSchema, str, ],
    }
)
...
request_path_virtual_group_id = api_client.PathParameter(
    name="virtualGroupId",
    style=api_client.ParameterStyle.SIMPLE,
    schema=VirtualGroupIdSchema,
    required=True,
)
# body param
...
class BaseApi(api_client.Api):
    @typing.overload
    def _add_members_to_virtual_group_oapg(
        self,
        # GTO-EA-2-2
    ...
    def _add_members_to_virtual_group_oapg(
        ...
        """
        Adds the specified entities to the given Virtual Group <p>Entities that are already assigned to the Virtual Group are ignored</p>
    
        API Parameters:
         - virtualGroupId: str - Mandatory placed in "path_params" param, The ID of the Virtual Group to add members to
         - x-mgmt-run-as-job: RunAsJob - Mandatory placed in "header_params" param
         - body: [str] - Mandatory , A list of entity IDs to add to the given Virtual Group <p>Virtual Groups can accepts entities of types 'USER', 'COMPUTER' and 'VIRTUAL_GROUP' (for supported servers)</p>
        """
        """
        Add entities to a Virtual Group
        ...
        serialized_data = parameter.serialize('add_members_to_virtual_group', prefix_separator_iterator)
        ...
        logger(f'Sending operation "add_members_to_virtual_group" url "{final_url}" ...')
        ...
        try:
            ...
            error_logger(f'Receiving operation "add_members_to_virtual_group" communication error for url "{final_url}" with status, reason "{e.reason}"')
        ...
        logger(f'Receiving operation "add_members_to_virtual_group" url "{final_url}" with status code "{response.status}"')
        network_logger(f'Receiving "{final_url}" with options: "{response_options}"')
    
        if not 200 <= response.status <= 299:
            error_logger(f'Receiving error operation "add_members_to_virtual_group" url "{final_url}" with "{response_options}"')
...
class AddMembersToVirtualGroup(BaseApi):
    # this class is used by api classes that refer to endpoints with operationId fn names




    def add_members_to_virtual_group(
        self,
        # GTO-EA-1-2
        body: SchemaForRequestBodyApplicationJson,
        ...
        return self._add_members_to_virtual_group_oapg(
            body=body,
