...
class BaseApi(api_client.Api):
    ...
    def _quarantine_file_oapg(
        ...
        """
        """
        Quarantine given files on computers matching a given query
        ...
        serialized_data = parameter.serialize('quarantine_file', prefix_separator_iterator)
        ...
        logger(f'Sending operation "quarantine_file" url "{final_url}" ...')
        ...
        try:
            ...
            error_logger(f'Receiving operation "quarantine_file" communication error for url "{final_url}" with status, reason "{e.reason}"')
        ...
        logger(f'Receiving operation "quarantine_file" url "{final_url}" with status code "{response.status}"')
        network_logger(f'Receiving "{final_url}" with options: "{response_options}"')
    
        if not 200 <= response.status <= 299:
            error_logger(f'Receiving error operation "quarantine_file" url "{final_url}" with "{response_options}"')
...
class QuarantineFile(BaseApi):
    ...
    def quarantine_file(
        ...
        return self._quarantine_file_oapg(
            body=body,
