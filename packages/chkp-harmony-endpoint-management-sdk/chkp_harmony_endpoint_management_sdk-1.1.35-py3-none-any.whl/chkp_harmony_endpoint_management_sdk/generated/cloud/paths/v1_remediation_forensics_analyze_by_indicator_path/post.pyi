...
class BaseApi(api_client.Api):
    ...
    def _analyze_by_path_oapg(
        ...
        """
         Collects forensics data whenever a computer that matches the given query accesses or executes a resource residing in the given path
        ...
        serialized_data = parameter.serialize('analyze_by_path', prefix_separator_iterator)
        ...
        logger(f'Sending operation "analyze_by_path" url "{final_url}" ...')
        ...
        try:
            ...
            error_logger(f'Receiving operation "analyze_by_path" communication error for url "{final_url}" with status, reason "{e.reason}"')
        ...
        logger(f'Receiving operation "analyze_by_path" url "{final_url}" with status code "{response.status}"')
        network_logger(f'Receiving "{final_url}" with options: "{response_options}"')
    
        if not 200 <= response.status <= 299:
            error_logger(f'Receiving error operation "analyze_by_path" url "{final_url}" with "{response_options}"')
...
class AnalyzeByPath(BaseApi):
    ...
    def analyze_by_path(
        ...
        return self._analyze_by_path_oapg(
            body=body,
