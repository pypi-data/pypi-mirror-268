...
SchemaFor200ResponseBodyApplicationJson = TenantsOverviewInfo
...
class HarmonyResponse:
    def __init__(self):
        self.payload: TenantsOverviewInfo = None
...
class BaseApi(api_client.Api):
    @typing.overload
    def _public_mssp_operational_oapg(
        self,
    ...
    def _public_mssp_operational_oapg(
        ...
        """
        Gets the operational information, such as the number of active endpoints and the deployment status.
        ...
        serialized_data = parameter.serialize('public_mssp_operational', prefix_separator_iterator)
        ...
        logger(f'Sending operation "public_mssp_operational" url "{final_url}" ...')
        ...
        try:
            ...
            error_logger(f'Receiving operation "public_mssp_operational" communication error for url "{final_url}" with status, reason "{e.reason}"')
        ...
        logger(f'Receiving operation "public_mssp_operational" url "{final_url}" with status code "{response.status}"')
        network_logger(f'Receiving "{final_url}" with options: "{response_options}"')
    
        if not 200 <= response.status <= 299:
            error_logger(f'Receiving error operation "public_mssp_operational" url "{final_url}" with "{response_options}"')
...
class PublicMsspOperational(BaseApi):
    # this class is used by api classes that refer to endpoints with operationId fn names



    def public_mssp_operational(
        self,
        ...
        return self._public_mssp_operational_oapg(
