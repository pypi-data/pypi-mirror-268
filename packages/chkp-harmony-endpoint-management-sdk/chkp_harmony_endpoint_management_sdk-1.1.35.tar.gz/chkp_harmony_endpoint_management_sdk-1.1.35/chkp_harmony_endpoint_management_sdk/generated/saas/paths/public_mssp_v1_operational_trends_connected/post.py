...
RequestOptionalQueryParams = typing_extensions.TypedDict(
    ...
    {
        'daysAgo': typing.Union[DaysAgoSchema, ],
    },
    total=False
)
...
request_query_days_ago = api_client.QueryParameter(
    name="daysAgo",
    style=api_client.ParameterStyle.FORM,
    schema=DaysAgoSchema,
    explode=True,
)
_auth = [
    'cloudInfraJwt',
...
SchemaFor200ResponseBodyApplicationJson = TenantsTrendsResponse
...
class HarmonyResponse:
    def __init__(self):
        self.payload: TenantsTrendsResponse = None
...
class BaseApi(api_client.Api):
    @typing.overload
    def _public_mssp_trends_connected_clients_oapg(
        self,
    ...
    def _public_mssp_trends_connected_clients_oapg(
        ...
        """
        Gets the the number of active endpoints over time.
    
        API Parameters:
         - daysAgo: DayRange - placed in "query_params" param
        """
        """
        ...
        serialized_data = parameter.serialize('public_mssp_trends_connected_clients', prefix_separator_iterator)
        for serialized_value in serialized_data.values():
            used_path += serialized_value
        ...
        logger(f'Sending operation "public_mssp_trends_connected_clients" url "{final_url}" ...')
        ...
        try:
            ...
            error_logger(f'Receiving operation "public_mssp_trends_connected_clients" communication error for url "{final_url}" with status, reason "{e.reason}"')
        ...
        logger(f'Receiving operation "public_mssp_trends_connected_clients" url "{final_url}" with status code "{response.status}"')
        network_logger(f'Receiving "{final_url}" with options: "{response_options}"')
    
        if not 200 <= response.status <= 299:
            error_logger(f'Receiving error operation "public_mssp_trends_connected_clients" url "{final_url}" with "{response_options}"')
...
class PublicMsspTrendsConnectedClients(BaseApi):
    # this class is used by api classes that refer to endpoints with operationId fn names



    def public_mssp_trends_connected_clients(
        self,
        query_params: RequestQueryParams = frozendict.frozendict(),
        ...
        return self._public_mssp_trends_connected_clients_oapg(
