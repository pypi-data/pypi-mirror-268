...
# Path params
MachineIdSchema = schemas.StrSchema
RequestRequiredPathParams = typing_extensions.TypedDict(
    'RequestRequiredPathParams',
    {
        'machineId': typing.Union[MachineIdSchema, str, ],
    }
)
...
request_path_machine_id = api_client.PathParameter(
    name="machineId",
    style=api_client.ParameterStyle.SIMPLE,
    schema=MachineIdSchema,
    required=True,
)
_auth = [
    'cloudInfraJwt',
...
class BaseApi(api_client.Api):
    @typing.overload
    def _public_terminate_single_endpoint_instance_oapg(
        self,
    ...
    def _public_terminate_single_endpoint_instance_oapg(
        ...
        """
        Terminates Harmony Endpoint Server. Warning! You cannot revert this action.
    
        API Parameters:
         - machineId: str - Mandatory placed in "path_params" param
        """
        """
        ...
        serialized_data = parameter.serialize('public_terminate_single_endpoint_instance', prefix_separator_iterator)
        ...
        logger(f'Sending operation "public_terminate_single_endpoint_instance" url "{final_url}" ...')
        ...
        try:
            ...
            error_logger(f'Receiving operation "public_terminate_single_endpoint_instance" communication error for url "{final_url}" with status, reason "{e.reason}"')
        ...
        logger(f'Receiving operation "public_terminate_single_endpoint_instance" url "{final_url}" with status code "{response.status}"')
        network_logger(f'Receiving "{final_url}" with options: "{response_options}"')
    
        if not 200 <= response.status <= 299:
            error_logger(f'Receiving error operation "public_terminate_single_endpoint_instance" url "{final_url}" with "{response_options}"')
...
class PublicTerminateSingleEndpointInstance(BaseApi):
    # this class is used by api classes that refer to endpoints with operationId fn names



    def public_terminate_single_endpoint_instance(
        self,
        path_params: RequestPathParams = frozendict.frozendict(),
        ...
        return self._public_terminate_single_endpoint_instance_oapg(
