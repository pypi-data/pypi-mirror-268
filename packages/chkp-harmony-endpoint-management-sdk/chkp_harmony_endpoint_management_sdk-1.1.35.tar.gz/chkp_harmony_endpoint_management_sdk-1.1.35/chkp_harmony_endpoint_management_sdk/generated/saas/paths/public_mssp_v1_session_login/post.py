...
SchemaFor200ResponseBodyApplicationJson = LoginStatusResponse
...
class HarmonyResponse:
    def __init__(self):
        self.payload: LoginStatusResponse = None
...
class BaseApi(api_client.Api):
    ...
    def _public_mssp_login_oapg(
        ...
        """
        Logs in as MSSP. The session expires after five minutes.
        ...
        serialized_data = parameter.serialize('public_mssp_login', prefix_separator_iterator)
        ...
        logger(f'Sending operation "public_mssp_login" url "{final_url}" ...')
        ...
        try:
            ...
            error_logger(f'Receiving operation "public_mssp_login" communication error for url "{final_url}" with status, reason "{e.reason}"')
        ...
        logger(f'Receiving operation "public_mssp_login" url "{final_url}" with status code "{response.status}"')
        network_logger(f'Receiving "{final_url}" with options: "{response_options}"')
    
        if not 200 <= response.status <= 299:
            error_logger(f'Receiving error operation "public_mssp_login" url "{final_url}" with "{response_options}"')
...
class PublicMsspLogin(BaseApi):
    # this class is used by api classes that refer to endpoints with operationId fn names



    def public_mssp_login(
        self,
        ...
        return self._public_mssp_login_oapg(
