...
class BaseApi(api_client.Api):
    @typing.overload
    def _remove_members_from_virtual_group_oapg(
        self,
        # GTO-EA-2-2
    ...
    def _remove_members_from_virtual_group_oapg(
        ...
        """
        Removes the specified entities from the given Virtual Group. <p>Entities that are not assigned to the Virtual Group are ignored</p>
    
        API Parameters:
         - virtualGroupId: str - Mandatory placed in "path_params" param, The ID of the Virtual Group to remove members from
         - x-mgmt-run-as-job: RunAsJob - Mandatory placed in "header_params" param
         - body: [str] - Mandatory , A list of entity IDs to remove from the given Virtual Group
        """
        """
        Remove entities from a Virtual Group
        ...
        serialized_data = parameter.serialize('remove_members_from_virtual_group', prefix_separator_iterator)
        ...
        logger(f'Sending operation "remove_members_from_virtual_group" url "{final_url}" ...')
        ...
        try:
            ...
            error_logger(f'Receiving operation "remove_members_from_virtual_group" communication error for url "{final_url}" with status, reason "{e.reason}"')
        ...
        logger(f'Receiving operation "remove_members_from_virtual_group" url "{final_url}" with status code "{response.status}"')
        network_logger(f'Receiving "{final_url}" with options: "{response_options}"')
    
        if not 200 <= response.status <= 299:
            error_logger(f'Receiving error operation "remove_members_from_virtual_group" url "{final_url}" with "{response_options}"')
...
class RemoveMembersFromVirtualGroup(BaseApi):
    # this class is used by api classes that refer to endpoints with operationId fn names




    def remove_members_from_virtual_group(
        self,
        # GTO-EA-1-2
        body: SchemaForRequestBodyApplicationJson,
        ...
        return self._remove_members_from_virtual_group_oapg(
            body=body,
