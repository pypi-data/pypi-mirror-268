...
# body param
SchemaForRequestBodyApplicationJson = PushOperationRequestAgentCollectLogsParameters


request_body_push_operation_request_agent_collect_logs_parameters = api_client.RequestBody(
    content={
...
class BaseApi(api_client.Api):
    ...
    def _collect_logs_oapg(
        ...
        """
        Collects all Harmony Endpoint Client logs as well as diagnostic information from computers matching the given query
    
        API Parameters:
         - x-mgmt-run-as-job: RunAsJobONOn - Mandatory placed in "header_params" param
         - body: PushOperationRequestAgentCollectLogsParameters - Mandatory 
        """
        """
        Collect diagnostic information from computers matching a given query
        ...
        serialized_data = parameter.serialize('collect_logs', prefix_separator_iterator)
        ...
        _fields = None
        _body = None
        serialized_data = request_body_push_operation_request_agent_collect_logs_parameters.serialize(body, content_type)
        ...
        logger(f'Sending operation "collect_logs" url "{final_url}" ...')
        ...
        try:
            ...
            error_logger(f'Receiving operation "collect_logs" communication error for url "{final_url}" with status, reason "{e.reason}"')
        ...
        logger(f'Receiving operation "collect_logs" url "{final_url}" with status code "{response.status}"')
        network_logger(f'Receiving "{final_url}" with options: "{response_options}"')
    
        if not 200 <= response.status <= 299:
            error_logger(f'Receiving error operation "collect_logs" url "{final_url}" with "{response_options}"')
...
class CollectLogs(BaseApi):
    ...
    def collect_logs(
        ...
        return self._collect_logs_oapg(
            body=body,
