...
SchemaFor200ResponseBodyApplicationJson = JobOrResultRuleAssignmentArray
...
class HarmonyResponse:
    def __init__(self):
        self.payload: JobOrResultRuleAssignmentArray = None
...
class BaseApi(api_client.Api):
    @typing.overload
    def _get_rule_assignments_oapg(
        self,
    ...
    def _get_rule_assignments_oapg(
        ...
        API Parameters:
         - ruleId: str - Mandatory placed in "path_params" param, The ID of the rule to get the assignments of
         - x-mgmt-run-as-job: RunAsJob - Mandatory placed in "header_params" param
        """
        """
        Get entities assigned to a rule
        ...
        serialized_data = parameter.serialize('get_rule_assignments', prefix_separator_iterator)
        ...
        logger(f'Sending operation "get_rule_assignments" url "{final_url}" ...')
        ...
        try:
            ...
            error_logger(f'Receiving operation "get_rule_assignments" communication error for url "{final_url}" with status, reason "{e.reason}"')
        ...
        logger(f'Receiving operation "get_rule_assignments" url "{final_url}" with status code "{response.status}"')
        network_logger(f'Receiving "{final_url}" with options: "{response_options}"')
    
        if not 200 <= response.status <= 299:
            error_logger(f'Receiving error operation "get_rule_assignments" url "{final_url}" with "{response_options}"')
...
class GetRuleAssignments(BaseApi):
    ...
    def get_rule_assignments(
        ...
        return self._get_rule_assignments_oapg(
