...
# body param
SchemaForRequestBodyApplicationJson = CreateVirtualGroupRequest


request_body_create_virtual_group_request = api_client.RequestBody(
    content={
...
SchemaFor201ResponseBodyApplicationJson = JobOrResultId
...
class HarmonyResponse:
    def __init__(self):
        self.payload: JobOrResultId = None
...
class BaseApi(api_client.Api):
    ...
    def _create_oapg(
        ...
        """
        Creates a new Virtual Group
    
        API Parameters:
         - x-mgmt-run-as-job: RunAsJob - Mandatory placed in "header_params" param
         - body: CreateVirtualGroupRequest - Mandatory , Parameters used to create the Virtual Group
        """
        """
        ...
        serialized_data = parameter.serialize('create', prefix_separator_iterator)
        ...
        _fields = None
        _body = None
        serialized_data = request_body_create_virtual_group_request.serialize(body, content_type)
        ...
        logger(f'Sending operation "create" url "{final_url}" ...')
        ...
        try:
            ...
            error_logger(f'Receiving operation "create" communication error for url "{final_url}" with status, reason "{e.reason}"')
        ...
        logger(f'Receiving operation "create" url "{final_url}" with status code "{response.status}"')
        network_logger(f'Receiving "{final_url}" with options: "{response_options}"')
    
        if not 200 <= response.status <= 299:
            error_logger(f'Receiving error operation "create" url "{final_url}" with "{response_options}"')
