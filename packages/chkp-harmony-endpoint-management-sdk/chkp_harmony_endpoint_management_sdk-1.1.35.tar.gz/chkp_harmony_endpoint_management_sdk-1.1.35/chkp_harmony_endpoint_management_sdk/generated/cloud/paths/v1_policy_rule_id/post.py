...
# body param
SchemaForRequestBodyApplicationJson = ICloneRuleInput


request_body_i_clone_rule_input = api_client.RequestBody(
    content={
...
class BaseApi(api_client.Api):
    ...
    def _clone_rule_oapg(
        ...
        """
         Clones the specified rule and puts the clone in the provided position inside the rulebase
        ...
        API Parameters:
         ...
         - body: ICloneRuleInput - Mandatory , properties of the new rule
        """
        """
        Clones the specified rule and puts the clone in the provided position inside the rulebase
        ...
        serialized_data = parameter.serialize('clone_rule', prefix_separator_iterator)
        ...
        _fields = None
        _body = None
        serialized_data = request_body_i_clone_rule_input.serialize(body, content_type)
        ...
        logger(f'Sending operation "clone_rule" url "{final_url}" ...')
        ...
        try:
            ...
            error_logger(f'Receiving operation "clone_rule" communication error for url "{final_url}" with status, reason "{e.reason}"')
        ...
        logger(f'Receiving operation "clone_rule" url "{final_url}" with status code "{response.status}"')
        network_logger(f'Receiving "{final_url}" with options: "{response_options}"')
    
        if not 200 <= response.status <= 299:
            error_logger(f'Receiving error operation "clone_rule" url "{final_url}" with "{response_options}"')
