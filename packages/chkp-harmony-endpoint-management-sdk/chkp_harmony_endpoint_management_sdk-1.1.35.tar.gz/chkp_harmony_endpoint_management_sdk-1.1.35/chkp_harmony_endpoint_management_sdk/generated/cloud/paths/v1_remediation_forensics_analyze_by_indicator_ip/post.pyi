...
class BaseApi(api_client.Api):
    ...
    def _analyze_by_ip_oapg(
        ...
        serialized_data = parameter.serialize('analyze_by_ip', prefix_separator_iterator)
        ...
        logger(f'Sending operation "analyze_by_ip" url "{final_url}" ...')
        ...
        try:
            ...
            error_logger(f'Receiving operation "analyze_by_ip" communication error for url "{final_url}" with status, reason "{e.reason}"')
        ...
        logger(f'Receiving operation "analyze_by_ip" url "{final_url}" with status code "{response.status}"')
        network_logger(f'Receiving "{final_url}" with options: "{response_options}"')
    
        if not 200 <= response.status <= 299:
            error_logger(f'Receiving error operation "analyze_by_ip" url "{final_url}" with "{response_options}"')
...
class AnalyzeByIp(BaseApi):
    ...
    def analyze_by_ip(
        ...
        return self._analyze_by_ip_oapg(
            body=body,
