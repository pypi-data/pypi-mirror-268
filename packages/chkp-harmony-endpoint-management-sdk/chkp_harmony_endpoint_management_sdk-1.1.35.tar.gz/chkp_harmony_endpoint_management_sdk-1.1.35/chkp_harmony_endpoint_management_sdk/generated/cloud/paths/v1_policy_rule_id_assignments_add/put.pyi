...
class BaseApi(api_client.Api):
    @typing.overload
    def _add_rule_assignments_oapg(
        self,
        # GTO-EA-2-2
    ...
    def _add_rule_assignments_oapg(
        ...
        """
        Assigns the specified entities to the given rule. <p>Specified IDs that are already assigned to the rule are ignored</p>
    
        API Parameters:
         - ruleId: str - Mandatory placed in "path_params" param, The ID of the rule to add assignments to
         - x-mgmt-run-as-job: RunAsJob - Mandatory placed in "header_params" param
         - body: [str] - Mandatory , A list of entity IDs to assign to the given rule <p>Assignments may be users, computers, organizational units and Virtual Groups</p>
        """
        """
        Assign entities to a given rule
        ...
        serialized_data = parameter.serialize('add_rule_assignments', prefix_separator_iterator)
        ...
        logger(f'Sending operation "add_rule_assignments" url "{final_url}" ...')
        ...
        try:
            ...
            error_logger(f'Receiving operation "add_rule_assignments" communication error for url "{final_url}" with status, reason "{e.reason}"')
        ...
        logger(f'Receiving operation "add_rule_assignments" url "{final_url}" with status code "{response.status}"')
        network_logger(f'Receiving "{final_url}" with options: "{response_options}"')
    
        if not 200 <= response.status <= 299:
            error_logger(f'Receiving error operation "add_rule_assignments" url "{final_url}" with "{response_options}"')
...
class AddRuleAssignments(BaseApi):
    # this class is used by api classes that refer to endpoints with operationId fn names




    def add_rule_assignments(
        self,
        # GTO-EA-1-2
        body: SchemaForRequestBodyApplicationJson,
        ...
        return self._add_rule_assignments_oapg(
            body=body,
