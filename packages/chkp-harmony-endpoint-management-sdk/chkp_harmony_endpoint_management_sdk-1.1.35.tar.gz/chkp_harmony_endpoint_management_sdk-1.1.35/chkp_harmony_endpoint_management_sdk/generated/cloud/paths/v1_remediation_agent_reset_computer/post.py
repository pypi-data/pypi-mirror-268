...
class BaseApi(api_client.Api):
    ...
    def _reset_computer_oapg(
        ...
        """
        """
        Restart computers matching a given query
        ...
        serialized_data = parameter.serialize('reset_computer', prefix_separator_iterator)
        ...
        logger(f'Sending operation "reset_computer" url "{final_url}" ...')
        ...
        try:
            ...
            error_logger(f'Receiving operation "reset_computer" communication error for url "{final_url}" with status, reason "{e.reason}"')
        ...
        logger(f'Receiving operation "reset_computer" url "{final_url}" with status code "{response.status}"')
        network_logger(f'Receiving "{final_url}" with options: "{response_options}"')
    
        if not 200 <= response.status <= 299:
            error_logger(f'Receiving error operation "reset_computer" url "{final_url}" with "{response_options}"')
...
class ResetComputer(BaseApi):
    ...
    def reset_computer(
        ...
        return self._reset_computer_oapg(
            body=body,
