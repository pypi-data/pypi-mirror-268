...
class BaseApi(api_client.Api):
    @typing.overload
    def _install_all_policies_oapg(
        self,
    ...
    def _install_all_policies_oapg(
        ...
        """
        Installs all policies. <p>Changes to rule settings are only enforced after policy installation</p>
        ...
        serialized_data = parameter.serialize('install_all_policies', prefix_separator_iterator)
        ...
        logger(f'Sending operation "install_all_policies" url "{final_url}" ...')
        ...
        try:
            ...
            error_logger(f'Receiving operation "install_all_policies" communication error for url "{final_url}" with status, reason "{e.reason}"')
        ...
        logger(f'Receiving operation "install_all_policies" url "{final_url}" with status code "{response.status}"')
        network_logger(f'Receiving "{final_url}" with options: "{response_options}"')
    
        if not 200 <= response.status <= 299:
            error_logger(f'Receiving error operation "install_all_policies" url "{final_url}" with "{response_options}"')
...
class InstallAllPolicies(BaseApi):
    # this class is used by api classes that refer to endpoints with operationId fn names



    def install_all_policies(
        self,
        header_params: RequestHeaderParams = frozendict.frozendict(),
        ...
        return self._install_all_policies_oapg(
