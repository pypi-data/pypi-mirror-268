...
def _PropertyContainer_create_property(self, name, dtype=None, components=None, data=None):
    ...
    if you are going to set the property values at a later time:

    .. literalinclude:: ../example_snippets/property_container.py
        :lines: 34-35

    If the property to be created already exists in the container, it gets replaced with a :ref:`modifiable copy <data_ownership>` if necessary.
    The values of the property get overwritten with *data* in this case.

    Note: If you're creating new :py:class:`PropertyContainer`, its element :py:attr:`count` is 0 initially.
    In this state the :py:meth:`!create_property` method allows you to initialize the count while adding the very first property
    ...
    :param int components: Number of vector components of the user-defined property (1 if not specified).
