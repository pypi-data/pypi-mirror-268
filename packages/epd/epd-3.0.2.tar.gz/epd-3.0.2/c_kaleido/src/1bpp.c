...
static uint8_t epd_header[16] = {0x45, 0x50, 0x44, 0x31, 0x40, 0x06, 0xB0, 0x04, 0x04, 0x80, 0xA9, 0x03, 0x00, 0x00, 0x00, 0x00};
...
void kaleido_convert_1bpp(uint8_t* in, uint8_t* out, uint16_t width, uint16_t height)
{
    uint8_t* outs = out+32;
    ...
    for (int y = 0; y < H; y+=STEP) // loop for rows
    {
        for (int x = 0; x < W; x+=STEP) // loop for columns
        {
            for(uint8_t a=0; a<STEP; a++) // loop for colors
            {
                ...
                if(q == -1)
                {
                    outs[(y+a)*(W>>3)+z] |= (calc_color(a, avg_blue, avg_green, avg_red)) >> 1;
                    if(z < (W>>3)-1) // if not last byte prepare next one
                        outs[(y+a)*(W>>3)+z+1] = (calc_color(a, avg_blue, avg_green, avg_red)) << 7;
                }
                else if(q == -2)
                {
                    outs[(y+a)*(W>>3)+z] |= (calc_color(a, avg_blue, avg_green, avg_red)) >> 2;
                    if(z < (W>>3)-1) // if not last byte prepare next one
                        outs[(y+a)*(W>>3)+z+1] = (calc_color(a, avg_blue, avg_green, avg_red)) << 6;
                }
                else
                    outs[((y+a)*(W>>3))+z] |= (calc_color(a, avg_blue, avg_green, avg_red)) << q;
