...
class DB:
	...
	def _reduce_container(self):
		...
		# 注意: latest_cont以外は現状書き込まれる可能性がないので、saveせずにメモリから消去できる
		if target_cont_name == self.index["latest_cont"]:
			self.save_container(target_cont_name)	# コンテナ保存 (指定されたコンテナを補助記憶装置 (HDD等) に保存する)
		# メモリ上から削除
