...
def add_files_to_clean(files: Set[str]) -> None:
    if hasattr(_app_ctx_stack.top, '_api_utils_files_to_clean'):
        for f in files:
            _app_ctx_stack.top._api_utils_files_to_clean.add(f)  # type: ignore
    else:
        _app_ctx_stack.top._api_utils_files_to_clean = files  # type: ignore


def clean_files() -> None:
    files: Set[str] = getattr(_app_ctx_stack.top, '_api_utils_files_to_clean', set())
    for f in files:
        try:
            os.unlink(f)
...
class ApiSdk:
    ...
    def _before_request(self) -> None:
        ...
        stat.collecting_enable(APPLICATION_DEBUG or APPLICATION_ENV_IS_LOCAL or self._debugger_enabled_with_pin())
