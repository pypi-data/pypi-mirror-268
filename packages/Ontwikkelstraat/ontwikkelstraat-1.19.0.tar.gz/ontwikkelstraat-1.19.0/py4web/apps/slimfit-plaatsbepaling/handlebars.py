...
def EDWH_URL_helper(this, *parts, **kwargs):
    if any(_.startswith("/") for _ in parts):
        warnings.warn(
            f'handlebars helper EDWH_URL there\'s a / prefixed in "{parts}", this will bother you. '
            "Remove prefixed slashes."
        )
    # te gebruiken in de front_end app
    if request.json and "FRONT_END_BASE" in request.json:
        return os.path.join(request.json["FRONT_END_BASE"], *parts)
    else:
        # als er geen post is, vallen we terug op site defaults, en die komt uit de omgevingsvariabelen.
        return os.path.join(
            "https://fragments." + os.getenv("HOSTINGDOMAIN"), "v1.0", *parts
        )
...
def FRONT_END_URL_helper(this, *parts):
    ...
    # # om te testen op de path-prefix methode (CORS testen etc.):
    #
    # pinned_version = "dev"  # e.g. 0.9
    #
    # url = "http://robin.edwh.nl:8000"  # fixme: customize
    #
    # return f"{url}/{pinned_version}/front_end/" + "/".join(parts)
