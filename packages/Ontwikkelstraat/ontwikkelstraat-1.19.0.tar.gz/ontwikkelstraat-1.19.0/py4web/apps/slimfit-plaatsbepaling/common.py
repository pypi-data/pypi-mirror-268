...
from py4web.core import Fixture, response
...
if settings.SESSION_TYPE == "cookies":
    ...
    session = Session(
        secret=settings.SESSION_SECRET_KEY, storage=conn, same_site="None"
    )
    ...
    session = Session(
        secret=settings.SESSION_SECRET_KEY, storage=DBStore(db), same_site="None"
    )
