...
import yatl
from edwh.core.backend import PLATFORM, NotFoundException, ValidationCode, edwh_asdict
...
from .common import db, flash, leidendb, session
from .fixtures import BaseTemplateSettings, TilesHelpers
from .handlebars import Handlebars, handlebars_helpers
from .helpers import PY4WEB_URL, DotMap, b64decode
from .hotspots import get_hotspots
from .models import _calculate_star_average, plaatsen, scholen
from .settings import APP_FOLDER
from .tasks import log_once

if typing.TYPE_CHECKING:
    from edwh.core.backend.engine import User
...
@action("index")
@action.uses(session, flash, backend, "component_loader.html")
def component_loader():
    token = backend.token
    return dict(backend_result=token, me=request.me)
...
def _item(id):
    ...
    orgs = item.organisations(db)

    def get_org_info(attribute_name: str, default: T) -> T:
        if len(orgs) == 1:
            # return the attribute of the first and only organisation
            return getattr(orgs[0], attribute_name, default) or default
        else:
            # don't get any attributes if no or multiple organisations are found
            return default

    def avatar_url(user: "User"):
        if user.avatar and user.avatar.uri:
            return user.avatar.uri
        else:
            return PY4WEB_URL(
                "static/images/users-solid.svg"
                if item.author.name.lower().startswith("team ")
                else "static/images/user-solid.svg"
            )
...
@action.uses(session)
def cookie_check():
    response.set_cookie("may-set-cookies", "yes")
    return redirect(URL("auth", vars={"ewform": "login"}))
...
def auth_form():
    ...
    if path == "cookie_check":
        return redirect(URL("cookie_check"))

    if request.get_cookie("may-set-cookies") != "yes":
        path = "no_cookies"
...
@action("simpleanalytics")
@action.uses("simple_analytics.html", backend)
def simple_analytics():
    ...
    # else:
    return {"url": "https://inzichtin-delen.meteddie.nl"}
