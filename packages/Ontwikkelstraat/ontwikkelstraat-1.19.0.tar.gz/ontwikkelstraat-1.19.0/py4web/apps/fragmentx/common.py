...
db = DAL(
    ...
    driver_args=dict(
        application_name="py4web:" + settings.APP_NAME,
    ),
)
...
if settings.SESSION_TYPE == "cookies":
    ...
    session = Session(
        secret=settings.SESSION_SECRET_KEY, same_site="None", storage=conn
    )
...
if settings.USE_CELERY:
    ...
    fragmentx_scheduler = Celery(
        "apps.fragmentx.tasks", broker=os.getenv("FRAGMENTX_CELERY_BROKER")
    )
    applog_scheduler = Celery(
        "edwh.core.applog.tasks", broker=os.getenv("APPLOG_CELERY_BROKER")
    )
...
# #######################################################
# Include applog library: setup the sink and make it commit after the request
# #######################################################
...
def setup_applog():
    ...
    app_id = os.getenvb(b"PY4WEB_APPLOG_ID")
    app_key = binascii.unhexlify(os.getenvb(b"PY4WEB_APPLOG_KEY"))
    ...
    bottle.default_app().add_hook("after_request", sink.commit)
    return sink


sink = setup_applog()

# The SignalEmitter is setup in backend.BackendSession.on_request
import edwh.core.data_model

edwh.core.data_model.setup_db_tables(db)

import edwh.core.pgcache.define_models

edwh.core.pgcache.define_models.define_model(db)

# todo: define_tables/define_models uitvoeren in models

# !!NOTE: should be fixed by using the proper Fixture(s)
...
#
#
# bottle.default_app().add_hook("after_request", commit_or_rollback)

from edwh.core.pgcache import (
    CachedResult,
    DillableAttrsClass,
    cache_hook_table_with_gid,
    cached,
)
