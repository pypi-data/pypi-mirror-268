...
class SelectiveKernelConv_SKA(nn.Module):

    def __init__(self,planes, in_channels, out_channels, kernel_size=None, stride=1, dilation=1, groups=1,
                 attn_reduction=16, min_attn_channels=32, keep_3x3=False, split_input=False,
                 drop_block=None, act_layer=nn.ReLU, norm_layer=nn.BatchNorm2d, aa_layer=None):
        """ Selective Kernel Convolution Module
        ...
        Args:
            ...
            norm_layer (nn.Module): batchnorm/norm layer to use
        """
        super(SelectiveKernelConv_SKA, self).__init__()
        kernel_size = kernel_size or [3, 5]  # default to one 3x3 and one 5x5 branch. 5x5 -> 3x3 + dilation
        _kernel_valid(kernel_size)
        if not isinstance(kernel_size, list):
            kernel_size = [kernel_size] * 2
        ...
        # attn_channels = max(int(out_channels / attn_reduction), min_attn_channels)
        # self.attn = SelectiveKernelAttn(out_channels, self.num_paths, attn_channels)
        self.attention = ASKCFuse(channels=out_channels, r=16)
        self.drop_block = drop_block
    ...
    def forward(self, x):
        if self.split_input:
            ...
            x_paths = [op(x) for op in self.paths]
        # x = torch.stack(x_paths, dim=1)
        x_attn = self.attention(x_paths[0],x_paths[1])
        # x = x * x_attn
        # x = torch.sum(x_attn, dim=0)
        return x_attn