...
def prepare_input_for_sampling(input_csv, db_path, spd_path=None, classifier_model_path=None,
    ...
    # Get the columns and inspect

    if isinstance(input_csv, str):
        data = pd.read_csv(input_csv, dtype='object', nrows=1)
        cols = data.columns
    elif isinstance(input_csv, pd.DataFrame):
        cols = input_csv.columns
    else:
        raise TypeError('input_csv should be path(str) or pd.DataFrame')

    assert all(col in cols for col in ['input_id', 'input_address']), \
        'Columns `input_id` and `input_address` should be present.'

    usecols = ['input_id', 'input_address']

    if all(col in cols for col in ['flap_match_score', 'flap_uprn', 'uprn_row']):
        skip_match = True
        usecols += ['flap_match_score', 'flap_uprn', 'uprn_row']
    else:
        skip_match = False
    ...
    if isinstance(input_csv, str):
        data = pd.read_csv(input_csv, dtype='object', usecols=usecols).sample(
            random_sample_size, random_state=random_state)

    elif isinstance(input_csv, pd.DataFrame):
        data = input_csv[usecols].sample(
    ...
    if not skip_match:
        ...
        if use_scoring:
            ...
            match_results = match(data, db_path, classifier_model_path=classifier_model_path,
                                  match_batch_size=int(random_sample_size/20))

        data = pd.merge(left=data, right=match_results[['input_id', 'flap_match_score', 'flap_uprn', 'uprn_row']],
                        on='input_id', how='left')

    data.flap_match_score.fillna('0', inplace=True)
    ...
    for col in stratify_config:
        try:
            for i, group in data.groupby(col, dropna=False):
                ...
                sampled_group = group[['input_id', 'input_address', 'uprn_row',
                                       'flap_match_score', 'flap_uprn']].sample(n)
                sampled_group['strata'] = col
                sampled_group['strata_value'] = i
                sampled_group['sample_weight_inverse'] = (len(data) * n) / len(group)
