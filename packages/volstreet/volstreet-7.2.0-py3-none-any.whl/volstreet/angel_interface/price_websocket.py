...
class PriceWebsocket(BaseWebsocket):
    ...
    def _prepare_subscription_dict(
        ...
        expiry_sub_modes: dict[str, int] = None,
    ) -> dict[int, list[str]]:
        subscription_dict = defaultdict(list)
        if expiry_sub_modes is None:
            expiry_sub_modes = {getattr(underlying, "current_expiry"): 3}
        else:
            expiry_sub_modes = {
                getattr(underlying, expiry): mode
    ...
    def subscribe_options(
        ...
        for underlying in underlyings:
            ...
            subscription_dict = self._prepare_subscription_dict(
                underlying, strike_range=strike_range, expiry_sub_modes=expiry_sub_modes
            )
            ...
            try:
                ...
                if command == "subscribe_indices":
                    ...
                    if command == "subscribe_options":
                        ...
                        expiry_sub_modes = args.get("expiry_sub_modes")
                        self.subscribe_options(
                            *underlyings,
                            range_of_strikes=range_of_strikes,
                            expiry_sub_modes=expiry_sub_modes,
                        )
