...
from volstreet.decorators import SingletonInstances
from volstreet.config import symbol_df, logger
...
class Action(Enum):
    ...
    def __init__(self, value):
        self.num_value = 1 if self.value == "BUY" else -1
...
class _Option:
    ...
    def fetch_greeks(
        ...
        return bs.greeks(
            ...
            theta_time_jump=theta_time_jump,
        )
...
class Strangle:
    ...
    def fetch_ltp(self, for_type: str | None = None):
        if for_type is not None:
            data_field = "best_ask" if for_type.upper() == "BUY" else "best_bid"
        else:
            data_field = "ltp"
        return fetch_ltp(
            self.exchange, self.call_symbol, self.call_token, field=data_field
        ), fetch_ltp(self.exchange, self.put_symbol, self.put_token, field=data_field)
    ...
    def fetch_total_ltp(self, for_type: str | None = None):
        call_ltp, put_ltp = self.fetch_ltp(for_type)
    ...
    def generate_order_params(
        ...
        return [
            ...
            generate_order_params(
                ...
                self.put_token,
                qty * self.lot_size,
                (~action).value,  # Inverse action for put
                put_price,
                order_tag=order_tag,
            )
            for qty in spliced_orders
...
def _set_underlying_attributes(instance, underlying: str):
    ...
    if not symbol_df.empty:
        ...
        logger.error(
            f"Symbol dataframe is empty. Using default freeze limit for {instance}"
        )
