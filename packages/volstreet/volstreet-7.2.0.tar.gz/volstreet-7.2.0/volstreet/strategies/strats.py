...
def intraday_trend(
    ...
    def get_futures_basis(traded_price):
        try:
            ...
            logger.error(f"Error in getting futures prices: {ex}")
            notifier(f"Error in getting futures prices: {ex}", notification_url, "INFO")
    ...
    while entries < max_entries and current_time().time() < exit_time_object:
        ...
        instructions[atm_synthetic_fut] = {
            "action": action,
            "quantity_in_lots": quantity_in_lots,
            "order_tag": strategy_tag,
        }
        ...
        if execute_hedge:
            hedge_strike_multiplier = (
                1 + hedge_offset if action == Action.BUY else 1 - hedge_offset
            )
            hedge_strike = find_strike(price * hedge_strike_multiplier, underlying.base)
            hedge_option_type: OptionType = (
                OptionType.CALL if action == Action.BUY else OptionType.PUT
            )
        ...
        basis_changes = [
            basis_change if action == Action.BUY else -basis_change
