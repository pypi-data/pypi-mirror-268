...
class FoffsetCls:
	...
	def set(self, freq_offset: float, freq_offset_enable: List[bool]) -> None:
		...
		Snippet: driver.configure.bluetooth.measurement.multiEval.limit.lowEnergy.le1M.foffset.set(freq_offset = 1.0, freq_offset_enable = [True, False, True]) \n
		...
		self._core.io.write(f'CONFigure:BLUetooth:MEASurement<Instance>:MEValuation:LIMit:LENergy:LE1M:FOFFset {param}'.rstrip())
	...
	def get(self) -> FoffsetStruct:
		"""SCPI: CONFigure:BLUetooth:MEASurement<Instance>:MEValuation:LIMit:LENergy[:LE1M]:FOFFset \n
		Snippet: value: FoffsetStruct = driver.configure.bluetooth.measurement.multiEval.limit.lowEnergy.le1M.foffset.get() \n
		Sets/gets the frequency offset limit. Commands for uncoded LE 1M PHY (..:LE1M..) , LE 2M PHY (..:LE2M..) , and LE coded
		PHY (..:LRANge..) are available. \n
			:return: structure: for return value, see the help for FoffsetStruct structure arguments."""
		return self._core.io.query_struct(f'CONFigure:BLUetooth:MEASurement<Instance>:MEValuation:LIMit:LENergy:LE1M:FOFFset?', self.__class__.FoffsetStruct())
