...
class ConnectorCls:
	...
	def set(self, cmws_connector: enums.CmwSingleConnector, segment=repcap.Segment.Default) -> None:
		...
		Snippet: driver.configure.bluetooth.measurement.multiEval.listPy.segment.setup.singleCmw.connector.set(cmws_connector = enums.CmwSingleConnector.R11, segment = repcap.Segment.Default) \n
	...
	def get(self, segment=repcap.Segment.Default) -> enums.CmwSingleConnector:
		"""SCPI: CONFigure:BLUetooth:MEASurement<Instance>:MEValuation:LIST:SEGMent<nr>[:SETup]:CMWS:CONNector \n
		Snippet: value: enums.CmwSingleConnector = driver.configure.bluetooth.measurement.multiEval.listPy.segment.setup.singleCmw.connector.get(segment = repcap.Segment.Default) \n
		No command help available \n
			:param segment: optional repeated capability selector. Default value: S1 (settable in the interface 'Segment')
