...
class P4HpCls:
	...
	def __init__(self, core: Core, parent):
		self._core = core
		self._cmd_group = CommandsGroup("p4Hp", core, parent)
	...
	def get_sacp(self) -> SacpStruct:
		...
		Snippet: value: SacpStruct = driver.configure.bluetooth.measurement.hdrp.limit.p4Hp.get_sacp() \n
		No command help available \n
			:return: structure: for return value, see the help for SacpStruct structure arguments.
		"""
		return self._core.io.query_struct('CONFigure:BLUetooth:MEASurement<Instance>:HDRP:LIMit:P4HP:SACP?', self.__class__.SacpStruct())
	...
	def set_sacp(self, value: SacpStruct) -> None:
		...
		driver.configure.bluetooth.measurement.hdrp.limit.p4Hp.set_sacp(value = structure) \n
		No command help available \n
			:param value: see the help for SacpStruct structure arguments.
		"""
		self._core.io.write_struct('CONFigure:BLUetooth:MEASurement<Instance>:HDRP:LIMit:P4HP:SACP', value)

	def clone(self) -> 'P4HpCls':
		"""Clones the group by creating new object from it and its whole existing subgroups
		Also copies all the existing default Repeated Capabilities setting,
		which you can change independently without affecting the original group"""
		new_group = P4HpCls(self._core, self._cmd_group.parent)
