...
class DminimumCls:
	...
	def set(self, freq_dev_f_1_lower: float, freq_dev_f_1_upper: float, freq_dev_f_1_enable: List[bool]) -> None:
		...
		Snippet: driver.configure.bluetooth.measurement.multiEval.limit.lowEnergy.dminimum.set(freq_dev_f_1_lower = 1.0, freq_dev_f_1_upper = 1.0, freq_dev_f_1_enable = [True, False, True]) \n
		...
		self._core.io.write(f'CONFigure:BLUetooth:MEASurement<Instance>:MEValuation:LIMit:LENergy:DMINimum {param}'.rstrip())
	...
	def get(self) -> DminimumStruct:
		"""SCPI: CONFigure:BLUetooth:MEASurement<Instance>:MEValuation:LIMit:LENergy:DMINimum \n
		Snippet: value: DminimumStruct = driver.configure.bluetooth.measurement.multiEval.limit.lowEnergy.dminimum.get() \n
		Defines the lower and upper Δf1 frequency deviation limits for LE 1M PHY. The mnemonics DAVerage, DMINimum, DMAXimum
		distinguish average, minimum and maximum frequency deviations. \n
			:return: structure: for return value, see the help for DminimumStruct structure arguments."""
		return self._core.io.query_struct(f'CONFigure:BLUetooth:MEASurement<Instance>:MEValuation:LIMit:LENergy:DMINimum?', self.__class__.DminimumStruct())
