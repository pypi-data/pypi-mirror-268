...
class CurrentCls:
	...
	def fetch(self) -> List[float]:
		...
		Snippet: value: List[float] = driver.bluetooth.measurement.multiEval.trace.spower.current.fetch() \n
	...
	def read(self) -> List[float]:
		...
		Snippet: value: List[float] = driver.bluetooth.measurement.multiEval.trace.spower.current.read() \n
		Returns the results of the power per CTE slot for LE CTE traces. The results of the current, average, minimum and maximum
		traces can be retrieved. \n
		Suppressed linked return values: reliability \n
			:return: power_vs_slot: No help available"""
