...
class CurrentCls:
	...
	def read(self) -> List[float]:
		...
		Snippet: value: List[float] = driver.bluetooth.measurement.hdr.trace.devMagnitude.current.read() \n
	...
	def fetch(self) -> List[float]:
		...
		Snippet: value: List[float] = driver.bluetooth.measurement.hdr.trace.devMagnitude.current.fetch() \n
		No command help available \n
		Suppressed linked return values: reliability \n
			:return: devm: No help available"""
		suppressed = ArgSingleSuppressed(0, DataType.Integer, False, 1, 'Reliability')
