...
class CurrentCls:
	...
	def calculate(self) -> CalculateStruct:
		...
		Snippet: value: CalculateStruct = driver.bluetooth.measurement.multiEval.powerVsTime.lowEnergy.le1M.current.calculate() \n
	...
	def read(self) -> ReadStruct:
		...
		Snippet: value: ReadStruct = driver.bluetooth.measurement.multiEval.powerVsTime.lowEnergy.le1M.current.read() \n
		Returns the power results for LE 1M PHY (uncoded) . The values described below are returned by FETCh and READ commands.
		CALCulate commands return limit check results instead, one value for each result listed below. \n
	...
	def fetch(self) -> FetchStruct:
		...
		Snippet: value: FetchStruct = driver.bluetooth.measurement.multiEval.powerVsTime.lowEnergy.le1M.current.fetch() \n
		Returns the power results for LE 1M PHY (uncoded) . The values described below are returned by FETCh and READ commands.
		CALCulate commands return limit check results instead, one value for each result listed below. \n
