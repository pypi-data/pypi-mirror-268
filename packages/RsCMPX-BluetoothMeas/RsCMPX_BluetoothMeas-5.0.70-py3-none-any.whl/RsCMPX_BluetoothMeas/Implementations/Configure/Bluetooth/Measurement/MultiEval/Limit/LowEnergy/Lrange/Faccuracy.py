...
class FaccuracyCls:
	...
	def set(self, freq_accuracy: float, freq_acc_enabled: List[bool]) -> None:
		...
		Snippet: driver.configure.bluetooth.measurement.multiEval.limit.lowEnergy.lrange.faccuracy.set(freq_accuracy = 1.0, freq_acc_enabled = [True, False, True]) \n
		...
		self._core.io.write(f'CONFigure:BLUetooth:MEASurement<Instance>:MEValuation:LIMit:LENergy:LRANge:FACCuracy {param}'.rstrip())
	...
	def get(self) -> FaccuracyStruct:
		"""SCPI: CONFigure:BLUetooth:MEASurement<Instance>:MEValuation:LIMit:LENergy:LRANge:FACCuracy \n
		Snippet: value: FaccuracyStruct = driver.configure.bluetooth.measurement.multiEval.limit.lowEnergy.lrange.faccuracy.get() \n
		Defines the limit for the frequency accuracy. Commands for uncoded LE 1M PHY (..:LE1M..) , LE 2M PHY (..:LE2M..) , and LE
		coded PHY (..:LRANge..) are available. \n
			:return: structure: for return value, see the help for FaccuracyStruct structure arguments."""
		return self._core.io.query_struct(f'CONFigure:BLUetooth:MEASurement<Instance>:MEValuation:LIMit:LENergy:LRANge:FACCuracy?', self.__class__.FaccuracyStruct())
