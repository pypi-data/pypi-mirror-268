...
class AllCls:
	...
	def fetch(self, timeout: float = None, target_main_state: enums.ResourceState = None, target_sync_state: enums.SyncState = None) -> enums.ResourceState:
		...
		Snippet: value: enums.ResourceState = driver.bluetooth.measurement.bhRate.state.all.fetch(timeout = 1.0, target_main_state = enums.ResourceState.ACTive, target_sync_state = enums.SyncState.ADJusted) \n
		No command help available \n
			:param timeout: No help available
			:param target_main_state: No help available
			:param target_sync_state: No help available
			:return: meas_states: No help available"""
		...
		response = self._core.io.query_str(f'FETCh:BLUetooth:MEASurement<Instance>:BHRate:STATe:ALL? {param}'.rstrip())
