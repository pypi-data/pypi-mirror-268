...
class CurrentCls:
	...
	def calculate(self) -> CalculateStruct:
		...
		Snippet: value: CalculateStruct = driver.bluetooth.measurement.multiEval.powerVsTime.lowEnergy.le2M.current.calculate() \n
	...
	def read(self) -> ReadStruct:
		...
		Snippet: value: ReadStruct = driver.bluetooth.measurement.multiEval.powerVsTime.lowEnergy.le2M.current.read() \n
		Returns the power results for LE 2M PHY (...:LE2M...) and LE coded PHY (...:LRANge...) . The values described below are
		returned by FETCh and READ commands. CALCulate commands return limit check results instead, one value for each result
		listed below. \n
	...
	def fetch(self) -> FetchStruct:
		...
		Snippet: value: FetchStruct = driver.bluetooth.measurement.multiEval.powerVsTime.lowEnergy.le2M.current.fetch() \n
		Returns the power results for LE 2M PHY (...:LE2M...) and LE coded PHY (...:LRANge...) . The values described below are
		returned by FETCh and READ commands. CALCulate commands return limit check results instead, one value for each result
		listed below. \n
