...
class XmaximumCls:
	...
	def fetch(self) -> ResultData:
		...
		Snippet: value: ResultData = driver.bluetooth.measurement.multiEval.modulation.cte.lowEnergy.le1M.xmaximum.fetch() \n
	...
	def calculate(self) -> CalculateStruct:
		...
		Snippet: value: CalculateStruct = driver.bluetooth.measurement.multiEval.modulation.cte.lowEnergy.le1M.xmaximum.calculate() \n
		Returns current, average, standard deviation, absolute min (xmin) , absolute max (xmax) , and max CTE modulation results
		for LE uncoded PHY (LE 1M PHY, LE 2M PHY) . The values described below are returned by FETCh and READ commands. CALCulate
		commands return limit check results instead, one value for each result listed below. \n
	...
	def read(self) -> ResultData:
		...
		Snippet: value: ResultData = driver.bluetooth.measurement.multiEval.modulation.cte.lowEnergy.le1M.xmaximum.read() \n
		Returns current, average, standard deviation, absolute min (xmin) , absolute max (xmax) , and max CTE modulation results
		for LE uncoded PHY (LE 1M PHY, LE 2M PHY) . The values described below are returned by FETCh and READ commands. CALCulate
		commands return limit check results instead, one value for each result listed below. \n
			:return: structure: for return value, see the help for ResultData structure arguments."""
