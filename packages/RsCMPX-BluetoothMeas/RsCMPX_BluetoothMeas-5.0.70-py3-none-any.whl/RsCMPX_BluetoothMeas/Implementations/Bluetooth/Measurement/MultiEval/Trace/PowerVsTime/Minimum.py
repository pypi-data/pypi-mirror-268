...
class MinimumCls:
	...
	def fetch(self) -> List[float]:
		...
		Snippet: value: List[float] = driver.bluetooth.measurement.multiEval.trace.powerVsTime.minimum.fetch() \n
	...
	def read(self) -> List[float]:
		...
		Snippet: value: List[float] = driver.bluetooth.measurement.multiEval.trace.powerVsTime.minimum.read() \n
		Returns the values of the power vs time traces. The results of the current, average minimum and maximum traces can be
		retrieved. \n
		Suppressed linked return values: reliability \n
			:return: power_vs_time: N power results, depending on the packet type and payload length; see 'PvT and modulation trace points (LE) '."""
