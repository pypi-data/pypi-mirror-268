...
class RsCMPX_BluetoothMeas:
	"""1320 total commands, 8 Subgroups, 0 group commands"""
	_driver_options = "SupportedInstrModels = CMX/CMP/CMW/PVT, SupportedIdnPatterns = CMX/CMP/CMW/PVT, SimulationIdnString = 'Rohde&Schwarz,CMP180,100001,5.0.70.0003'"
	...
	def __init__(self, resource_name: str, id_query: bool = True, reset: bool = False, options: str = None, direct_session: object = None):
		...
		self._core = Core(resource_name, id_query, reset, RsCMPX_BluetoothMeas._driver_options, options, direct_session)
		self._core.driver_version = '5.0.70.0003'
	...
	@classmethod
	def from_existing_session(cls, session: object, options: str = None) -> 'RsCMPX_BluetoothMeas':
	...
	def __str__(self) -> str:
		if self._core.io:
			return f"RsCMPX_BluetoothMeas session '{self._core.io.resource_name}'"
		else:
			return f"RsCMPX_BluetoothMeas with session closed"
	...
	def assert_minimum_version(min_version: str) -> None:
		...
		curr_version_list = '5.0.70.0003'.split('.')
		...
		for i in range(count):
			...
			if curr > minimum:
				break
			if curr < minimum:
				raise RsInstrException(f"Assertion for minimum RsCMPX_BluetoothMeas version failed. Current version: '5.0.70.0003', minimum required version: '{min_version}'")
	...
	def _sync_to_custom_properties(self, cloned: 'RsCMPX_BluetoothMeas') -> None:
	...
	def bluetooth(self):
		...
		if not hasattr(self, '_bluetooth'):
			from .Implementations.Bluetooth import BluetoothCls
	...
	def clone(self) -> 'RsCMPX_BluetoothMeas':
		"""Creates a deep copy of the RsCMPX_BluetoothMeas object. Also copies:
		...
		cloned = RsCMPX_BluetoothMeas.from_existing_session(self.get_session_handle(), self._options)
