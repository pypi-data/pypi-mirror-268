...
class LSVMArena(LocalArena):
    ...
    def save_player_to_json(self, name, player):
        ...
        data[name] = {
            ...
            "event_history": [{"mu": ev.mu, "rating_sig": ev.rating_sig, "perf_score": ev.perf_score, "place": ev.place} for ev in player.event_history],
    ...
    def load_player_from_json(self,name):
        ...
        if name in data:
            ...
            player.event_history = [PlayerEvent(ev['mu'], ev['rating_sig'], ev['perf_score'], ev['place']) for ev in player_dict["event_history"]]
    ...
    def _find_matches(self, player_name):
        ...
        for name in self.game_reports: 
            if name != player_name and not self.game_reports[name]['disconnected']: 
    ...
    # TODO: Draft up an announcement for friendly competition being up for spectrum
    # TODO: Connect the code with google drive to uplink the files to a shared drive with the students. 
    # TODO: Add a pygame visualizer for the actual games themselves
