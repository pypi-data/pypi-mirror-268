...
NeedToRun = Callable[[List[Path], List[Path], List[Path]], bool]
...
def need_to_run(
    ...
    lg.debug(f'Prods: {prods}, exists: {[p.exists for p in prods]}')
...
def tsk(
        *cmds: Cmd | str,
        ...
        prods: Paths = [],
        updts: Paths = [],
        need_to_run: NeedToRun = need_to_run) -> Job:
