...
from spinqit.qiskit.extensions import UnitaryGate
...
def control(
    ...
    import spinqit.qiskit.circuit.controlledgate as controlledgate
    ...
    if operation.name == "x" or (
        ...
        for gate, qargs, _ in definition.data:
            if gate.name == "x":
                ...
                from spinqit.qiskit.circuit.library import MCPhaseGate
...
def _gate_to_dag(operation):
    from spinqit.qiskit.converters.circuit_to_dag import circuit_to_dag
...
def _unroll_gate(operation, basis_gates):
    from spinqit.qiskit.converters.dag_to_circuit import dag_to_circuit
    from spinqit.qiskit.transpiler.passes import Unroller
