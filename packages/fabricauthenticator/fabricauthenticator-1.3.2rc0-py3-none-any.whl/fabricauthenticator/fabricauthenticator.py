...
class FabricAuthenticator(oauthenticator.CILogonOAuthenticator):
    ...
    def check_username_claim(self, claimlist, resp_json):
        ...
        if not username:
            if len(claimlist) < 2:
                self.log.error(
                    f"Sub: {sub} Username claim: '{self.username_claim}' not found in response: {resp_json}"
                )
            else:
                self.log.error(
                    f"Sub: {sub} No username claim: '{self.username_claim}' found in response: {resp_json}"
                )
