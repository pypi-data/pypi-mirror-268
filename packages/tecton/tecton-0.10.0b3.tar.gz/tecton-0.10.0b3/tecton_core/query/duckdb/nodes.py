...
class AsofJoinFullAggDuckDBNodeV2(nodes.AsofJoinFullAggNode):
    ...
    def _find_all_related_spine_rows(
        ...
        if self.fdw.has_lifetime_aggregate or self.fdw.is_continuous:
            join_condition = _join_condition(features, spine, self.fdw.join_keys)
            join_condition &= features.field(anchor_time()) <= spine.field(anchor_time())
            if self.fdw.is_continuous:
                earliest_window_start = convert_timedelta_for_version(
                    self.fdw.earliest_window_start, self.fdw.get_feature_store_format_version
                )
                join_condition &= features.field(anchor_time()) > (spine.field(anchor_time()) + earliest_window_start)
            return (
                self.func.query()
                .from_(features)
                .join(spine)
                .on(join_condition)
                .select(*[*select_cols, spine.field(anchor_time()).as_(spine_anchor_column)])
            )

        join_anchor_column = f"_join_{anchor_time()}"
        augmented_spine = self._augment_spine_with_join_anchor_column(spine, join_anchor_column)
