...
class MaterializedFeatureView(FeatureView):
    ...
    def _use_feature_param(self) -> bool:
        ...
        return bool(self._args.materialized_feature_view_args.attributes)
        ...
        *,
        ...
        if stream_processing_mode is None:
            stream_processing_mode_ = _compute_default_stream_processing_mode(
                has_push_source, batch_compaction_enabled, stream_compaction_enabled, aggregations
            )
        else:
...
def _build_materialized_feature_view_args(
    ...
    if features:
        if all(isinstance(f, feature.Aggregate) for f in features):
            ...
            aggregation_protos = [
                agg._to_proto(aggregation_interval, is_continuous, batch_compaction_enabled) for agg in features
            ]
        else:
...
def _compute_default_stream_processing_mode(
    ...
    elif batch_compaction_enabled and not stream_compaction_enabled:
