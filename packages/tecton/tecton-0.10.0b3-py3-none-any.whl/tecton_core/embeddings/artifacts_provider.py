...
class TokenBudget:
    ...
    def __init__(self, hardware_token_budget_map: Optional[Dict[InferenceDeviceHardware, int]] = None):  # noqa: ANN204
        if not hardware_token_budget_map:
            self.hardware_token_budget_map = {}
        for k, v in hardware_token_budget_map.items():
            ...
            if not isinstance(v, int) or v <= 0:
                ...
                raise ValueError(msg)
        self.hardware_token_budget_map = MappingProxyType(copy.deepcopy(hardware_token_budget_map))

    def get(self, hardware: InferenceDeviceHardware) -> Optional[int]:
        token_budget = self.hardware_token_budget_map.get(hardware)
        if token_budget is None:
            token_budget = (
                self._DEFAULT_CPU_TOKEN_BUDGET
                if hardware == InferenceDeviceHardware.CPU
                else self._DEFAULT_GPU_TOKEN_BUDGET
            )
            logger.info(f"Token budget for {hardware} not found. Using default of: {token_budget}")
        return token_budget
...
class ArtifactsProvider:
    def __init__(self, artifact_s3_map: Optional[Dict[str, Tuple[str, TokenBudget]]] = None):  # noqa: ANN204
        artifact_s3_map = CURRENT_ARTIFACTS if artifact_s3_map is None else artifact_s3_map
        self.artifacts = {
            k: EmbeddingModelInfo(k, s3_path, token_budget=token_budget)
            for k, (s3_path, token_budget) in artifact_s3_map.items()
        }
