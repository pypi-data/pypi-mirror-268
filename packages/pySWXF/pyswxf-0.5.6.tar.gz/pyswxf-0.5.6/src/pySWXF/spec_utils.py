...
def get_reflectivity_CLS(fname, datadir, scan):
    ...
    if os.path.isfile(specfile):
        ...
        print(f'File {specfile} does not exist')
    return(x, y,dy)
...
def get_fluor_data(spec_filename,  mca_filename, scan_number,):
  data,scan_info  = readscan(spec_filename,scan_number)
  norm = data['IC2']
  angles = data['MU']
  exposure_time = np.mean(data['Seconds'])
  npt = np.size(angles)
  mca_full_filename = mca_filename
  mca,ndset = get_mca_data_CLS_Nov(mca_filename,scan_number,exposure_time,quiet=True)
  mca_sum = np.sum(mca,0) # sum over each point in the scan
  return mca,mca_sum,ndset,angles,npt,exposure_time,norm

def fit_fluor_data(mca, ndset, npt, exposure_time, 
                   norm, peak_model, pars, E, data_range):
