...
def sample_sim_new(d_overlay, sig_overlay, G,D,sig_W,sig_SiC,
    ...
    if new_cell: 
        ...
        layers += [(H2O.formula,H2O.density,1000,30)]
        if bilayer:  
            layers += make_bilayer_model()
...
def multilayer_ref_new(theta,I0,thoff,bg,foot,G,D,d_overlay, sig_overlay,
    ...
    layers = sample_sim_new(d_overlay, sig_overlay, G,D,sig_W,sig_SiC,
                   D_Cr, sig_Cr, sig_Si_Bot,water=water,bilayer=bilayer,new_cell=new_cell)
    ...
    # Include resolution
    if (res>0):
        npt = np.size(Intensity)
        signal = np.concatenate((Intensity[::-1],Intensity,Intensity[::-1]))
        step = dth
        sig = res/step
        nwin = 8*int(sig)+16
        x2 = np.arange(-nwin/2,nwin/2+1)
        win = np.exp(-x2**2/2/sig**2)
        win /= np.sum(win)
