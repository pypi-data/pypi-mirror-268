...
'''
	import euphoria.clouds.food_USDA.interpret.packageWeight.interpret as package_weight_interpreter
...
import euphoria.measures.mass.swap as mass_swap
import euphoria.measures.volume.swap as volume_swap

import euphoria.clouds.food_USDA.interpret.packageWeight.label_splitter as label_splitter
...
def start (
	...
	if (records >= 1):
		print ()
		print ("given quantities:", given_quantities)
	...
	for given_quantity in given_quantities:
		[ amount, unit ] = label_splitter.start (given_quantity)
		
		amount = str (Fraction (amount))
		
		if (records >= 1):
			print (f"	given quantity: '{ amount }' '{ unit }'")
