...
class Prisma(ContainerScan):
    ...
    def create_issue(self, dat: dict = None) -> Issue:
        """
        ...
        if self.attributes.app.config["issues"][self.name.lower()]["useKev"]:
            kev_due_date = self.lookup_kev(dat["CVE ID"])
        iss = Issue(
            ...
            recommendedActions=(dat["Fix Status"] if dat["Fix Status"] else "Upgrade affected package"),
            cve=dat["CVE ID"],
            autoApproved="No",
    ...
    def create_asset(self, dat: Optional[dict] = None) -> Asset:
        ...
        return Asset(
            **{
                ...
                "operatingSystem": Asset.find_os(dat["Distro"]),
    ...
    def create_vuln(self, dat: Optional[dict] = None) -> Optional[Vulnerability]:
        ...
        if asset_match:
            regscale_vuln = Vulnerability(
                ...
                dns=dat["Hostname"],
                mitigated=None,
                operatingSystem=(Asset.find_os(dat["Distro"]) if Asset.find_os(dat["Distro"]) else None),
                severity=severity,
                ...
                extra_data={"solution": dat["Fix Status"], "proof": dat["Fix Status"]},
            )
