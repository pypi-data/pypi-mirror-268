...
def load_controls(controls_to_be_added: dict, component_id: int, base_catalog: str) -> None:
    """
    ...
    try:
        imp_r = api.get(
            url=f'{config["domain"]}/api/controlImplementation/getAllByParent/{component_id}\
                /components'
        )
    ...
    if len(cat) > 0:
        cat_id = cat[0]["id"]
        try:
            controls = api.get(f'{config["domain"]}/api/SecurityControls/getList/{cat_id}').json()
        ...
        for imp_control in controls_to_be_added:
            ...
            try:
                ...
                if len(reg_control) > 0:
                    ...
                    dat = control_implementation.dict()
                    control = SecurityControl.lookup_control(app=app, control_id=dat["controlID"])
                    if control.controlId not in [
                        imp["controlName"]
                        for imp in all_implementations
                        if imp["parentId"] == control_implementation.parentId
                    ]:
                        ...
                        dat = [
                            imp
                            for imp in all_implementations
                            if imp["controlName"] == control.controlId and imp["parentId"] == dat["parentId"]
                        ][0]
    ...
    logger.debug("Controls to insert: %i", len(control_implementations_to_insert))
    logger.debug("Controls to update: %i", len(control_implementations_to_update))
    logger.debug("control_implementations_to_insert\n%s", control_implementations_to_insert)
...
def upload_catalog(
    ...
    if "fedramp" in catalog_arr["metadata"]["version"].lower():
        ...
        date_published = reformat_str_date(catalog_arr["metadata"]["published"], dt_format="%Y-%m-%d %H:%M:%S.%f %z")
        last_modified = reformat_str_date(
            catalog_arr["metadata"]["last-modified"],
            dt_format="%Y-%m-%d %H:%M:%S.%f %z",
        )
    else:
        date_published = catalog_arr["metadata"]["version"]
        last_modified = catalog_arr["metadata"]["version"]
    ...
    if upload_flag is True:
        ...
        try:
            ...
            logger.info("Created Catalogue ID: %s in RegScale", json_response["id"])
            # get the catalog ID
            catalogue_id = json_response["id"]
        except (requests.exceptions.RequestException, KeyError) as ex:
            error_and_exit(
                f"Unable to create catalogue in RegScale, try logging in again to pull a fresh token.\n{ex}",
            )
    else:
...
def extract_text(html_string: str, elem: str) -> str:
    """
    Function to return an object with an representation of a string formatted and cleaned

    :param str html_string: A string of HTML
    :param str elem: An HTML tag type (ex. li)
    :return: An object with an representation of a string formatted and cleaned
    :rtype: str
    """
    result = {
        "key": "",
...
def post_alternative_controls(args: Tuple, thread: int) -> None:
    ...
    :rtype: None
    """
    new_objectives, parts, task, api = args

    # find which records should be executed by the current thread
    threads = thread_assignment(thread=thread, total_items=len(new_objectives))

    # iterate through the thread assignment items and process them
    for i in range(len(threads)):
        obj = new_objectives[threads[i]]
        # Please don't automate my job ChatGPT
        title = reformat_title(obj["name"])
        label = [part["part_label"] for part in parts if part["name"] == obj["name"]][0]
        # Create a Child Control for this special case
...
def process_fedramp_objectives(part: Union[list, dict], str_obj: str, objectives: list, ctrl) -> str:
    """
    Function to handle processing of FedRAMP objectives

    :param Union[list, dict] part: Part from OSCAL
    ...
    if "name" in part:
        ...
        if part["name"] == "statement":
            # if it is a statement, it is generally a prefix such as "the organization" or "determine if"
            str_prefix = part["prose"]
            str_obj += str_prefix + " "

    # check the name
    if str_name == "item" or str_name == "objective":
        # append the prose
        str_obj += str_prose

    # flag the type
    strType = "objective"
    if str_name == "objective":
        strType = "assessment"
    ...
    if "parts" in part:
        for p in part["parts"]:
            strObj2 = str_obj
            if p["name"] == "item" or p["name"] == "objective":
                if "props" in p:
                    if str_name != "objective":
                        strObj2 += p["props"][0]["value"] + " "
                if "prose" in p:
                    strObj2 += p["prose"] + " "
            if "parts" in p:
                for p2 in p["parts"]:
                    strObj3 = strObj2
                    if p2["name"] == "item" or p2["name"] == "objective":
                        if "props" in p2:
                            if str_name != "objective":
                                strObj3 += p2["props"][0]["value"] + " "
                        if "prose" in p2:
                            strObj3 += p2["prose"] + " "
                        if "parts" in p2:
                            for p3 in p2["parts"]:
                                strObj4 = strObj3
                                if p3["name"] == "item" or p3["name"] == "objective":
                                    if "props" in p3:
                                        if str_name != "objective":
                                            strObj4 += p3["props"][0]["value"] + " "
                                    if "prose" in p3:
                                        strObj4 += p3["prose"] + " "
                                    if "parts" in p3:
                                        for p4 in p3["parts"]:
                                            strObj5 = strObj4
                                            if p4["name"] == "item" or p4["name"] == "objective":
                                                if "props" in p4:
                                                    if str_name != "objective":
                                                        strObj5 += p4["props"][0]["value"] + " "
                                                if "prose" in p4:
                                                    strObj5 += p4["prose"] + " "
                                                if "parts" in p4:
                                                    for p5 in p4["parts"]:
                                                        strObj6 = strObj5
                                                        if p5["name"] == "item" or p5["name"] == "objective":
                                                            if (str_name != "objective") and "props" in p5:
                                                                strObj6 += p5["props"][0]["value"] + " "
                                                            if "prose" in p4:
                                                                strObj6 += p5["prose"] + " "
                                                            if strObj6 != "":
                                                                newObj = {
        ...
        for dat in ctrl["parts"]:
            if (
                ...
                if str_name in [
                    ...
                    parts.append(part)
                    try:
                        str_parts += "<li>{{" + dat["id"] + "}} - " + str_prose + "</li>"
                    except Exception:
                ...
                if dat["name"] in ["guidance", "overview"]:
                    ...
                    if "prose" in dat:
                        str_guidance += "<ul>"
                        str_guidance += "<li>" + dat["prose"] + "</li>"
                        str_guidance += "</ul>"
                    if "links" in dat:
                        str_guidance += "<ul>"
                        for lkp in dat["links"]:
                            str_guidance += "<li>" + lkp["href"] + ", " + lkp["rel"] + "</li>"
                        str_guidance += "</ul>"
                    str_guidance += "</ul>"
            else:
                # process assessments
...
def process_assessments(dat: dict, ctrl: dict, assessments: list) -> None:
    ...
    if dat["name"].startswith("assessment") is True:
        ...
        if "prose" in dat:
            ...
            if "props" in dat:
                if len(dat["props"]) > 0:
                    if "value" in dat["props"][0]:
                        ast["testType"] = dat["props"][0]["value"]
            if "parts" in dat:
                if len(dat["parts"]) > 0:
                    if "prose" in dat["parts"][0]:
                        ast["description"] = dat["parts"][0]["prose"]
        ...
        if "props" in dat:
            if len(dat["props"]) > 0:
                if "value" in dat["props"][0]:
                    strTestType = dat["props"][0]["value"]
        ast["testType"] = strTestType
        ...
        if "parts" in dat:
            if len(dat["parts"]) > 0:
                if "prose" in dat["parts"][0]:
                    strDescription = dat["parts"][0]["prose"]
        ast["description"] = strDescription
...
def oscal_version(metadata: dict) -> str:
    ...
    if oscal_schema not in ["1.0.0", "1.0.2", "1.0.4"]:
        raise ValueError(
            f"The RegScale CLI does not support OSCAL version {oscal_schema}, only 1.0.0, 1.0.2 \
                and 1.0.4 are supported."
        )
