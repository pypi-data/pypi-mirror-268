    ...
    def on_post_build(self, config):
        ...
        if self.config.unknown_warning:
            for item in self.unknowns:
                self._logger.warning(f'Glossary ignore {item["count"]} appearance{"" if item["count"] == 1 else "s"} of term "{item["term"]}" --> not in Glossary list (found first appearance at "{item["first_found"]})"')
            ...
            for item in self.unknowns:
                self._logger.debug(f'Glossary ignore {item["count"]} appearance{"" if item["count"] == 1 else "s"} of term "{item["term"]}" --> not in Glossary list (found first appearance at "{item["first_found"]})"')
                total_count += item["count"]
