...
async def test_device_update_no_type():
    ...
    device = Device(TEST_NODE_ID, TEST_API)
...
async def test_device_update_type_set(type_id, expected_params):
    ...
    with patch(
        DEVICE_DATA_PACKAGE, return_value=DUMMY_RESPONSE
    ) as device_data_mock, patch("asyncio.sleep", wraps=sleep_mock):
        await device.update()
...
def test_has_channel_type_non_exists():
    """Test the has channel type function with a type that was fetched."""
    device: Device = Device(TEST_NODE_ID, TEST_API)
