...
def userDataDir_创建(用户名=None,目录='user_data'):
    if not 用户名:
        i=0
        while True:
            path = os.path.join(os.path.abspath(目录), f'user_{i}')
            if not os.path.exists(path):
                用户名=f'user_{i}'
                break
            i+=1
    path = os.path.join(os.path.abspath(目录), 用户名)
    os.makedirs(path,exist_ok=1)
    return path
def userDataDir_读取(用户名=None,目录='user_data'):
    os.makedirs(目录,exist_ok=True)
    if 用户名:
        return os.path.join(os.path.abspath(目录),用户名)
    lst=os.listdir(os.path.abspath(目录))
    return os.path.join(os.path.abspath(目录),random.choice(lst)) if lst else None
class PYPP():
    host = ''
    port = ''
    user=''
