...
def _min_max_score(candidates: dict[object, int], reverse: bool = False) -> int:
...
def highest_remaining_card(player: object, trick: Trick,  trumps: Suit = None) -> Card:
    """Return the highest card remaining in hand if player does not win trick."""
    ...
    while value < 13 and value < winning_card.value:
        ...
        if test_card not in trick.cards + player.opponents_unplayed_cards[suit]:
...
def play_to_unblock(hand_to_play: list[Card], other_hand: list[Card]) -> Card | None:
    ...
    # The lowest card in the short hand is lower than the highest card in the long hand
...
def _card_in_sequence(hand_to_play: list[Card], other_hand: list[Card]) -> Card | None:
...
def _all_low_in_short(hand_to_play: list[Card], other_hand: list[Card]) -> Card | None:
    (long_hand, short_hand) = get_long_hand(hand_to_play, other_hand)
    long_values, short_values = card_values(long_hand), card_values(short_hand)
