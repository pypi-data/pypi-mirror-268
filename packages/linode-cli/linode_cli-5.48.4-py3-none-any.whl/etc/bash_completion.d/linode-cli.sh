...
case "${prev}" in
    linode-cli | linode | lin)
        COMPREPLY=( $(compgen -W "account betas events payment-methods service-transfers users databases domains images linodes kernels stackscripts lke longview managed networking firewalls vlans nodebalancers object-storage profile security-questions sshkeys phone regions tickets tags volumes vpcs _spec --help" -- ${cur}) )
        return 0
        ;;
    account)
        ...
        ;;
        linodes)
        COMPREPLY=( $(compgen -W "list create view update delete backups-list snapshot backups-cancel backups-enable backup-view backup-restore boot clone configs-list config-create config-view config-update config-delete config-interfaces-list config-interface-add config-interface-view config-interface-update config-interface-delete config-interfaces-order disks-list disk-create disk-view disk-update disk-delete disk-clone disk-reset-password disk-resize firewalls-list ips-list ip-add ip-view ip-update ip-delete migrate upgrade nodebalancers linode-reset-password reboot rebuild rescue resize shutdown transfer-view volumes types type-view --help" -- ${cur}) )
        return 0
        ;;
        kernels)
        ...
        ;;
        vpcs)
        COMPREPLY=( $(compgen -W "list create view update delete subnets-list subnet-create subnet-view subnet-update subnet-delete --help" -- ${cur}) )
        return 0
        ;;
    *)
        ;;
