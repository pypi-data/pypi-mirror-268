...
def imports(
    ...
    lines = [
        ...
        "from edgedb import RelativeDuration, AsyncIOClient, create_async_client, RetryOptions",
...
def build_client(db_config: DBConfig) -> str:
    return f"CLIENT = create_async_client(max_concurrency={db_config.max_concurrency}).with_retry_options(options=RetryOptions(attempts=0))"
...
async def build_enums_from_config(include_strawberry: bool) -> str:
    client = edgedb.create_async_client().with_retry_options(
        options=edgedb.RetryOptions(attempts=0)
    )
