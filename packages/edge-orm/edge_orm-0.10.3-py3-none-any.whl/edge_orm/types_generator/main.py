...
def build_client(db_config: DBConfig) -> str:
    return f'from datetime import timedelta\nCLIENT = create_async_client(max_concurrency={db_config.max_concurrency}).with_config(query_execution_timeout=timedelta(seconds=float(os.environ["EDGEDB_QUERY_EXECUTION_TIMEOUT_SECONDS"])))'
