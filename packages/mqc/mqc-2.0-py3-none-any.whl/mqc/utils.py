...
def max_reduced_degree_rate(model, metId, model_info):
    ...
    c_met_degree, max_rate, s_met_degree = 0, 0, 0
    temp = []
        ...
        for r,v in need_fluxes.items() :
            ...
            if len(all_mets) == 1 and metId not in all_mets and 'S' in mets.elements.keys():
                if (rxn.reactants and v < 0) or (rxn.products and v > 0):
                    ...
                    # print('test:',rxn,r,v,mets.elements.items(),mets.charge,'s_met_degree',s_met_degree,'degree',degree)
            if len(all_mets) == 1 and metId not in all_mets and 'C' in mets.elements.keys():
                if (rxn.reactants and v < 0) or (rxn.products and v > 0):
                    ...
                    # print('test:',rxn,r,v,mets.elements.items(),mets.charge,'c_met_degree',c_met_degree,'degree',degree)
        max_rate = (s_met_degree + c_met_degree)/abs(degree)
    ...
    for rxn in model_info["reactions"]:
        ...
        if rxn["id"] in model_info["exchange_rxns"]:
            if (rxn['reactants_mets'] and rxn["bounds"][0] <= 0) or (rxn['products_mets'] and rxn['bounds'][1] >= 0):
                ...
                if re.search("C[A-Z]",str(r_met.formula)) or re.search("C[\d]",str(r_met.formula)) or r_met_name in C_LIST or str(r_met.formula).find('R') != -1 or str(r_met.formula).find('X') != -1 or str(r_met.formula) == 'null': # C后边是大写字母或者数字，或者包含X
...
def del_model_boundary(model_info, model, check_model, model_control_info):
    ...
    find_autotrophic_and_carbon_source(model_info, model) # 获取自养、异样碳源
    ...
    if model_info["single_carbon"]: # 当模型为自动构建的模型时，就不用找最小碳源集，直接将葡萄糖等单碳作为碳源即可，在后续进行gap，添加必要的途径
        print('single_carbon:',model.slim_optimize(),'........................................111.......................')
...
def is_auto_build(model_info, model):
    ...
    try:
        if (num/total) >= 0.7:
            model_info["single_carbon"] = 1
        else:
            model_info["single_carbon"] = 0
...
def add_respiratory_chain_rxn(model_info, model, check_model):
    ...
    if count/len(model.metabolites) > 0.8:
        print(count,len(model.metabolites))
        mets = [met.id for met in model.metabolites]
        if 'ATPS4rpp' not in model.reactions:
            if set(['adp_c','h_p','pi_c','atp_c','h2o_c','h_c']) & set(mets) == 6:
        ...
        if 'CYTBO3_4pp' not in model.reactions:
            if set(['o2_c','h_p','q8h2_c','q8_c','h2o_c','h_c']) & set(mets) == 6:
        ...
        if 'NADH16pp' not in model.reactions:
            if set(['nadh_c','h_p','q8_c','q8h2_c','nad_c','h_c']) & set(mets) == 6:
...
def del_h2(model_info, model, check_model):
    ...
    if 'rxn08734_c0' in model.reactions:
        ...
        model_info["special_boundary_reaction"].extend([f"The reaction cpd11640_c0 is not correct. No NADH is needed to generate proton driving force,turn it off and add H2 efflux reaction"])
...
def set_special_boundary_rxn(model_info, model, check_model):
    ...
    for rxn in model_info["reactions"]:
        if rxn['bounds'][0] > 0 or rxn['bounds'][1] < 0:
            ...
            temp.append(rxn['id'])
            model_info['modify_rxn'].append(rxn['id']) # 会把一些有问题的边界的反应记录进去：比如ATPM
    for rxns in model.reactions:
        if rxns.id in temp:
...
def write_flux_file2(model_info, model, objectiveId,cfg:object):
    """
    get flux file
    """
    if model.slim_optimize() > 0:
        ...
        need_fluxes = {}
    with open(cfg.flux_path2, 'w') as flux:  
        for r,v in need_fluxes.items():
            for rxn in model_info['reactions']:
                if r == rxn['id'] and r != objectiveId:
                    rxns = model.reactions.get_by_id(r)
...
def keep_decimal_places(reaction):
    ...
    reactant_str = ' + '.join([f"{abs(round(coefficient, 3))} {metabolite.name}" for metabolite, coefficient in reaction.metabolites.items() if coefficient < 0])
    product_str = ' + '.join([f"{abs(round(coefficient, 3))} {metabolite.name}" for metabolite, coefficient in reaction.metabolites.items() if coefficient > 0])
    equation_rounded = f"{reactant_str} {arrow} {product_str}"
    reactant_str_id = ' + '.join([f"{abs(round(coefficient, 3))} {metabolite.id}" for metabolite, coefficient in reaction.metabolites.items() if coefficient < 0])
    product_str_id = ' + '.join([f"{abs(round(coefficient, 3))} {metabolite.id}" for metabolite, coefficient in reaction.metabolites.items() if coefficient > 0])
