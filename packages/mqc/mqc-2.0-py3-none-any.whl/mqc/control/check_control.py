...
class Check():
    ...
    def find_yield_initial_flux(self,model_info, model, model_control_info):
        ...
        maxRate=[maxRate[k] for k in range(10) if k<len(maxRate)]
        objRate=[objRate[k] for k in range(10) if k<len(objRate)]
        ...
        if len(yield_generation) != 0:
            ...
            plt.bar(x_middle, maxRate, width=width, label='maximum_yield')
            plt.bar(x_middle + width, objRate, width=width, label='objective_value')
    ...
    def check_control(self, model_info, model, check_model, model_control_info, model_check_info):
        ...
        self.get_reducing_power_initial_flux(model_info, model, model_check_info)
        self.get_energy_initial_flux(model_info, model, model_check_info)
        ...
        if is_autotrophic(model_info):
            ...
            get_c_source_supply(model_info, model)
            self.find_yield_initial_flux(model_info, model, model_check_info)
