    ...
    def atp_control(self, model_info, model, check_model, model_control_info, model_check_info, obj):
        ...
        for k in xtp:
            ...
            if not atpm_id:
                ...
                model_info["control_analysis"].append("无")
                continue
            ...
            model_info["control_analysis"].append((initial1,final1))
            if model_control_info["check_energy_production"][name]["score"] == 0:
                temp["score"] = 0
                model_control_info["check_energy_production"][name]["Optimal_rate_in_initial_model"] = f"Optimal {k}TP production rate was {initial1} mmol/gDCW/h when no carbon sourse was supplied."
