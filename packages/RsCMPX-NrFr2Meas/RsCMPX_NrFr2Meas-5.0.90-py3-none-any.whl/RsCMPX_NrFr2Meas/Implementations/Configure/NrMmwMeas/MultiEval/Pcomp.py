...
class PcompCls:
	...
	def set(self, phase_comp: enums.PhaseComp, user_def_freq: float) -> None:
		"""SCPI: CONFigure:NRMMw:MEASurement<Instance>:MEValuation:PCOMp \n
		Snippet: driver.configure.nrMmwMeas.multiEval.pcomp.set(phase_comp = enums.PhaseComp.CAF, user_def_freq = 1.0) \n
		Specifies the phase compensation applied by the UE during the modulation and upconversion. \n
			...
			:param user_def_freq: Frequency for PhaseComp = UDEF
		"""
		param = ArgSingleList().compose_cmd_string(ArgSingle('phase_comp', phase_comp, DataType.Enum, enums.PhaseComp), ArgSingle('user_def_freq', user_def_freq, DataType.Float))
		self._core.io.write(f'CONFigure:NRMMw:MEASurement<Instance>:MEValuation:PCOMp {param}'.rstrip())
	...
	def get(self) -> PcompStruct:
		"""SCPI: CONFigure:NRMMw:MEASurement<Instance>:MEValuation:PCOMp \n
		Snippet: value: PcompStruct = driver.configure.nrMmwMeas.multiEval.pcomp.get() \n
		Specifies the phase compensation applied by the UE during the modulation and upconversion. \n
			:return: structure: for return value, see the help for PcompStruct structure arguments."""
		return self._core.io.query_struct(f'CONFigure:NRMMw:MEASurement<Instance>:MEValuation:PCOMp?', self.__class__.PcompStruct())
