...
class ExtremeCls:
	...
	def read(self, carrierComponent=repcap.CarrierComponent.Default, layer=repcap.Layer.Default) -> ResultData:
		"""SCPI: READ:NRMMw:MEASurement<Instance>:MEValuation[:CC<no>][:LAYer<layer>]:MODulation:EXTReme \n
		Snippet: value: ResultData = driver.nrMmwMeas.multiEval.cc.layer.modulation.extreme.read(carrierComponent = repcap.CarrierComponent.Default, layer = repcap.Layer.Default) \n
		Return the extreme single value results for carrier <no>, layer <l>. The values described below are returned by FETCh and
		...
		return self._core.io.query_struct(f'READ:NRMMw:MEASurement<Instance>:MEValuation:CC{carrierComponent_cmd_val}:LAYer{layer_cmd_val}:MODulation:EXTReme?', self.__class__.ResultData())

	def fetch(self, carrierComponent=repcap.CarrierComponent.Default, layer=repcap.Layer.Default) -> ResultData:
		"""SCPI: FETCh:NRMMw:MEASurement<Instance>:MEValuation[:CC<no>][:LAYer<layer>]:MODulation:EXTReme \n
		Snippet: value: ResultData = driver.nrMmwMeas.multiEval.cc.layer.modulation.extreme.fetch(carrierComponent = repcap.CarrierComponent.Default, layer = repcap.Layer.Default) \n
		Return the extreme single value results for carrier <no>, layer <l>. The values described below are returned by FETCh and
		READ commands. CALCulate commands return limit check results instead, one value for each result listed below. \n
			:param carrierComponent: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Cc')
			:param layer: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Layer')
		...
		carrierComponent_cmd_val = self._cmd_group.get_repcap_cmd_value(carrierComponent, repcap.CarrierComponent)
		layer_cmd_val = self._cmd_group.get_repcap_cmd_value(layer, repcap.Layer)
		return self._core.io.query_struct(f'FETCh:NRMMw:MEASurement<Instance>:MEValuation:CC{carrierComponent_cmd_val}:LAYer{layer_cmd_val}:MODulation:EXTReme?', self.__class__.ResultData())
	...
	class CalculateStruct(StructBase):
		"""Response structure. Fields: \n
			...
			- Psd_Min: float or bool: No parameter help available
			- Psd_Max: float or bool: No parameter help available
			...
			- Freq_Err_Ppm: float or bool: Carrier frequency error in ppm
			- Iq_Imbalance: float or bool: I/Q gain imbalance
			- Iq_Quadrature_Err: float or bool: No parameter help available
			- Ant_1_Pow_Min: float or bool: Minimum power at RX antenna 1
			- Ant_1_Pow_Max: float or bool: Maximum power at RX antenna 1
			- Ant_2_Pow_Min: float or bool: Minimum power at RX antenna 2
			- Ant_2_Pow_Max: float or bool: Maximum power at RX antenna 2"""
		__meta_args_list = [
			ArgStruct.scalar_int('Reliability', 'Reliability'),
			ArgStruct.scalar_int('Out_Of_Tolerance'),
			...
			ArgStruct.scalar_float_ext('Psd_Min'),
			ArgStruct.scalar_float_ext('Psd_Max'),
			...
			ArgStruct.scalar_float_ext('Freq_Err_Ppm'),
			ArgStruct.scalar_float_ext('Iq_Imbalance'),
			ArgStruct.scalar_float_ext('Iq_Quadrature_Err'),
			ArgStruct.scalar_float_ext('Ant_1_Pow_Min'),
			ArgStruct.scalar_float_ext('Ant_1_Pow_Max'),
			ArgStruct.scalar_float_ext('Ant_2_Pow_Min'),
			ArgStruct.scalar_float_ext('Ant_2_Pow_Max')]
		...
		def __init__(self):
			...
			self.Psd_Min: float or bool = None
			self.Psd_Max: float or bool = None
			...
			self.Freq_Err_Ppm: float or bool = None
			self.Iq_Imbalance: float or bool = None
			self.Iq_Quadrature_Err: float or bool = None
			self.Ant_1_Pow_Min: float or bool = None
			self.Ant_1_Pow_Max: float or bool = None
			self.Ant_2_Pow_Min: float or bool = None
			self.Ant_2_Pow_Max: float or bool = None

	def calculate(self, carrierComponent=repcap.CarrierComponent.Default, layer=repcap.Layer.Default) -> CalculateStruct:
		"""SCPI: CALCulate:NRMMw:MEASurement<Instance>:MEValuation[:CC<no>][:LAYer<layer>]:MODulation:EXTReme \n
		Snippet: value: CalculateStruct = driver.nrMmwMeas.multiEval.cc.layer.modulation.extreme.calculate(carrierComponent = repcap.CarrierComponent.Default, layer = repcap.Layer.Default) \n
		Return the extreme single value results for carrier <no>, layer <l>. The values described below are returned by FETCh and
		READ commands. CALCulate commands return limit check results instead, one value for each result listed below. \n
			:param carrierComponent: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Cc')
			:param layer: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Layer')
		...
		carrierComponent_cmd_val = self._cmd_group.get_repcap_cmd_value(carrierComponent, repcap.CarrierComponent)
		layer_cmd_val = self._cmd_group.get_repcap_cmd_value(layer, repcap.Layer)
		return self._core.io.query_struct(f'CALCulate:NRMMw:MEASurement<Instance>:MEValuation:CC{carrierComponent_cmd_val}:LAYer{layer_cmd_val}:MODulation:EXTReme?', self.__class__.CalculateStruct())
