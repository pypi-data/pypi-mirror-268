...
class CurrentCls:
	...
	def fetch(self, carrierComponentExt=repcap.CarrierComponentExt.Default, maximum=repcap.Maximum.Default) -> List[int]:
		"""SCPI: FETCh:NRMMw:MEASurement<Instance>:MEValuation:LIST[:CC<carrier>]:ESFLatness:SCINdex:MAXimum<nr>:CURRent \n
		Snippet: value: List[int] = driver.nrMmwMeas.multiEval.listPy.cc.esFlatness.scIndex.maximum.current.fetch(carrierComponentExt = repcap.CarrierComponentExt.Default, maximum = repcap.Maximum.Default) \n
		...
		response = self._core.io.query_bin_or_ascii_int_list_suppressed(f'FETCh:NRMMw:MEASurement<Instance>:MEValuation:LIST:CC{carrierComponentExt_cmd_val}:ESFLatness:SCINdex:MAXimum{maximum_cmd_val}:CURRent?', suppressed)
		return response
