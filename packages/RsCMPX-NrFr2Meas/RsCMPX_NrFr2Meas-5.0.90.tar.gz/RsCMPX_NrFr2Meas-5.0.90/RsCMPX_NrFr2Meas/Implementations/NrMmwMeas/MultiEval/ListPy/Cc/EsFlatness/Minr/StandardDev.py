...
class StandardDevCls:
	...
	def fetch(self, carrierComponentExt=repcap.CarrierComponentExt.Default, minRange=repcap.MinRange.Default) -> List[float]:
		"""SCPI: FETCh:NRMMw:MEASurement<Instance>:MEValuation:LIST[:CC<carrier>]:ESFLatness:MINR<nr>:SDEViation \n
		Snippet: value: List[float] = driver.nrMmwMeas.multiEval.listPy.cc.esFlatness.minr.standardDev.fetch(carrierComponentExt = repcap.CarrierComponentExt.Default, minRange = repcap.MinRange.Default) \n
		...
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'FETCh:NRMMw:MEASurement<Instance>:MEValuation:LIST:CC{carrierComponentExt_cmd_val}:ESFLatness:MINR{minRange_cmd_val}:SDEViation?', suppressed)
		return response
