...
class IqOffsetCls:
	...
	def set(self, enable: bool, offset_0: float, offset_1: float) -> None:
		"""SCPI: CONFigure:NRMMw:MEASurement<Instance>:MEValuation:LIMit:PHBPsk:IQOFfset \n
		Snippet: driver.configure.nrMmwMeas.multiEval.limit.phbpsk.iqOffset.set(enable = False, offset_0 = 1.0, offset_1 = 1.0) \n
		Defines upper limits for the I/Q origin offset (π/2-BPSK modulation) . Two different I/Q origin offset limits can be set
		for two TX power ranges. \n
		...
		self._core.io.write(f'CONFigure:NRMMw:MEASurement<Instance>:MEValuation:LIMit:PHBPsk:IQOFfset {param}'.rstrip())
	...
	def get(self) -> IqOffsetStruct:
		"""SCPI: CONFigure:NRMMw:MEASurement<Instance>:MEValuation:LIMit:PHBPsk:IQOFfset \n
		Snippet: value: IqOffsetStruct = driver.configure.nrMmwMeas.multiEval.limit.phbpsk.iqOffset.get() \n
		Defines upper limits for the I/Q origin offset (π/2-BPSK modulation) . Two different I/Q origin offset limits can be set
		for two TX power ranges. \n
			:return: structure: for return value, see the help for IqOffsetStruct structure arguments."""
		return self._core.io.query_struct(f'CONFigure:NRMMw:MEASurement<Instance>:MEValuation:LIMit:PHBPsk:IQOFfset?', self.__class__.IqOffsetStruct())
