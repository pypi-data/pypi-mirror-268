...
class EvmSymbolCls:
	...
	def set(self, enable: bool, symbol: int, low_high: enums.LowHigh) -> None:
		...
		Snippet: driver.configure.nrMmwMeas.multiEval.result.evMagnitude.evmSymbol.set(enable = False, symbol = 1, low_high = enums.LowHigh.HIGH) \n
		Enables or disables the measurement of EVM vs modulation symbol results and configures the scope of the measurement.
		Do not use anymore. Use instead method RsCMPX_NrFr2Meas.Configure.NrMmwMeas.MultiEval.Result.modulation. \n
			:param enable: OFF: Do not measure the results. ON: Measure the results.
		...
		self._core.io.write(f'CONFigure:NRMMw:MEASurement<Instance>:MEValuation:RESult:EVMagnitude:EVMSymbol {param}'.rstrip())
	...
	def get(self) -> EvmSymbolStruct:
		"""SCPI: CONFigure:NRMMw:MEASurement<Instance>:MEValuation:RESult:EVMagnitude:EVMSymbol \n
		Snippet: value: EvmSymbolStruct = driver.configure.nrMmwMeas.multiEval.result.evMagnitude.evmSymbol.get() \n
		Enables or disables the measurement of EVM vs modulation symbol results and configures the scope of the measurement.
		Do not use anymore. Use instead method RsCMPX_NrFr2Meas.Configure.NrMmwMeas.MultiEval.Result.modulation. \n
			:return: structure: for return value, see the help for EvmSymbolStruct structure arguments."""
		return self._core.io.query_struct(f'CONFigure:NRMMw:MEASurement<Instance>:MEValuation:RESult:EVMagnitude:EVMSymbol?', self.__class__.EvmSymbolStruct())
