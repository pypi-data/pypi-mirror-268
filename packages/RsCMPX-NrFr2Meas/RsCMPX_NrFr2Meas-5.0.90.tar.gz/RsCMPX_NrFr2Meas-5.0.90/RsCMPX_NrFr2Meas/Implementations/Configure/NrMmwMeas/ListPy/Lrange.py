...
class LrangeCls:
	...
	def set(self, start_index: int, nr_segments: int) -> None:
		...
		Snippet: driver.configure.nrMmwMeas.listPy.lrange.set(start_index = 1, nr_segments = 1) \n
		...
		self._core.io.write(f'CONFigure:NRMMw:MEASurement<Instance>:LIST:LRANge {param}'.rstrip())
	...
	def get(self) -> LrangeStruct:
		"""SCPI: CONFigure:NRMMw:MEASurement<Instance>:LIST:LRANge \n
		Snippet: value: LrangeStruct = driver.configure.nrMmwMeas.listPy.lrange.get() \n
		Select a range of measured segments. \n
			:return: structure: for return value, see the help for LrangeStruct structure arguments."""
		return self._core.io.query_struct(f'CONFigure:NRMMw:MEASurement<Instance>:LIST:LRANge?', self.__class__.LrangeStruct())
