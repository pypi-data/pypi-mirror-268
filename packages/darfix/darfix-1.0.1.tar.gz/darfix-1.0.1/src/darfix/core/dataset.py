...
class Dataset:
    ...
    def __init__(
        ...
        if data is not None:
            ...
            if filenames is None and first_filename is None:
                filenames = sorted(
                    [
                        ...
                        for x in glob.glob(
                            os.path.join(
                                _dir if raw_folder is None else raw_folder, "*"
                            )
                        )
                        if os.path.isfile(x)
                    ]
            ...
            if self._isH5:
                if isinstance(first_filename, DataUrl):
                    url = first_filename
                else:
                    url = DataUrl(first_filename)
                # load data

                def get_metadata(data_slice) -> tuple:
                    if metadata_url is None:
                        return _HDF5MetadataReader(metadata={})
                    else:
                        metadata = {}
                        with silx.io.open(metadata_url.file_path()) as h5:
                            ...
                            metadata_group = h5[metadata_path]
                            for dataset in metadata_group.keys():
                                # we only handle 1D dataset
                                if (
                                    isinstance(metadata_group[dataset], h5py.Dataset)
                                    and metadata_group[dataset].ndim == 1
                                ):
                                    if len(metadata_group[dataset]) <= data_slice:
                                        # TODO: is this warning always correct and relevant ?
                                        _logger.warning(
                                            f"Unable to access to index {data_slice} of the dataset {metadata_group[dataset].name}"
                                        )
                                        metadata[dataset] = None
                                    else:
                                        metadata[dataset] = metadata_group[dataset][
                                            data_slice
                                        ]
                        return _HDF5MetadataReader(metadata=metadata)

                with silx.io.open(url.file_path()) as h5:
                    hdf5_item = h5[url.data_path()]
                ...
                for i in range(n_frames):
                    ...
                    metadata.append(get_metadata(data_slice=i))
            else:
    ...
    def partition_by_intensity(self, bins=None, bottom_bin=None, top_bin=None):
        """
    ...
    @dims.setter
    def dims(self, _dims):
        assert isinstance(_dims, AcquisitionDims), (
    ...
    def find_dimensions(self, kind, tolerance=1e-9) -> None:
        """
        ...
        unique_values = [
            numpy.unique(value, return_index=True, return_counts=True)
            for value in values
        ]
        dimensions = []
        ...
        for dimension in sorted(
            ...
            _logger.info(
                "Dimension {} of size {} has been added for reshaping".format(
                    dimension.name, dimension.size
                )
            )

    def get_metadata_values(
        self, kind, key, indices=None, dimension=None
    ) -> numpy.ndarray:
        values = []
        missing_value = False
        for data in self.get_data(indices, dimension).metadata:
            try:
    ...
    def compute_mosaicity_colorkey(
        self, dimensions=[0, 1], scale=100, indices=None, third_motor=None
    ):
        """
    ...
    def apply_mask_removal(self, mask, indices=None, _dir=None):
        """
    ...
    def apply_fit(
        self, indices=None, int_thresh=None, chunk_shape=[100, 100], _dir=None
    ):
        """
    ...
    def compute_transformation(
        self, d, kind="magnification", rotate=False, topography=[False, 0], center=True
    ):
        """
        ...
        if self.dims.ndim == 1 and kind == "rsm":
            ...
            obpitch = numpy.unique(
                self.get_metadata_values(POSITIONER_METADATA, "obpitch")
            )
            obpitch = obpitch[len(obpitch) // 2]
            mainx = -self.get_metadata_values(POSITIONER_METADATA, "mainx")[0]
            x, y = compute_magnification(
                H, W, d, obx, obpitch, mainx, topography, center
            )
    ...
    def project_data(self, dimension, indices=None, _dir=None):
        ...
        if len(dimension) == 1:
            ...
            return ValueError("Only 1D and 2D projection is allowed")
    ...
    def compute_rsm(
        ...
        if transformation is None and self.transformation is not None:
            transformation = self.transformation
        elif self.transformation is None:
            raise ValueError(
                "Transformation has to first be computed using the Transformation Widget (before any ROI)"
            )
    ...
    def apply_binning(self, scale, _dir=None):
        ...
        data = rescale_data(self.get_data(), scale)
        shape = data.shape
        data = data.view(Data)
        data.save(
            os.path.join(_dir, "binned_data.hdf5"),
            in_memory=self._in_memory,
            new_shape=shape,
        )
...
class _HDF5MetadataReader:
    ...
    def get_value(self, kind, name):
        return self.__metadata.get(name, None)

    def get_keys(self, kind):
        return tuple(self.__metadata.keys())
