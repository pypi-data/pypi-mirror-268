...
class AllCls:
	...
	def fetch(self, timeout: float = None, target_main_state: enums.TargetMainState = None, target_sync_state: enums.TargetSyncState = None) -> List[enums.ResourceState]:
		...
		Snippet: value: List[enums.ResourceState] = driver.iqVsSlot.state.all.fetch(timeout = 1.0, target_main_state = enums.TargetMainState.OFF, target_sync_state = enums.TargetSyncState.ADJusted) \n
		...
		response = self._core.io.query_str(f'FETCh:GPRF:MEASurement<Instance>:IQVSlot:STATe:ALL? {param}'.rstrip())
