...
class IqRecorderCls:
	...
	def initiate(self, save_to_iq_file: enums.FileSave = None) -> None:
		...
		Use FETCh...STATe? to query the current measurement state. \n
			:param save_to_iq_file: OFF | ON | ONLY Optional parameter, selecting whether the results are written to an I/Q file, to the memory or both. For file selection, see method RsCmwGprfMeas.Configure.IqRecorder.iqFile. OFF: The results are only stored in the memory. ON: The results are stored in the memory and in a file. ONLY: The results are only stored in a file.
		"""
		param = ''
		if save_to_iq_file:
	...
	def read(self) -> List[float]:
		...
		formats, see 'ASCII and binary data formats'. For the number of values n, see method RsCmwGprfMeas.Configure.IqRecorder.
		Capture.set. \n
		Use RsCmwGprfMeas.reliability.last_value to read the updated reliability indicator. \n
			:return: iq_samples: float For ASCII format: Comma-separated list of I and Q amplitudes {I, Q}1, ..., {I, Q}n For REAL format: Binary block data as listed in the table below. There are no commas within this parameter. Unit: V (for ASCII format)"""
