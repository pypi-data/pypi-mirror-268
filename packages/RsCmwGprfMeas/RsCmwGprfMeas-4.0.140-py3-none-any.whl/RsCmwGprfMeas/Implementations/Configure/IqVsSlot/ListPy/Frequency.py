...
class FrequencyCls:
	...
	def set(self, index: int, frequency: float) -> None:
		...
		Snippet: driver.configure.iqVsSlot.listPy.frequency.set(index = 1, frequency = 1.0) \n
	...
	def get(self, index: int) -> float:
		"""SCPI: CONFigure:GPRF:MEASurement<Instance>:IQVSlot:LIST:FREQuency \n
		Snippet: value: float = driver.configure.iqVsSlot.listPy.frequency.get(index = 1) \n
		Defines or queries the frequency of subsweep <Index>. For the supported frequency range, see 'Frequency ranges'. \n
			:param index: integer Range: 0 to 199
	...
	def get_all(self) -> List[float]:
		...
		Snippet: value: List[float] = driver.configure.iqVsSlot.listPy.frequency.get_all() \n
		Defines the frequencies for all subsweeps. For the supported frequency range, see 'Frequency ranges'. \n
			:return: frequency: numeric Comma-separated list of frequencies, one value per subsweep Unit: Hz
		"""
	...
	def set_all(self, frequency: List[float]) -> None:
		"""SCPI: CONFigure:GPRF:MEASurement<Instance>:IQVSlot:LIST:FREQuency:ALL \n
		Snippet: driver.configure.iqVsSlot.listPy.frequency.set_all(frequency = [1.1, 2.2, 3.3]) \n
		Defines the frequencies for all subsweeps. For the supported frequency range, see 'Frequency ranges'. \n
			:param frequency: numeric Comma-separated list of frequencies, one value per subsweep Unit: Hz
		"""
