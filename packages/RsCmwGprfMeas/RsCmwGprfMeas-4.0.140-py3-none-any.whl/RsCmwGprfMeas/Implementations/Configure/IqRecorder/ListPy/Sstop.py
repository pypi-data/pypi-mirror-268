...
class SstopCls:
	...
	def set(self, start_index: int, stop_index: int) -> None:
		...
		Snippet: driver.configure.iqRecorder.listPy.sstop.set(start_index = 1, stop_index = 1) \n
		...
		self._core.io.write(f'CONFigure:GPRF:MEASurement<Instance>:IQRecorder:LIST:SSTop {param}'.rstrip())
	...
	def get(self) -> SstopStruct:
		"""SCPI: CONFigure:GPRF:MEASurement<Instance>:IQRecorder:LIST:SSTop \n
		Snippet: value: SstopStruct = driver.configure.iqRecorder.listPy.sstop.get() \n
		No command help available \n
			:return: structure: for return value, see the help for SstopStruct structure arguments."""
		return self._core.io.query_struct(f'CONFigure:GPRF:MEASurement<Instance>:IQRecorder:LIST:SSTop?', self.__class__.SstopStruct())
