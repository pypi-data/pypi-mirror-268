...
class CaptureCls:
	...
	def set(self, capt_samp_bef_trig: int, capt_samp_aft_trig: int) -> None:
		...
		Snippet: driver.configure.iqRecorder.capture.set(capt_samp_bef_trig = 1, capt_samp_aft_trig = 1) \n
		...
		sum does not exceed the maximum number of samples. \n
			:param capt_samp_bef_trig: integer Samples before trigger event Range: 1 to 4194303
			:param capt_samp_aft_trig: integer Samples after trigger event Range: 1 to 4194303
		"""
	...
	def get(self) -> CaptureStruct:
		"""SCPI: CONFigure:GPRF:MEASurement<Instance>:IQRecorder:CAPTure \n
		Snippet: value: CaptureStruct = driver.configure.iqRecorder.capture.get() \n
		Selects the number of samples to be recorded before and after the trigger event. Configure the two settings so that their
		sum does not exceed the maximum number of samples. \n
