...
class GaussCls:
	...
	def get_bandwidth(self) -> float:
		...
		Selects the bandwidth for a filter of Gaussian shape. \n
			:return: gauss_bw: numeric Only the following values can be configured: 1 kHz, 10 kHz, 100 kHz, 1 MHz, 10 MHz Other values are rounded to the next allowed value. Unit: Hz
		"""
	...
	def set_bandwidth(self, gauss_bw: float) -> None:
		...
		Selects the bandwidth for a filter of Gaussian shape. \n
			:param gauss_bw: numeric Only the following values can be configured: 1 kHz, 10 kHz, 100 kHz, 1 MHz, 10 MHz Other values are rounded to the next allowed value. Unit: Hz
		"""
