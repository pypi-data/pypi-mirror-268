...
class OsStopCls:
	...
	def set(self, offset_start: float, offset_stop: float) -> None:
		...
		Snippet: driver.trigger.fftSpecAn.osStop.set(offset_start = 1.0, offset_stop = 1.0) \n
		Defines the start and stop values for the trigger offset mode VARiable. The start value must be smaller than the stop
		value. \n
			:param offset_start: numeric Range: -0.15 s to 0.15 s, Unit: s
			:param offset_stop: numeric Range: -0.15 s to 0.15 s, Unit: s
		"""
	...
	def get(self) -> OsStopStruct:
		"""SCPI: TRIGger:GPRF:MEASurement<Instance>:FFTSanalyzer:OSSTop \n
		Snippet: value: OsStopStruct = driver.trigger.fftSpecAn.osStop.get() \n
		Defines the start and stop values for the trigger offset mode VARiable. The start value must be smaller than the stop
		value. \n
