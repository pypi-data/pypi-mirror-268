...
class RsCmwGprfMeas:
	...
	_driver_options = "SupportedInstrModels = CMW500/CMW100/CMW270/CMW280/CMP, SupportedIdnPatterns = CMW, SimulationIdnString = 'Rohde&Schwarz,CMW500,100001,4.0.140.0033'"
	...
	def __init__(self, resource_name: str, id_query: bool = True, reset: bool = False, options: str = None, direct_session: object = None):
		...
		self._core = Core(resource_name, id_query, reset, RsCmwGprfMeas._driver_options, options, direct_session)
		self._core.driver_version = '4.0.140.0033'
	...
	@classmethod
	def from_existing_session(cls, session: object, options: str = None) -> 'RsCmwGprfMeas':
	...
	def __str__(self) -> str:
		if self._core.io:
			return f"RsCmwGprfMeas session '{self._core.io.resource_name}'"
		else:
			return f"RsCmwGprfMeas with session closed"
	...
	def assert_minimum_version(min_version: str) -> None:
		...
		curr_version_list = '4.0.140.0033'.split('.')
		...
		for i in range(count):
			...
			if curr > minimum:
				break
			if curr < minimum:
				raise RsInstrException(f"Assertion for minimum RsCmwGprfMeas version failed. Current version: '4.0.140.0033', minimum required version: '{min_version}'")
	...
	def _sync_to_custom_properties(self, cloned: 'RsCmwGprfMeas') -> None:
	...
	@property
	def configure(self):
		"""configure commands group. 10 Sub-classes, 1 commands."""
	...
	def iqVsSlot(self):
		...
		if not hasattr(self, '_iqVsSlot'):
			from .Implementations.IqVsSlot import IqVsSlotCls
	...
	def extPwrSensor(self):
		...
		if not hasattr(self, '_extPwrSensor'):
			from .Implementations.ExtPwrSensor import ExtPwrSensorCls
	...
	def nrpm(self):
		...
		if not hasattr(self, '_nrpm'):
			from .Implementations.Nrpm import NrpmCls
	...
	def iqRecorder(self):
		...
		if not hasattr(self, '_iqRecorder'):
			from .Implementations.IqRecorder import IqRecorderCls
	...
	def spectrum(self):
		...
		if not hasattr(self, '_spectrum'):
			from .Implementations.Spectrum import SpectrumCls
	...
	def fftSpecAn(self):
		...
		if not hasattr(self, '_fftSpecAn'):
			from .Implementations.FftSpecAn import FftSpecAnCls
	...
	def ploss(self):
		...
		if not hasattr(self, '_ploss'):
			from .Implementations.Ploss import PlossCls
	...
	def canalyzer(self):
		...
		if not hasattr(self, '_canalyzer'):
			from .Implementations.Canalyzer import CanalyzerCls
			self._canalyzer = CanalyzerCls(self._core, self._cmd_group)
		return self._canalyzer

	def clone(self) -> 'RsCmwGprfMeas':
		"""Creates a deep copy of the RsCmwGprfMeas object. Also copies:
		...
		cloned = RsCmwGprfMeas.from_existing_session(self.get_session_handle(), self._options)
