...
class Main:
    ...
    def faas(self, parser):
        ...
        parser.add_argument('--lang', choices=LANGUAGE_OPTIONS, metavar="", help=f"language options allowed are - {LANGUAGE_OPTIONS_STR}")
        parser.add_argument('--name', metavar="", help=f"This is your function name.")
        faas_sub_parser.add_parser("setup", help="To setup e2e functions on your system", formatter_class=show_messages.cli_formatter())
        faas_create = faas_sub_parser.add_parser("deploy", help="To create the e2e function", formatter_class=show_messages.cli_formatter())
        faas_create.add_argument('--mem', choices=MEMORY_CHOICES, metavar="", help="Memory alloted to your function in MB")
        faas_create.add_argument('--timeout', metavar="", help="Function timeout value in second")
        faas_sub_parser.add_parser("redeploy", help="To update the existing function", formatter_class=show_messages.cli_formatter())
        faas_sub_parser.add_parser("list", help="To list all e2e functions", formatter_class=show_messages.cli_formatter())
        faas_sub_parser.add_parser("destroy", help="To delete the function", formatter_class=show_messages.cli_formatter())
