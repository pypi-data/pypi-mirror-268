...
def get_primary_columns(table_name, filepath):
    try:
...
def create_table(table_name, columns, filepath, primary_key, overwrite=False, logging=False):
    ...
    Note:
        - Raises only work when logging is set to True.
    
    Example:
        >>> create_table(table_name="my_table", columns="id INTEGER, name TEXT, age INTEGER",
        >>>                        filepath="my_database.db", primary_key="id", overwrite=True, logging=True) """
...
def table_exists(filepath:str, table_name:str ,logging = False):
    """
...
def remove_data_from_table(filepath, table_name, key_values, logging=False):
    """
...
def purge_table(filepath, table_name, confirm=False, logging=False):
    """
...
def edit_table(filepath, table_name, new_structure, primary_key_column, logging=False):
    """
...
def load_data_from_key(filepath, table_name, value, logging = False):
    """
...
def load_all_data(filepath, table_name, logging=False):
    """
