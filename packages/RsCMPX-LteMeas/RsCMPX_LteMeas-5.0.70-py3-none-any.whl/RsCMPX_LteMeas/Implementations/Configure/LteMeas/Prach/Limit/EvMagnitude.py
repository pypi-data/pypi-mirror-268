...
class EvMagnitudeCls:
	...
	def set(self, rms: float or bool, peak: float or bool) -> None:
		...
		Snippet: driver.configure.lteMeas.prach.limit.evMagnitude.set(rms = 1.0, peak = 1.0) \n
		...
		self._core.io.write(f'CONFigure:LTE:MEASurement<Instance>:PRACh:LIMit:EVMagnitude {param}'.rstrip())
	...
	def get(self) -> EvMagnitudeStruct:
		"""SCPI: CONFigure:LTE:MEASurement<Instance>:PRACh:LIMit:EVMagnitude \n
		Snippet: value: EvMagnitudeStruct = driver.configure.lteMeas.prach.limit.evMagnitude.get() \n
		Defines upper limits for the RMS and peak values of the error vector magnitude (EVM) . \n
			:return: structure: for return value, see the help for EvMagnitudeStruct structure arguments."""
		return self._core.io.query_struct(f'CONFigure:LTE:MEASurement<Instance>:PRACh:LIMit:EVMagnitude?', self.__class__.EvMagnitudeStruct())
