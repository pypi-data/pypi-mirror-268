...
class MerrorCls:
	...
	def fetch(self, xvalue: int or bool, trace_select: enums.TraceSelect) -> float:
		...
		Snippet: value: float = driver.lteMeas.multiEval.referenceMarker.merror.fetch(xvalue = 1, trace_select = enums.TraceSelect.AVERage) \n
		...
		response = self._core.io.query_str_suppressed(f'FETCh:LTE:MEASurement<Instance>:MEValuation:REFMarker:MERRor? {param}'.rstrip(), suppressed)
