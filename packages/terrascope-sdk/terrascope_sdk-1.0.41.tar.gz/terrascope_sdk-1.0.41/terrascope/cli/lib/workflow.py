...
# Init
############################################################################
def check_environment_complete(raise_on_failure=False, print_missing=True, verbose=False):
    """
    ...
    missing = set()
    required_environment = ['TERRASCOPE_API_TOKEN', 'TERRASCOPE_API_ADMIN_TOKEN']
    var_list = []
...
def create_algorithm_manifest(manifest_dict):
    ...
    inputs = manifest_dict['inputs'][0]
    outputs = manifest_dict['outputs']

    manifest = AlgorithmManifest()

    # metadata
    manifest.metadata_required(description=metadata['description'], version=metadata['version'])
    ...
    # inputs/outputs
    kwargs = {'parameters': inputs.get('parameters', {})}
    if 'bands' in inputs:
        kwargs['bands'] = inputs['bands']
    manifest.inputs_add_data_type(data_type_name=inputs['data_type_name'], **kwargs)
    for output in outputs:
        manifest.outputs_add_data_type(data_type_name=output['data_type_name'])
...
def create_analysis_manifest(
    ...
    Args:
        ...
        version (str): The version of the analysis manifest.
    ...
    manifest.metadata(description=description, version=version, tags=tags)
...
async def deactivate_algorithm_versions(
    ...
    """
    """
    oi_papi_host = os.environ["TERRASCOPE_API_HOST"]
    oi_papi_port_string = os.getenv("TERRASCOPE_API_PORT", default="443")
    oi_papi_port = int(oi_papi_port_string)
    secure_string = os.getenv("TERRASCOPE_API_SECURE", default="True")
    secure = True if secure_string in ["True", "true", "Yes", "yes", "1"] else False
    # admin_api_token = os.environ["TERRASCOPE_API_ADMIN_TOKEN"]

    client = TerrascopeAsyncClient(oi_papi_host, oi_papi_port, secure=secure)
    # admin_client = TerrascopeAsyncClient(oi_papi_host, oi_papi_port, secure=secure, api_token=admin_api_token)
...
async def update_algorithm(
    ...
    await set_credit(algorithm_version_id=algorithm_version.id,
                     value_price=value_price,
                     execution_price=execution_price)
    if visualizer_config_names:
        await create_visualizer_config_algo(visualizer_config_names, algorithm_version.id)
...
async def create_algorithm_config(
    ...
    config.add_data_source(data_type=data_type, data_parameters=data_parameters)
...
async def list_analyses(analysis_name: str = None):
    ...
    kwargs = {}
    if analysis_name:
        kwargs['search_text'] = analysis_name
    sdk = TerraScopeSDK()
...
async def update_analysis(
    ...
    await set_credit(analysis_version_id=analysis_version.id,
                     value_price=value_price,
                     execution_price=execution_price)
...
async def set_credit(
    ...
    export TERRASCOPE_API_ADMIN_TOKEN=<admin_token>
    ...
    terrascope_api_admin_token = os.getenv('TERRASCOPE_API_ADMIN_TOKEN')
    assert terrascope_api_admin_token is not None
    client = TerraScopeSDK().create_client(terrascope_host=terrascope_host,
                                           terrascope_api_token=terrascope_api_admin_token)
    sdk = TerraScopeSDK(client=client)
...
async def download_algorithm_computation_results(
    ...
    """
    sdk = TerraScopeSDK()
    algorithm_computation_id_to_data_type_to_downloaded_paths = await sdk.result.export(
        algorithm_computation_ids=algorithm_computation_ids, analysis_computation_ids=analysis_computation_ids,
        source_aoi_version=source_aoi_version, min_observation_start_ts=min_observation_start_ts,
        max_observation_start_ts=max_observation_start_ts, download_dir=download_dir
    )
...
async def resource_permission_create(
        resource_ids: List,
        user_emails: List,
        permission_type=Permission.PermissionType.READ,
        resource_type=Permission.ResourceType.ALGORITHM,
        public=False,
    ...
    Args:
        ...
        permission_type (Permission.PermissionType, optional): The type of permission to grant.
            Defaults to Permission.PermissionType.READ.
        resource_type (Permission.ResourceType, optional): The type of resource for which to grant permissions.
            Defaults to Permission.ResourceType.ALGORITHM.
...
async def analysis_permission_create(
    ...
    await resource_permission_create(algorithms, user_emails,
                                     permission_type, Permission.ResourceType.ALGORITHM,
                                     public=public, public_confirm=public_confirm)
    await resource_permission_create(analyses, user_emails,
                                     permission_type, Permission.ResourceType.ANALYSIS,
                                     public=public, public_confirm=public_confirm)
    for algo_configs in tsu.chunk_array(algorithm_configs, chunk_size):
        await resource_permission_create(algo_configs, user_emails,
                                         permission_type, Permission.ResourceType.ALGORITHM_CONFIG,
                                         public=public, public_confirm=public_confirm)
    await resource_permission_create([analysis_config_id], user_emails,
                                     permission_type, Permission.ResourceType.ANALYSIS_CONFIG,
                                     public=public, public_confirm=public_confirm)
...
# Visualization
############################################################################
async def create_visualizer_config_algo(
