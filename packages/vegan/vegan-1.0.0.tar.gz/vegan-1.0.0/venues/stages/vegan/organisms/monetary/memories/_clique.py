...
def monetary_memories_clique ():
	...
	'''
		itinerary:
			[ ] vegan_1 monetary structures export --export-name export.2.json
	'''
	@group.command ("export")
	@click.option (
		'--essence-path', 
		default = 'essence.py',
		help = 'vegan monetary structures --essence-path essence.py', 
		required = False
	)
	def save (essence_path):
		...
		the_exports_path = vegan_essence ["monetary"] ["saves"] ["exports"] ["path"]
	
	
	'''
		vegan_1 monetary structures import --import-name export.1.json
	'''
	...
	def insert (essence_path, import_name, db, drop):
		...
		the_exports_path = vegan_essence ["monetary"] ["saves"] ["exports"] ["path"]
		the_exports_collections = vegan_essence ["monetary"] ["saves"] ["exports"] ["collections"]
