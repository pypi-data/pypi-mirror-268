...
class model_builder():
    ...
    """
    def __init__(self, IMG_DIM=None, path_training=None, path_validation=None,label = None,
    ...
    def display_data_distribution(self):
        ...
        train_labels = [self.label[0] if self.label[0] in os.path.basename(fn) else self.label[1] for fn in train_files]
        validation_labels = [self.label[0] if self.label[0] in os.path.basename(fn) else self.label[1] for fn in validation_files]
        print('number of training samples norm: {}'.format(train_labels.count(self.label[0])))
        print('number of training samples pheno: {}'.format(train_labels.count(self.label[1])))
