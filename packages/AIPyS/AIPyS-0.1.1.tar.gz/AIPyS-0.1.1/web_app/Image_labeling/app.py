...
def image_labeling(imagePath,dataPath):
  # normlise intensity and sd and mask area by origImage.

  df_Orig = pd.read_csv(os.path.join(dataPath,'imageseq_data.csv'))
  # copy data before labeling to new folder 
  df_Nolabel = os.path.join(dataPath,"prelabel_table")
  if not os.path.exists(df_Nolabel):
    os.makedirs(df_Nolabel, exist_ok=True)
    shutil.copy('imageseq_data.csv', df_Nolabel)
