...
from AIPyS.supportFunctions import display_and_xml as dx
...
def deployRatioCalc(image_input,mask):
    return pd.DataFrame(measure.regionprops_table(mask,intensity_image=image_input,properties=['area', 'label', 'centroid','intensity_mean']))

def call_bin(table_sel_cor, threshold ,img_blank):
    '''
    :parameter:
