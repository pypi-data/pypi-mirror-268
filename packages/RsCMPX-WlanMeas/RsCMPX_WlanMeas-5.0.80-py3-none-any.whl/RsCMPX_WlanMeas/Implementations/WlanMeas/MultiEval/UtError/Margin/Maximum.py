...
class MaximumCls:
	...
	def read(self, utError=repcap.UtError.Default) -> List[float]:
		...
		Snippet: value: List[float] = driver.wlanMeas.multiEval.utError.margin.maximum.read(utError = repcap.UtError.Default) \n
		...
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'READ:WLAN:MEASurement<Instance>:MEValuation:UTERror{utError_cmd_val}:MARGin:MAXimum?', suppressed)
		return response
	...
	def fetch(self, utError=repcap.UtError.Default) -> List[float]:
		...
		Snippet: value: List[float] = driver.wlanMeas.multiEval.utError.margin.maximum.fetch(utError = repcap.UtError.Default) \n
		Returns the margin values of the unused tone error measurement for the current, average, minimum and maximum traces.
		A positive margin indicates a violation of the unused tone error limit line. The respective trace value is located above
		the upper limit line. The values described below are returned by FETCh and READ commands. CALCulate commands return limit
		check results instead, one value for each result listed below. \n
		...
		utError_cmd_val = self._cmd_group.get_repcap_cmd_value(utError, repcap.UtError)
		suppressed = ArgSingleSuppressed(0, DataType.Integer, False, 1, 'Reliability')
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'FETCh:WLAN:MEASurement<Instance>:MEValuation:UTERror{utError_cmd_val}:MARGin:MAXimum?', suppressed)
		return response

	# noinspection PyTypeChecker
	def calculate(self, utError=repcap.UtError.Default) -> List[enums.ResultStatus2]:
		"""SCPI: CALCulate:WLAN:MEASurement<Instance>:MEValuation:UTERror<n>:MARGin:MAXimum \n
		Snippet: value: List[enums.ResultStatus2] = driver.wlanMeas.multiEval.utError.margin.maximum.calculate(utError = repcap.UtError.Default) \n
		Returns the margin values of the unused tone error measurement for the current, average, minimum and maximum traces.
		A positive margin indicates a violation of the unused tone error limit line. The respective trace value is located above
		the upper limit line. The values described below are returned by FETCh and READ commands. CALCulate commands return limit
		check results instead, one value for each result listed below. \n
		...
		utError_cmd_val = self._cmd_group.get_repcap_cmd_value(utError, repcap.UtError)
		suppressed = ArgSingleSuppressed(0, DataType.Integer, False, 1, 'Reliability')
		response = self._core.io.query_str_suppressed(f'CALCulate:WLAN:MEASurement<Instance>:MEValuation:UTERror{utError_cmd_val}:MARGin:MAXimum?', suppressed)
