...
class MinimumCls:
	...
	def fetch(self, segmentB=repcap.SegmentB.Default) -> FetchStruct:
		"""SCPI: FETCh:WLAN:MEASurement<Instance>:MEValuation:LIST:SEGMent<segment>:TSMask:FREQuency:MINimum \n
		Snippet: value: FetchStruct = driver.wlanMeas.multiEval.listPy.segment.tsMask.frequency.minimum.fetch(segmentB = repcap.SegmentB.Default) \n
		...
		return self._core.io.query_struct(f'FETCh:WLAN:MEASurement<Instance>:MEValuation:LIST:SEGMent{segmentB_cmd_val}:TSMask:FREQuency:MINimum?', self.__class__.FetchStruct())
