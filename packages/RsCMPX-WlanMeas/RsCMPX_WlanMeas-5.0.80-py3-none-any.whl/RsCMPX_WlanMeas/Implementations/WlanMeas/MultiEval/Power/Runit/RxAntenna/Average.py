...
class AverageCls:
	...
	def fetch(self, resourceUnit=repcap.ResourceUnit.Default, rxAntenna=repcap.RxAntenna.Default) -> float:
		"""SCPI: FETCh:WLAN:MEASurement<instance>:MEValuation:POWer:RUNit<ru>:RXANtenna<n>:AVERage \n
		Snippet: value: float = driver.wlanMeas.multiEval.power.runit.rxAntenna.average.fetch(resourceUnit = repcap.ResourceUnit.Default, rxAntenna = repcap.RxAntenna.Default) \n
		...
		response = self._core.io.query_str_suppressed(f'FETCh:WLAN:MEASurement<Instance>:MEValuation:POWer:RUNit{resourceUnit_cmd_val}:RXANtenna{rxAntenna_cmd_val}:AVERage?', suppressed)
