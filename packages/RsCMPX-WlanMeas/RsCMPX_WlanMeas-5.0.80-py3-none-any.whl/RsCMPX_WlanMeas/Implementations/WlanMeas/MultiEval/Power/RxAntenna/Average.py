...
class AverageCls:
	...
	def fetch(self, rxAntenna=repcap.RxAntenna.Default) -> List[float]:
		...
		Snippet: value: List[float] = driver.wlanMeas.multiEval.power.rxAntenna.average.fetch(rxAntenna = repcap.RxAntenna.Default) \n
		...
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'FETCh:WLAN:MEASurement<Instance>:MEValuation:POWer:RXANtenna{rxAntenna_cmd_val}:AVERage?', suppressed)
		return response
