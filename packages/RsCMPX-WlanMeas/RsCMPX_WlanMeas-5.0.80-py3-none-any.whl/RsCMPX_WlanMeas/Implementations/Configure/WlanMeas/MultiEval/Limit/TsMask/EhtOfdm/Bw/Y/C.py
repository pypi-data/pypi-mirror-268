...
class CCls:
	...
	def set(self, tsm_lim_yrel_lev_c: float, bandwidthF=repcap.BandwidthF.Default) -> None:
		...
		Snippet: driver.configure.wlanMeas.multiEval.limit.tsMask.ehtOfdm.bw.y.c.set(tsm_lim_yrel_lev_c = 1.0, bandwidthF = repcap.BandwidthF.Default) \n
	...
	def get(self, bandwidthF=repcap.BandwidthF.Default) -> float:
		"""SCPI: CONFigure:WLAN:MEASurement<Instance>:MEValuation:LIMit:TSMask:EHTofdm:BW<bandwidth>:Y:C \n
		Snippet: value: float = driver.configure.wlanMeas.multiEval.limit.tsMask.ehtOfdm.bw.y.c.get(bandwidthF = repcap.BandwidthF.Default) \n
		Defines the relative spectral density limit for point C (frequency offset: 1*bandwidth) on the transmit spectrum mask for
		802.11be signals with the specified <bandwidth>. See 'Transmit spectrum mask OFDM, default masks' for background
		information. \n
			:param bandwidthF: optional repeated capability selector. Default value: Bw20 (settable in the interface 'Bw')
