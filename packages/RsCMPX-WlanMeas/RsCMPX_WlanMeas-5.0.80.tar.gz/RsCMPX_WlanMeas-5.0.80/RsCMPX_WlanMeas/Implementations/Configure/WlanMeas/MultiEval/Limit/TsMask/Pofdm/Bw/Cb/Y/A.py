...
class ACls:
	...
	def set(self, tsm_lim_yrel_lev_a: float, bandwidthB=repcap.BandwidthB.Bw5) -> None:
		...
		Snippet: driver.configure.wlanMeas.multiEval.limit.tsMask.pofdm.bw.cb.y.a.set(tsm_lim_yrel_lev_a = 1.0, bandwidthB = repcap.BandwidthB.Bw5) \n
		Defines the Y-value of point A (Δf = 2 <bandwidth>) on the 802.11p spectrum mask for power class B and the specified
		...
		self._core.io.write(f'CONFigure:WLAN:MEASurement<Instance>:MEValuation:LIMit:TSMask:POFDm:BW{bandwidthB_cmd_val}:CB:Y:A {param}')

	def get(self, bandwidthB=repcap.BandwidthB.Bw5) -> float:
		"""SCPI: CONFigure:WLAN:MEASurement<Instance>:MEValuation:LIMit:TSMask:POFDm:BW<bandwidth>:CB:Y:A \n
		Snippet: value: float = driver.configure.wlanMeas.multiEval.limit.tsMask.pofdm.bw.cb.y.a.get(bandwidthB = repcap.BandwidthB.Bw5) \n
		Defines the Y-value of point A (Δf = 2 <bandwidth>) on the 802.11p spectrum mask for power class B and the specified
		<bandwidth>. For background information, see 'Transmit spectrum mask OFDM, by regulation'. \n
			:param bandwidthB: optional repeated capability selector. Default value: Bw5
			:return: tsm_lim_yrel_lev_a: No help available"""
		bandwidthB_cmd_val = self._cmd_group.get_repcap_cmd_value(bandwidthB, repcap.BandwidthB)
		response = self._core.io.query_str(f'CONFigure:WLAN:MEASurement<Instance>:MEValuation:LIMit:TSMask:POFDm:BW{bandwidthB_cmd_val}:CB:Y:A?')
