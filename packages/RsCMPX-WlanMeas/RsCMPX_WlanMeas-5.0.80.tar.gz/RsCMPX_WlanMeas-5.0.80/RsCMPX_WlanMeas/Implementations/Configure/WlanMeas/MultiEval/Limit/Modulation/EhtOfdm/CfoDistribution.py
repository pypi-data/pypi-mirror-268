...
class CfoDistributionCls:
	...
	def set(self, cfo_percentage: float or bool, cfo_frequency: float) -> None:
		...
		Snippet: driver.configure.wlanMeas.multiEval.limit.modulation.ehtOfdm.cfoDistribution.set(cfo_percentage = 1.0, cfo_frequency = 1.0) \n
		Configure the limit of carrier frequency offset (CFO) error distribution for EHT modulation measurements. Exceeding the
		limit has no impact on the stop On Limit Failure condition or out of tolerance counter. \n
			:param cfo_percentage: (float or boolean) Upper limit for the tolerated CFO errors (CFO exceeding the specified CFO_Frequency)
			:param cfo_frequency: Border value defining CFO error
		"""
	...
	def get(self) -> CfoDistributionStruct:
		"""SCPI: CONFigure:WLAN:MEASurement<Instance>:MEValuation:LIMit:MODulation:EHTofdm:CFDistrib \n
		Snippet: value: CfoDistributionStruct = driver.configure.wlanMeas.multiEval.limit.modulation.ehtOfdm.cfoDistribution.get() \n
		Configure the limit of carrier frequency offset (CFO) error distribution for EHT modulation measurements. Exceeding the
		limit has no impact on the stop On Limit Failure condition or out of tolerance counter. \n
