...
class MinimumCls:
	...
	def read(self, mimo=repcap.Mimo.Default) -> ResultData:
		"""SCPI: READ:WLAN:MEASurement<Instance>:MEValuation:TSMask:MIMO<n>:SEGMents:FREQuency:MINimum \n
		Snippet: value: ResultData = driver.wlanMeas.multiEval.tsMask.mimo.segments.frequency.minimum.read(mimo = repcap.Mimo.Default) \n
		...
		return self._core.io.query_struct(f'READ:WLAN:MEASurement<Instance>:MEValuation:TSMask:MIMO{mimo_cmd_val}:SEGMents:FREQuency:MINimum?', self.__class__.ResultData())

	def fetch(self, mimo=repcap.Mimo.Default) -> ResultData:
		"""SCPI: FETCh:WLAN:MEASurement<Instance>:MEValuation:TSMask:MIMO<n>:SEGMents:FREQuency:MINimum \n
		Snippet: value: ResultData = driver.wlanMeas.multiEval.tsMask.mimo.segments.frequency.minimum.fetch(mimo = repcap.Mimo.Default) \n
		No command help available \n
			:param mimo: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Mimo')
			:return: structure: for return value, see the help for ResultData structure arguments."""
		mimo_cmd_val = self._cmd_group.get_repcap_cmd_value(mimo, repcap.Mimo)
		return self._core.io.query_struct(f'FETCh:WLAN:MEASurement<Instance>:MEValuation:TSMask:MIMO{mimo_cmd_val}:SEGMents:FREQuency:MINimum?', self.__class__.ResultData())
	...
	def calculate(self, mimo=repcap.Mimo.Default) -> CalculateStruct:
		"""SCPI: CALCulate:WLAN:MEASurement<Instance>:MEValuation:TSMask:MIMO<n>:SEGMents:FREQuency:MINimum \n
		Snippet: value: CalculateStruct = driver.wlanMeas.multiEval.tsMask.mimo.segments.frequency.minimum.calculate(mimo = repcap.Mimo.Default) \n
		No command help available \n
			:param mimo: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Mimo')
			:return: structure: for return value, see the help for CalculateStruct structure arguments."""
		mimo_cmd_val = self._cmd_group.get_repcap_cmd_value(mimo, repcap.Mimo)
		return self._core.io.query_struct(f'CALCulate:WLAN:MEASurement<Instance>:MEValuation:TSMask:MIMO{mimo_cmd_val}:SEGMents:FREQuency:MINimum?', self.__class__.CalculateStruct())
