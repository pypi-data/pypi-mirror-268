...
class CmimoCls:
	...
	def __init__(self, core: Core, parent):
		self._core = core
		self._cmd_group = CommandsGroup("cmimo", core, parent)

	@property
	def psts(self):
		"""psts commands group. 5 Sub-classes, 0 commands."""
		if not hasattr(self, '_psts'):
			from .Psts import PstsCls
			self._psts = PstsCls(self._core, self._cmd_group)
		return self._psts
	...
	def clone(self) -> 'CmimoCls':
		"""Clones the group by creating new object from it and its whole existing subgroups
		Also copies all the existing default Repeated Capabilities setting,
		which you can change independently without affecting the original group"""
		new_group = CmimoCls(self._core, self._cmd_group.parent)
