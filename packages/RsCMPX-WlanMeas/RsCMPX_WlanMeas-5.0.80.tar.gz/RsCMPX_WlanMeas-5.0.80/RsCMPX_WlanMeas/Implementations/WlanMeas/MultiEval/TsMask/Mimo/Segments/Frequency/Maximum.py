...
class MaximumCls:
	...
	def read(self, mimo=repcap.Mimo.Default) -> ResultData:
		...
		Snippet: value: ResultData = driver.wlanMeas.multiEval.tsMask.mimo.segments.frequency.maximum.read(mimo = repcap.Mimo.Default) \n
	...
	def fetch(self, mimo=repcap.Mimo.Default) -> ResultData:
		...
		Snippet: value: ResultData = driver.wlanMeas.multiEval.tsMask.mimo.segments.frequency.maximum.fetch(mimo = repcap.Mimo.Default) \n
		No command help available \n
			:param mimo: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Mimo')
			:return: structure: for return value, see the help for ResultData structure arguments."""
		mimo_cmd_val = self._cmd_group.get_repcap_cmd_value(mimo, repcap.Mimo)
	...
	def calculate(self, mimo=repcap.Mimo.Default) -> CalculateStruct:
		...
		Snippet: value: CalculateStruct = driver.wlanMeas.multiEval.tsMask.mimo.segments.frequency.maximum.calculate(mimo = repcap.Mimo.Default) \n
		No command help available \n
			:param mimo: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Mimo')
