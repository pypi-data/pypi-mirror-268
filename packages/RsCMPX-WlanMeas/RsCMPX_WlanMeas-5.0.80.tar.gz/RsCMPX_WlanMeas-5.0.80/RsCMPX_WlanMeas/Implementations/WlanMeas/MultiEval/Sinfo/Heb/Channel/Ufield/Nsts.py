...
class NstsCls:
	...
	def fetch(self, channel=repcap.Channel.Default, userIx=repcap.UserIx.Default) -> FetchStruct:
		"""SCPI: FETCh:WLAN:MEASurement<instance>:MEValuation:SINFo:HEB:CHANnel<ch_index>:UFIeld<usr_index>:NSTS \n
		Snippet: value: FetchStruct = driver.wlanMeas.multiEval.sinfo.heb.channel.ufield.nsts.fetch(channel = repcap.Channel.Default, userIx = repcap.UserIx.Default) \n
		...
		return self._core.io.query_struct(f'FETCh:WLAN:MEASurement<Instance>:MEValuation:SINFo:HEB:CHANnel{channel_cmd_val}:UFIeld{userIx_cmd_val}:NSTS?', self.__class__.FetchStruct())
