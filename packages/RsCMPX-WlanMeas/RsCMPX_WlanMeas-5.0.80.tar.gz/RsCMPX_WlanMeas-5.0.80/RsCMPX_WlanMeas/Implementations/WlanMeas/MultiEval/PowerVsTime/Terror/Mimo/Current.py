...
class CurrentCls:
	...
	def read(self, mimo=repcap.Mimo.Default) -> float:
		...
		Snippet: value: float = driver.wlanMeas.multiEval.powerVsTime.terror.mimo.current.read(mimo = repcap.Mimo.Default) \n
		...
		Suppressed linked return values: reliability \n
			...
			:return: timing_error_curr: No help available"""
		mimo_cmd_val = self._cmd_group.get_repcap_cmd_value(mimo, repcap.Mimo)
		suppressed = ArgSingleSuppressed(0, DataType.Integer, False, 1, 'Reliability')
		response = self._core.io.query_str_suppressed(f'READ:WLAN:MEASurement<Instance>:MEValuation:PVTime:TERRor:MIMO{mimo_cmd_val}:CURRent?', suppressed)
	...
	def fetch(self, mimo=repcap.Mimo.Default) -> float:
		...
		Snippet: value: float = driver.wlanMeas.multiEval.powerVsTime.terror.mimo.current.fetch(mimo = repcap.Mimo.Default) \n
		Return the current, average, minimum, maximum and standard deviation timing error single value results of the power vs
		time MIMO measurement. The commands are only supported for OFDM standards. The values described below are returned by
		FETCh and READ commands. CALCulate commands return limit check results instead, one value for each result listed below. \n
		Suppressed linked return values: reliability \n
		...
		mimo_cmd_val = self._cmd_group.get_repcap_cmd_value(mimo, repcap.Mimo)
		suppressed = ArgSingleSuppressed(0, DataType.Integer, False, 1, 'Reliability')
		response = self._core.io.query_str_suppressed(f'FETCh:WLAN:MEASurement<Instance>:MEValuation:PVTime:TERRor:MIMO{mimo_cmd_val}:CURRent?', suppressed)
		return Conversions.str_to_float(response)
	...
	def calculate(self, mimo=repcap.Mimo.Default) -> enums.ResultStatus2:
		...
		Snippet: value: enums.ResultStatus2 = driver.wlanMeas.multiEval.powerVsTime.terror.mimo.current.calculate(mimo = repcap.Mimo.Default) \n
		Return the current, average, minimum, maximum and standard deviation timing error single value results of the power vs
		time MIMO measurement. The commands are only supported for OFDM standards. The values described below are returned by
		FETCh and READ commands. CALCulate commands return limit check results instead, one value for each result listed below. \n
		Suppressed linked return values: reliability \n
		...
		mimo_cmd_val = self._cmd_group.get_repcap_cmd_value(mimo, repcap.Mimo)
		suppressed = ArgSingleSuppressed(0, DataType.Integer, False, 1, 'Reliability')
		response = self._core.io.query_str_suppressed(f'CALCulate:WLAN:MEASurement<Instance>:MEValuation:PVTime:TERRor:MIMO{mimo_cmd_val}:CURRent?', suppressed)
