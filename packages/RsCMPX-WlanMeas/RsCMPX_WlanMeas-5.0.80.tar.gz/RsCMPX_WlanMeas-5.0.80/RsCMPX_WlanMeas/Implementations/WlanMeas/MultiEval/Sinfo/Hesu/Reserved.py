...
class ReservedCls:
	...
	def fetch(self, reserved=repcap.Reserved.Default) -> FetchStruct:
		...
		Snippet: value: FetchStruct = driver.wlanMeas.multiEval.sinfo.hesu.reserved.fetch(reserved = repcap.Reserved.Default) \n
		...
		return self._core.io.query_struct(f'FETCh:WLAN:MEASurement<Instance>:MEValuation:SINFo:HESU:REServed{reserved_cmd_val}?', self.__class__.FetchStruct())
