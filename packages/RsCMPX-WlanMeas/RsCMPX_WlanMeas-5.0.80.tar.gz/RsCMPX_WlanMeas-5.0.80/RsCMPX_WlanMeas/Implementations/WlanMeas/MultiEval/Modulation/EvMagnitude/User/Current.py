...
class CurrentCls:
	...
	class FetchStruct(StructBase):
		...
		__meta_args_list = [
			...
			ArgStruct.scalar_float('Evm_Vs_User_All'),
			ArgStruct.scalar_float('Evm_Vs_User_Data'),
			ArgStruct.scalar_float('Evm_Vs_User_Pilot')]

		def __init__(self):
			StructBase.__init__(self, self)
			self.Reliability: int = None
			self.Evm_Vs_User_All: float = None
			self.Evm_Vs_User_Data: float = None
			self.Evm_Vs_User_Pilot: float = None

	def fetch(self, user=repcap.User.Default) -> FetchStruct:
		"""SCPI: FETCh:WLAN:MEASurement<instance>:MEValuation:MODulation:EVMagnitude:USER<user>:CURRent \n
		Snippet: value: FetchStruct = driver.wlanMeas.multiEval.modulation.evMagnitude.user.current.fetch(user = repcap.User.Default) \n
		...
		return self._core.io.query_struct(f'FETCh:WLAN:MEASurement<Instance>:MEValuation:MODulation:EVMagnitude:USER{user_cmd_val}:CURRent?', self.__class__.FetchStruct())
