...
class CurrentCls:
	...
	def fetch(self, resourceUnit=repcap.ResourceUnit.Default, rxAntenna=repcap.RxAntenna.Default) -> float:
		"""SCPI: FETCh:WLAN:MEASurement<instance>:MEValuation:POWer:RUNit<ru>:RXANtenna<n>:CURRent \n
		Snippet: value: float = driver.wlanMeas.multiEval.power.runit.rxAntenna.current.fetch(resourceUnit = repcap.ResourceUnit.Default, rxAntenna = repcap.RxAntenna.Default) \n
		...
		Suppressed linked return values: reliability \n
			...
			:return: power_vs_antenna_vs_ru: Power vs RU vs antenna"""
		...
		response = self._core.io.query_str_suppressed(f'FETCh:WLAN:MEASurement<Instance>:MEValuation:POWer:RUNit{resourceUnit_cmd_val}:RXANtenna{rxAntenna_cmd_val}:CURRent?', suppressed)
