...
class CurrentCls:
	...
	def read(self, start: float = None, count: float = None, decimation: float = None) -> List[float]:
		...
		Snippet: value: List[float] = driver.wlanMeas.multiEval.trace.spectrFlatness.acsiso.current.read(start = 1.0, count = 1.0, decimation = 1.0) \n
		...
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'READ:WLAN:MEASurement<Instance>:MEValuation:TRACe:SFLatness:ACSiso:CURRent? {param}'.rstrip(), suppressed)
		return response
	...
	def fetch(self, start: float = None, count: float = None, decimation: float = None) -> List[float]:
		...
		Snippet: value: List[float] = driver.wlanMeas.multiEval.trace.spectrFlatness.acsiso.current.fetch(start = 1.0, count = 1.0, decimation = 1.0) \n
		No command help available \n
		Suppressed linked return values: reliability \n
			:param start: No help available
			:param count: No help available
		...
		param = ArgSingleList().compose_cmd_string(ArgSingle('start', start, DataType.Float, None, is_optional=True), ArgSingle('count', count, DataType.Float, None, is_optional=True), ArgSingle('decimation', decimation, DataType.Float, None, is_optional=True))
		suppressed = ArgSingleSuppressed(0, DataType.Integer, False, 1, 'Reliability')
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'FETCh:WLAN:MEASurement<Instance>:MEValuation:TRACe:SFLatness:ACSiso:CURRent? {param}'.rstrip(), suppressed)
		return response
