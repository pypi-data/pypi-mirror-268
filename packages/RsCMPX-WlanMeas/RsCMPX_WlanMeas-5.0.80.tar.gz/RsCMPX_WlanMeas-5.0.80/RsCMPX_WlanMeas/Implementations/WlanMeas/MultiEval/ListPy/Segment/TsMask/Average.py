...
class AverageCls:
	...
	def fetch(self, segmentB=repcap.SegmentB.Default) -> FetchStruct:
		"""SCPI: FETCh:WLAN:MEASurement<Instance>:MEValuation:LIST:SEGMent<segment>:TSMask:AVERage \n
		Snippet: value: FetchStruct = driver.wlanMeas.multiEval.listPy.segment.tsMask.average.fetch(segmentB = repcap.SegmentB.Default) \n
		Return limit line margin results for segment <no> in list mode. A positive result indicates that the trace is located
		above the limit line, i.e. the limit is exceeded. Margins for the current, average, minimum, maximum and standard
		deviation results are returned. \n
		...
		return self._core.io.query_struct(f'FETCh:WLAN:MEASurement<Instance>:MEValuation:LIST:SEGMent{segmentB_cmd_val}:TSMask:AVERage?', self.__class__.FetchStruct())
