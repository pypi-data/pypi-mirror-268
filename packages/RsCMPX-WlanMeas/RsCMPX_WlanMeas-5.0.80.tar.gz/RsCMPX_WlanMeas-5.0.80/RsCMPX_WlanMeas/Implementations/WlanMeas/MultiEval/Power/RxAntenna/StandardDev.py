...
class StandardDevCls:
	...
	def fetch(self, rxAntenna=repcap.RxAntenna.Default) -> List[float]:
		"""SCPI: FETCh:WLAN:MEASurement<instance>:MEValuation:POWer:RXANtenna<n>:SDEViation \n
		Snippet: value: List[float] = driver.wlanMeas.multiEval.power.rxAntenna.standardDev.fetch(rxAntenna = repcap.RxAntenna.Default) \n
		...
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'FETCh:WLAN:MEASurement<Instance>:MEValuation:POWer:RXANtenna{rxAntenna_cmd_val}:SDEViation?', suppressed)
		return response
