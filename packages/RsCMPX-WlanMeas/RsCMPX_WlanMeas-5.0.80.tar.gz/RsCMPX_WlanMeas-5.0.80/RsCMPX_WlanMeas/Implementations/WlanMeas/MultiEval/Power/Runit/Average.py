...
class AverageCls:
	...
	def fetch(self, resourceUnit=repcap.ResourceUnit.Default) -> List[float]:
		...
		Snippet: value: List[float] = driver.wlanMeas.multiEval.power.runit.average.fetch(resourceUnit = repcap.ResourceUnit.Default) \n
		...
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'FETCh:WLAN:MEASurement<Instance>:MEValuation:POWer:RUNit{resourceUnit_cmd_val}:AVERage?', suppressed)
		return response
