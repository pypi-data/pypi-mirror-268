...
class CurrentCls:
	...
	def fetch(self, user=repcap.User.Default, stream=repcap.Stream.Default) -> FetchStruct:
		...
		Snippet: value: FetchStruct = driver.wlanMeas.multiEval.modulation.evMagnitude.user.stream.current.fetch(user = repcap.User.Default, stream = repcap.Stream.Default) \n
