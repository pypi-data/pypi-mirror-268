...
class Flags(SendableMixin, LoggingMixin):
    ...
    def __init__(self, *initial_flags):
        ...
        self._logger.debug(f"creating Flags with initial flags {initial_flags}")
    ...
    def _add(self, section):
        ...
        self._logger.debug(f"adding {raw} to flags. Flags are now {self.flags}")
    ...
    def __iadd__(self, other):
        self.__add__(other)
        self._logger.debug(
            f"adding {other} to flags inplace. Flags are now {self.flags}"
        )
        return self
    ...
    def _sub(self, section):
        ...
        self._logger.debug(f"subtracting {raw} from flags. Flags are now {self.flags}")
    ...
    def __isub__(self, other):
        self.__sub__(other)
        self._logger.debug(
            f"subtracting {other} from flags inplace. Flags are now {self.flags}"
        )
        return self
