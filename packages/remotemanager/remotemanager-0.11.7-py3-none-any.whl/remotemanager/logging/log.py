...
class Handler:
    ...
    _filetype = ".yaml"
    _default_file = "remotemanager_log"
    ...
    def _delete_log(self, warn: bool = False, force: bool = False):
        ...
        def delete_log(path):
            try:
                ...
                warnings.warn(f"could not remove log at {self._path} (file open)")
    ...
    def runtime(self, msg, *args, **kwargs):
        """Direct passthrough for custom `runtime` logging method"""
        if not isinstance(msg, str):
            msg = format_iterable(msg)
        self._external.runtime(msg, *args, **kwargs)
    ...
    def important(self, msg, *args, **kwargs):
        """Direct passthrough for custom `important` logging method"""
        if not isinstance(msg, str):
            msg = format_iterable(msg)
        self._external.important(msg, *args, **kwargs)
