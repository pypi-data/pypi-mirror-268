...
def pull(items, *, start=None, end=None, grid='15min', header='__auto__', group_by=None,
    ...
    if shape == 'capsules':
        ...
        _ensure_column_types_for_capsules_shape(output.df)
...
def _ensure_column_types_for_capsules_shape(df):
    for column_name, column_type in reversed([
        ('Condition', 'object'),
        ('Capsule Start', 'datetime64[ns, UTC]'),
        ('Capsule End', 'datetime64[ns, UTC]'),
        ('Capsule Is Uncertain', 'bool')]
    ):
        if column_name in df.columns:
            if not df[column_name].dtype.name.startswith(column_type):
                try:
...
def _pull_condition_via_formula_api(session: Session, item_name, formula, parameters, current_start, pd_end,
    ...
    if shape == 'samples':
        ...
        result_df = (pd.concat([result_df, df_to_append]) if len(result_df) != 0 else df_to_append)
