...
from profiles_rudderstack.go_client import init_go_rpc, ClientTokenAuthInterceptor
...
from profiles_rudderstack.tunnel.tunnel_pb2_grpc import add_PythonServiceServicer_to_server, WhtServiceStub
...
class ProfilesRPCServer:
    ...
    def __server_init(self, token: str, current_supported_schema_version: int, pb_version: str):
        # check if Go RPC server is up
        if self.__go_rpc_status(token) == False:
            raise Exception("Go RPC server is not up")
    ...
    def __go_rpc_status(self, token: str):
        """Check if Go RPC server is up

        Returns:
            bool: True if WHT RPC server is up, False otherwise
        """
        max_retries = 10
        base_delay = 0.1  # seconds
        interceptors = [ClientTokenAuthInterceptor(token)]
        for i in range(max_retries):
            delay = base_delay * math.pow(2, i)
            try:
                ...
                print(
                    f"Error: connecting to WHT RPC server: {e}. Retrying in {delay} seconds")
            time.sleep(delay)
