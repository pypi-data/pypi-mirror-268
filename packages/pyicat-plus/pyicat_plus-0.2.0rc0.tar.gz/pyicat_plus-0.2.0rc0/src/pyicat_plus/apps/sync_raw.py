...
def sync_raw(
    ...
    with ExperimentalSessionStore(
        ...
        for session_dir in session_dirs:
            proposal_from_dir, beamline_from_dir, session_from_dir = (
...
def _discover_raw_datasets(
    ...
    dataset_filter = raw_data.get_dataset_filter(
        exp_session.raw_root_dir, raw_data_format=raw_data_format
    )

    logger.info("Extracting metadata from HDF5 for %s ...", dataset_filter)
    ...
    dataset_dirs = glob(path_utils.markdir(dataset_filter))
