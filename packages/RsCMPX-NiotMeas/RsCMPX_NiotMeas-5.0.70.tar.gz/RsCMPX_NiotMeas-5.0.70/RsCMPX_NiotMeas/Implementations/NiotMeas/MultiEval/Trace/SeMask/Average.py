...
class AverageCls:
	...
	def read(self) -> List[float]:
		...
		Snippet: value: List[float] = driver.niotMeas.multiEval.trace.seMask.average.read() \n
		...
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'READ:NIOT:MEASurement<Instance>:MEValuation:TRACe:SEMask:AVERage?', suppressed)
		return response

	def fetch(self) -> List[float]:
		"""SCPI: FETCh:NIOT:MEASurement<Instance>:MEValuation:TRACe:SEMask:AVERage \n
		Snippet: value: List[float] = driver.niotMeas.multiEval.trace.seMask.average.fetch() \n
		Returns the values of the spectrum emission traces. The results of the current, average and maximum traces can be
		retrieved. See also 'Square Spectrum Emission Mask'. \n
		Suppressed linked return values: reliability \n
			:return: power: Comma-separated list of 1067 power results The value in the middle of the result array corresponds to the center frequency. The test point separation between two results equals 3.75 kHz."""
		suppressed = ArgSingleSuppressed(0, DataType.Integer, False, 1, 'Reliability')
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'FETCh:NIOT:MEASurement<Instance>:MEValuation:TRACe:SEMask:AVERage?', suppressed)
		return response
