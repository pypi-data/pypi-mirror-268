...
class MaximumCls:
	...
	def read(self) -> List[float]:
		...
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'READ:NIOT:MEASurement<Instance>:MEValuation:TRACe:SEMask:MAXimum?', suppressed)
		return response
	...
	def fetch(self) -> List[float]:
		...
		Suppressed linked return values: reliability \n
			:return: power: Comma-separated list of 1067 power results The value in the middle of the result array corresponds to the center frequency. The test point separation between two results equals 3.75 kHz."""
		suppressed = ArgSingleSuppressed(0, DataType.Integer, False, 1, 'Reliability')
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'FETCh:NIOT:MEASurement<Instance>:MEValuation:TRACe:SEMask:MAXimum?', suppressed)
		return response
