...
class PreambleCls:
	...
	def read(self, preamble=repcap.Preamble.Default) -> ResultData:
		...
		Snippet: value: ResultData = driver.niotMeas.prach.modulation.preamble.read(preamble = repcap.Preamble.Default) \n
		Return the single value results of the EVM vs Preamble and Power vs Preamble squares, for a selected preamble. See also
		...
		return self._core.io.query_struct(f'READ:NIOT:MEASurement<Instance>:PRACh:MODulation:PREamble{preamble_cmd_val}?', self.__class__.ResultData())
	...
	def fetch(self, preamble=repcap.Preamble.Default) -> ResultData:
		...
		Snippet: value: ResultData = driver.niotMeas.prach.modulation.preamble.fetch(preamble = repcap.Preamble.Default) \n
		Return the single value results of the EVM vs Preamble and Power vs Preamble squares, for a selected preamble. See also
		'Square EVM vs Preamble, Power vs Preamble'. \n
			:param preamble: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Preamble')
			:return: structure: for return value, see the help for ResultData structure arguments."""
		preamble_cmd_val = self._cmd_group.get_repcap_cmd_value(preamble, repcap.Preamble)
		return self._core.io.query_struct(f'FETCh:NIOT:MEASurement<Instance>:PRACh:MODulation:PREamble{preamble_cmd_val}?', self.__class__.ResultData())
