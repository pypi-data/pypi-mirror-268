...
class RsCMPX_NiotMeas:
	...
	@classmethod
	def from_existing_session(cls, session: object, options: str = None) -> 'RsCMPX_NiotMeas':
	...
	def __str__(self) -> str:
		if self._core.io:
			return f"RsCMPX_NiotMeas session '{self._core.io.resource_name}'"
		else:
			return f"RsCMPX_NiotMeas with session closed"
	...
	def niotMeas(self):
		...
		if not hasattr(self, '_niotMeas'):
			from .Implementations.NiotMeas import NiotMeasCls
	...
	def clone(self) -> 'RsCMPX_NiotMeas':
		"""Creates a deep copy of the RsCMPX_NiotMeas object. Also copies:
		...
		cloned = RsCMPX_NiotMeas.from_existing_session(self.get_session_handle(), self._options)
