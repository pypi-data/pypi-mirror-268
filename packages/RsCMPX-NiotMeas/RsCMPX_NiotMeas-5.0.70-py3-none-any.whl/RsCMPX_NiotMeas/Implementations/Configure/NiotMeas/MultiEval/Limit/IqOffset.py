...
class IqOffsetCls:
	...
	def set(self, enable: bool, offset_1: float, offset_2: float, offset_3: float) -> None:
		...
		Snippet: driver.configure.niotMeas.multiEval.limit.iqOffset.set(enable = False, offset_1 = 1.0, offset_2 = 1.0, offset_3 = 1.0) \n
		Defines upper limits for the I/Q origin offset. Three different limits can be set for three TX power ranges. For details,
		see 'Modulation limits: I/Q offset'. \n
		...
		self._core.io.write(f'CONFigure:NIOT:MEASurement<Instance>:MEValuation:LIMit:IQOFfset {param}'.rstrip())
	...
	def get(self) -> IqOffsetStruct:
		"""SCPI: CONFigure:NIOT:MEASurement<Instance>:MEValuation:LIMit:IQOFfset \n
		Snippet: value: IqOffsetStruct = driver.configure.niotMeas.multiEval.limit.iqOffset.get() \n
		Defines upper limits for the I/Q origin offset. Three different limits can be set for three TX power ranges. For details,
		see 'Modulation limits: I/Q offset'. \n
			:return: structure: for return value, see the help for IqOffsetStruct structure arguments."""
		return self._core.io.query_struct(f'CONFigure:NIOT:MEASurement<Instance>:MEValuation:LIMit:IQOFfset?', self.__class__.IqOffsetStruct())
