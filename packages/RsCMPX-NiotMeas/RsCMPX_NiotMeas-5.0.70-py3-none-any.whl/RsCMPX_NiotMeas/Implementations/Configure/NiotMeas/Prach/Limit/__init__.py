...
class LimitCls:
	...
	def get_freq_error(self) -> float or bool:
		...
		Snippet: value: float or bool = driver.configure.niotMeas.prach.limit.get_freq_error() \n
		Defines an upper limit for the carrier frequency error. \n
			:return: frequency_error: (float or boolean) No help available
		"""
		response = self._core.io.query_str('CONFigure:NIOT:MEASurement<Instance>:PRACh:LIMit:FERRor?')
		return Conversions.str_to_float_or_bool(response)

	def set_freq_error(self, frequency_error: float or bool) -> None:
		"""SCPI: CONFigure:NIOT:MEASurement<Instance>:PRACh:LIMit:FERRor \n
		Snippet: driver.configure.niotMeas.prach.limit.set_freq_error(frequency_error = 1.0) \n
		Defines an upper limit for the carrier frequency error. \n
			:param frequency_error: (float or boolean) No help available
		"""
		param = Conversions.decimal_or_bool_value_to_str(frequency_error)
		self._core.io.write(f'CONFigure:NIOT:MEASurement<Instance>:PRACh:LIMit:FERRor {param}')
