...
class ModulationCls:
	...
	def get_ew_position(self) -> enums.LowHigh:
		...
		Snippet: value: enums.LowHigh = driver.configure.niotMeas.prach.modulation.get_ew_position() \n
		...
		"""
		response = self._core.io.query_str('CONFigure:NIOT:MEASurement<Instance>:PRACh:MODulation:EWPosition?')
		return Conversions.str_to_scalar_enum(response, enums.LowHigh)

	def set_ew_position(self, evm_window_pos: enums.LowHigh) -> None:
		"""SCPI: CONFigure:NIOT:MEASurement<Instance>:PRACh:MODulation:EWPosition \n
		Snippet: driver.configure.niotMeas.prach.modulation.set_ew_position(evm_window_pos = enums.LowHigh.HIGH) \n
		Specifies the position of the EVM window used for calculation of the trace results. \n
			:param evm_window_pos: No help available
		"""
		param = Conversions.enum_scalar_to_str(evm_window_pos, enums.LowHigh)
		self._core.io.write(f'CONFigure:NIOT:MEASurement<Instance>:PRACh:MODulation:EWPosition {param}')
