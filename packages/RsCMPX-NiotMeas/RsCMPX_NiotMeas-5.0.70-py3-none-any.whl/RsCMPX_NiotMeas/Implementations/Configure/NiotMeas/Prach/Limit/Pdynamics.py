...
class PdynamicsCls:
	...
	def set(self, enable: bool, on_power_upper: float, on_power_lower: float, off_power_upper: float) -> None:
		...
		Snippet: driver.configure.niotMeas.prach.limit.pdynamics.set(enable = False, on_power_upper = 1.0, on_power_lower = 1.0, off_power_upper = 1.0) \n
		...
		self._core.io.write(f'CONFigure:NIOT:MEASurement<Instance>:PRACh:LIMit:PDYNamics {param}'.rstrip())
	...
	def get(self) -> PdynamicsStruct:
		"""SCPI: CONFigure:NIOT:MEASurement<Instance>:PRACh:LIMit:PDYNamics \n
		Snippet: value: PdynamicsStruct = driver.configure.niotMeas.prach.limit.pdynamics.get() \n
		Defines limits for the ON power and OFF power determined with the power dynamics measurement. \n
			:return: structure: for return value, see the help for PdynamicsStruct structure arguments."""
		return self._core.io.query_struct(f'CONFigure:NIOT:MEASurement<Instance>:PRACh:LIMit:PDYNamics?', self.__class__.PdynamicsStruct())
