...
class EwLengthCls:
	...
	def get_value(self) -> List[int]:
		...
		Snippet: value: List[int] = driver.configure.niotMeas.prach.modulation.ewLength.get_value() \n
		Specifies the EVM window length in samples for all preamble formats. \n
			:return: evm_window_length: No help available
		"""
		response = self._core.io.query_bin_or_ascii_int_list('CONFigure:NIOT:MEASurement<Instance>:PRACh:MODulation:EWLength?')
		return response

	def set_value(self, evm_window_length: List[int]) -> None:
		"""SCPI: CONFigure:NIOT:MEASurement<Instance>:PRACh:MODulation:EWLength \n
		Snippet: driver.configure.niotMeas.prach.modulation.ewLength.set_value(evm_window_length = [1, 2, 3]) \n
		Specifies the EVM window length in samples for all preamble formats. \n
			:param evm_window_length: No help available
		"""
		param = Conversions.list_to_csv_str(evm_window_length)
		self._core.io.write(f'CONFigure:NIOT:MEASurement<Instance>:PRACh:MODulation:EWLength {param}')
