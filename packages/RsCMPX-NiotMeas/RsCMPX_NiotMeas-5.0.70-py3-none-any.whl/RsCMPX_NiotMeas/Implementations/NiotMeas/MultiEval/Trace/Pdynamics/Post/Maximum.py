...
class MaximumCls:
	...
	def read(self) -> List[float]:
		...
		Snippet: value: List[float] = driver.niotMeas.multiEval.trace.pdynamics.post.maximum.read() \n
		...
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'READ:NIOT:MEASurement<Instance>:MEValuation:TRACe:PDYNamics:POST:MAXimum?', suppressed)
		return response

	def fetch(self) -> List[float]:
		"""SCPI: FETCh:NIOT:MEASurement<Instance>:MEValuation:TRACe:PDYNamics:POST:MAXimum \n
		Snippet: value: List[float] = driver.niotMeas.multiEval.trace.pdynamics.post.maximum.fetch() \n
		Return the values of the right power dynamics trace (end of last allocated RU) . The results of the current, average and
		maximum traces can be retrieved. See also 'Square Power Dynamics'. \n
		Suppressed linked return values: reliability \n
			:return: power: Comma-separated list of 1408 power values The 705th value refers to the time 0 µs, the end of the last allocated RU. The other details depend on the subcarrier spacing, see table."""
		suppressed = ArgSingleSuppressed(0, DataType.Integer, False, 1, 'Reliability')
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'FETCh:NIOT:MEASurement<Instance>:MEValuation:TRACe:PDYNamics:POST:MAXimum?', suppressed)
		return response
