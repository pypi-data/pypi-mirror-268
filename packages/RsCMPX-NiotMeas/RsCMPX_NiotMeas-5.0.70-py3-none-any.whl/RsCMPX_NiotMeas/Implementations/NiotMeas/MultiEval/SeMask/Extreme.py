...
class ExtremeCls:
	...
	def read(self) -> ResultData:
		...
		Snippet: value: ResultData = driver.niotMeas.multiEval.seMask.extreme.read() \n
		...
		return self._core.io.query_struct(f'READ:NIOT:MEASurement<Instance>:MEValuation:SEMask:EXTReme?', self.__class__.ResultData())

	def fetch(self) -> ResultData:
		"""SCPI: FETCh:NIOT:MEASurement<Instance>:MEValuation:SEMask:EXTReme \n
		Snippet: value: ResultData = driver.niotMeas.multiEval.seMask.extreme.fetch() \n
		Return the extreme single value results of the spectrum emission measurement. The values described below are returned by
		FETCh and READ commands. CALCulate commands return limit check results instead, one value for each result listed below. \n
			:return: structure: for return value, see the help for ResultData structure arguments."""
		return self._core.io.query_struct(f'FETCh:NIOT:MEASurement<Instance>:MEValuation:SEMask:EXTReme?', self.__class__.ResultData())
	...
	class CalculateStruct(StructBase):
		...
		__meta_args_list = [
			...
			ArgStruct.scalar_enum('Tx_Power_Min', enums.ResultStatus2),
			ArgStruct.scalar_enum('Tx_Power_Max', enums.ResultStatus2)]
		...
		def __init__(self):
			...
			self.Tx_Power_Min: enums.ResultStatus2 = None
			self.Tx_Power_Max: enums.ResultStatus2 = None

	def calculate(self) -> CalculateStruct:
		"""SCPI: CALCulate:NIOT:MEASurement<Instance>:MEValuation:SEMask:EXTReme \n
		Snippet: value: CalculateStruct = driver.niotMeas.multiEval.seMask.extreme.calculate() \n
		Return the extreme single value results of the spectrum emission measurement. The values described below are returned by
		FETCh and READ commands. CALCulate commands return limit check results instead, one value for each result listed below. \n
			:return: structure: for return value, see the help for CalculateStruct structure arguments."""
		return self._core.io.query_struct(f'CALCulate:NIOT:MEASurement<Instance>:MEValuation:SEMask:EXTReme?', self.__class__.CalculateStruct())
