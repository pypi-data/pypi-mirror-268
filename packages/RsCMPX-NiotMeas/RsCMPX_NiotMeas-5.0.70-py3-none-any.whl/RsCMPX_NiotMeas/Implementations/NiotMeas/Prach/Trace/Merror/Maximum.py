...
class MaximumCls:
	...
	def read(self) -> List[float]:
		...
		Snippet: value: List[float] = driver.niotMeas.prach.trace.merror.maximum.read() \n
		...
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'READ:NIOT:MEASurement<Instance>:PRACh:TRACe:MERRor:MAXimum?', suppressed)
		return response

	def fetch(self) -> List[float]:
		"""SCPI: FETCh:NIOT:MEASurement<Instance>:PRACh:TRACe:MERRor:MAXimum \n
		Snippet: value: List[float] = driver.niotMeas.prach.trace.merror.maximum.fetch() \n
		Return the values of the magnitude error traces. Each value is averaged over the samples in one preamble symbol.
		The results of the current, average and maximum traces can be retrieved. See also 'Squares EVM, Magnitude Error, Phase
		Error'. \n
		Suppressed linked return values: reliability \n
			:return: results: Comma-separated list of 20 results, one result per symbol"""
		suppressed = ArgSingleSuppressed(0, DataType.Integer, False, 1, 'Reliability')
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'FETCh:NIOT:MEASurement<Instance>:PRACh:TRACe:MERRor:MAXimum?', suppressed)
		return response
