...
class PgxSession(PgxContextManager):
    ...
    def read_graph_by_name(
        ...
        :param options: Tuple of read graph options as strings. The possible options are in the list
                        below.
    ...
    def read_subgraph_from_pg_pgql(
        ...
        reader = java_handler(generic_reader.fromPgPgql, [view])
        return self._read_subgraph_with_pgql(reader, queries=queries, config=config,
                                             num_connections=num_connections,
                                             data_source_id=data_source_id, jdbc_url=jdbc_url,
                                             keystore_alias=keystore_alias, owner=owner,
                                             password=password, schema=schema, username=username,
    ...
    def read_subgraph_from_pg_sql(
        ...
        """
        generic_reader = java_handler(self._session.readSubgraph, [])
        reader = java_handler(generic_reader.fromPgSql, [sql_graph_name])
        return self._read_subgraph_with_pgql(reader, queries=queries, config=config,
                                             num_connections=num_connections,
                                             data_source_id=data_source_id, jdbc_url=jdbc_url,
                                             keystore_alias=keystore_alias, owner=owner,
    ...
    def _read_subgraph_with_pgql(
        ...
        if owner is not None:
            java_handler(reader.owner, [owner])
        if password is not None:
            java_handler(reader.password, [password])
        if schema is not None:
            java_handler(reader.schema, [schema])
