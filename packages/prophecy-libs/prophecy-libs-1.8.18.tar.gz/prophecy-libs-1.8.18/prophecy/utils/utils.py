...
class ProphecyDataFrame:
    ...
    def apply_expression(self, userExpression, selectedColumnNames, castDataType, addPrefix, addSuffix, sparkSession) -> DataFrame:
        result = self.extended_dataframe.apply_expression(
            userExpression, createScalaList(self.spark, selectedColumnNames), createScalaOption(self.spark, castDataType), addPrefix, addSuffix, sparkSession._jsparkSession
        )
        return DataFrame(result, self.sqlContext)
