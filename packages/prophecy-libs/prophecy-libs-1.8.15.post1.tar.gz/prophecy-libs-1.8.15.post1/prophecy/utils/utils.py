        ...
        def wrapper(f):
            try:
                ...
                try:
                    ...
                    if len(spark.streams.active) > 0:  # if there are active streams, wait for them to finish
...
def extractHierarchicalGemName(stack, function) -> str:
    ...
    return ".".join(stack_function_names[start_index + 1:end_index] + [function.__name__])


def sendGemProgressEvent(spark: SparkSession, userSession: str, gemName: str, state: str,
                         startTime: str, endTime: str = ""):
    spark.sparkContext._jvm.org.apache.spark.sql.ProphecySparkSession.sendGemProgressEvent(
        spark._jsparkSession, userSession, gemName, state, startTime, endTime
    )


def withInstrumentation():
    def decorator(function):
        def inner_wrapper(spark, *args, **kwargs):
            global interimConfig
            userSession = interimConfig.interimOutput.session()
            gemName = extractHierarchicalGemName(inspect.stack(), function)
            startTime = int(time.time() * 1000)
            state = TaskState.LAUNCHING
            sendGemProgressEvent(spark, userSession, gemName, state.value.__str__(), str(startTime))

            try:
                state = TaskState.RUNNING
                sendGemProgressEvent(spark, userSession, gemName, state.value.__str__(),
                                     str(startTime))
