...
from saleboxdjango.lib.common import (
    get_rating_display,
    json_to_datetime_local,
    json_to_datetime_utc,
)
...
class Analytic(models.Model):
    ...
    ip_country = models.CharField(
        max_length=5, blank=True, null=True, verbose_name="Country"
    )
    ip_city = models.CharField(
        max_length=32, blank=True, null=True, verbose_name="City"
    )
    ip_tz = models.CharField(
        max_length=32, blank=True, null=True, verbose_name="Timezone"
    )
    ip_lat = models.DecimalField(
        max_digits=9, decimal_places=6, blank=True, null=True, verbose_name="Latitude"
    )
    ip_lng = models.DecimalField(
        max_digits=9, decimal_places=6, blank=True, null=True, verbose_name="Longitude"
    )
    ua_browser_family = models.CharField(
        max_length=32, blank=True, null=True, verbose_name="Browser family"
    )
    ua_browser_version = models.CharField(
        max_length=20, blank=True, null=True, verbose_name="Browser version"
    )
    ua_os_family = models.CharField(
        max_length=20, blank=True, null=True, verbose_name="OS family"
    )
    ua_os_version = models.CharField(
        max_length=20, blank=True, null=True, verbose_name="OS version"
    )
    ua_device_family = models.CharField(
        max_length=32, blank=True, null=True, verbose_name="Device family"
    )
    ua_device_brand = models.CharField(
        max_length=20, blank=True, null=True, verbose_name="Device brand"
    )
    ua_device_model = models.CharField(
        max_length=20, blank=True, null=True, verbose_name="Device model"
    )
...
class Attribute(models.Model):
    ...
    last_update_local = models.DateTimeField(auto_now=True)
...
class AttributeItem(models.Model):
    ...
    extras = models.JSONField(default=dict, blank=True, db_index=True)
...
class CheckoutStoreManager(models.Manager):
    def apply_timeout(self):
        ...
        gw = sb.get("GATEWAY", {})
        to = gw.get("TIMEOUT", {})
        default_timeout = to.get("DEFAULT", 60 * 60 * 24 * 3)
        payment_methods = to.get("PAYMENT_METHODS", {})

        # perform default timeout
        cutoff = now() - datetime.timedelta(seconds=default_timeout)
        checkouts_closed = (
            CheckoutStore.objects.filter(status__lt=30)
        ...
        try:
            ...
            open_checkouts = CheckoutStore.objects.filter(status__lt=30).filter(
                created__lt=cutoff
            )
    ...
    def update_checked_out_stock(self):
        ...
        stores = CheckoutStore.objects.filter(status__lt=31)
        for store in stores:
        ...
        variants = ProductVariant.objects.filter(id__in=list(stock.keys()))
        for pv in variants:
            if pv.stock_checked_out != stock[pv.id]:
                pv.set_stock_checked_out(stock[pv.id])

        # update the zero quantities in the DB
        variants = ProductVariant.objects.exclude(id__in=list(stock.keys())).exclude(
            stock_checked_out=0
        )
        for pv in variants:
...
class ContentKeyValueStore(models.Model):
...
class ContentPage(models.Model):
    ...
    parent = models.ForeignKey(
        "ContentPage", on_delete=models.CASCADE, blank=True, null=True
    )
    ...
    parent_ids = models.CharField(max_length=255, blank=True, null=True)
    ...
    def __str__(self):
        return self.combined_slug


class ContentPageItem(models.Model):
...
class DiscountGroup(models.Model):
    GROUP_TYPE_CHOICES = (("S", "Seasonal"), ("M", "Manual"))
    FIELD_CHOICES = (
        (0, "Inactive"),
...
class DiscountRuleset(models.Model):
    TYPE_CHOICES = (("flat_percent", "Flat Percentage"),)
    group = models.ForeignKey(
        DiscountGroup, blank=True, null=True, on_delete=models.CASCADE
    )
    name = models.CharField(max_length=25)
    discount_type = models.CharField(
        default="flat_percent", max_length=12, choices=TYPE_CHOICES
    )
...
class Event(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if self.id is None:
            duplicate = (
                Event.objects.filter(event=self.event)
...
class Member(models.Model):
    ...
    group = models.ForeignKey(
        MemberGroup, null=True, blank=True, on_delete=models.CASCADE
    )
    parent = models.ForeignKey(
    ...
    salebox_member_id = models.UUIDField(
        blank=True, db_index=True, editable=True, null=True
    )
    ...
    country_state = models.ForeignKey(
        CountryState, blank=True, null=True, on_delete=models.CASCADE
    )
    ...
    salebox_transactionhistory_request_dt = models.DateTimeField(
        default=datetime.datetime.now, blank=True, null=True
    )
    salebox_transactionhistory_count = models.IntegerField(default=0)
    salebox_transactionhistory_data = models.JSONField(blank=True, null=True)
    ...
    def transactionhistory_get_data(self):
        ...
        for t in transactions:
            ...
            for b in t["basket"]:
                try:
                    pv = (
                        ProductVariant.objects.filter(id=b["product_variant"]["id"])
...
class ProductCategory(MPTTModel):
    ...
    # local fields
    local_image_version = models.CharField(max_length=70, blank=True, null=True)
    ...
    def get_trans_for_lang(
        self, lang, field, fallback_to_primary=True, non_primary_fallback=None
    ):
    ...
    def get_trans(self, field, fallback_to_primary=True, non_primary_fallback=None):
        return self.get_trans_for_lang(
            get_language(), field, fallback_to_primary, non_primary_fallback
        )
...
class Product(models.Model):
    ...
    attribute_1 = models.ManyToManyField(
        AttributeItem, related_name="product_attr_1", blank=True
    )
    attribute_2 = models.ManyToManyField(
        AttributeItem, related_name="product_attr_2", blank=True
    )
    attribute_3 = models.ManyToManyField(
        AttributeItem, related_name="product_attr_3", blank=True
    )
    attribute_4 = models.ManyToManyField(
        AttributeItem, related_name="product_attr_4", blank=True
    )
    attribute_5 = models.ManyToManyField(
        AttributeItem, related_name="product_attr_5", blank=True
    )
    attribute_6 = models.ManyToManyField(
        AttributeItem, related_name="product_attr_6", blank=True
    )
    attribute_7 = models.ManyToManyField(
        AttributeItem, related_name="product_attr_7", blank=True
    )
    attribute_8 = models.ManyToManyField(
        AttributeItem, related_name="product_attr_8", blank=True
    )
    attribute_9 = models.ManyToManyField(
        AttributeItem, related_name="product_attr_9", blank=True
    )
    attribute_10 = models.ManyToManyField(
        AttributeItem, related_name="product_attr_10", blank=True
    )
    ...
    website_json = models.JSONField(default=dict)
    ...
    def update_rating(self):
        variant_ids = ProductVariant.objects.filter(product=self).values_list(
            "id", flat=True
        )

        self.rating_score = 0
        self.rating_vote_count = ProductVariantRating.objects.filter(
            variant__in=variant_ids
        ).count()

        if self.rating_vote_count > 0:
            self.rating_score = ProductVariantRating.objects.filter(
                variant__in=variant_ids
...
class ProductVariant(models.Model):
    ...
    size_uom = models.CharField(
        max_length=2, choices=SIZE_UOM_CHOICES, blank=True, default=""
    )
    ...
    attribute_1 = models.ManyToManyField(
        AttributeItem, related_name="variant_attr_1", blank=True
    )
    attribute_2 = models.ManyToManyField(
        AttributeItem, related_name="variant_attr_2", blank=True
    )
    attribute_3 = models.ManyToManyField(
        AttributeItem, related_name="variant_attr_3", blank=True
    )
    attribute_4 = models.ManyToManyField(
        AttributeItem, related_name="variant_attr_4", blank=True
    )
    attribute_5 = models.ManyToManyField(
        AttributeItem, related_name="variant_attr_5", blank=True
    )
    attribute_6 = models.ManyToManyField(
        AttributeItem, related_name="variant_attr_6", blank=True
    )
    attribute_7 = models.ManyToManyField(
        AttributeItem, related_name="variant_attr_7", blank=True
    )
    attribute_8 = models.ManyToManyField(
        AttributeItem, related_name="variant_attr_8", blank=True
    )
    attribute_9 = models.ManyToManyField(
        AttributeItem, related_name="variant_attr_9", blank=True
    )
    attribute_10 = models.ManyToManyField(
        AttributeItem, related_name="variant_attr_10", blank=True
    )
    active_flag = models.BooleanField(default=True)
    ...
    def save(self, *args, **kwargs):
        ...
        pvi = (
            ProductVariantImage.objects.filter(variant=self)
    ...
    def set_stock_checked_out(self, qty=0):
        if self.id:
            ProductVariant.objects.filter(id=self.id).update(
                stock_checked_out=qty, stock_total=max((self.stock_count - qty), 0)
            )
...
class SaleboxUser(AbstractUser):
    ...
    salebox_member_sync = models.JSONField(blank=True, null=True)
    ...
    def get_member(self):
        ...
        return Member.objects.filter(salebox_member_id=self.salebox_member_id).first()
...
class SyncLog(models.Model):
    ...
    status = models.CharField(choices=STATUS_CHOICES, default="INFO", max_length=7)
...
class SyncQueue(models.Model):
    model_id = models.CharField(max_length=36)  # IDs can be UUIDs
