...
class SaleboxBasket:
    ...
    def update_basket(self, request, variant, qty, relative, **kwargs):
        if isinstance(variant, int):
            variant = ProductVariant.objects.get(id=variant)
        ...
        if items["basket"] is None:
            if items["wishlist"] is None:
                # create basket item
                if int(qty) > 0:
                    self._add_variant(request, variant, qty, True, **kwargs)
            else:
    ...
    def _add_variant(self, request, variant, qty, basket, **kwargs):
        bwl = BasketWishlist(
            variant=variant,
            quantity=int(qty),
            weight=int(kwargs["weight"]) if "weight" in kwargs else 0,
            basket_flag=basket,
        )
    ...
    def _calculate_price(self, request):
        ...
        for i in self.data["basket"]["items"]:
            if i.get("weight", 0) == 0:
                ...
                weight_modifier = i["weight"]

            # price for this row item
            i["variant"]["qty_price"] = (
                i["variant"]["price"] * i["qty"] * weight_modifier
            )
            i["variant"]["qty_sale_price"] = (
                i["variant"]["sale_price"] * i["qty"] * weight_modifier
            )

            # total basket price
            self.data["basket"]["orig_price"] += i["variant"]["qty_price"]
            self.data["basket"]["sale_price"] += i["variant"]["qty_sale_price"]
            if i["variant"]["preorder_flag"]:
                self.data["basket"]["orig_price_preorder"] += i["variant"]["qty_price"]
                self.data["basket"]["sale_price_preorder"] += i["variant"][
                    "qty_sale_price"
                ]
            else:
                self.data["basket"]["orig_price_onshelf"] += i["variant"]["qty_price"]
                self.data["basket"]["sale_price_onshelf"] += i["variant"][
                    "qty_sale_price"
                ]
        ...
        for q in qs:
            pv = {
                ...
                "product": {
                    ...
                    "website_json": q.variant.product.website_json,
                },
