...
##### 2. Merchant config:
   ```python
    ...
    MobioAdminSDK().gen_jwt_anonymous_user(merchant_id: str, data_jwt: dict)
    """
    :param merchant_id: 
    :param data_jwt: dict
    là dữ liệu sẽ được đóng gói vào trong jwt, sau này từ request của client có thể đọc thông tin này.
    trong data_jwt bắt buộc có thông tin này:
    "permissions": {"/path/api/v1.0/submit-form": ["post"],
                    "/tag/api/v1.0/tags/(.*)/list": ["get"]
                    }
    giá trị permissions là object chứa tất cả các path api và phương thức(viết thường) mà jwt này sẽ được thao tác, module chủ động cấu hình thông tin này.
    :return: string jwt 
    """
