    ...
    def find(
        ...
        **kwargs,
        ...
        Args:
            ...
            status: final status to match (FINISHED/FAILED)
        ...
        if status is not None:
            extra_filters.append(f'?mex/@value_str = "{status}"')
        ...
        where = f"""
                ...
                {extra_patterns}
                FILTER( ?mod/@name = "{module_name}" AND
                        ?bld/@name = "{version}"
                        {extra_filters} )
            """
...
class VQBuild(VQModule):
    ...
    def last_run(
        ...
        **kwargs,
        ...
        res = self._sess.find("mex", module_name=module_name, version=version, status=_last_run_status, _execute_options=_execute_options, **kwargs)
        ...
        if len(add_iterables) > 0 or _keep_log or _execute_options:
            ...
            if _execute_options:
                for key, val in _execute_options.items():
                    ex_opt_tag.add_tag(key, value=val)
