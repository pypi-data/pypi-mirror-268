...
def addparameters():
	...
	parser.add_argument('--paramids', type=int, nargs='+', help='Manually specify the to-be-tested parameter IDs')
	parser.add_argument('--overwritepar', action='store_true', help='Overwrite the .par file with the best fit')
...
def addparameters_core(par, lin, VIB_DIGITS=1, ALL_STATES=9, qnu='qnu4', qnl='qnl4', parameters=None, skipfixed=False, skipinterstate=False, skiprotational=False, skipglobal=False, newinteraction=None, initialvalues=None, skipsavebest=False, parameters_to_test=None, overwritepar=False, report=True):
	...
	for i, stats in enumerate(results):
		...
		prit(report, f"{id:14} | {rms:12.2f} | {rejected_lines:8.0f} | {diverging:10} | {initial:10.2e} | {final:10.2e} ")
