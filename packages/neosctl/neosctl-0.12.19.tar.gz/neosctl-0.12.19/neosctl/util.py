...
def dumps_formatted_json(
    payload: typing.Union[dict, list[dict], pydantic.BaseModel, list[pydantic.BaseModel]],
    *,
    sort_keys: bool = True,
) -> str:
    """Dump formatted json.
...
def prettify_json(payload: typing.Union[dict, list], *, sort_keys: bool = True) -> str:
...
def prettify_yaml(payload: typing.Union[dict, list], *, sort_keys: bool = True) -> str:  # noqa: ARG001
...
def prettify_toml(payload: typing.Union[dict, list], *, sort_keys: bool = True) -> str:  # noqa: ARG001
...
def tabulate_data(payload: typing.Union[dict, list], *, sort_keys: bool = True) -> str:
    ...
    if not isinstance(payload, list):
        ...
        if isinstance(payload[0], pydantic.BaseModel):
            headers = (k.title() for k in payload[0].model_dump())
        else:
...
render_callables: dict[str, typing.Callable[[typing.Union[dict, list], bool], str]] = {
    "json": prettify_json,
    "yaml": prettify_yaml,
    "toml": prettify_toml,
    "text": tabulate_data,
...
def process_payload(payload: typing.Union[dict, list], output_format: str = "json", *, sort_keys: bool = True) -> str:
...
def process_response(
    ...
    render_callable: typing.Union[typing.Callable[[typing.Union[dict, list]], str], None] = None,
    data_key: typing.Union[str, None] = None,
    *,
    sort_keys: bool = True,
) -> None:
...
def check_refresh_token_exists(ctx: typer.Context) -> bool:
    ...
    if ctx.obj.active_env:
        if not ctx.obj.active_env.refresh_token:
            raise exit_with_output(
...
def update_profile(
    ...
    return schema.Profile(
        ...
        hub_api_url=ctx.obj.hub_api_url,
...
def check_profile_exists(ctx: typer.Context) -> bool:
    """Check if a profile (or env) exists in neosctl configuration or exit."""
    if ctx.obj.active_env:
        return True
...
def upsert_credential(
    ...
    name = name or ctx.obj.profile_name
...
def load_json_file(fp: pathlib.Path, content_type: str) -> typing.Union[dict, list]:
...
class GenericHttpClient(base.NeosClient):
    ...
    def __init__(  # noqa: D107
        ...
        # TODO(edgy): Replace with better error
...
def user_profile_callback(
    ...
    user_profile = get_user_profile(ctx.obj.config, ctx.obj.profile_name, allow_missing=ctx.obj.active_env is not None)
...
def extract_account(
...
) -> tuple[dict, typing.Optional[dict]]:
    """Set up account header and params."""
    profile_account = ctx.obj.account
    ...
    headers, params = {"X-Account": profile_account}, None
    if account:
