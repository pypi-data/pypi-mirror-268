...
def list_envs(
    ctx: typer.Context,
    _output: constant.Output = typer.Option(
        "text",
        "--output",
        "-o",
        help="Output format",
        callback=util.output_format_callback,
    ),
    _verbose: int = util.Verbosity,
) -> None:
    ...
    raise util.exit_with_output(
        msg=util.process_payload(data, ctx.obj.output_format),
    )
...
def view(
    ...
    env = util.get_env_section(ctx.obj.env, name)
    raise util.exit_with_output(
        msg=util.process_payload(env.model_dump(mode="json"), ctx.obj.output_format),
    )
...
def active(
    ...
    raise util.exit_with_output(
        msg=util.process_payload(ctx.obj.active_env.model_dump(mode="json"), ctx.obj.output_format),
    )
...
def list_cores(
    ...
    raise util.exit_with_output(
        msg=util.process_payload(payload, ctx.obj.output_format),
    )
