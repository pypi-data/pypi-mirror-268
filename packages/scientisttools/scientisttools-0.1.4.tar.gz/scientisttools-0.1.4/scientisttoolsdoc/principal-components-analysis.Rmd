---
title: '**ACP sous Python avec scientisttools**'
...
Ce tutoriel a pour objectif de présenter rapidement les principales fonctionnalités offertes par le package \og scientisttools \fg{} pour réaliser une Analyse en Composantes Principales. 
...
res_pca = PCA(normalize=True,
              n_components = None,
              row_labels=actif.index,
              col_labels=actif.columns,
              row_sup_labels=None,
...
\begin{itemize}
\item \texttt{normalize} :  pour choisir de réduire ou non les variables
\item \texttt{n\_components} : le nombre de dimensions à garder dans les résultats
\item \texttt{row\_labels} : les noms des lignes actives
\item \texttt{col\_labels} : les noms des colonnes actives
\item \texttt{row\_sup\_labels} : les nombes des lignes supplémentaires
\item \texttt{quanti\_sup\_labels} : les noms des colonnes quantitatives supplémentaires
\item \texttt{quali\_sup\_labels} : les noms des colonnes qualitatives supplémentaires
...
print(fviz_eigenvalue(res_pca,choice="eigenvalue"))
```

```{python}
print(fviz_eigenvalue(res_pca,choice="proportion"))
```
...
plotPCA(res_pca,choice="var",color="blue",quanti_sup=True,ax=axe,repel=True)
plt.show()
```
...
```{python}
fig, axe = plt.subplots(figsize=(10,10))
plotPCA(res_pca,repel=True,ax=axe,random_state=123,habillage="Competition")
plt.show()
```
...
```{python}
# Correlation Variable - Dim 1
from scipy.stats import pearsonr
corDim1 = pd.DataFrame(columns=["statistic","pvalue"]).astype("float")
for i, col in enumerate(decathlon.columns):
  if(decathlon.iloc[:,i].dtypes in ["float64","int64","float32","int32"]):
    res = pearsonr(decathlon.iloc[:,i],row["coord"]["Dim.1"])
    row_RD = pd.DataFrame({"statistic" : res.statistic,"pvalue":res.pvalue},
                          index = [col])
    corDim1 = pd.concat([corDim1,row_RD])
# Tri décroissant et significatif
corDim1 = (corDim1.query('pvalue < 0.05')
                  .sort_values(by="statistic",ascending=False))
corDim1
```

```{python}
# Correlation Variable - Dim 2
corDim2 = pd.DataFrame(columns=["statistic","pvalue"]).astype("float")
for i, col in enumerate(decathlon.columns):
  if(decathlon.iloc[:,i].dtypes in ["float64","int64","float32","int32"]):
    res = pearsonr(decathlon.iloc[:,i],row["coord"]["Dim.2"])
    row_RD = pd.DataFrame({"statistic" : res.statistic,"pvalue":res.pvalue},
                          index = [col])
    corDim2 = pd.concat([corDim2,row_RD])
# Tri décroissant et significatif
corDim2 = (corDim2.query('pvalue < 0.05')
                  .sort_values(by="statistic",ascending=False))
corDim2
```
...
```{python}
from scientisttools.extractfactor import get_pca_ind,get_pca_var,get_eig
eig = get_eig(res_pca)
```
...
print(fviz_cosines(res_pca,choice="ind",axis=0,top_cos2=10))
```

```{python,out.width="85%"}
# Classement des points colonnes en fonction de leur cos2 sur le 1er axe
print(fviz_cosines(res_pca,choice="var",axis=0))
```
...
```{python}
res_pca.transform(actif)[:5,:]
```


```{python}
res_pca.fit_transform(decathlon)[:5,:]
```
