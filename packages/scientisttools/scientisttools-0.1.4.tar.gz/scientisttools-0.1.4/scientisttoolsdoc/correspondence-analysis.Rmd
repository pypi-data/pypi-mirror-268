---
title: '**AFC sous Python avec scientisttools**'
...
Ce tutoriel a pour objectif de présenter rapidement les principales fonctionnalités offertes par le package \og scientisttools \fg{} pour réaliser une Analyse Factorielle des Correspondances. 
...
my_ca_fa = CA(n_components=None,
               row_labels=wfemmes.index,
               col_labels=wfemmes.columns).fit(wfemmes.values)
my_ca_fa.mapping(num_x_axis=1, num_y_axis=2)
```
...
plot_contrib(my_ca,choice="ind",axis=0,top_contrib=10,ax=axe)
plt.show()
```
...
fig,axe = plt.subplots(figsize=(16,10))
plot_contrib(my_ca2,choice="var",axis=0,ax=axe)
plt.show()
```
...
\href{https://github.com/enfantbenidedieu/scientisttools/blob/master/notebooks/ca_example2.ipynb}{https://github.com/enfantbenidedieu/scientisttools/blob/master/notebooks/ca\_example2.ipynb}
