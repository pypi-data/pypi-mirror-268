...
def test_short_lvm():
    ...
    np.testing.assert_equal(data['data'][0, 1], 0.914018)
    ...
    data = read('./data/short_new_line_end.lvm',
                read_from_pickle=True, dump_file=False)
    np.testing.assert_equal(data['data'][0, 1], 0.914018)
...
def test_with_empty_fields_lvm():
    ...
    np.testing.assert_equal(data['data'][0, 7], -0.011923)


def test_with_multi_time_column_lvm():
    data = read('./data/multi_time_column.lvm',
                read_from_pickle=False, dump_file=False)
    np.testing.assert_allclose(data['data'][0],
                               np.array([0.000000, -0.035229,
                                         0.000000, 0.532608]))
...
def test_no_decimal_separator():
    data = read('./data/no_decimal_separator.lvm',
                read_from_pickle=False, dump_file=False)
    np.testing.assert_equal(data['data'][0, 1], -0.008807)


def test_several_comments():
    data = read('./data/with_comments.lvm',
                read_from_pickle=False, dump_file=False)
    np.testing.assert_equal(data['data'][0, 1], 1.833787)
