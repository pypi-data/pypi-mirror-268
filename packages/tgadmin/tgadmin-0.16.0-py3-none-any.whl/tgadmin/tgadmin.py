...
@click.option(
    "--threaded", help="use multithreading for crud requests (experimental, try without in case of errors)", is_flag=True
)
@pass_tgclient
def put_aggregation(client, project_id, aggregation_file, threaded):
    """upload an aggregation and referenced objects recursively"""

    click.echo(f"Starting import of {aggregation_file.name}. This may take some time. If you want so see tgadmin working try --verbose")
    start_time = time.time()

    try:
        res = aggregation_import(client.crud, client.sid, project_id, aggregation_file.name, threaded)
    except TextgridImportException as error:
        exit_with_error(error)

    time_format =time.strftime("%Hh%Mm%Ss", time.gmtime(time.time() - start_time))
    click.echo(f"Done: imported {res['objects_uploaded']} files in {time_format}. Find the imex file at {res['imex_location']}")
