...
def aggregation_import(tgcrud: TextgridCrud, sid: str, project_id: str, filename: str, threaded: bool):
    """
    ...
    tguri = handle_aggregation_upload(tgcrud, sid, project_id, filename, agg_meta, imex_map, threaded)
...
def handle_aggregation_upload(tgcrud, sid, project_id, filename, agg_meta, imex_map, threaded):
    ...
    if is_edition(agg_meta):
        ...
        tguri = upload_file(tgcrud, sid, project_id, work_path, imex_map, referenced_in=filename)
    ...
    if not threaded:
        for ore_aggregates in agg_xml_root.findall(".//ore:aggregates", NAMESPACES):
            _handle_upload_op(tgcrud, sid, project_id, filename, ore_aggregates, imex_map, threaded)
    else:
        with ThreadPoolExecutor(max_workers=10) as ex:
            futures = [
                ex.submit(_handle_upload_op, tgcrud, sid, project_id, filename, ore_aggregates, imex_map, threaded)
                for ore_aggregates in agg_xml_root.findall(".//ore:aggregates", NAMESPACES)
            ]
    ...
    tguri = upload_modified(tgcrud, sid, project_id, agg_xml_root, agg_meta, filename)
...
def _handle_upload_op(tgcrud, sid, project_id, filename, ore_aggregates, imex_map, threaded):
    ...
    if is_aggregation(meta):
        tguri = handle_aggregation_upload(tgcrud, sid, project_id, data_path, meta, imex_map, threaded)
    else:
        tguri = upload_file(tgcrud, sid, project_id, data_path, imex_map)

    # TODO: is this thread safe?
    ore_aggregates.set(RDF_RESOURCE, base_uri_from(tguri))  # update the xml with the uri


def upload_file(tgcrud, sid, project_id: str, data_path: PurePath, imex_map: dict, referenced_in: str = "" ) -> str:
    ...
    try:
        with open(data_path, "r") as the_data:
            ...
            for crudwarn in res.object_value.generic.generated.warning:
                log.warning(f" ⚠️ Warning from tgcrud for {data_path.name}: {crudwarn}")
...
def upload_modified(tgcrud, sid, project_id: str, etree_data, metadata, filename="") -> str:
