...
def test_auction_in_pbn():
    # hands only needed to create board
    hand_dict = {
        'N': Hand('A8765.QT.K9.AT87'),
        'E': Hand('J42.AJ7632.J.632'),
        'S': Hand('QT3.85.Q86.KQJ54'),
    ...
    board.dealer = 'W'
    bids = ['2H', 'P', 'P', 'D', 'P', 'P', 'R', 'P', 'P', 'P']
    board.auction.calls = [Call(bid) for bid in bids]
    pbn_list = board.board_to_pbn()

    assert '[Auction "W"]' in pbn_list
    assert '[Auction "N"]' not in pbn_list
    assert '2H Pass Pass X' in pbn_list
    assert 'Pass Pass XX Pass' in pbn_list
    assert 'Pass Pass' in pbn_list


def test_contract_in_pbn():
    board = Board()
    board.contract.name = '2H'
    pbn_list = board.board_to_pbn()
    assert '[Contract "2H"]' in pbn_list


def test_tricks_in_pbn():
    board = Board()
    trick = Trick()
    board.tricks.append(trick)
    trick.cards = [Card('2H'), Card('3H'), Card('AH')]
    trick.leader = 'E'
    pbn_list = board.board_to_pbn()
    assert '[Play "E"]' in pbn_list
    assert 'H2 H3 HA' in pbn_list
