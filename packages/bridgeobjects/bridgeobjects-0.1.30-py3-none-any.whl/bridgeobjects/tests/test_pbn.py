...
import datetime
import pytest
from ..src.file_operations import load_rbn, save_pbn, load_pbn
from ..src.card import Card

DATA_PATH = 'tests/test_data'
LOAD_PATH = Path(DATA_PATH, 'pbn_input_file_1.pbn')
events_loaded = load_pbn(LOAD_PATH)

# Create a pbn file from an rbn input to test saves
INPUT_PATH = Path(DATA_PATH, 'rbn_input_file_1.rbn')
...
LOAD_PATH = Path(DATA_PATH, 'pbn_input_3.pbn')
events_loaded_BBO = load_pbn(LOAD_PATH)


@pytest.mark.parametrize("events, expected", [
    (events_loaded_BBO, 1)
])
def test_event_counts_BBO(events, expected):
    """Ensure that the correct number of events are loaded and saved."""
    assert len(events) == expected
