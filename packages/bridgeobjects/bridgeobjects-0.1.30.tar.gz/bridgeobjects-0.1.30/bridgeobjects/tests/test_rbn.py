...
from ..src.file_operations import load_rbn, save_rbn, load_pbn

DATA_PATH = 'tests/test_data'
load_path = Path(DATA_PATH, 'rbn_input_file_1.rbn')
...
input_path = Path(DATA_PATH, 'pbn_input_file_1.pbn')
test_events = load_pbn(input_path)

# They are then saved in rbn format to a file at save_path.
save_path = Path(DATA_PATH, 'rbn_output_file_1.rbn')
