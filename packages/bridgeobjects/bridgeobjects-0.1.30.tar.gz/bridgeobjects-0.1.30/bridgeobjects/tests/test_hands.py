...
from ..src.suit import Suit, Spades, Hearts, Diamonds, Clubs
...
hand_list_6322 = ['JS', '8S', '5S', '4S', '3S', '2S',
                  'JH', '8H', '6H',
                  'AD', 'JD',
                  'KC', 'JC']

hand_list_6322_error_one = ['JS', '1S', '5S', '4S', '3S', '2S',
                  'JH', '8H', '6H',
                  'AD', 'JD',
                  'KC', 'JC']

hand_list_6322_error_two = ['JS', 66, '5S', '4S', '3S', '2S',
                  'JH', '8H', '6H',
                  'AD', 'JD',
                  'KC', 'JC']
...
hand_pbn_one_error_one = '.61.AKQJ87.A9732'
...
def test_hand_from_list_card_in_list():
    ...
    names = [card.name for card in Hand(hand_list_6322).cards]
    assert Card('4S').name in names
    assert Card('KC').name in names

def test_invalid_card_name_1():
    try:
        names = [card.name for card in Hand(hand_list_6322_error_one).cards]
    except ValueError as err:
        assert err.args[0] == '1S is not a valid card name'

def test_invalid_card_name_2():
    try:
        names = [card.name for card in Hand(hand_list_6322_error_two).cards]
    except TypeError as err:
        assert err.args[0] == 'Invalid card type for "66" (int)'
...
def test_hand_from_pbn_error():
    """Ensure that a hand created from a pbn string is correct: card in list."""
    try:
        names = [card.name for card in Hand(hand_pbn_one_error_one).cards]
    except ValueError as err:
        assert err.args[0] == '1 is not a valid card rank'
...
def test_hand_is_valid_str_list_invalid_item():
    ...
    hand_list = [card for card in hand_list_6322]
    hand_list[4] = Card('4S')
...
def test_hand_is_valid_card_list_invalid_item():
    """Ensure that a hand is_valid flag is correct for number of cards: from list."""
    hand_list = [Card(card) for card in hand_list_6322]
    hand_list[4] = '4S'
    with pytest.raises(TypeError) as exec_info:
        assert Hand(hand_list)
...
def test_hand_is_valid_list():
    """Ensure that a hand is_valid flag is correct for number of cards: from list."""
    assert Hand(hand_list_6322).is_valid
...
def test_hand_suit_cards_list_spades():
    ...
    assert Hand(hand_list_6322).spades == 6
...
def test_hand_suit_cards_list_hearts():
    ...
    assert Hand(hand_list_6322).hearts == 3
...
def test_hand_suit_cards_list_diamonds():
    ...
    assert Hand(hand_list_6322).diamonds == 2
...
def test_hand_suit_cards_list_clubs():
    ...
    assert Hand(hand_list_6322).clubs == 2
...
def test_hand_suit_holding():
    ...
    assert Hand(hand_list_6322).suit_holding['S'] == 6
    assert Hand(hand_list_6322).suit_holding['H'] == 3
    assert Hand(hand_list_6322).suit_holding['D'] == 2
    assert Hand(hand_list_6322).suit_holding['C'] == 2
    assert Hand(hand_list_6322).suit_holding[Spades()] == 6
    assert Hand(hand_list_6322).suit_holding[Suit('H')] == 3
    assert Hand(hand_list_6322).suit_holding[Diamonds()] == 2
    assert Hand(hand_list_6322).suit_holding[Suit('C')] == 2
...
def test_hand_shape_list_valid():
    ...
    assert Hand(hand_list_6322).shape == [6, 3, 2, 2]
...
def test_hand_honour_counts_list_aces():
    ...
    assert Hand(hand_list_6322).aces == 1
...
def test_hand_honour_counts_list_kings():
    ...
    assert Hand(hand_list_6322).kings == 1
...
def test_hand_honour_counts_list_queens():
    ...
    assert Hand(hand_list_6322).queens == 0
...
def test_hand_honour_counts_list_jacks():
    ...
    assert Hand(hand_list_6322).jacks == 4
...
def test_hand_honour_counts_list_tens():
    ...
    assert Hand(hand_list_6322).tens == 0
...
def test_hand_honour_counts_list_nines():
    ...
    assert Hand(hand_list_6322).nines == 0
...
def test_hand_honour_counts_list_tens_and_nines():
    ...
    assert Hand(hand_list_6322).tens_and_nines == 0
...
def test_hand_seven_card_suit_or_better():
    ...
    assert not Hand(hand_list_6322).seven_card_suit_or_better
...
def test_hand_six_card_suit_or_better():
    ...
    assert Hand(hand_list_6322).six_card_suit_or_better
...
def test_hand_five_card_suit_or_better():
    ...
    assert Hand(hand_list_6322).five_card_suit_or_better
...
def test_hand_seven_card_suit():
    ...
    assert not Hand(hand_list_6322).seven_card_suit
...
def test_hand_six_card_suit():
    ...
    assert Hand(hand_list_6322).six_card_suit
...
def test_hand_five_card_suit():
    ...
    assert not Hand(hand_list_6322).five_card_suit
...
def test_hand_five_card_major_or_better():
    ...
    assert Hand(hand_list_6322).five_card_major_or_better
...
def test_hand_five_card_major_suit():
    ...
    assert Hand(hand_list_6322).five_card_major_suit == spade_suit
...
def test_hand_four_card_major_or_better():
    ...
    assert Hand(hand_list_6322).four_card_major_or_better
...
def test_hand_five_four():
    ...
    assert not Hand(hand_list_6322).five_four
...
def test_hand_five_five():
    ...
    assert not Hand(hand_list_6322).five_five
...
def test_hand_six_four():
    ...
    assert not Hand(hand_list_6322).six_four
...
def test_hand_six_sic():
    """Ensure that six/four works."""
    assert not Hand(hand_list_6322).six_six
...
def test_hand_four_card_major_suit():
    ...
    assert Hand(hand_list_6322).four_card_major_suit == spade_suit
...
def test_hand_five_card_major():
    ...
    assert not Hand(hand_list_6322).five_card_major
...
def test_hand_four_card_major():
    ...
    assert Hand(hand_list_6322).four_card_major
...
def test_hand_equal_long_suits():
    ...
    assert not Hand(hand_list_6322).equal_long_suits
...
def test_hand_get_third_longest_suit():
    ...
    assert Hand(hand_list_6322).third_suit == diamond_suit
    assert Hand(hand_list_4513).third_suit == club_suit


def test_hand_suits_by_length():
    """Ensure the suits_by_length list is correct."""
    assert Hand(hand_list_6322).suits_by_length[0].name == 'S'
    assert Hand(hand_list_6322).suits_by_length[1].name == 'H'
    assert Hand(hand_list_6322).suits_by_length[2].name == 'D'
    assert Hand(hand_list_6322).suits_by_length[3].name == 'C'
...
def test_hand_longest_suit():
    ...
    assert Hand(hand_list_6322).longest_suit.name == 'S'
...
def test_hand_second_suit():
    ...
    assert Hand(hand_list_6322).second_suit.name == 'H'
...
def test_hand_distribution_points():
    ...
    assert Hand(hand_list_6322).distribution_points == 2
...
def test_hand_high_card_points():
    ...
    assert Hand(hand_list_6322).high_card_points == 11
...
def test_hand_is_semi_balanced():
    ...
    assert Hand(hand_list_6322).is_semi_balanced
...
def test_hand_rule_of_twenty():
    ...
    assert Hand(hand_list_6322).rule_of_twenty
...
def test_hand_rule_of_nineteen():
    ...
    assert Hand(hand_list_6322).rule_of_nineteen
...
def test_hand_suit_points():
    ...
    assert Hand(hand_list_6322).suit_points(spade_suit) == 1
    assert Hand(hand_list_6322).suit_points(club_suit) == 4
...
def test_hand_rule_of_fourteen():
    ...
    assert not Hand(hand_list_6322).rule_of_fourteen(spade_suit)
    assert not Hand(hand_list_6322).rule_of_fourteen(spade_suit)
...
def test_hand_solid_suit_honours():
    ...
    assert not Hand(hand_list_6322).solid_suit_honours(spade_suit)
...
def test_cards_by_suit():
    ...
    assert Card('8S') in Hand(hand_list_6322).cards_by_suit['S']
    assert Card('AS') not in Hand(hand_list_6322).cards_by_suit['S']
    assert Card('AD') in Hand(hand_list_6322).cards_by_suit['D']
    assert Card('AD') not in Hand(hand_list_6322).cards_by_suit['S']
    assert Card('QD') not in Hand(hand_list_6322).cards_by_suit['D']
...
def test_suit_order_assignment():
    """Ensure suit_order assignment works correctly."""
    # hand_pbn_two = 'A8654.KQ5.T.QJT6'
    hand = Hand(hand_pbn_two)
    with pytest.raises(TypeError) as exec_info:
...
def test_cards_in_suit():
    """Ensure cards in suit works correctly."""
    hand = Hand(hand_list_6322)
...
def test_suit_holding():
    """Ensure suit_lengths works correctly."""
    hand = Hand(hand_list_6322)
    assert hand.suit_holding == {
        'S': 6,
        'H': 3,
        'D': 2,
        'C': 2,
