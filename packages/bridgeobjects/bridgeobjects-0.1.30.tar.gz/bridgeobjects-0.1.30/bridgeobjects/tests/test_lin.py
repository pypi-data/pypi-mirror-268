"""A set of tests for the bridgeobjects lin file format."""
from pathlib import Path
import pytest
from ..src.file_operations import load_lin
...
# Create a lin file from a pbn input to test saves
INPUT_PATH = Path(DATA_PATH, 'lin_input_file_1.lin')
# SAVE_PATH = Path([DATA_PATH, 'pbn_output_file_1.pbn'])
events_loaded = load_lin(INPUT_PATH)
# save_pbn(rbn_events, SAVE_PATH)
...
def test_lin_board_counts(event, expected):
...
def test_lin_hands_counts(board, expected):
...
# ])
# def test_lin_event_load_name(event, expected):
...
# ])
# def test_lin_event_load_location(event, expected):
...
# ])
# def test_lin_event_load_date(event, expected):
...
#     (events_saved[0], "M")
# ])
# def test_lin_board_scoring_method(event, expected):
#     """Ensure that the board is constructed correctly: scoring method."""
#     assert event.scoring_method == expected


@pytest.mark.parametrize("board, expected", [
    (events_loaded[0].boards[3], "Board 4"),
    # (events_saved[0].boards[0], "8")
])
def test_lin_board_identifier(board, expected):
...
# ])
# def test_lin_board_north(board, expected):
...
# ])
# def test_lin_board_east(board, expected):
...
# ])
# def test_lin_board_south(board, expected):
...
# ])
# def test_lin_board_west(board, expected):
...
def test_lin_board_vulnerable(board, expected):
...
# ])
# def test_lin_board_declarer(board, expected):
...
# ])
# def test_lin_board_contract(board, expected):
#     """Ensure that the board is constructed correctly: contract."""
#     assert board.contract.call.name == expected


# @pytest.mark.parametrize("board, expected", [
#     (events_loaded[0].boards[3], "D"),
# ])
# def test_lin_board_contract_doubled(board, expected):
#     """Ensure that the board is constructed correctly: contract."""
#     assert board.contract.modifier == expected


# @pytest.mark.parametrize("board, expected", [
#     (events_loaded[0].boards[12], "R"),
# ])
# def test_lin_board_contract_redoubled(board, expected):
#     """Ensure that the board is constructed correctly: contract."""
#     assert board.contract.modifier == expected


def test_lin_board_hands():
    """Ensure that the board's hands are constructed correctly."""
    assert Card("6D") in events_loaded[0].boards[3].hands[3].cards
    assert Card("4D") in events_loaded[0].boards[3].hands[0].cards
    ...
    assert Card("4D") in events_loaded[0].boards[3].hands['N'].cards
