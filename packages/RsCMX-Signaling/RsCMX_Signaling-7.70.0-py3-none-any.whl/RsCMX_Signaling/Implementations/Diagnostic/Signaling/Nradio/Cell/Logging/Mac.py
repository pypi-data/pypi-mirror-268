...
class MacCls:
	...
	def set(self, enable: bool, log_type: enums.LogType, payload: int = None) -> None:
		...
		Snippet: driver.diagnostic.signaling.nradio.cell.logging.mac.set(enable = False, log_type = enums.LogType.DISable, payload = 1) \n
		...
		self._core.io.write(f'DIAGnostic:SIGNaling:NRADio:CELL:LOGGing:MAC {param}'.rstrip())
	...
	def get(self) -> MacStruct:
		"""SCPI: DIAGnostic:SIGNaling:NRADio:CELL:LOGGing:MAC \n
		Snippet: value: MacStruct = driver.diagnostic.signaling.nradio.cell.logging.mac.get() \n
		No command help available \n
			:return: structure: for return value, see the help for MacStruct structure arguments."""
		return self._core.io.query_struct(f'DIAGnostic:SIGNaling:NRADio:CELL:LOGGing:MAC?', self.__class__.MacStruct())
