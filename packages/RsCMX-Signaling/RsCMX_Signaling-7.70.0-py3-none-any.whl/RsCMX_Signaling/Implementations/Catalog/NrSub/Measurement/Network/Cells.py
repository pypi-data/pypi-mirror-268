...
class CellsCls:
	...
	def get(self, measInstance=repcap.MeasInstance.Default) -> List[str]:
		...
		Snippet: value: List[str] = driver.catalog.nrSub.measurement.network.cells.get(measInstance = repcap.MeasInstance.Default) \n
		...
		response = self._core.io.query_str(f'CATalog:NRSub:MEASurement{measInstance_cmd_val}:NETWork:CELLs?')
