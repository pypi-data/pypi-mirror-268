...
class McsTableCls:
	...
	def set(self, cell_name: str, mcs_table: enums.McsTableB) -> None:
		...
		Snippet: driver.configure.signaling.nradio.cell.ueScheduling.userDefined.downlink.mcsTable.set(cell_name = 'abc', mcs_table = enums.McsTableB.L64) \n
	...
	def get(self, cell_name: str) -> enums.McsTableB:
		"""SCPI: [CONFigure]:SIGNaling:NRADio:CELL:UESCheduling:UDEFined:DL:MCSTable \n
		Snippet: value: enums.McsTableB = driver.configure.signaling.nradio.cell.ueScheduling.userDefined.downlink.mcsTable.get(cell_name = 'abc') \n
		Selects the MCS table that the UE must use to determine the modulation order and the target code rate of the PDSCH, for
		the initial BWP. \n
			:param cell_name: No help available
