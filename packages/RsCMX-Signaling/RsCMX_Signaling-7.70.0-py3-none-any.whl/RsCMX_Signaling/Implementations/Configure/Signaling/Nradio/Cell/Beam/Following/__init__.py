...
class FollowingCls:
	...
	def __init__(self, core: Core, parent):
		self._core = core
		self._cmd_group = CommandsGroup("following", core, parent)
	...
	@property
	def ibChange(self):
		"""ibChange commands group. 0 Sub-classes, 1 commands."""
		if not hasattr(self, '_ibChange'):
			from .IbChange import IbChangeCls
			self._ibChange = IbChangeCls(self._core, self._cmd_group)
		return self._ibChange
	...
	def clone(self) -> 'FollowingCls':
		"""Clones the group by creating new object from it and its whole existing subgroups
		Also copies all the existing default Repeated Capabilities setting,
		which you can change independently without affecting the original group"""
		new_group = FollowingCls(self._core, self._cmd_group.parent)
