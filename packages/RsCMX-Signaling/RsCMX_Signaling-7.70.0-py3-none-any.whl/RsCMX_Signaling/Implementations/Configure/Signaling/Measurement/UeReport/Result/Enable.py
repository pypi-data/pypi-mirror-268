...
class EnableCls:
	...
	def set(self, rsrp: bool, rsrq: bool, rssi_nr: bool) -> None:
		...
		Snippet: driver.configure.signaling.measurement.ueReport.result.enable.set(rsrp = False, rsrq = False, rssi_nr = False) \n
		...
		self._core.io.write(f'CONFigure:SIGNaling:MEASurement:UEReport:RESult:ENABle {param}'.rstrip())
	...
	def get(self) -> EnableStruct:
		"""SCPI: [CONFigure]:SIGNaling:MEASurement:UEReport:RESult:ENABle \n
		Snippet: value: EnableStruct = driver.configure.signaling.measurement.ueReport.result.enable.get() \n
		Selects the quantities to be reported by the UE for serving cell measurements. \n
			:return: structure: for return value, see the help for EnableStruct structure arguments."""
		return self._core.io.query_struct(f'CONFigure:SIGNaling:MEASurement:UEReport:RESult:ENABle?', self.__class__.EnableStruct())
