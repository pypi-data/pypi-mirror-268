...
class SendCls:
	...
	def set(self, cell_name: str, ccrntis_end: enums.CcrntisEnd) -> None:
		...
		Snippet: driver.configure.signaling.lte.cell.ueScheduling.laa.rburst.ccrnti.send.set(cell_name = 'abc', ccrntis_end = enums.CcrntisEnd.ASF) \n
		...
		self._core.io.write(f'CONFigure:SIGNaling:LTE:CELL:UESCheduling:LAA:RBURst:CCRNti:SEND {param}'.rstrip())

	# noinspection PyTypeChecker
	def get(self, cell_name: str) -> enums.CcrntisEnd:
		"""SCPI: [CONFigure]:SIGNaling:LTE:CELL:UESCheduling:LAA:RBURst:CCRNti:SEND \n
		Snippet: value: enums.CcrntisEnd = driver.configure.signaling.lte.cell.ueScheduling.laa.rburst.ccrnti.send.get(cell_name = 'abc') \n
		Selects subframes for transmission of CC-RNTI, for random bursts. \n
			:param cell_name: No help available
			:return: ccrntis_end: Send CC-RNTI: F2SF: final 2 SF LSF: last SF BLSF: before last SF SASF: skip all SF ASF: all SF"""
		param = Conversions.value_to_quoted_str(cell_name)
		response = self._core.io.query_str(f'CONFigure:SIGNaling:LTE:CELL:UESCheduling:LAA:RBURst:CCRNti:SEND? {param}')
