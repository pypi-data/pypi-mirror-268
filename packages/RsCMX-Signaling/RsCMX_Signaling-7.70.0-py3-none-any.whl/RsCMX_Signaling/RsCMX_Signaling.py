...
class RsCMX_Signaling:
	"""1155 total commands, 14 Subgroups, 0 group commands"""
	_driver_options = "SupportedInstrModels = CMX/CMW, SupportedIdnPatterns = CMX/CMW, SimulationIdnString = 'Rohde&Schwarz,CMX,100001,7.70.0.0018'"
	...
	def __init__(self, resource_name: str, id_query: bool = True, reset: bool = False, options: str = None, direct_session: object = None):
		...
		self._core = Core(resource_name, id_query, reset, RsCMX_Signaling._driver_options, options, direct_session)
		self._core.driver_version = '7.70.0.0018'
	...
	@classmethod
	def from_existing_session(cls, session: object, options: str = None) -> 'RsCMX_Signaling':
	...
	def __str__(self) -> str:
		if self._core.io:
			return f"RsCMX_Signaling session '{self._core.io.resource_name}'"
		else:
			return f"RsCMX_Signaling with session closed"
	...
	def assert_minimum_version(min_version: str) -> None:
		...
		curr_version_list = '7.70.0.0018'.split('.')
		...
		for i in range(count):
			...
			if curr > minimum:
				break
			if curr < minimum:
				raise RsInstrException(f"Assertion for minimum RsCMX_Signaling version failed. Current version: '7.70.0.0018', minimum required version: '{min_version}'")
	...
	def _sync_to_custom_properties(self, cloned: 'RsCMX_Signaling') -> None:
	...
	@property
	def create(self):
		"""create commands group. 1 Sub-classes, 0 commands."""
	...
	@property
	def signaling(self):
		"""signaling commands group. 12 Sub-classes, 0 commands."""
		if not hasattr(self, '_signaling'):
			from .Implementations.Signaling import SignalingCls
			self._signaling = SignalingCls(self._core, self._cmd_group)
		return self._signaling

	@property
	def restart(self):
		"""restart commands group. 1 Sub-classes, 0 commands."""
		if not hasattr(self, '_restart'):
			from .Implementations.Restart import RestartCls
	...
	@property
	def catalog(self):
		"""catalog commands group. 5 Sub-classes, 0 commands."""
	...
	@property
	def procedure(self):
		"""procedure commands group. 1 Sub-classes, 0 commands."""
	...
	def clone(self) -> 'RsCMX_Signaling':
		"""Creates a deep copy of the RsCMX_Signaling object. Also copies:
		...
		cloned = RsCMX_Signaling.from_existing_session(self.get_session_handle(), self._options)
