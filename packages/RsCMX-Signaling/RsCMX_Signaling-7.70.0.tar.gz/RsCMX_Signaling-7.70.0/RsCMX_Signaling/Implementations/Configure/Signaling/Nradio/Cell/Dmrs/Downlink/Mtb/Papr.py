...
class PaprCls:
	...
	def set(self, cell_name: str, enable: bool) -> None:
		...
		Snippet: driver.configure.signaling.nradio.cell.dmrs.downlink.mtb.papr.set(cell_name = 'abc', enable = False) \n
		...
		self._core.io.write(f'CONFigure:SIGNaling:NRADio:CELL:DMRS:DL:MTB:PAPR {param}'.rstrip())

	def get(self, cell_name: str) -> bool:
		"""SCPI: [CONFigure]:SIGNaling:NRADio:CELL:DMRS:DL:MTB:PAPR \n
		Snippet: value: bool = driver.configure.signaling.nradio.cell.dmrs.downlink.mtb.papr.get(cell_name = 'abc') \n
		Enables the usage of a DMRS with a low PAPR, for PDSCH, mapping type B, initial BWP. \n
			:param cell_name: No help available
			:return: enable: No help available"""
		param = Conversions.value_to_quoted_str(cell_name)
		response = self._core.io.query_str(f'CONFigure:SIGNaling:NRADio:CELL:DMRS:DL:MTB:PAPR? {param}')
