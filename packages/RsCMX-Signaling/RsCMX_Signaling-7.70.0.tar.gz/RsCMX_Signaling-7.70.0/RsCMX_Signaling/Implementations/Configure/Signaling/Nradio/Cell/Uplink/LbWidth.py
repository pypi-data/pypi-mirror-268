...
class LbWidthCls:
	...
	def set(self, cell_name: str, riv: int) -> None:
		...
		Snippet: driver.configure.signaling.nradio.cell.uplink.lbWidth.set(cell_name = 'abc', riv = 1) \n
		...
		self._core.io.write(f'CONFigure:SIGNaling:NRADio:CELL:UL:LBWidth {param}'.rstrip())

	def get(self, cell_name: str) -> int:
		"""SCPI: [CONFigure]:SIGNaling:NRADio:CELL:UL:LBWidth \n
		Snippet: value: int = driver.configure.signaling.nradio.cell.uplink.lbWidth.get(cell_name = 'abc') \n
		Defines the resource indication value (RIV) signaled as 'locationAndBandwidth', for the uplink, for the initial BWP. \n
			:param cell_name: No help available
			:return: riv: No help available"""
		param = Conversions.value_to_quoted_str(cell_name)
		response = self._core.io.query_str(f'CONFigure:SIGNaling:NRADio:CELL:UL:LBWidth? {param}')
