...
class RmsCls:
	...
	def set(self, cell_name: str, max_exp_rms_pwr: float) -> None:
		...
		Snippet: driver.configure.signaling.lte.cell.power.uplink.rms.set(cell_name = 'abc', max_exp_rms_pwr = 1.0) \n
		Sets the maximum RMS power expected in the UL, for user-defined configuration. For automatic configuration, you can query
		the value. \n
	...
	def get(self, cell_name: str) -> float:
		"""SCPI: [CONFigure]:SIGNaling:LTE:CELL:POWer:UL:RMS \n
		Snippet: value: float = driver.configure.signaling.lte.cell.power.uplink.rms.get(cell_name = 'abc') \n
		Sets the maximum RMS power expected in the UL, for user-defined configuration. For automatic configuration, you can query
		the value. \n
			:param cell_name: No help available
