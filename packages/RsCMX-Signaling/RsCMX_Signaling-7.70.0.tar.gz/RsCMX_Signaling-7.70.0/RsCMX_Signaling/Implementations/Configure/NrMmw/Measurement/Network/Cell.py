...
class CellCls:
	...
	def set(self, cell_name: str, used_ul: List[int] = None, measInstance=repcap.MeasInstance.Default) -> None:
		"""SCPI: [CONFigure]:NRMMw:MEASurement<Instance>:NETWork:CELL \n
		Snippet: driver.configure.nrMmw.measurement.network.cell.set(cell_name = 'abc', used_ul = [1, 2, 3], measInstance = repcap.MeasInstance.Default) \n
		...
		self._core.io.write(f'CONFigure:NRMMw:MEASurement{measInstance_cmd_val}:NETWork:CELL {param}'.rstrip())
	...
	def get(self, measInstance=repcap.MeasInstance.Default) -> CellStruct:
		"""SCPI: [CONFigure]:NRMMw:MEASurement<Instance>:NETWork:CELL \n
		Snippet: value: CellStruct = driver.configure.nrMmw.measurement.network.cell.get(measInstance = repcap.MeasInstance.Default) \n
		No command help available \n
			:param measInstance: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Measurement')
			:return: structure: for return value, see the help for CellStruct structure arguments."""
		measInstance_cmd_val = self._cmd_group.get_repcap_cmd_value(measInstance, repcap.MeasInstance)
		return self._core.io.query_struct(f'CONFigure:NRMMw:MEASurement{measInstance_cmd_val}:NETWork:CELL?', self.__class__.CellStruct())
