...
class BandsCls:
	...
	def set(self, rat: List[enums.CellType], fbi: List[int]) -> None:
		"""SCPI: ADD:SIGNaling:EPS:UECapability:MRDC:BANDs \n
		Snippet: driver.add.signaling.eps.ueCapability.mrdc.bands.set(rat = [CellType.LTE, CellType.NR], fbi = [1, 2, 3]) \n
		Adds entries to the list of requested frequency bands for the container type 'UE-MRDC-Capability', for EPS tracking areas.
		The bands are defined as pairs of values: {<Rat>, <Fbi>}1, {<Rat>, <Fbi>}2, ... \n
		...
		self._core.io.write(f'ADD:SIGNaling:EPS:UECapability:MRDC:BANDs {param}'.rstrip())
