...
class AbsoluteCls:
	...
	def fetch(self, cword=repcap.Cword.Default) -> FetchStruct:
		...
		Snippet: value: FetchStruct = driver.signaling.measurement.bler.cword.absolute.fetch(cword = repcap.Cword.Default) \n
		Returns the absolute DL results of the BLER measurement, for code word <no>. There is one set of results {...} per cell:
		<Reliability>, {<CellName>, <ACK>, <NACK>, <DTX>, <ThroughputAvg>}, {...}, ... \n
		...
		return self._core.io.query_struct(f'FETCh:SIGNaling:MEASurement:BLER:CWORd{cword_cmd_val}:ABSolute?', self.__class__.FetchStruct())
