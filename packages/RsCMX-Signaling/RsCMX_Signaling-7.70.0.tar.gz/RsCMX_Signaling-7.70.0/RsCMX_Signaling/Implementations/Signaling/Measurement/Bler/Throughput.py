...
class ThroughputCls:
	...
	class FetchStruct(StructBase):
		...
		__meta_args_list = [
			...
			ArgStruct('Rel_Avg', DataType.FloatList, None, False, True, 1),
			ArgStruct('Abs_Ack', DataType.IntegerList, None, False, True, 1),
		...
		def __init__(self):
			...
			self.Rel_Avg: List[float] = None
			self.Abs_Ack: List[int] = None
			self.Abs_Scheduled: List[int] = None

	def fetch(self) -> FetchStruct:
		"""SCPI: FETCh:SIGNaling:MEASurement:BLER:THRoughput \n
		Snippet: value: FetchStruct = driver.signaling.measurement.bler.throughput.fetch() \n
		Returns the DL throughput results of the BLER measurement. There is one set of results {...} per cell: <Reliability>,
		{<CellName>, <RelAvg>, <AbsAck>, <AbsScheduled>}, {...}, ... \n
			:return: structure: for return value, see the help for FetchStruct structure arguments."""
		return self._core.io.query_struct(f'FETCh:SIGNaling:MEASurement:BLER:THRoughput?', self.__class__.FetchStruct())
