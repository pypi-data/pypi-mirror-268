...
class TNObsLocal(_TNObsBase):
    ...
    **Arguments**
    ...
    def __init__(self, name, operator):
        ...
        # Old parameter, should be eliminated but is still
        # used by write_input_v1
        self.periodicity = []
    ...
    def write(self, fh, **kwargs):
        ...
        str_buffer = "------------------- tnobslocal\n"

        str_buffer += "%d\n" % (len(self))
        for ii in range(len(self)):
            # Fix for the MPS not having the operator descriptor
            try:
                str_buffer += "%d\n" % (operator_map[(self.operator[ii], None)])
            except KeyError:
                str_buffer += "%d\n" % (operator_map[self.operator[ii]])
    ...
    def write_input_v1(self, folder_name, params):
        ...
        for ii in range(len(self)):
            ...
            with open(full_filename, "w+") as fh:
                ...
                fh.write(".true.\n")
                fh.write("# Periodicity\n")
                fh.write("%d\n" % (self.periodicity[ii]))
                fh.write("# Operators\n")
                fh.write("%s\n" % (self.operator[ii]))

            yield "observable_%s.in" % (self.name[ii])
