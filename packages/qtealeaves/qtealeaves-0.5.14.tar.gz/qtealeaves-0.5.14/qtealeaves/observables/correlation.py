    ...
    **Arguments**
    ...
    def write(self, fh, **kwargs):
        ...
        **Arguments**
        ...
        str_buffer = "------------------- tnobscorr\n"

        str_buffer += "%d\n" % (len(self))

        for ii in range(len(self)):
            if len(self.operators[ii]) != 2:
                raise Exception("Correlation requires two operators")

            str_buffer += "%d, %d\n" % (
                operator_map[(self.operators[ii][0], "l")],
                operator_map[(self.operators[ii][1], "r")],
            )
            str_buffer += ".true.\n"
...
class TNObsCorr4(TNObsCorr):
    ...
    **Details**
    ...
    def read_hdf5(self, dataset, params):
        ...
        """
        raise Exception("This observables cannot read HDF5 files yet.")
    ...
    def write(self, fh, **kwargs):
        ...
        **Arguments**
        ...
        str_buffer = "------------------- tnobscorr4\n"

        str_buffer += "%d\n" % (len(self))

        for ii in range(len(self)):
            if len(self.operators[ii]) != 4:
                raise Exception("Correlation requires two operators")

            try:
                op_a = operator_map[(self.operators[ii][0], None)]
            except KeyError:
                op_a = operator_map[(self.operators[ii][0])]

            try:
                op_b = operator_map[(self.operators[ii][1], None)]
            except KeyError:
                op_b = operator_map[(self.operators[ii][1])]

            try:
                op_c = operator_map[(self.operators[ii][2], None)]
            except KeyError:
                op_c = operator_map[(self.operators[ii][2])]

            try:
                op_d = operator_map[(self.operators[ii][3], None)]
            except KeyError:
                op_d = operator_map[(self.operators[ii][3])]
