...
"""
Observable to recap all the observables that can be measured by the MPS
"""
...
class TNObservables(_ParameterizedClass):
    ...
    **Arguments**
    ...
    filename_observables : str
        ...
        will be choosen by the backend.
    ...
    **Details**
    ...
    def get_num_trajectories(self, **kwargs):
        ...
        **Arguments**
        ...
        if num_trajectories > 4095:
            raise Exception("Number of trajectory over the limit for the seed.")
    ...
    def add_observable(self, obj):
        ...
        if repr(obj) not in self.obs_list:
            raise Exception("Observables not of valid type.")
    ...
    def write(self, folder_name, params, operator_map):
        """
        Write the information of the observables for fortran
        as an input file.
        ...
        **Arguments**
        ...
        with open(full_path_file, "w+") as fh:
            ...
            fh.write("T\n" if eval_do_write_hdf5 else "F\n")
            # write the observables
            for elem in self.obs_list:
                self.obs_list[elem].write(fh, operator_map=operator_map, params=params)

            fh.write("-------------------- end of file\n")

        return self.filename_observables

    def write_results(self, filename, is_measured, params):
        """
        ...
        **Arguments**
        ...
        is_measured : array-like of bools
            Array containg the information about the measurability
            of a given observable with a given backend
        ...
        with open(filename, "w+") as fh:
            ...
            fh.write(str(self.results_buffer["energy"]) + "\n")
            ...
            for ii, elem in enumerate(self.obs_list):
                self.obs_list[elem].write_results(fh, is_measured[ii])
        ...
        with open(full_path_file, "w+") as fh:
            for elem in self.obs_list:
                for subfile in self.obs_list[elem].write_input_v1(
                    ...
                    fh.write(self.folder_observables + "/" + subfile + "\n")
        ...
        **Returns**
        ...
        with open(log_file_path, "r") as log_file:
            for line in log_file:
                if match in line:
                    cpu_time = float(line.replace(match, "").strip())
                    break

        if cpu_time is None:
            # Found no match for some reason ... empty iterator
            return

        yield "cpu_time", cpu_time
    ...
    def get_seed_trajectories(params):
        ...
        **Returns**
        ...
        """
        if "seed" in params:
            raise Exception("The seed cannot be specified by the user.")
