...
def download_file(ali_api: Aligo, pan_file: BaseFile, local_path: str,
                  pan_file_filter: Callable[[BaseFile], bool] = lambda x: False,
                  share_token: GetShareTokenResponse = None) -> str:
    """
    下载文件/文件夹
    :param ali_api: Aligo
    :param pan_file: 网盘文件
    ...
    if pan_file.type == 'file':
        download_url = pan_file.download_url or pan_file.url
        if download_url:
            local_file = ali_api.download_file(file=pan_file, local_folder=local_path)
        else:
            if share_token is None:
                share_token = aliyun_pan_share_tools.get_share_token(ali_api, pan_file.share_id)
            down_file = ali_api.get_share_link_download_url(pan_file.file_id, share_token)
            local_file = ali_api.download_file(file=down_file, local_folder=local_path)
