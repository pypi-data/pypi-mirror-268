...
class MarketHandle():
    def __init__(self, tsocket: TSocket):
        ...
        self.__BaseTick_Columns_Types = {"ExchangeID": str, "InstrumentID": str, "ActionDay": str,
        ...
        self.__OHLC_Columns_Types = {
            "ExchangeID": str, "InstrumentID": str, "ActionDay": str, "ActionTime": str, "ActionTimeSpan": int,
            "StartTime": str, "EndTime": str, "Range": int,
            "OpenPrice": float, "OpenVolume": int, "OpenBidPrice": float, "OpenAskPrice": float, "OpenBidVolume": int, "OpenAskVolume": int,
            "HighestPrice": float, "HighestVolume": int, "HighestBidPrice": float, "HighestAskPrice": float, "HighestBidVolume": int, "HighestAskVolume": int,
            "LowestPrice": float, "LowestVolume": int, "LowestBidPrice": float, "LowestAskPrice": float, "LowestBidVolume": int, "LowestAskVolume": int,
    ...
    def get_basetick_column_types(self):
        return self.__BaseTick_Columns_Types
    ...
    def fin_save_basetick_list(self, instrument_id, df, **kwargs) -> Tuple[bool, str]:
        if not isinstance(df, pd.DataFrame):
            return [False, "df 数据类型格式不是 DataFrame", {}]

        if df.columns.to_list() != list(self.__BaseTick_Columns_Types.keys()):
            return [False, "df 数据列名称不匹配,当前:%s 应为:%s" % (df.columns.to_list(), self.__BaseTick_Columns_Types.keys())]
    ...
    def fin_read_basetick_list(self, params: FinPersistReadParamData, is_return_list: bool = False) -> Tuple[bool, str, pd.DataFrame]:
        ...
        if is_return_list is True:
            ...
            ret = [rsp.RspSuccess, rsp.RspMsg, self.__unpack_fin_persist_read_param_data_to_df(req_rsp, list(self.__BaseTick_Columns_Types.keys()))]
