...
def add_margin(pil_img, top, right, bottom, left, color=0):
    width, height = pil_img.size
    new_width = width + right + left
    new_height = height + top + bottom
    result = Image.new(pil_img.mode, (new_width, new_height), color)
    result.paste(pil_img, (left, top))
    print('before-after', pil_img.size, result.size)
    return result

def zoom_and_maintain_center(img, zoom_factor, offset_x=0, offset_y=0):
    ...
    new_width, new_height = X / zoom_factor, Y / zoom_factor

    left = a - new_width / 2 - offset_x
    top = b - new_height / 2 - offset_y
    right = left + new_width 
    bottom = top + new_height

    cropped_img = img.crop((left, top, right, bottom))
        
    zoomed_img = cropped_img.resize((X, Y), Image.LANCZOS)
...
class LwViewW:
    ...
    #    self.intensity_min = np.amin(self.input_img.data)
       self.intensity_max = np.quantile(self.input_img.data, 0.995)
       self.intensity_min = np.quantile(self.input_img.data, 0.005)
       ...
       # Initialize zoom scale factors for each view
       self.zoom_scale_axi = 1.0
       self.zoom_scale_cor = 1.0
       self.zoom_scale_sag = 1.0
    ...
    def update_images(self):
        ...
        self.axi_pilimg = zoom_and_maintain_center(self.axi_pilimg, self.zoom_scale_axi, self.offset_x_axi, self.offset_y_axi)
        self.cor_pilimg = zoom_and_maintain_center(self.cor_pilimg, self.zoom_scale_cor, self.offset_x_cor, self.offset_y_cor)
        self.sag_pilimg = zoom_and_maintain_center(self.sag_pilimg, self.zoom_scale_sag, self.offset_x_sag, self.offset_y_sag)
        # self.axi_pilimg =  self.axi_pilimg.resize((int(self.axi_pilimg.width * self.zoom_scale_axi), int(self.axi_pilimg.height * self.zoom_scale_axi)), Image.LANCZOS)
        # # 
        # self.cor_pilimg = self.cor_pilimg.resize((int(self.cor_pilimg.width * self.zoom_scale_cor), int(self.cor_pilimg.height * self.zoom_scale_cor)), Image.LANCZOS)
        # self.sag_pilimg = self.sag_pilimg.resize((int(self.sag_pilimg.width * self.zoom_scale_sag), int(self.sag_pilimg.height * self.zoom_scale_sag)), Image.LANCZOS)

        # Update the Tk images and canvases as before
    ...
    def on_mod_key_press(self,event):
        self.mod_key_pressed = True
        print('hello', self.mod_key_pressed)

    def on_mod_key_release(self,event):
        self.mod_key_pressed = False
        

    def on_mouse_down(self,event):
        # Check for Command (macOS) or Control (Windows/Linux) key
        self.dragging = True
        if event.widget == self.canvas_axi:
            self.last_x, self.last_y = self.find_original_coordinates(event.x, event.y, self.i_width_px, self.j_width_px, self.zoom_scale_axi)
        elif event.widget == self.canvas_cor:
            self.last_x, self.last_y = self.find_original_coordinates(event.x, event.y, self.i_width_px, self.k_width_px, self.zoom_scale_cor)
        elif event.widget == self.canvas_sag:
            self.last_x, self.last_y = self.find_original_coordinates(event.x, event.y, self.j_width_px, self.k_width_px, self.zoom_scale_sag)
        
    def on_mouse_move(self,event):
        if self.dragging:
            if event.widget == self.canvas_axi:
                x, y = self.find_original_coordinates(event.x, event.y, self.i_width_px, self.j_width_px, self.zoom_scale_axi)
            elif event.widget == self.canvas_cor:
                x, y = self.find_original_coordinates(event.x, event.y, self.i_width_px, self.k_width_px, self.zoom_scale_cor)
            elif event.widget == self.canvas_sag:
                x, y = self.find_original_coordinates(event.x, event.y, self.j_width_px, self.k_width_px, self.zoom_scale_sag)
            ...
            if event.widget == self.canvas_axi:
                self.offset_x_axi += delta_x
                self.offset_y_axi += delta_y
            elif event.widget == self.canvas_cor:
                self.offset_x_cor += delta_x
                self.offset_y_cor += delta_y
            elif event.widget == self.canvas_sag:
                self.offset_x_sag += delta_x
                self.offset_y_sag += delta_y
                
            # Redraw the zoomed and translated image
    ...
    def on_mouse_up(self,event):
        # This can be used to finalize the drag
        # global dragging
        # End dragging
        self.dragging = False
        # print("End Dragging")
    ...
    def move_annotation(self, event):        
        ...
        if event.widget == self.canvas_axi:
            ...
            x -= self.offset_x_axi
            y -= self.offset_y_axi
            i,j = (self.i_width_px - x - 1) * self.n_i / self.i_width_px  , (self.j_width_px - y - 1) * self.n_j / self.j_width_px
            ...
            i,k = (self.i_width_px - x - 1) * self.n_i / self.i_width_px  , (self.k_width_px - y - 1) * self.n_k / self.k_width_px
            ...
            j,k = (self.j_width_px - x - 1) * self.n_j / self.j_width_px  , (self.k_width_px - y - 1) * self.n_k / self.k_width_px
            self.focus_j, self.focus_k = j, k 
            
        
        # self.focus_i, self.focus_j = i, j
        # self.focus_i /= self.zoom_scale_axi
        # self.focus_j /= self.zoom_scale_axi
        self.update_annotations_axi()
        self.update_annotations_cor()
        self.update_annotations_sag()
        # self.our_update()
    ...
    def handle_zoom(self, event):
        # Determine which canvas and focus point to use
        if event.widget == self.canvas_axi:
            scale_factor = self.zoom_scale_axi
        elif event.widget == self.canvas_cor:
            scale_factor = self.zoom_scale_cor
        elif event.widget == self.canvas_sag:
            scale_factor = self.zoom_scale_sag
        ...
        if event.widget == self.canvas_axi:
            self.zoom_scale_axi = scale_factor
        elif event.widget == self.canvas_cor:
            self.zoom_scale_cor = scale_factor
        elif event.widget == self.canvas_sag:
            self.zoom_scale_sag = scale_factor
            
        # Update the image display based on the new scale factor
    ...
    def make_widgets(self, parent):
        ...
        mod_key_jog = "Alt" if sys.platform == "darwin" else "Option"
        ...
        self.canvas_axi.bind(f'<{mod_key_jog}-Button-1>', self.axi_jog_callback)
        ...
        self.canvas_cor.bind(f'<{mod_key_jog}-Button-1>', self.cor_jog_callback)
        #
        ...
        self.canvas_sag.bind(f'<{mod_key_jog}-Button-1>', self.sag_jog_callback)
        ...
        if platform.system() == 'Linux':
            ...
            self.canvas_cor.bind(f"<{mod_key}-ButtonPress-1>", self.on_mouse_down)
            self.canvas_cor.bind(f"<{mod_key}-B1-Motion>", self.on_mouse_move)
            self.canvas_cor.bind(f"<{mod_key}-ButtonRelease-1>", self.on_mouse_up)
            
            self.canvas_sag.bind(f"<{mod_key}-ButtonPress-1>", self.on_mouse_down)
            self.canvas_sag.bind(f"<{mod_key}-B1-Motion>", self.on_mouse_move)
            self.canvas_sag.bind(f"<{mod_key}-ButtonRelease-1>", self.on_mouse_up)
            
            self.canvas_axi.bind("<Shift-Motion>", self.move_annotation)

            self.canvas_cor.bind(f'<{mod_key}-MouseWheel>', self.handle_zoom)
            self.canvas_cor.bind("<Shift-Motion>", self.move_annotation)
            
            self.canvas_sag.bind(f'<{mod_key}-MouseWheel>', self.handle_zoom)
            self.canvas_sag.bind(f"<Shift-Motion>", self.move_annotation)
        ...
        # self.canvas_axi.bind(f"<{mod_key}-KeyPress>", self.on_mod_key_press)
        # self.canvas_axi.bind(f"<{mod_key}-KeyRelease>", self.on_mod_key_release)
        #
    ...
    def map_coordinates_to_zoomed_image(self, original_position, width, height, zoom):
        # X, Y = self.i_width_px, self.j_width_px
        r, t = original_position

        # Center of the original image
        a, b = width // 2, height // 2

        # Calculate new dimensions
        new_width, new_height = width / zoom, height / zoom

        # Calculate the crop box
        left = max(a - new_width / 2, 0)
        top = max(b - new_height / 2, 0)
        right = min(left + new_width, width)
        bottom = min(top + new_height, height)

        # Check if (r, t) is within the crop box
        if left <= r <= right:
            zoomed_x = r - left
        else:
            if left>=r:
        ...
        if top <= t <= bottom:
            ...
            if top>=t:
                ...
                zoomed_y = 0
        # Calculate position relative to the top-left corner of the crop box
        # Given the resize back to original dimensions, the positions map directly
        # zoomed_x = r - left
        # zoomed_y = t - top
        return zoom*zoomed_x, zoom*zoomed_y
        # else:
        #     # Point is outside the visible area after zooming
        #     return None
    ...
    def make_annotations_axi(self):
        ...
        focus_i_px -= self.offset_x_axi
        focus_j_px -= self.offset_y_axi
        
        # Apply zoom to annotation
        focus_i_px, focus_j_px = self.map_coordinates_to_zoomed_image((focus_i_px,focus_j_px), self.i_width_px, self.j_width_px, self.zoom_scale_axi)
        ...
        if self.show_reticles:
            for i in range(-5,6):
                d = self.zoom_scale_axi*(10 * i) / self.mm_per_pixel
    ...
    def make_annotations_cor(self):
        ...
        focus_i_px -= self.offset_x_cor
        focus_k_px -= self.offset_y_cor
        
        # Apply zoom to annotation
        focus_i_px, focus_k_px = self.map_coordinates_to_zoomed_image((focus_i_px,focus_k_px), self.i_width_px, self.k_width_px, self.zoom_scale_cor)
        ...
        if self.show_reticles:
            for i in range(-5,6):
                d = self.zoom_scale_cor*(10 * i) / self.mm_per_pixel
    ...
    def make_annotations_sag(self):
        ...
        focus_j_px -= self.offset_x_sag
        focus_k_px -= self.offset_y_sag
        
        # Apply zoom to annotation
        focus_j_px, focus_k_px = self.map_coordinates_to_zoomed_image((focus_j_px,focus_k_px), self.j_width_px, self.k_width_px, self.zoom_scale_sag)
        ...
        if self.show_reticles:
            for i in range(-5,6):
                d = self.zoom_scale_sag*(10 * i) / self.mm_per_pixel
    ...
    def find_original_coordinates(self, c, d, width, height, zoom):
        c = c / zoom
        d = d / zoom
    ...
    def axi_callback(self, event):
        ...
        center_position  = (self.i_width_px // 2, self.j_width_px // 2)
...
def zoom_image_around_point(img, point, zoom_factor):
    """
    Zoom into an image around a specific point.
    
    :param img: PIL Image object.
    :param point: A tuple (a, b) representing the point around which to zoom.
    :param zoom_factor: The zoom factor.
    :return: A PIL Image object that has been zoomed.
    """
    ...
    # Determine the crop box, centered around (a, b)
    left = max(a - new_width / 2, 0)
    top = max(b - new_height / 2, 0)
    right = min(left + new_width, original_width)
    bottom = min(top + new_height, original_height)
    
    # Adjust the box to ensure its size is correct (in case of clipping at edges)
    ...
    cropped_img = img.crop((left, top, right, bottom))
    zoomed_img = cropped_img.resize((original_width, original_height), Image.LANCZOS)
    
    return zoomed_img, [left,top,right,bottom]
