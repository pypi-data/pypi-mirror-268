...
async def parse_cli(
    ...
    parser = hub.lib.argparse.ArgumentParser(description=f"{cli.title().replace('_', ' ')} CLI Parser")
    sparser = None
    subparsers = {}
    ...
    for subcommand, opts in subcommands.items():
        ...
        if "description" not in opts:
            opts["description"] = f"{cli.title().replace('_', ' ')} {subcommand.title().replace('_', ' ')} CLI Parser"
    ...
    for namespace, args in opt.items():
        for key, val in args.items():
            ...
            try:
                ...
                if key.endswith(("_dir", "_path", "_file")) and await path.is_absolute():
