    ...
    async def react_mutate(
        ...
        if method == 'Write':
            ...
            raise resemble.aio.aborted.SystemAborted(
                resemble.v1alpha1.errors_pb2.InvalidMethod(),
                message=
                f"Method 'WriteTask' can not be called via React (for now)"
            )
        elif method == 'Read':
            # Invariant here is that users should not have called this
            # directly but only through code generated React
            ...
            raise resemble.aio.aborted.SystemAborted(
                resemble.v1alpha1.errors_pb2.InvalidMethod(),
                message=
                f"Method 'DeleteTask' can not be called via React (for now)"
            )
        else:
            logger.warning(
                "Got a react mutate request with an invalid method name: "
...
class Secret:
    ...
    async def Write(
        ...
        *,
        ...
        return (
            reference,
            await reference._writer(__context_or_workflow__).Write(
                request,
                idempotency=idempotency,
                metadata=metadata,
                bearer_token=bearer_token,
    ...
    async def Delete(
        ...
        return (
            reference,
            await reference._writer(__context_or_workflow__).Delete(
                request,
                idempotency=idempotency,
                metadata=metadata,
                bearer_token=bearer_token,
