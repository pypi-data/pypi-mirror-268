...
from pragy_dl.utils import Popen

rootDir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
LAZY_EXTRACTORS = 'pragy_dl/extractor/lazy_extractors.py'


class TestExecution(unittest.TestCase):
    def run_pragy_dl(self, exe=(sys.executable, 'pragy_dl/__main__.py'), opts=('--version', )):
    ...
    def test_import(self):
        self.run_pragy_dl(exe=(sys.executable, '-c', 'import pragy_dl'))

    def test_module_exec(self):
        self.run_pragy_dl(exe=(sys.executable, '-m', 'pragy_dl'))

    def test_cmdline_umlauts(self):
        _, stderr = self.run_pragy_dl(opts=('ä', '--version'))
    ...
    def test_lazy_extractors(self):
        try:
            ...
            _, stderr = self.run_pragy_dl(opts=('-s', 'test:'))
