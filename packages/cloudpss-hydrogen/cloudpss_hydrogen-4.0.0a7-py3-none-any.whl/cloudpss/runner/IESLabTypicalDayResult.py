...
class IESLabTypicalDayResult(Result):
    def __init__(self, *args, **kwargs):
        ...
        self.__type_list =['电负荷', '热负荷','冷负荷','氢负荷','散射辐射', '直射辐射',  '天顶角', '环境温度', '土壤温度', '10m风速', '50m风速']
        self.result = {'TypicalMonth': [],'TypicalDay': []}
        for i in range(12):
            self.result['TypicalMonth'].append({'月份': int, '持续天数': [],'电负荷': [], '热负荷': [],'冷负荷':[],'氢负荷':[],'散射辐射': [], '直射辐射': [],'天顶角': [],'环境温度': [], '土壤温度': [], '10m风速': [], '50m风速': []})
        ...
        if (length > self.__plotIndex):
            for num in range(self.__plotIndex, length):# update TypicalMonth
                ...
                if val['type'] == 'plot':
                    ...
                    if key_list[0] == '散射辐射':#每个月各个典型日的天数
            ...
            for m in range(12):
                for i in range(len(self.result['TypicalMonth'][m]['持续天数'])):
                    self.result['TypicalDay'].append({'info':{'typicalDayID': int, 'name': str, 'duration': int, 'maxElectricalLoad': 0.0, 'maxHeatLoad': 0.0, 'maxCoolLoad': 0.0}, 
                                                      'data': {'电负荷': [], '热负荷': [],'冷负荷':[],'氢负荷':[],'散射辐射': [], '直射辐射': [],'天顶角': [],'环境温度': [], '土壤温度': [], '10m风速': [], '50m风速': []}})
                    ...
                    if self.result['TypicalMonth'][m]['氢负荷']:
                        if self.result['TypicalMonth'][m]['氢负荷'][i]:
                            self.result['TypicalDay'][-1]['info']['maxHydrogenLoad'] = max(self.result['TypicalMonth'][m]['氢负荷'][i])
