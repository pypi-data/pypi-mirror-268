...
class QuantizationErrorAnalyzer:
    ...
    def _plot_metric(self, attr: str, label: str = None, with_tags=True):
        y = [getattr(cmp, attr)() for cmp in self._compare_dict.values()]
        plt.plot(y, ".-", label=label)

        if with_tags:
            ticks = [cmp.layer_type for cmp in self._compare_dict.values()]
    ...
    def plot_qsnr(self, with_tags=True, show=False, fname: str = None):
        ...
        """
        self._plot_metric("qsnr", with_tags=with_tags)
        plt.ylabel("qSNR [dB]")
        plt.title("qSNR vs. Layer")
    ...
    def plot_dynamic_range_utilization(
        self, with_tags=True, show=False, fname: str = None
    ):
        ...
        Arguments:
            ...
            fname (str, optional): If not None, plt.savefig() will be called, saving the
                figure as a .png file under the given filename. Default None.
        """
        self._plot_metric(attr="std_over_dynamic", label="std", with_tags=with_tags)
        self._plot_metric(
            attr="meanabs_over_dynamic", label="meanabs", with_tags=with_tags
        )
        self._plot_metric(attr="max_over_dynamic", label="max", with_tags=with_tags)
        plt.grid(True)
        plt.legend()
        plt.yscale("log", base=2)
        plt.ylabel("Ratio over i16 Dynamic Range")
        plt.title("Dynamic Range Utilization vs. Network Depth")
