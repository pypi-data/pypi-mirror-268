...
def list(count, reverse):
    ...
    traces = list_traces_from_db(db_path, count=count, reverse=reverse)
...
def list_queries(trace_id, syntax_highlight):
    ...
    data = load_msgpack(msgpack_data)
    for frame in data["frames_of_interest"]:
...
@click.option(
    "--template", default="", help="The path to a custom test generation template."
)
@click.option(
    "--use-saved-schemas",
    default=False,
...
def generate_test(
    trace_ids, test_name, test_class, file, settings, template, use_saved_schemas
):
