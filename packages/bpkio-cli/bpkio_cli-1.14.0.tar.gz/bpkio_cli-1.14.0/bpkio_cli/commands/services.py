...
def source_commands(endpoint_path: str):
    ...
    @click.pass_obj
    def source(obj: AppContext, **kwargs):
    ...
    @click.pass_obj
    def ad_server(obj: AppContext, **kwargs):
        resource = retrieve_resource(endpoint_path=endpoint_path)
        source = None
        keys = ["vodAdInsertion", "adBreakInsertion", "liveAdReplacement"]
        for k in keys:
    ...
    @click.pass_obj
    def profile(obj: AppContext, **kwargs):
        resource = retrieve_resource(endpoint_path=endpoint_path)
        if hasattr(resource, "transcodingProfile"):
    ...
    return [source, ad_server, profile]
