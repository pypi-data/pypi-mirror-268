...
To install the latest release from [PyPI](https://pypi.org/project/mattmazda1122/), run `pip3 install mattmazda1122`.
...
```python
import asyncio
import mattmazda1122

async def main() -> None:
    # Initialize API client (MNAO = North America)
    client = mattmazda1122.Client("myemail", "mypassword", "MNAO")
...
```python
client = mattmazda1122.Client(email, password, region, websession)
```
...
Returns an instance of `mattmazda1122.Client` which can be used to invoke the below methods.
