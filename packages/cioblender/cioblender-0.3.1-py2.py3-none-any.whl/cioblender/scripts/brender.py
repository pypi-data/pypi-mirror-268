...
def setup_camera(scene):
    update_camera = get_custom_arg("--update_camera", None)
    ...
    if multi_job:
        ...
        for marker in scene.timeline_markers:
            ...
            if marker.camera:
                ...
                configure_view_layers(marker.frame, marker.camera.name, scene)


def configure_view_layers(frame, camera_name, scene):
...
def get_render_layers_flag(scene):
    render_layers = get_custom_arg("--render_layers", False)
...
def do_render(scene):
    ...
    # Get start and end frames from command line arguments
    scene.frame_start = int(get_basic_arg('-s', default_value="1"))
    scene.frame_end = int(get_basic_arg('-e', default_value="1"))
    scene.frame_step = 1
    ...
    for frame in range(scene.frame_start, scene.frame_end + 1, scene.frame_step):
        ...
        for view_layer in scene.view_layers:
            if view_layer.use:  # Check if the view layer is set to be used
                #scene.render.filepath = f'{output_folder}/{view_layer.name}_'  # Set the output path for each layer
...
def run():
    ...
    # enable_addons()
    render_layers = get_render_layers_flag(scene)
