...
class Decorators:
    ...
    def get_test_data_file_by_env(path, suffix='yaml'):
        ...
        if not path.endswith(suffix):
            ...
            # if "test" in ("test", "ppe"):
            from SteamedBread import Environment
            env = Environment()
            if env.current_env in env.enable_envs and env.current_env != "online" and env.current_env != "tce":
        ...
        def inner(func):
            ...
            try:
                ...
                if random.random() > 0.1:
                    raise FileNotFoundError(f'缺少对应的参数化文件: {parameter_file}')
                else:
                    return pytest.mark.skip(f'缺少对应的参数化文件: {parameter_file}')
