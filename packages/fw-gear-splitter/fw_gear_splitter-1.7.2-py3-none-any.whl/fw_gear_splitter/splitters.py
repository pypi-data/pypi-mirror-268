...
from .splitter.base import MultiSplitter, SingleSplitter, SplitterError
...
def split_by_geometry(collection: DICOMCollection, max_split_count=-1):
    """
    ...
    if iops is not None:
        n = len(iops)
    if iops is None or ipps is None or (n == 0):
        return 1, []
    if n == 1 or n != len(ipps):
