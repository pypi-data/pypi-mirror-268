    ...
    def submit(self) -> Tuple[bool, str]:
        ...
        for service, service_submission in self.serviceowner_submission.items():
            ...
            if response.status_code == 201:
                ...
                print(f"FAILED to submit service: `{service}`. Reason: {self.check_status_code(response)}")
            ...
            if service_submission.get("readme"):
                service_id = response["id"] if "id" in response else self.get_service_id_by_name(service)
                ...
                files = {"md_file": (f"{service}.md", open(service_submission.get("readme"), "rb"))}
                ...
                if response.status_code == 201:
                    ...
                    submitted_readme.append(service)
                if response.status_code == 200:
                    print(f"UPDATED README file for service: `{service}`.")
                    submitted_readme.append(service)
                else:
                    response = self.check_status_code(response)
                    print(f"FAILED to submit README file for service: `{service}`. Reason: `{response}.`")
            else:
    ...
    def get_service_id_by_name(self, service_name: str) -> int:
        ...
        try:
            SUBMIT_SERVICEOWNER_REQUEST_PATH = (
                f"{self.auth.fqdn}{SUBMIT_SERVICEOWNER_SUBMISSION_ENDPOINT}?name={service_name}"
            )
