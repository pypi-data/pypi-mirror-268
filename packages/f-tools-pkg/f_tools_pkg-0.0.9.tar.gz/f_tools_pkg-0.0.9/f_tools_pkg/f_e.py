...
class yc_e:
    ...
    def run(self,command, timeout_duration=None):
        try:
            ...
            print(f"Calling {command}, timed out after {timeout_duration} seconds.")
            raise 
    ...
    def e_bu(self,addr,type=0):
        ret = self.run("e bu " + self.tohex(addr) + self.tohex(type))[0]
        print(ret)
    
    def e_ru(self,reg,val=None):
        if(val!=None):
            ret = self.run("e ru "+reg+" "+self.tohex(val))[0]
        else:
            ret = self.run("e ru "+reg)[0]
            print(ret)
    ...
    def get_mem_data(self,addr,len):
        result = self.run("e "+self.tohex(addr)+"l"+self.tohex(len),self.timeout+int(len/500))
        odata=[]
...
# try:
#     t=time.time()
#     print(e.get_mem_data(0x8000,1000))
#     print(time.time()-t)
