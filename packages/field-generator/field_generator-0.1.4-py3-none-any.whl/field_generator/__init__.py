...
def solutionCurve(func, xinit, yinit, xmin=-10, xmax=10, ymin=-10, ymax=10, figureNumber=None):
    ...
    if not (type(func) is str or hasattr(func, '__call__')):
        raise Exception("L, bad argument")
    if type(func) is str:
        ylambda = lambda x, y: _parseFunc(func, x, y)
    else:
        ylambda = lambda x, y: func(x, y)

    leftTimes = np.linspace(xinit, xmin, 500)
    solution = solve_ivp(ylambda, y0=(xinit, yinit), t_span=(xinit, xmin), t_eval=leftTimes)
    xvals = []
    yvals = []
    if len(solution.y) > 0:
    ...
    rightTimes = np.linspace(xinit, ymax, 500)
    solution = solve_ivp(ylambda, y0=(xinit, yinit), t_span=(xinit, xmax), t_eval=rightTimes)
    if len(solution.y) > 0:
        for i in range(len(solution.y[0])):
            x = solution.t[i]
            y = solution.y[1][i]
