...
from agit.common import LLMResp, ToolCall, ToolDesc, Usage
...
def _extract_tool_calls(api_resp)->Tuple[List[ToolCall], Any]:
    tool_calls = []
...
def call_llm(messages: List[str], model: str, request_id: str,
             ...
             temperature=0.7, top_p=0.95, max_tokens: int = None, do_sample=True,
             **kwargs) -> LLMResp:
    ...
    with logger.contextualize(request_id=request_id):
        ...
        # total_speed = usage.total_tokens/total_cost
        # perf = Perf(first_token_latency=first_token_latency, total_cost=total_cost, decode_speed=decode_speed, encode_speed=encode_speed, total_speed=total_speed)
        resp = LLMResp(content=content, details=details, usage=usage, perf=None, tool_calls=tool_calls)
        return resp
