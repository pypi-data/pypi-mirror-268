...
class IssueWorkflowJSON():
    ...
    def get_workflow(json_data,issue_key,status):
        issue_data = IssueWorkflowJSON.get_issue_from_json(issue_key,json_data)
        ...
        print(issue_key,status,to_time,max_to_time)
        return max_to_time
    
    @staticmethod
    def get_lead_time(json_data,issue_key, first_status, last_status):
        workflow1 = IssueWorkflowJSON.get_workflow(json_data, issue_key,first_status)
        workflow2 = IssueWorkflowJSON.get_workflow(json_data, issue_key,last_status)
        print('worflow1,workflow2',workflow1,workflow2)
        if workflow1 and workflow2:
            print(workflow1 > workflow2)
