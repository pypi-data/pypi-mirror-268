...
class IssueS2Thread(threading.Thread):
    ...
    def run(self):
        params = {'jql': self.jql_query, "startAt": self.start_at, "maxResults": self.batch_size, "fields": self.fields,"expand": 'changelog'}
...
class IssueS2(Issue):
    ...
    def find_by(jql_query):
        return IssueS2.get_issue_data_from_jira_with_thread(jql_query=jql_query)
    ...
    def all_lite():
        ...
        return [IssueS2(key=issue["key"], updated=issue["fields"].get("updated", None),
                                      status_updated=issue["fields"].get("statuscategorychangedate")) for issue in issues]
