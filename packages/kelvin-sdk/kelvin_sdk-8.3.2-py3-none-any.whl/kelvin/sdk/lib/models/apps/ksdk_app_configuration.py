...
class ProjectType(BaseEnum):
    ...
    @classmethod
    def app_types(cls) -> List:
        """
        Applications supported by the schema.

        Returns
        -------

        """
        return [cls.kelvin, cls.docker]
    ...
    def app_types_as_str(cls) -> List[str]:
        ...
        """
        return [cls.kelvin.value, cls.docker.value]

    def project_class_name(self) -> str:
        return_result: str = "project"
        if self in [ProjectType.kelvin, ProjectType.kelvin_legacy, ProjectType.docker, ProjectType.bridge]:
            return_result = "application"
