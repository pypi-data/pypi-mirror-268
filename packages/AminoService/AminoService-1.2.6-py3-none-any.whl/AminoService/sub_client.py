...
class SubClient(Client):
    ...
    def live_notify(self, chatId: str, userId: str):
        ...
        response = self.session.post(f"{self.api}/x{self.comId}/s/chat/thread/{chatId}/member/{userId}/invite-av-chat", headers=self.parse_headers(data=data), data=data, proxies=self.proxies, verify=self.certificatePath)
        if response.status_code != 200: return exceptions.CheckException(json.loads(response.text))
        else: return response.status_code

    def get_chat_lurkers(self, start: int = 0, size: int = 100):
        return self.session.get(f"{self.api}/x{self.comId}/s/live-layer/public-chats?start={start}&size={size}", headers=self.parse_headers(), proxies=self.proxies, verify=self.certificatePath)
