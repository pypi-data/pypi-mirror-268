...
def get_jwt_secret():
    return os.environ.get("aa_ui_AUTH_SECRET")


def ensure_jwt_secret():
    if require_login() and get_jwt_secret() is None:
        raise ValueError(
            "You must provide a JWT secret in the environment to use authentication. Run `aa_ui create-secret` to generate one."
        )
...
def require_login():
    return (
        bool(os.environ.get("aa_ui_CUSTOM_AUTH"))
