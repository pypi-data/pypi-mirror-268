...
from aa_ui.oauth_providers import get_oauth_provider
...
from aa_ui.auth import create_jwt, get_configuration, get_current_user
...
from aa_ui.data.acl import is_thread_author
from aa_ui.logger import logger
from aa_ui.markdown import get_markdown_str
from aa_ui.playground.config import get_llm_providers
...
app.mount(
    "/assets",
    StaticFiles(
        packages=[("aa_ui", os.path.join(build_dir, "assets"))],
...
app.mount(
    "/copilot",
    StaticFiles(
        packages=[("aa_ui", copilot_build_dir)],
        follow_symlink=config.project.follow_symlink,
    ),
    name="copilot",
)
...
def get_html_template():
    ...
    default_url = "https://github.com/aa_ui/aa_ui"
    ...
    <meta property="og:image" content="https://aa_ui-cloud.s3.eu-west-3.amazonaws.com/logo/aa_ui_banner.png">
...
def get_user_facing_url(url: URL):
    ...
    aa_ui_url = os.environ.get("aa_ui_URL")
    ...
    config_url = URL(aa_ui_url).replace(
        query="",
        fragment="",
    )
...
async def oauth_login(provider_id: str, request: Request):
    ...
    samesite = os.environ.get("aa_ui_COOKIE_SAMESITE", "lax")  # type: Any
