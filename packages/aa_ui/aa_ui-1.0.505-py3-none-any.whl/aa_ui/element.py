...
from aa_ui.types import FileDict
...
class Element:
    ...
    # The key of the element hosted on aa_ui.
    aa_ui_key: Optional[str] = None
    ...
    async def _create(self) -> bool:
        ...
        if not self.url and (not self.aa_ui_key or self.updatable):
    ...
    async def send(self, for_id: str):
        ...
        if not self.url and not self.aa_ui_key:
            raise ValueError("Must provide url or aa_ui key to send element")
