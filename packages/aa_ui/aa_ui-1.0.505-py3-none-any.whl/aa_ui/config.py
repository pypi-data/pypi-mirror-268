...
from aa_ui.translations import lint_translation_json
...
config_dir = os.path.join(APP_ROOT, ".aa_ui")
...
# Follow symlink for asset mount (see https://github.com/aa_ui/aa_ui/issues/317)
...
@dataclass()
class aa_uiConfig:
    # Directory where the aa_ui project is located
    root = APP_ROOT
    # aa_ui server URL. Used only for cloud features
    aa_ui_server: str
    run: RunSettings
...
def load_config():
    ...
    aa_ui_server = os.environ.get("aa_ui_SERVER", "https://cloud.aa_ui.io")

    config = aa_uiConfig(
        aa_ui_server=aa_ui_server,
        run=RunSettings(),
        **settings,
    )
...
def lint_translations():
    # Load the ground truth (en-US.json file from aa_ui source code)
