...
class PhotovoltaicPowerPlant(Generator):
    ...
    def step(self):
        ...
        if not self.config.is_static_t_module:
            ...
            self.state.t_module_deg_celsius += t_module_change
        ...
        # print(
        #     f"Now: {self.inputs.now_dt.timestamp()} -- "
        #     f"{irradiance_w_per_m2} * {self.config.eta_percent}"
        #     f"/ 100 * {k_temp} * {k_irradicance} * {self.config.a_m2} / 1e3"
        #     f" = {self.state.p_kw} kW"
        # )
