...
class DEnsembleModel(Model, FeatureInt):
    ...
    def __init__(
        self,
        base_model="catboost",#"catboost",#"gbm",
        loss="RMSE",#"mse",
        num_models=6,
        enable_sr=True,
        enable_fs=True,
...
#         self.params = {"objective": loss}
    ...
    def fit(self, df_train, df_valid, savepath): #(self, dataset: DatasetH):
#         df_train, df_valid = dataset.prepare(
#             ["train", "valid"], col_set=["feature", "label"], data_key=DataHandlerLP.DK_L
#         )
        if df_train.empty or df_valid.empty:
            raise ValueError("Empty data from dataset, please check your dataset config.")
        x_train, y_train = df_train["feature"][0], df_train["label"][0]
        ...
        for k in range(self.num_models):
            ...
            model_k = self.train_submodel(df_train, df_valid, weights, features,savepath,k)
...
#             print("loss_values =", loss_values)###
    ...
    def train_submodel(self, df_train, df_valid, weights, features,savepath,k):
        if self.base_model == "gbm":
            ...
            x_train, y_train = df_train["feature"][0].loc[:, features], df_train["label"][0]
            x_valid, y_valid = df_valid["feature"][0].loc[:, features], df_valid["label"][0]
            ...
            model.save_model(savepath+"/sub"+str(k)+".json")
...
#         print("loss_curve_norm =", loss_curve_norm)###
        loss_values_norm = (-loss_values).rank(pct=True)
#         print("loss_values_norm =", loss_values_norm)###
        ...
        show_h = pd.concat([
            pd.DataFrame(h1,columns = ["h1"]),
            pd.DataFrame(h2,columns = ["h2"]),
            h
        ], axis=1)
        self.show_h.append(show_h)
...
#         loss_curve.columns = ["loss_curve"]
        show = pd.concat([
            loss_curve,
            pd.DataFrame(loss_values,columns = ["loss_values"]),
...
#             raise ValueError("not implemented yet")
            return np.sqrt(((np.array(label) - pred) ** 2).mean())
...
#             raise ValueError("not implemented yet")
            ...
            pred_tree = np.zeros(N, dtype=float)
            for i_tree in range(num_trees):
                pred_tree += model.predict(np.array(x_train))###
                loss_curve.iloc[:, i_tree] = self.get_loss(y_train, pred_tree)
        return loss_curve

    def predict(self, df_test): # (self, dataset: DatasetH, segment: Union[Text, slice] = "test"):
        if self.ensemble is None:
            raise ValueError("model is not fitted yet!")
#         x_test = dataset.prepare(segment, col_set="feature", data_key=DataHandlerLP.DK_I)
        x_test, y_test = df_test["feature"][0], df_test["label"][0]
    ...
    def predict_sub(self, submodel, df_data, features):
        x_data = df_data["feature"][0].loc[:, features]
