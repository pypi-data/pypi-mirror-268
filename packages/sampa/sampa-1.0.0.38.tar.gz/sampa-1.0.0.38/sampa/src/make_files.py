...
for i in range(len(files)):
    ...
    A1 = poscar.readline().split();  A1x = A1[0]*param;  A1y = A1[1]*param;  A1z = A1[2]*param;  A1 = np.array([A1x, A1y, A1z]);  mA1 = np.linalg.norm(A1)
    A2 = poscar.readline().split();  A2x = A2[0]*param;  A2y = A2[1]*param;  A2z = A2[2]*param;  A2 = np.array([A2x, A2y, A2z]);  mA2 = np.linalg.norm(A2)
    A3 = poscar.readline().split();  A3x = A3[0]*param;  A3y = A3[1]*param;  A3z = A3[2]*param;  A3 = np.array([A3x, A3y, A3z]);  mA3 = np.linalg.norm(A3)
