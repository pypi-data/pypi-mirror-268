    ...
    def load_fen(self, x: Iterable[str]):
        for fen in x:
            ...
            for move in board.legal_moves:
                ...
                match pt:
                    case chess.QUEEN:
                        actions += [VEC_BLOCK * 2 + idx, VEC_BLOCK * 3 + idx]
                    case chess.KING:
                        actions += [VEC_BLOCK * 4 + idx]
                    case _:
                        actions += [VEC_BLOCK * (pt - 1) + idx]

            mat = scipy.sparse.csr_matrix(
                (
                    np.ones_like(actions, dtype=np.uint8),
                    (np.zeros_like(actions), actions),
                ),
                shape=(1, VEC_SIZE),
            )
