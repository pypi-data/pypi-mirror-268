...
class SubmissionWatcher(BaseWatcher):
    def __init__(self,
                 w3: Web3,
        ...
        while True:
            try:
                ...
                for new_voting_entry in new_voting_entries:
                    # print(f" ------------ {new_voting_entry[1]}")
                    await self._handle_topic_data(new_voting_entry[1])
                    await self._handle_event_data(new_voting_entry[0])
    ...
    async def _handle_topic_data(self, data: any):
        pass
        # print(data)
    ...
    def handle_new_voting_round_event(self, event_data: AttributeDict):
        ...
        self.log_with_clock(log_level=logging.INFO,
                            msg=f"New voting round {vr.voting_round} issued at block - {vr.block_number}. ")
