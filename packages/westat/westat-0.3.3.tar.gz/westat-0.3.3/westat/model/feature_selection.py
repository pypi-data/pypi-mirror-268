...
def get_feature_by_ivcorr(data: pd.DataFrame,
    ...
    # 根据每一列的IV值，计算相关矩阵
    data_iv_filter = data_iv[data_iv['Name'].isin(df.columns)]
    ...
    data_corr = df[data_iv_filter['Name']].corr()

    col_iv_result = []
    for i in range(len(data_iv_filter)):
        for j in range(len(data_iv_filter)):
            col1 = data_iv_filter.iloc[i, 0]
            iv1 = data_iv_filter.iloc[i, 1]
            col2 = data_iv_filter.iloc[j, 0]
            iv2 = data_iv_filter.iloc[j, 1]
