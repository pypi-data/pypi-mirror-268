...
def plot_iv(data: pd.DataFrame,
    ...
    iv_result = get_woe_iv(data=data,
                           col=col,
                           bins=bins,
                           qcut=qcut,
                           missing=missing,
                           ...
                           precision=precision,
                           language=language)
    result = iv_result.iloc[:-1, :]
    x = [str(x) for x in result['Bin']]
    iv = pd.to_numeric(result['IV'])
    badrate = pd.to_numeric(result['%BadRate'].apply(lambda x:(x[:-1])))/100
    total_badrate = pd.to_numeric((iv_result['%BadRate'].iloc[-1])[:-1])/100
    good = result['#Good']
    bad = result['#Bad']
    total_iv = iv.sum()
