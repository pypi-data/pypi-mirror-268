...
def plot_ks(data: pd.DataFrame,
            data_proba: pd.DataFrame = pd.DataFrame(),
            score_card: pd.DataFrame = pd.DataFrame(),
            init_score: int = 600,
            pdo: int = 50,
            odds: float = 0,
            target: str = 'y',
    ...
    Args:
        ...
        data_proba:pd.DataFrame，预测得分,列名需要为 "Proba"
    ...
    if data_proba.empty:
        data_proba = get_predict_score(data,
                                       score_card,
    ...
    fpr, tpr, thresholds = roc_curve(data['y'], data_proba['Proba'], drop_intermediate=False)
    plt.figure(figsize=(10, 8))

    # KS曲线
    plt.subplot(1, 1, 1)
    pre = sorted(data_proba['Proba'], reverse=True)
