...
class DIOSENSORSCOPE(QtWidgets.QDialog, ui_dio_sensor_scope.Ui_Dialog):
    def __init__(self, parent, sensor, addr, dev):
        ...
        self.initialize = sensor['init']
        self.address = addr
        self.scopereg = sensor['scopereg']
        self.scopebytes = sensor['scopebytes']
        self.offset = 0
    ...
    def iterateScope(self):
        ...
        if not self.busy: # initialize the sensor capture routine
            ...
            self.fitcurve.clear()
            self.p.__capture_i2c_init__(self.address, self.scopereg+self.offset, self.scopebytes, self.NP, self.TG)
            self.start_time = time.time()
            self.fetched_samples = 0
            self.busy = True
            self.nosingleshot = True
            ...
            if available_samples > (self.p.fetched_i2c_scope_buffer+chunksize*.8) or available_samples == self.NP or elapsed_time>0.2:
                ...
                v = self.p.__retrieve_incremental_buffer__(available_samples)
                available_samples  = len(v) #Overwrite if packets were dropped , and data is incomplete.
                if available_samples == self.NP:
                    self.busy = False

                self.time = np.linspace(0, 1e-6 * self.TG * available_samples, available_samples)
                self.values = v
                self.datapoints = available_samples
                self.curve.setData(self.time, v)
    ...
    def sineFit(self):
        ...
        res = 'Amp, Freq, Phase, Offset<br>'
        try:
            fa = utils.fit_sine(self.time[start:end], self.values[start:end])
            if fa is not None:
                amp = abs(fa[0])
                freq = fa[1]
                phase = fa[2]
    ...
    def dampedSineFit(self):
        ...
        try:
            fa = utils.fit_dsine(self.time[start:end], self.values[start:end])
            if fa is not None:
                amp = abs(fa[0])
                freq = fa[1]
                decay = fa[4]
                ...
                x = np.linspace(self.time[start], self.time[end], 1000)
                self.fitcurve.clear()
                self.fitcurve.setData(x - self.T, utils.dsine_eval(x, fa))
                self.fitcurve.setVisible(True)
        except Exception as e:
            res += '--<br>'
            print(e)
