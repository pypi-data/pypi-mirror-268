    ...
    * loads calibration values.
    ...
    def __runInitSequence__(self, **kwargs):
        ...
        if kwargs.get('load_calibration', True) and self.H.connected and 'ip' not in kwargs:
            ...
            polynomials = self.read_bulk_flash(self.ADC_POLYNOMIALS_LOCATION, 600)
            polyDict = {}
            if polynomials[:7] == b'ExpEYES':  # PYTHON3
                ...
                poly_sections = polynomials.split(
                    b'STOP')  # The 2K array is split into sections containing data for ADC_INL fit, ADC_CHANNEL fit, DAC_CHANNEL fit, PCS, CAP ... #PYTHON3
    ...
    def __capture_fullspeed_init__(self, chan, samples, tg, *args, **kwargs):
        ...
        try:
            ...
            if 'FIRE_PULSE' in args:
                ...
                if kwargs.get('pulse_type', 'high_true') == 'high_true':
                    self.H.__sendInt__(0x8000 | pwd)
                else:
                    self.H.__sendInt__(pwd)
                time.sleep(pwd * 1e-6)
    ...
    def __capture_fullspeed__(self, chan, samples, tg, *args, **kwargs):
        ...
        **Arguments**
        ...
        return np.linspace(0, 1e-3 * self.timebase * (self.samples - 1), self.samples), self.analogInputSources[
            chan].calPoly12(y)  # time in mS
        ...
        **Arguments**
        ...
        try:
            ...
            if num == 1:
                ...
                self.achans[0].set_params(channel=channel_one_input, length=samples, timebase=self.timebase,
                                          resolution=10, source=self.analogInputSources[channel_one_input])
                self.achans[1].set_params(channel='A2', length=samples, timebase=self.timebase, resolution=10,
                                          source=self.analogInputSources['A2'])
        ...
        **Arguments**
        ...
        try:
            ...
            for C in np.sort(results.keys()):
                name = results[C]
                data[name] = [np.linspace(0, self.timebase * (self.samples - 1), self.samples),
                              self.analogInputSources[name].calPoly12(self.__retrieveBufferData__(num, self.samples))]
                num += 1
            for name in args:
                yield data[name][0]
                yield data[name][1]
    ...
    def __fetch_channel__(self, channel_number):
        ...
        **Arguments**
        ...
        try:
            ...
            self.achans[channel_number - 1].fetched_length = samples
        except Exception as ex:
            msg = "Incorrect Number of bytes received.\n"
            return False
    ...
    def __fetch_incremental_channel__(self, channel_number, upto):
        max_samples = self.achans[channel_number - 1].length
        offset = self.achans[channel_number - 1].fetched_length
        if upto > max_samples:
            upto = max_samples
        ...
        try:
            ...
            self.H.__sendByte__(CP.GET_CAPTURE_CHANNEL)
            self.H.__sendByte__(channel_number - 1)  # starts with A0 on PIC
            self.H.__sendInt__(upto - offset)  # Samples to get
            self.H.__sendInt__(offset)  # Starting position
            data += self.H.fd.read(
                int((upto - offset) * 2))  # reading int by int sometimes causes a communication error.
            self.H.__get_ack__()
    ...
    def __retrieve_incremental_buffer__(self, upto):
        #print('fetch upto', upto, 'from', self.fetched_i2c_scope_buffer)
        ...
        if len(data) == samples*2:
            for a in range(samples):
                self.buff[self.fetched_i2c_scope_buffer+a] = np.int16((data[a * 2] << 8) | data[a * 2 + 1])
        else:
            print('insufficient data . return.', self.fetched_i2c_scope_buffer)
            return self.buff[:self.fetched_i2c_scope_buffer]
    ...
    def DoublePinEdges(self, channel1, channel2, edge1, edge2, points1, points2, timeout=1.0, **kwargs):
        ...
        **Arguments**
        ...
        try:
            ...
            self.H.__sendByte__(SEQ | points1 | (
                    points2 << 4))  # Number of points to fetch from either channel |0x80 if sequential is True
    ...
    def get_states(self):
        ...
        try:
            ...
            return {'IN2': (s & 1 != 0), 'SQR1': (s & 2 != 0), 'OD1': (s & 4 != 0), 'SEN': (s & 8 != 0),
                    'SQR1_OUT': (s & 16 != 0), 'OD1_OUT': (s & 32 != 0), 'CCS': (s & 64 == 0)}  # CCS is inverted
        except Exception as ex:
            self.raiseException(ex, "Communication Error , Function : " + inspect.currentframe().f_code.co_name)
    ...
    bp = 2;
    bn = 8;
    stepper_positions = [ap + bp, an + bp, an + bn, ap + bn]  # {AP+BP,AN+BP,AN+BN,AP+BN}; //full step sequence.

    def stepper_move(self, steps, direction, step_delay=0.01):
        for a in range(steps):
            if (direction):
                self.stepper_forward()
            else:
                self.stepper_reverse()
            time.sleep(step_delay)
        self.set_multiplexer(15)

    def stepper_forward(self):
        self.stepper_position += 1
        if (self.stepper_position > 3): self.stepper_position = 0
        self.set_multiplexer(self.stepper_positions[self.stepper_position])

    def stepper_reverse(self):
        self.stepper_position -= 1
        if (self.stepper_position < 0): self.stepper_position = 3
        self.set_multiplexer(self.stepper_positions[self.stepper_position])
    ...
    def tim_helper(self, cmd, src, dst, timeout=2.5):
        ...
        if cmd == 'multi_r2r':
            ...
            # print (edge,count)
            T = self.SinglePinEdges(src, edge, count,
                                    timeout)  # src,src,rising edges , rising edges, total 2, total 2, 1 second timeout.
            # print ('multi_r2r',T)
            if T is not None:
                return T[count - 1] - T[0]
            else:
                return -1
            ...
            T1, T2 = self.DoublePinEdges(src, dst, edge1, edge2, 1, 2, timeout,
    ...
    def duty_cycle(self, pin, timeout=2.0):
        ...
        if T1 is not None and T2 is not None:
            if T2[1]:
                return 100 * (T2[1]) / (T1[1] - T1[
                    0])  # T2[0] will always equal 0 because sequential mode is enabled, but one falling edge must occur between two rising edges.
            elif T2[2]:
                return 100 * (T2[2]) / (T1[1] - T1[
                    0])  # T2[1] can be zero on rare occassions if input frequency is too high causing a falling edge to be recorded before the T2 timer is active
        else:
            return -1
    ...
    def readbackWaveform(self, chan):
        ...
        **Arguments**
        ...
        if chan == 'WG':
            ...
            return self.sqr1freq
        elif chan[:3] == 'SQ2':
            return self.sqr2freq
    ...
    def set_sq1(self, freq, duty_cycle=50):
        return self.set_sqr1(freq, duty_cycle)

    def set_sq2(self, freq, duty_cycle=50):
        return self.set_sqr2(freq, duty_cycle)

    def set_sq1_dc(self, duty_cycle):
        self.set_sqr1(self.sqr1freq, duty_cycle)

    def set_sq2_dc(self, duty_cycle):
        self.set_sqr2(self.sqr2freq, duty_cycle)
    ...
    def set_sqr1(self, freq, duty_cycle=50):
        ...
        **Arguments**
        ...
        self.sqr1freq = 64e6 / wavelength / p[prescaler & 0x3]
        self.sqr1dc = duty_cycle
        return self.sqr1freq
    ...
    def set_sqr2(self, freq, duty_cycle=50):
        ...
        **Arguments**
        ...
        try:
            ...
            self.raiseException(ex, "Communication Error , Function : " + inspect.currentframe().f_code.co_name)
        self.WaveMode = 'sqr2'
        self.sqr2freq = 64e6 / wavelength / p[prescaler & 0x3]
        return self.sqr2freq
    ...
    def set_sqrs(self, freq, diff=0):  # Freq in Hertz, phase difference in % of T
        ...
        **Arguments**
        ...
        print(int(round(wavelength)), ' : ', int(round(phase)), ':', prescaler)
        try:
            self.H.__sendByte__(CP.WAVEGEN)
            self.H.__sendByte__(CP.SET_SQR1)
            self.H.__sendInt__(int(round(wavelength)))
