...
def service_status(stdscr, idx):
    ...
    if platform.system() != 'Windows':
        ...
        real_stdsrc(idx, ll*2-2, scr_str("sg-gg".ljust(ll)))
        idx+=1
    ...
    for d in range(0, max([len(test_lst),len(us_lst),len(sg_lst)])):
        ...
        widget_stdscr(d, sg_lst, idx, ll*2-2, "sg-gg" if platform.system() == 'Windows' else "")
        idx+=1
    return idx
...
def status():
    ...
    if platform.system() == 'Windows':
        ...
        print(scr_line("-" * ll*3))
        print(scr_str(f"Mecord".ljust(ll*3-2)))
        ...
        try:
            ...
            while True:
                ...
                idx+=1
                stdscr.addstr(idx, 0, scr_str(f"Mecord".ljust(ll*3-2)))
                idx+=1
