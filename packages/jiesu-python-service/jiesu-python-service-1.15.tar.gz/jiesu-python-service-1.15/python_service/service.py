...
rm dist/ -rf
rm jiesu_python_service.egg-info/ -rf
...
def register_eureka(eureka_server: str, name: str, port: int, instance: str) -> None:
...
class BaseHttpHandler:
    ...
    def get_body(self) -> bytes:
        content_len = int(self.request_handler.headers.get("Content-Length"))  # type: ignore
...
class Service:
    ...
    def ping_monitor(self, name: str):
        if self.args.service_monitor is None:
            self.log.warn("Can't ping monitor, monitor service URL was not provided.")
        else:
            try:
                requests.post(self.args.service_monitor + "/" + name)
            except Exception:
                self.log.warn("Failed to ping service monitor.")
    ...
    def send_email(self, email: Email):
        try:
            ...
            self.log.warn("Failed to send email.")
            self.log.exception(e)

    def get_eureka_service_urls(self, serviceName: str) -> List[str]:
        response = requests.get(self.eureka_server + '/eureka/apps/' + serviceName)
        root = ET.fromstring(response.content)
        ...
        for instance in instances:
            ...
            if ip is not None and port is not None and ip.text is not None and port.text is not None:
                results.append("http://" + ip.text + ":" + port.text)
        return results
