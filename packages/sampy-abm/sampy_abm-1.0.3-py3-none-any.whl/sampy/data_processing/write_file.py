...
def counts_to_csv(list_count_arr, graph, path_to_csv, sep=';', timesteps=None, use_index_as_col_name=False):
    """
    ...
    :param timesteps: optional, list or array, default None. If not None, used to create a column 'timesteps' in the
                      csv.
    :param use_index_as_col_name: optional, boolean, default False. If True, the names of the columns in the CSV will
                                  be the index (i.e. integers) of the vertices of the graph. Else, it will be their
                                  id (generally a string or a couple).
    """
    row_stack = np.row_stack(list_count_arr)
    if use_index_as_col_name:
        columns = [str(i) for i in range(graph.connections.shape[0])]
    else:
