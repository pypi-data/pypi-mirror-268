...
def fix_encode_html(raw_html, encoding="utf-8"):
    ...
    if encode_type is not None and encode_type.lower() not in guesses:
        guesses.append(encode_type.lower())
    detection_results = from_bytes(raw_html[:15000]) or from_bytes(raw_html)
    for r in detection_results:
        if r.encoding.lower() not in guesses:
            guesses.append(r.encoding.lower())
    if encoding not in guesses:
        guesses.append(encoding)

    guesses = [i for i in guesses if i not in UNICODE_ALIASES]

    for guess_type in guesses:
        try:
            content = raw_html.decode(guess_type)
            return content, guess_type
        except:
            pass
