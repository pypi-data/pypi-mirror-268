...
def return_format_datetime(date_strs):
                        ...
                        if "刚刚" in mid_str:
                            ...
                            if "天前" in mid_str:
                                ...
                                public_time = return_format_datetime(
                                    ...
                                    + " "
                                    + date_strs.split("昨天")[1].strip()
                                )
                                if public_time == "":
                                    public_time = return_format_datetime(
                                        int(
                                            return_stand(return_now_stamp()) - all_stamp
                                        )
                                    )
                            elif "前天" in mid_str:
                                all_stamp = 2 * 86399
    ...
    if (">" in public_time_ok) or ("<" in public_time_ok):
        ...
        if len(date_strs) == 14:
            try:
                ...
                if int(public_time_ok_split[0]) > int(year_now()):
                    need_check_times = (
                        year_now()[:2]
                        + public_time_ok_split[2].split(" ")[0]
                        + "-"
                        + public_time_ok_split[1]
                        + "-"
                        + public_time_ok_split[0][2:]
                        + " "
        ...
        if need_check_times == "":
            ...
            if len(re.findall("-", need_check_times)) > 2:
                if need_check_times.endswith("--"):
                    need_check_times_ok = need_check_times.replace("--", "").strip()
                else:
                    try:
                        need_check_times_ok = need_check_times.replace(
                            ("-" + need_check_times.split("-")[-1]), ""
                        ).strip()
                    except:
                        need_check_times_ok = ""
            else:
        ...
        if (
            len(re.findall("-", need_check_times_ok)) > 2
            or len(re.findall("/", need_check_times_ok)) > 1
        ):
            return ""
        else:
