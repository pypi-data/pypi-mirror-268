...
def preserved_arguments(
    func: t.Callable,
    *args: t.Any,
    **kwargs: t.Any,
) -> t.Any:
    fn_code: CodeType = func.__code__
    possible_args: Integer = fn_code.co_argcount
    possible_kwargs: t.Tuple[String, ...] = fn_code.co_varnames
    dispatched_args = dict(
        zip(
            possible_kwargs[:possible_args],
            args[:possible_args],
        ),
    )
    dispatched_args |= kwargs
    return func(**dispatched_args)
