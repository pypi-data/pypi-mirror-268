...
def add_account(dest_path):
    ...
    if 'mode' in os.environ:
        ...
        IBC_tradingMode = 'live' if click.confirm('Is it a Live-Account?',
                                                  default=True) else 'paper'
...
def connected_ib(config, api_ip, api_port, clientId):
    ...
    try:
        ...
        if 'ibcIniPath' in config:
            IBC_commands['ibcIni'] = os.path.join(config['ibcIniPath'],
                                                  str(api_port) + '.ini')
...
def check_order(symbol, fb_ist, fb_soll, ib_ist, ib_soll, order):
    ...
    if fb_soll + fb_ist == 0 and fb_soll != ib_soll:
        ...
        return order
    if fb_ist + fb_soll == 0 and ib_ist + ib_soll > 0:
...
def starter(env, action, ip, port):
    ...
    for folioname in list(folios):
        ...
        if f['ib_sync']:
            ...
            sync(f['ib_account'], config, ip or f['ib_ip'],
                 port or f['ib_port'], f['positions'], f['orders'], clientId)
            time.sleep(1)
            clientId = clientId + 1
...
def upload_positions(env):
    ...
    for folioname in list(folios):
        ...
        if f['ib_sync']:
            ...
            # fb_orders = f['orders']
            # debug_meldungen_fb(account, api_ip, api_port,
            #                   fb_positions, fb_orders)
            ib = connected_ib(config, api_ip, api_port, clientId)
            accountlist = ib.managedAccounts()
            if account not in accountlist:
                ib.disconnect()
