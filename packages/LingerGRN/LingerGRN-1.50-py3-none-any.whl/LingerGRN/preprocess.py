...
def preprocess(TG_pseudobulk,RE_pseudobulk,GRNdir,genome,method,outdir,workdir):
...
### if you have the label (cell type annotation)
    ...
    adata_RNA=adata_RNA[adata_RNA.obs['barcode'].isin(label['barcode_use'].values)]
    label.index=label['barcode_use']
    adata_RNA.obs['label']=label.loc[adata_RNA.obs['barcode']]['label'].values
    adata_ATAC=adata_ATAC[adata_ATAC.obs['barcode'].isin(label['barcode_use'].values)]
