...
from .common import _test_arguments, make_time_unit_str
...
def read_nexrad_cdm(
    ...
    **kwargs
    ...
    fdata = {
        "Reflectivity": np.ma.masked_equal(
            np.ones((nrays, ngates), dtype="float32"), 1
        ),
        "RadialVelocity": np.ma.masked_equal(
            np.ones((nrays, ngates), dtype="float32"), 1
        ),
        "SpectrumWidth": np.ma.masked_equal(
            np.ones((nrays, ngates), dtype="float32"), 1
        ),
        "DifferentialReflectivity": np.ma.masked_equal(
            np.ones((nrays, ngates), dtype="float32"), 1
        ),
        "CorrelationCoefficient": np.ma.masked_equal(
            np.ones((nrays, ngates), dtype="float32"), 1
        ),
        "DifferentialPhase": np.ma.masked_equal(
            np.ones((nrays, ngates), dtype="float32"), 1
        ),
    }
    ...
    if (
        hasattr(dataset, "StationLatitude")
        and hasattr(dataset, "StationLongitude")
        and hasattr(dataset, "StationElevationInMeters")
    ):
...
def _scan_info(dvars):
    ...
    for time_var in time_variables:
        for time_var_i, time in enumerate(dvars[time_var][:, 0]):
            ...
            if time_var == "timeV":
                _populate_scan_dic(
                    scan_dic, time_var, time_var_i, "SpectrumWidth", dvars
                )
            if time_var == "timeV_HI":
                _populate_scan_dic(
                    scan_dic, time_var, time_var_i, "SpectrumWidth_HI", dvars
                )
