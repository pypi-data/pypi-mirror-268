...
class MobSpawner:
    ...
    def spawn(self):
        ...
        if math.floor(random.random() * 100) < size and \
    ...
    def performSpawning(self, count, entity, loader):
        ...
        for i in range(count):
            math.floor(random.random() * 5)
            blockX = math.floor(random.random() * self.__worldObj.width)
            blockY = int(min(random.random(), random.random()) * self.__worldObj.height)
            blockZ = math.floor(random.random() * self.__worldObj.length)
            if not self.__worldObj.isBlockNormalCube(blockX, blockY, blockZ) and \
               ...
               (not self.__worldObj.isHalfLit(blockX, blockY, blockZ) or \
                math.floor(random.random() * 5) == 0):
                for j in range(8):
                    xx = blockX
                    yy = blockY
                    zz = blockZ
                    for k in range(3):
                        xx += math.floor(random.random() * 6) - math.floor(random.random() * 6)
                        yy += math.floor(random.random() * 1) - math.floor(random.random() * 1)
                        zz += math.floor(random.random() * 6) - math.floor(random.random() * 6)
                        if xx >= 0 and zz >= 0 and yy >= 0 and \
                           ...
                           self.__worldObj.isBlockNormalCube(xx, yy + 1, zz):
                            ...
                            if self.__worldObj.checkIfAABBIsClear(mob.boundingBox):
                                mobs += 1
                                self.__worldObj.releaseEntitySkin(mob)
