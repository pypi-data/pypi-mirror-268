...
def initParser(parser):
    parser.add_argument("names", metavar="name", help="Reader name", nargs="+")

def main():
    args = ClientArgs.ParsePlatform(description="Readers info", init=initParser, defaultformat="yaml")
    ClientConfig.UpdatePlatformFromConfiguration(args)
    clientv2 = MetadataGenClientV2(**vars(args))

    responses = (clientv2.GetReaderV2(n) for n in args.names)
