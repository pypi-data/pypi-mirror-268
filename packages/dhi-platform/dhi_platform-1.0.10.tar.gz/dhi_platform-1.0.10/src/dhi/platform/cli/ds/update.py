...
def main():
    args = ClientArgs.ParseForDataset(description="Update dataset", init=__initParser, defaultformat="yaml")
    ClientConfig.UpdateDatasetFromConfiguration(args)
    clientv3 = MetadataGenClientV3(**vars(args))

    input = MetadataClientV3Contracts.PrepareUpdateInput(
    ...
    response = clientv3.UpdateFromFileV3(input, args.datasetid)
