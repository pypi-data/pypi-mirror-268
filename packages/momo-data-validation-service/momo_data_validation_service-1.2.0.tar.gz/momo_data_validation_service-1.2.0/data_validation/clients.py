...
def get_ibis_table(client, schema_name, table_name, database_name=None):
    ...
    if client.name == "trino":
        schema_arr = schema_name.split('.')
        if len(schema_arr) < 2:
            raise exc.NoSuchTableError("Catalog and schema is required in tables-list")
        database_name = schema_arr[0]
        schema_name = schema_arr[1]
