...
def sync_scripts():
    ...
    for client in p4.run_clients("--me"):
        ...
        try:
            scripts_depot_path = p4.run_where(f"{client_root}\{SCRIPTS_DIR}\*")[0]["depotFile"]
        except:
...
def get_script_path(script_file_name):
    main_root = f"s:/sn2-main/{SCRIPTS_DIR}"
    main_path = f"{main_root}/{script_file_name}"
    if os.path.isfile(main_path):
        return main_path
    if p4_client:
        p4_path = f"{get_client_root(p4_client)}/{SCRIPTS_DIR}/{script_file_name}"
        if os.path.isfile(p4_path):
            return p4_path
    client_root = p4.run_info()[0]["clientRoot"]
    if client_root:
        p4_path = f"{client_root}/{SCRIPTS_DIR}/{script_file_name}"
        if os.path.isfile(p4_path):
            return p4_path
