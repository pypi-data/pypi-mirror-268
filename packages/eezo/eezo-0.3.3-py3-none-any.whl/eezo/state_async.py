...
class StateProxyAsync:
    ...
    Attributes:
        client: The asynchronous client instance that this proxy is managing state for.
    ...
    async def load(self) -> None:
        """
        Asynchronously load the state data from the database using the configured user ID.
        ...
        result: Dict[str, Any] = await self.client.read_state(self.client.user_id)
    ...
    async def save(self) -> None:
        """
        Asynchronously save the state data to the database.

        If the state was loaded and is not empty, the state is asynchronously updated through the
        client using the configured user ID. If the state was not loaded, it logs a warning.
        """
        logging.info(">> Saving state")
        if not self.client.state_was_loaded:
            logging.warning("State was not loaded, skipping save.")
        elif self._state:
            await self.client.update_state(self.client.user_id, self._state)
        else:
