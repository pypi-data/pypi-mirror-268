...
METADATA_HEADERS_V1 = (
    ...
    "timeStamp", "ramanExcitationWavelength", "movementMode", "posX", "posY", \
    "posZ", "posA", "posB", "posC", "roiName", \
    "sessionGUID", "sessionName", "spectrumCounter", "exposureTime", "shutterMode", \
    "aisApplied", "smoothingLevel", "aiaOffsetHorizontal", "aiaOffsetVertical", "binningHorizontal", \
    ...
    "laserEnergySD", "laserPulses", "laserFrequency", "laserPower", "cleaningShots", \
    "average", "subtractDark", "temperatureDetector", "temperatureSpectrometer", "temperatureSample", \
    "temperatureSampleChamber", "temperatureEnvironment")
"""Headers of the unified metadata defined for aryx. All functions will return this metadata."""


METADATA_DEFAULT_V1 = (
    1, "", "", "", "", \
    TIMESTAMP_MISSING, np.NaN, "AtRest", np.NaN, np.NaN, \
    np.NaN, np.NaN, np.NaN, np.NaN, "", \
    "", "", 1, np.NaN, "Open", \
    False, np.NaN, np.NaN, np.NaN, np.NaN, \
...
def __read_ary_meta(meta: List[str]) -> Dict:
    ...
    head["timeStamp"] = time.isoformat().replace("+00:00", "Z")
...
def read_ltb_ary(file: Union[Path,str,IO[bytes]], *, sort_wl: bool=True) -> Spectra:
    ...
    with zipfile.ZipFile(file) as f_zip:
        ...
        head["timestamp"] = parser.isoparse(head["timeStamp"])
...
def __read_aryx_meta(f_zip: zipfile.ZipFile) -> dict:
    head = {}
    file_list = f_zip.namelist()
    if "summary.json" in file_list:
        head_s = f_zip.read("summary.json").decode("utf-8")
...
def read_ltb_aryx(file: Union[Path,str,IO[bytes]], *, sort_wl:bool=True) -> Spectra:
    ...
    with zipfile.ZipFile(file) as f_zip:
        ...
        if isinstance(file, (Path, str)):
            ...
            head['filename'] = "memory"
        head["timestamp"] = parser.isoparse(str(head["timeStamp"]))
...
def write_ltb_aryx(file: Union[str,Path,IO[bytes]], spec: Spectra) -> None:
    ...
    with zipfile.ZipFile(file, mode="w") as f_zip:
        ...
        f_zip.writestr("summary.json", json.dumps(head_write, allow_nan=False))
