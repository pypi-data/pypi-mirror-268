...
class ApiClient(BaseClient):
    def __init__(
        ...
        *,
        ...
        if should_verify_compatibility:
            ...
            uncompatible_statuses = [CompatibilityStatus.incompatible, CompatibilityStatus.unknown]
    ...
    def forgotten_password(self, email: str, timeout: Optional[int] = None) -> None:
        self.auth.forgotten_password(ForgottenPasswordRequest(email=email), timeout=timeout or self.timeout)

    def reset_password(self, email: str, new_password: str, new_password_repeated: str, token: UUID, timeout: Optional[int] = None) -> None:
        self.auth.reset_password(ResetPasswordRequest(email=email, new_password=new_password, new_password_repeated=new_password_repeated, token=token), timeout=timeout or self.timeout)
