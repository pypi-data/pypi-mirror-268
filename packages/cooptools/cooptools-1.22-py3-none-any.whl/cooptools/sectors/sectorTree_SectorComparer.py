...
SectorComparer = Callable[[rect.Rect, Any], bool]
...
class SectorTree:
    ...
    def _add_child_layer(self, grid_pos: Tuple[int, int], sector_comparer: SectorComparer):
        ...
        logging.info(f"child layer added at Lvl {self.lvl}: {grid_pos} with area rect: {child_rect}")

    def _handle_child_layer(self, grid_pos: Tuple[int, int], sector_comparer: SectorComparer):
    ...
    def add_update_client(self, client, sector_comparer: SectorComparer):
        if self.lvl == 0:
            logging.info(f"User requests adding [{client}]")
        ...
        for grid_pos, _ in self.grid.grid_enumerator:
            ...
            if sector_comparer(area, client):
                ...
                logging.info(f"client [{client}] added to Lvl {self.lvl}: {grid_pos}")
...
if __name__ == "__main__":
    ...
    def test2():
        ...
        check_areas = {
            ii: area_gen(_rect, max_w=25, max_h=25) for ii in range(10)
        }
        pprint(check_areas)
