...
def check_ccode(ccode):
    ...
    """
    ccode = ccode.upper()
    shpfile = pathlib.Path(__file__).parent / "data" / COUNTRIES_SHP
    ccodes = []
...
def _get_utm_proj(lat, lon):
    ...
    if lat < 0:
        south = "+south"
    fmt = (
        f"+proj=utm +zone={zone} {south} +ellps=WGS84 " "+datum=WGS84 +units=m +no_defs"
    )
    proj = pyproj.Proj(fmt)
    return proj
...
def _get_pshape(polygon, buffer_km):
    ...
    utm_crs = pyproj.CRS(utmproj.definition_string())
    project = pyproj.Transformer.from_crs(geo_crs, utm_crs, always_xy=True).transform
