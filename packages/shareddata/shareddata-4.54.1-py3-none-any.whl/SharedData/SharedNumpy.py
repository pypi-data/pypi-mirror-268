...
class SharedNumpy(np.ndarray):
    ...
    def subscribe(self, host, port, lookbacklines=1000, lookbackdate=None, method='socket', snapshot=False, bandwidth=1e6):
        self.table.subscribe(host, port, lookbacklines,
                             lookbackdate, method, snapshot, bandwidth)
        
    def publish(self, host, port, lookbacklines=1000, lookbackdate=None, method='socket', snapshot=False, bandwidth=1e6):
    ...
    def tags2df(self, dt, seltags, datetags=None):
        ...
        for tag in seltags:
            try:
                rec = self.loc[dt,tag]    
                values = self.records2df(rec).droplevel(0).droplevel(0)
                ...
                Logger.log.error('Could not get %s!' % (tag))
        return df, dfmtime
