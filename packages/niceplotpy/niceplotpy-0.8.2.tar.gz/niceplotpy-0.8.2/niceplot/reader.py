...
class Reader(DotAccessibleDict):
    ...
    def __init__(self, config_file: str) -> None:
        print(f"Making reader based on config file: {config_file}")
    ...
    def prepdfdict(self) -> dict[pandas.DataFrame]:
        ...
        if os.path.isdir(self.input_file):
            treelist = [uproot.open(os.path.join(self.input_file, filen))[self.treename] for filen in os.listdir(self.input_file)]
        else:
            treelist = [uproot.open(self.input_file)[self.treename]]
        ...
        for config in self.configurations:
            ...
            if config.seedfrom is not None:
                ...
                dfdict[config.name] = pandas.concat([ak.to_dataframe(tree.arrays(grab_columns, library='ak')) for tree in treelist])
