...
class SentinelApi:
    ...
    def _find_query(
        ...
        polygon_query = f"OData.CSC.Intersects(area=geography'SRID=4326;{polygon}')" if polygon else ""

        tile_query = f"{'and ' if polygon else ''}contains(Name, '_T{tile}_')" if tile else ""
        ...
        search_query = f"{self.catalogue_odata_url}/Products?$filter={polygon_query}{tile_query}{cloud_cover} and Online eq true and Attributes/OData.CSC.StringAttribute/any(att:att/Name eq 'productType' and att/OData.CSC.StringAttribute/Value eq '{productType.value}'){init_date}{end_date}&$expand=Attributes{order_by}{limit}"
        return search_query
    ...
    def findGeom(
        ...
        """
        search_query = self._find_query(
            productType, None, polygon, init_date, end_date, cloud_cover, limit, order_desc
        )
