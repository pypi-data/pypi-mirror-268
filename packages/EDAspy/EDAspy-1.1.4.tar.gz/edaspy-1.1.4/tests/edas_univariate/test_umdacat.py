...
class TestUMDAcat(TestCase):
    ...
    def test_constructor(self):
        eda = UMDAcat(size_gen=100, max_iter=100, dead_iter=10, n_variables=self.n_variables, alpha=0.5,
                      frequency=self.frequency, possible_values=self.possible_values)

        assert eda.size_gen == 100
        assert eda.max_iter == 100
        assert eda.dead_iter == 10
        assert eda.n_variables == self.n_variables
    ...
    def test_data_init(self):
        ...
        n_variables = 3
        eda = UMDAcat(size_gen=20, max_iter=1, dead_iter=1, n_variables=n_variables, alpha=0.5,
                      possible_values=self.possible_values, frequency=self.frequency, init_data=gen)

        eda.best_mae_global = 999
