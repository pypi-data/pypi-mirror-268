...
def _fn_crawl(args: Args) -> None:
    with AugmentedJenkinsClient(
        **extract_credentials(args.credentials), timeout=args.timeout
    ) as jenkins_client:
        for path, node in jenkins_client.traverse_job_tree():
            if node.type == "Folder":
                print(f"Folder: {node.name}")
                continue
            job = jenkins_client.job_info(path)
            job.expand(jenkins_client, max_build_infos=None)
            print(job)
            for build_nr, build in job.build_infos.items():
                assert (build.result is None) == build.inProgress
                print(f"  - {build_nr}: {build}")

    # download_build_artifacts(
    #    "checkmk/master/builders/build-linux-agent-updater", 63, out_dir="artifacts"
    # )
    # download_build_artifacts("checkmk/master/winagt-build", 632, out_dir="artifacts")
