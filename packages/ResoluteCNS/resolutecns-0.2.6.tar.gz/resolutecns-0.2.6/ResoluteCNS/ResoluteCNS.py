...
class CNS:
    ...
    def download_excel(self, download_search):
        ...
        check_download = download_wait(download_folder(download_filename), 30)
        if check_download < 30:
            #Open the file
...
def download_wait(file, timeout):

    seconds = 0

    while not os.path.exists(file) and seconds < timeout:
        time.sleep(1)
        seconds += 1
    return seconds