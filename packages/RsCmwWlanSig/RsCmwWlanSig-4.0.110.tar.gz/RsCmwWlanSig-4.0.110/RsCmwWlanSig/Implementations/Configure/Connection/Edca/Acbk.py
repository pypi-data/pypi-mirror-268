...
class AcbkCls:
	...
	def __init__(self, core: Core, parent):
		self._core = core
		self._cmd_group = CommandsGroup("acbk", core, parent)
	...
	def set(self, aif_sn: int, ecw_min: int, ecw_max: int, tx_op_lim: int) -> None:
		...
		Snippet: driver.configure.connection.edca.acbk.set(aif_sn = 1, ecw_min = 1, ecw_max = 1, tx_op_lim = 1) \n
		...
		self._core.io.write(f'CONFigure:WLAN:SIGNaling<Instance>:CONNection:EDCA:ACBK {param}'.rstrip())
	...
	def get(self) -> AcbkStruct:
		"""SCPI: CONFigure:WLAN:SIGNaling<instance>:CONNection:EDCA:ACBK \n
		Snippet: value: AcbkStruct = driver.configure.connection.edca.acbk.get() \n
		Configures the record fields of EDCA parameter set. \n
			:return: structure: for return value, see the help for AcbkStruct structure arguments."""
		return self._core.io.query_struct(f'CONFigure:WLAN:SIGNaling<Instance>:CONNection:EDCA:ACBK?', self.__class__.AcbkStruct())
