...
class AcbkCls:
	...
	def set(self, aif_sn: int, ecw_min: int, ecw_max: int, timer: int) -> None:
		...
		Snippet: driver.configure.connection.muedca.acbk.set(aif_sn = 1, ecw_min = 1, ecw_max = 1, timer = 1) \n
		...
		self._core.io.write(f'CONFigure:WLAN:SIGNaling<Instance>:CONNection:MUEDca:ACBK {param}'.rstrip())
	...
	def get(self) -> AcbkStruct:
		"""SCPI: CONFigure:WLAN:SIGNaling<instance>:CONNection:MUEDca:ACBK \n
		Snippet: value: AcbkStruct = driver.configure.connection.muedca.acbk.get() \n
		Configures the record fields of MU EDCA parameter set. \n
			:return: structure: for return value, see the help for AcbkStruct structure arguments."""
		return self._core.io.query_struct(f'CONFigure:WLAN:SIGNaling<Instance>:CONNection:MUEDca:ACBK?', self.__class__.AcbkStruct())
