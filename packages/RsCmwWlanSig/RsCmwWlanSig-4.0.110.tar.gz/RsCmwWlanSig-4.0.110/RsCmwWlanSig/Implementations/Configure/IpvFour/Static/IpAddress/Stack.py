...
class StackCls:
	...
	def __init__(self, core: Core, parent):
		self._core = core
		self._cmd_group = CommandsGroup("stack", core, parent)
	...
	def set(self, first: int, sec: int, third: int, fourth: int) -> None:
		...
		Snippet: driver.configure.ipvFour.static.ipAddress.stack.set(first = 1, sec = 1, third = 1, fourth = 1) \n
		...
		self._core.io.write(f'CONFigure:WLAN:SIGNaling<Instance>:IPVFour:STATic:IPADdress:STACk {param}'.rstrip())
	...
	def get(self) -> StackStruct:
		"""SCPI: CONFigure:WLAN:SIGNaling<instance>:IPVFour:STATic:IPADdress:STACk \n
		Snippet: value: StackStruct = driver.configure.ipvFour.static.ipAddress.stack.get() \n
		No command help available \n
			:return: structure: for return value, see the help for StackStruct structure arguments."""
		return self._core.io.query_struct(f'CONFigure:WLAN:SIGNaling<Instance>:IPVFour:STATic:IPADdress:STACk?', self.__class__.StackStruct())
