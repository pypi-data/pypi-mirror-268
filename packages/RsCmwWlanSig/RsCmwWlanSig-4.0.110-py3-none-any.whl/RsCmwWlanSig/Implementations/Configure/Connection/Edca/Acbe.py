...
class AcbeCls:
	...
	def set(self, aif_sn: int, ecw_min: int, ecw_max: int, tx_op_lim: int) -> None:
		...
		Snippet: driver.configure.connection.edca.acbe.set(aif_sn = 1, ecw_min = 1, ecw_max = 1, tx_op_lim = 1) \n
		...
		self._core.io.write(f'CONFigure:WLAN:SIGNaling<Instance>:CONNection:EDCA:ACBE {param}'.rstrip())
	...
	def get(self) -> AcbeStruct:
		"""SCPI: CONFigure:WLAN:SIGNaling<instance>:CONNection:EDCA:ACBE \n
		Snippet: value: AcbeStruct = driver.configure.connection.edca.acbe.get() \n
		Configures the record fields of EDCA parameter set. \n
			:return: structure: for return value, see the help for AcbeStruct structure arguments."""
		return self._core.io.query_struct(f'CONFigure:WLAN:SIGNaling<Instance>:CONNection:EDCA:ACBE?', self.__class__.AcbeStruct())
