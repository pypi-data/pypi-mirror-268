...
class AcvoCls:
	...
	def __init__(self, core: Core, parent):
		self._core = core
		self._cmd_group = CommandsGroup("acvo", core, parent)
	...
	def set(self, aif_sn: int, ecw_min: int, ecw_max: int, tx_op_lim: int) -> None:
		...
		Snippet: driver.configure.connection.edca.acvo.set(aif_sn = 1, ecw_min = 1, ecw_max = 1, tx_op_lim = 1) \n
		...
		self._core.io.write(f'CONFigure:WLAN:SIGNaling<Instance>:CONNection:EDCA:ACVO {param}'.rstrip())
	...
	def get(self) -> AcvoStruct:
		"""SCPI: CONFigure:WLAN:SIGNaling<instance>:CONNection:EDCA:ACVO \n
		Snippet: value: AcvoStruct = driver.configure.connection.edca.acvo.get() \n
		Configures the record fields of EDCA parameter set. \n
			:return: structure: for return value, see the help for AcvoStruct structure arguments."""
		return self._core.io.query_struct(f'CONFigure:WLAN:SIGNaling<Instance>:CONNection:EDCA:ACVO?', self.__class__.AcvoStruct())
