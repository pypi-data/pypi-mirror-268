...
class AllocationCls:
	...
	def set(self, ch_20_index: enums.Ch20Index, ru_index: enums.RuIndex, station=repcap.Station.Default, user=repcap.User.Default) -> None:
		"""SCPI: CONFigure:WLAN:SIGNaling<instance>:CONNection:STA<s>:DFRame:HEMU:USER<index>:ALLocation \n
		Snippet: driver.configure.connection.sta.dframe.hemu.user.allocation.set(ch_20_index = enums.Ch20Index.CHA1, ru_index = enums.RuIndex.RU1, station = repcap.Station.Default, user = repcap.User.Default) \n
		...
		self._core.io.write(f'CONFigure:WLAN:SIGNaling<Instance>:CONNection:STA{station_cmd_val}:DFRame:HEMU:USER{user_cmd_val}:ALLocation {param}'.rstrip())
