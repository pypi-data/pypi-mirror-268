...
class CmwCls:
	...
	def set(self, first: int, sec: int, third: int, fourth: int) -> None:
		...
		Snippet: driver.configure.ipvFour.static.ipAddress.cmw.set(first = 1, sec = 1, third = 1, fourth = 1) \n
		...
		self._core.io.write(f'CONFigure:WLAN:SIGNaling<Instance>:IPVFour:STATic:IPADdress:CMW {param}'.rstrip())
	...
	def get(self) -> CmwStruct:
		"""SCPI: CONFigure:WLAN:SIGNaling<instance>:IPVFour:STATic:IPADdress:CMW \n
		Snippet: value: CmwStruct = driver.configure.ipvFour.static.ipAddress.cmw.get() \n
		Defines the static IP V4 address of the R&S CMW. The setting is relevant for instruments without a DAU. \n
			:return: structure: for return value, see the help for CmwStruct structure arguments."""
		return self._core.io.query_struct(f'CONFigure:WLAN:SIGNaling<Instance>:IPVFour:STATic:IPADdress:CMW?', self.__class__.CmwStruct())
