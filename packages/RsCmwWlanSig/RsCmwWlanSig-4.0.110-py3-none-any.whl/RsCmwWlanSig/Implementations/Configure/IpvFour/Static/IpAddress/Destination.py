...
class DestinationCls:
	...
	def set(self, first_number: int, sec_number: int, third_number: int, fourth_number: int) -> None:
		...
		self._core.io.write(f'CONFigure:WLAN:SIGNaling<Instance>:IPVFour:STATic:IPADdress:DESTination {param}'.rstrip())
	...
	def get(self) -> DestinationStruct:
		"""SCPI: CONFigure:WLAN:SIGNaling<instance>:IPVFour:STATic:IPADdress:DESTination \n
		...
		return self._core.io.query_struct(f'CONFigure:WLAN:SIGNaling<Instance>:IPVFour:STATic:IPADdress:DESTination?', self.__class__.DestinationStruct())
