from .....Internal.Core import Core
from .....Internal.CommandsGroup import CommandsGroup


# noinspection PyPep8Naming,PyAttributeOutsideInit,SpellCheckingInspection
class SconnectionCls:
	"""Sconnection commands group definition. 1 total commands, 0 Subgroups, 1 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._cmd_group = CommandsGroup("sconnection", core, parent)

	def set(self, opc_timeout_ms: int = -1) -> None:
		"""SCPI: CALL:WLAN:SIGNaling<Instance>:ACTion:WDIRect:SCONnection \n
		Snippet: driver.call.action.wdirect.sconnection.set() \n
		No command help available \n
			:param opc_timeout_ms: Maximum time to wait in milliseconds, valid only for this call."""
		self._core.io.write_with_opc(f'CALL:WLAN:SIGNaling<Instance>:ACTion:WDIRect:SCONnection', opc_timeout_ms)
