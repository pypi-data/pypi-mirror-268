...
class RsCmwWlanSig:
	...
	_driver_options = "SupportedInstrModels = CMW500/CMW100/CMW270/CMW280/CMP, SupportedIdnPatterns = CMW, SimulationIdnString = 'Rohde&Schwarz,CMW500,100001,4.0.110.0044'"
	...
	def __init__(self, resource_name: str, id_query: bool = True, reset: bool = False, options: str = None, direct_session: object = None):
		...
		self._core = Core(resource_name, id_query, reset, RsCmwWlanSig._driver_options, options, direct_session)
		self._core.driver_version = '4.0.110.0044'
	...
	@classmethod
	def from_existing_session(cls, session: object, options: str = None) -> 'RsCmwWlanSig':
	...
	def __str__(self) -> str:
		if self._core.io:
			return f"RsCmwWlanSig session '{self._core.io.resource_name}'"
		else:
			return f"RsCmwWlanSig with session closed"
	...
	def assert_minimum_version(min_version: str) -> None:
		...
		curr_version_list = '4.0.110.0044'.split('.')
		...
		for i in range(count):
			...
			if curr > minimum:
				break
			if curr < minimum:
				raise RsInstrException(f"Assertion for minimum RsCmwWlanSig version failed. Current version: '4.0.110.0044', minimum required version: '{min_version}'")
	...
	def _sync_to_custom_properties(self, cloned: 'RsCmwWlanSig') -> None:
	...
	@property
	def configure(self):
		"""configure commands group. 14 Sub-classes, 0 commands."""
	...
	@property
	def sense(self):
		"""sense commands group. 5 Sub-classes, 0 commands."""
	...
	@property
	def pswitched(self):
		"""pswitched commands group. 1 Sub-classes, 0 commands."""
		if not hasattr(self, '_pswitched'):
			from .Implementations.Pswitched import PswitchedCls
	...
	def second(self):
		...
		if not hasattr(self, '_second'):
			from .Implementations.Second import SecondCls
	...
	def third(self):
		...
		if not hasattr(self, '_third'):
			from .Implementations.Third import ThirdCls
	...
	@property
	def hetBased(self):
		"""hetBased commands group. 2 Sub-classes, 3 commands."""
		if not hasattr(self, '_hetBased'):
			from .Implementations.HetBased import HetBasedCls
			self._hetBased = HetBasedCls(self._core, self._cmd_group)
		return self._hetBased

	@property
	def packRate(self):
		"""packRate commands group. 0 Sub-classes, 1 commands."""
		if not hasattr(self, '_packRate'):
			from .Implementations.PackRate import PackRateCls
			self._packRate = PackRateCls(self._core, self._cmd_group)
		return self._packRate

	@property
	def per(self):
		"""per commands group. 1 Sub-classes, 5 commands."""
		if not hasattr(self, '_per'):
			from .Implementations.Per import PerCls
	...
	def clone(self) -> 'RsCmwWlanSig':
		"""Creates a deep copy of the RsCmwWlanSig object. Also copies:
		...
		cloned = RsCmwWlanSig.from_existing_session(self.get_session_handle(), self._options)
