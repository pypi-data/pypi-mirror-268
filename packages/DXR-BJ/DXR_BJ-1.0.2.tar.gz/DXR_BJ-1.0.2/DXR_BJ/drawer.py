...
class Drawer(object):
    ...
    def Resizemessage(self, message):
        ...
        x_scale = (self.imgwidth / 1920.0) * 1920
        y_scale = (self.imgheight / 1080.0) * 1080

        # x_scale = (self.imgwidth / 1920.0) * 1
        # y_scale = (self.imgheight / 1080.0) * 1
    ...
    def label_rect_message(self, outimg, message):
        ...
        if message["sType"] in locationtypes:
            self.labelcolor = self.green
            self.rectcolor = self.green
            for num, target_rect in enumerate(rects):
                label = sValues[num][0]  ##标签
                ...
                if message["sType"] == 'HKIR':
                    ...
                    zh_label = str(int(sValues[num][0])) + '_' + str(int(sValues[num][1]))  ##标签
                else:
                    zh_label = self.global_label_name[sValues[num][0].split('_')[0]] + self.global_label_name[
        ...
        ###人脸
        facetypes = ['FaceRec']
        if message["sType"] in facetypes and texts:
            self.labelcolor = self.green
            self.rectcolor = self.green
            for num, target_rect in enumerate(rects):
                name = texts[num][0]
...
if __name__ == "__main__":
    ...
    outmessage={'error': False, 'bFlag': True, 'bState': True, 'sType': 'IRPerson', 'sValue': '1', 'lResults': {'rect': [[0.17447916666666666, 0.49166666666666664, 0.23697916666666666, 0.1824074074074074, 4.552219]], 'track': [], 'region': [], 'line': [], 'point': [], 'text': [['car']], 'sValue': [], 'res_key': 'rect'}}
    # outmessage={
    #     'error': False,
    #     'bFlag': True,
    #     'bState': True,
