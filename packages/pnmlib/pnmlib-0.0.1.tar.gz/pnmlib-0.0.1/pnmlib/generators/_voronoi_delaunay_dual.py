...
from pnmlib.tools import conns_to_am
from pnmlib.queries import isoutside
from pnmlib.queries import find_neighbor_nodes
...
def voronoi_delaunay_dual(points, shape, trim=True, reflect=True, relaxation=0):
    ...
    network['pore.delaunay'] = np.zeros(n_nodes, dtype=bool)
    network['pore.delaunay'][0:vor.npoints] = True
    network['pore.voronoi'] = np.zeros(n_nodes, dtype=bool)
    network['pore.voronoi'][vor.npoints:] = True
    # Label throats between Delaunay pores
    network['throat.delaunay'] = np.zeros(n_edges, dtype=bool)
    Ts = np.all(network['throat.conns'] < vor.npoints, axis=1)
    network['throat.delaunay'][Ts] = True
    # Label throats between Voronoi pores
    network['throat.voronoi'] = np.zeros(n_edges, dtype=bool)
    Ts = np.all(network['throat.conns'] >= vor.npoints, axis=1)
    network['throat.voronoi'][Ts] = True
    # Label throats connecting a Delaunay and a Voronoi pore
    Ts = np.sum(network['throat.delaunay'][conns].astype(int), axis=1) == 1
    network['throat.interconnect'] = Ts

    if trim:
        # Find all delaunay nodes outside the domain
        Ps = isoutside(network=network, shape=shape)*network['pore.delaunay']
        if np.any(Ps):  # only occurs if points were reflected
            ...
            network['pore.surface'] = np.zeros(n_nodes, dtype=bool)
            network['pore.surface'][Ns] = True
