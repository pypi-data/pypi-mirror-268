...
class SentenceTransformerVectorizer(BaseEstimator, TransformerMixin):
    def __init__(
        self, model_name: Union[str, SentenceTransformer] = "BAAI/bge-small-en-v1.5"
    ):
...
def _fetch_curation_artifacts(
    ...
    **options,
    ...
    if curate_model in supported["curate_model"] or isinstance(
        curate_model, ClassifierMixin
    ):
        is_model_valid = True

        if isinstance(curate_model, str):
    ...
    if not is_model_valid:
        ...
        msg += f"Currently, passing the string {', '.join(supported['curate_model'])} or any sklearn classifier object are supported\n"
