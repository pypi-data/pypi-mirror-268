...
file_dir = "/".join(os.path.abspath(__file__).split("/")[:-3])
...
class OPTProcessor:
    ...
    def reconstruct(self, sinogram: np.ndarray):
        ...
        if self.rec_process == Rec_Modes.FBP_GPU.value:
            ...
            artifact_path = "/".join(file_dir.split("/")[:-2])+"/models"+"/model.ckpt"
            tomodl_checkpoint = torch.load(artifact_path)

            self.iradon_functor.load_state_dict(tomodl_checkpoint['model_state_dict'])
