...
_keys = ['report_type', 'year', 'month', 'day',
         ...
         'air_temperature', 
         'methodOfWetBulbTemperatureMeasurement',
         ...
         'periodOfSwellWaves',
         'heightOfSwellWaves',
         'swellWavesDirection_1',
         'periodOfSwellWaves_1',
         'heightOfSwellWaves_1',
...
def parse_synop(message: str, year: int, month: int) -> dict:
    ...
    output['callsign'] = decoded['callsign']['value']
    
    if decoded.get('displacement') is not None:
        try:
            output['displacement_direction'] = decoded['displacement']['direction']['_code']
        except Exception:
            output['displacement_direction'] = None
    else:
        output['displacement_direction'] = None

    if decoded.get('displacement') is not None:
        try:
            output['displacement_speed'] = decoded['displacement']['speed']['_code']
        except Exception:
            output['displacement_speed'] = None
    else:
        output['displacement_speed'] = None
    ...
    if decoded.get('station_position') is not None:
        try:
            output['latitude'] = \
                round(decoded['station_position']['latitude'], 2)
        except Exception:
    ...
    if decoded.get('station_position') is not None:
        try:
            output['longitude'] = \
                round(decoded['station_position']['longitude'], 2)
        except Exception:
            output['longitude'] = None
    else:
        output['longitude'] = None

    if decoded.get('wet_bulb_temperature') is not None:
        if decoded['wet_bulb_temperature']['_code'] in [0, 1]:
            output['methodOfWetBulbTemperatureMeasurement'] = 0
        elif decoded['wet_bulb_temperature']['_code'] in [2]:
            output['methodOfWetBulbTemperatureMeasurement'] = 1        
        elif decoded['wet_bulb_temperature']['_code'] in [5, 6]:
            output['methodOfWetBulbTemperatureMeasurement'] = 2        
        elif decoded['wet_bulb_temperature']['_code'] in [7]:
            output['methodOfWetBulbTemperatureMeasurement'] = 3        
        else:
            output['methodOfWetBulbTemperatureMeasurement'] = 7        
    else:
        output['methodOfWetBulbTemperatureMeasurement'] = 7
            ...
            for windWave in decoded.get('wind_waves'):
                if windWave['instrumental'] == True:
                    output['periodOfWaves'] = \
                        round(windWave['period']['value'], 0)
                    output['heightOfWaves'] = \
                        round(windWave['height']['value'], 1) 
                else:
                    output['periodOfWindWaves'] = \
                        round(windWave['period']['value'], 0)
                    output['heightOfWindWaves'] = \
                        round(windWave['height']['value'], 1)                                    
        except Exception:
            output['periodOfWaves'] = None
            output['heightOfWaves'] = None
            output['periodOfWindWaves'] = None
            output['heightOfWindWaves'] = None                     
    ...
    if decoded.get('swell_waves') is not None:
        try:
            swellWaves = decoded.get('swell_waves')
            output['swellWavesDirection'] = \
                    round(swellWaves[0]['direction']['value'], 0)
            output['periodOfSwellWaves'] = \
                    round(swellWaves[0]['period']['value'], 0)
            output['heightOfSwellWaves'] = \
                    round(swellWaves[0]['height']['value'], 1) 
            if len(swellWaves) >= 2:  
                output['swellWavesDirection_1'] = \
                    round(swellWaves[1]['direction']['value'], 0)
                output['periodOfSwellWaves_1'] = \
                    round(swellWaves[1]['period']['value'], 0)
                output['heightOfSwellWaves_1'] = \
                    round(swellWaves[1]['height']['value'], 1) 
            else:
                output['swellWavesDirection_1'] = None
                output['periodOfSwellWaves_1'] = None
                output['heightOfSwellWaves_1'] = None               
        except Exception:
            output['swellWavesDirection_1'] = None
            output['periodOfSwellWaves_1'] = None
            output['heightOfSwellWaves_1'] = None                      
            output['swellWavesDirection'] = None
            output['periodOfSwellWaves'] = None
            output['heightOfSwellWaves'] = None 
    else:
        output['swellWavesDirection_1'] = None
        output['periodOfSwellWaves_1'] = None
        output['heightOfSwellWaves_1'] = None                      
    ...
    '''Translate wind instrument flag from the SYNOP code to the BUFR code
       wind_indicator also decides the output template '''  
    ...
    if (decoded.get('sunshine') is not None):
        if (decoded['sunshine'][0].get('amount') is not None):
            ...
            try:
                sun_time = decoded['sunshine'][0]['duration']['value']
            except Exception:
                sun_time = None

            try:
                # Sunshine amount should be given in minutes
                sun_amount = decoded['sunshine'][0]['amount']['value'] * 60
            except Exception:
                sun_amount = None
    ...
    num_s3_clouds = 0
    '''This group (8NsChshs) shall be repeated to report 
    a number of different layers or masses of cloud.'''
    ...
    '''When two or more cloud layers with their bases below station level occur at different 
    levels, two or more groups N´C´H´H´Ct shall be used. Ct shall be reported as 9 in the 
    groups indicating the layer of the smaller cloud amount and, in the remaining group, Ct 
    shall be coded in Code table 0552.'''
...
def extract_individual_synop(data: str) -> list:
    ...
    start_position = data.find("BBXX")

    # Start position is -1 if AAXX is not present in the message
    if start_position == -1:
        raise ValueError(
            "Invalid SYNOP message: BBXX could not be found."
        )

    data = re.split('(BBXX)', data[start_position:])
...
def transform(data: str, year: int,
    ...
    #     metadata_dict = {}
    #     '''from tsi to wsi. tsi: traditional station ID. wsi: wigos_station_identifier '''
    #     tsi_mapping = {}
    #     for row in reader:
    #         if len(row) == 0:
    #             continue
    #         single_row = dict(zip(col_names, row))
    #         '''tsi: traditional station ID'''
    #         tsi = single_row['traditional_station_identifier']
    #         try:
    #             '''wsi: wigos_station_identifier'''
