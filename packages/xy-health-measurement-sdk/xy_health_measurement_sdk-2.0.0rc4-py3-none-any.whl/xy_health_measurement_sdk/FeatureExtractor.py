...
from .protos.Validation_pb2 import TooLowFps, YawOutOfRange, PitchOutOfRange, FaceWobble
...
class FeatureExtractor(object):
    ...
    def validate(self, *args):
        landmarks, min_x, min_y, max_x, max_y = args
        self.__validate_skewing(landmarks)
        self.__validate_wobble(min_x, min_y, max_x, max_y)
    ...
    def __extract(frame, *args):
        landmarks, blendshapes, width, height = args
        ...
        feature.blendShape.values.extend(blendshapes)
        return feature
    ...
    def __validate_skewing(self, landmarks):
        """
        校验人脸角度偏移
        """
        ...
        yaw, pitch = 0, 0  # TODO:获取真实角度值
        if yaw_config and abs(yaw) > yaw_config['yaw_angle']:
            self.__skewing_cnt += 1
            util.generate_error(config={
                'code': YawOutOfRange,
                'level': 0 if self.__skewing_cnt < max_failed_times else 1,
                'msg': 0 if yaw > yaw_config['yaw_angle'] else 1
            })
        if pitch_config and abs(pitch) > pitch_config['pitch_angle']:
            self.__skewing_cnt += 1
            util.generate_error(config={
    ...
    def __validate_wobble(self, *position):
        """
        校验人脸晃动
        """
        ...
        self.__slided_window.append(position)

        if len(self.__slided_window) <= wobbling_config['interval']:
            return

        pre_position = self.__slided_window.pop(0)
        overlap_area_ratio = self.__calculate_overlap_area_ratio(pre_position, position)
        ...
        if self.__wobbling_cnt < wobbling_config['max_failed_times']:
            util.generate_error(FaceWobble)
