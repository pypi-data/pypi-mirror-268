...
from .protos.Validation_pb2 import TooSmallImage, FaceLost, FaceOutOfBoundary, TooFarDistance, TooCloseDistance
    ...
    def validate(cls, *args):
        ...
        if image_size_validation:
            ...
            if longer_side < image_size_validation['min_height'] or shorter_side < image_size_validation['min_width']:
                util.generate_error(TooSmallImage)

        # 校验是否存在人脸
        if not landmarks:
            util.generate_error(FaceLost)
        ...
        if util.get_validation(FaceOutOfBoundary):
            if len(landmarks) < 478 or min_x < 0 or min_y < 0 or max_x > width or max_y > height:
                util.generate_error(FaceOutOfBoundary)

        # 校验测量距离（太远或太近）
        min_face_ratio, max_face_ratio = (util.get_validation(TooFarDistance),
                                          util.get_validation(TooCloseDistance))
        if min_face_ratio or max_face_ratio:
            # 计算人脸区域面积占比
            face_ratio = (max_x - min_x) * (max_y - min_y)
            if min_face_ratio and face_ratio < min_face_ratio['min_ratio']:
                util.generate_error(TooFarDistance)
            if max_face_ratio and face_ratio > max_face_ratio['max_ratio']:
                util.generate_error(TooCloseDistance)
