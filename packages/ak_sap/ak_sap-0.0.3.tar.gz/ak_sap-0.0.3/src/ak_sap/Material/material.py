...
class Material(MasterClass):
    def __init__(self, mySapObject) -> None:
        ...
        self.__PropMaterial = mySapObject.SapModel.PropMaterial
    ...
    def rename(self, old: str, new: str):
        ...
        return self.__PropMaterial.ChangeName(old, new)
    
    @smooth_sap_do
    def total(self) -> int:
        """returns the total number of defined material properties in the model."""
        return self.__PropMaterial.Count()
    
    @smooth_sap_do
    def delete(self, name: str):
        return self.__PropMaterial.Delete(name)
    
    @smooth_sap_do
    def list_all(self) -> list[str]:
        """retrieves the names of all defined material properties of the specified type"""
        return self.__PropMaterial.GetNameList()[1:]
    
    def __get_type(self, name: str) -> dict:
        *_result, _ret = self.__PropMaterial.GetTypeOAPI(name)
        assert _ret == 0
        return {
    ...
    def get_props(self, name: str) -> dict:
        ...
        *_result, _ret = self.__PropMaterial.GetMaterial(name)
        assert _ret == 0
        return {
            'MaterialType': typing.get_args(MaterialTypesStr)[_result[0] - 1],
            'Color': _result[1],
    ...
    def add(self, name: str, material_type: MaterialTypesStr):
        ...
        return self.__PropMaterial.SetMaterial(name, typing.get_args(MaterialTypesStr).index(material_type) + 1)
    ...
    def set_density(self, name: str, mass_per_vol: float):
        ...
        return self.__PropMaterial.SetWeightAndMass(name, 2, mass_per_vol)
