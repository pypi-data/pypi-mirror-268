...
def attach_to_model():
    ...
    try:
        if st.session_state.get('sap_path') is not None and st.session_state['sap_path'].strip() != "":
            st.session_state['SAP'] = Sap2000Wrapper(attach_to_exist=True, program_path=st.session_state['sap_path'])
            success_msg()
        else:
