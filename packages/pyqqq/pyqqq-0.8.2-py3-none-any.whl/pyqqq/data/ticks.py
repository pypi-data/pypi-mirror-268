...
def get_tick_data(date: dtm.date, asset_code: str) -> pd.DataFrame:
    ...
    with BytesIO() as byte_stream:
        ...
        def __custom_parser(str):
            d = pd.to_datetime(str, format="%H%M%S")
            return d if pd.isnull(d) else d.time()

        df = pd.read_csv(
            byte_stream,
            compression="xz",
            dtype=dtype,
            date_parser=__custom_parser,
            parse_dates=["chetime", "opentime", "hightime", "lowtime"],
        )
