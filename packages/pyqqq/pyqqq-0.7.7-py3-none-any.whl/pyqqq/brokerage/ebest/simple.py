...
class EBestSimpleDomesticStock:
    ...
    def get_positions(self) -> List[EBestStockPosition]:
        ...
        while fetching:
            r = self.stock_api.get_stock_balance(
                tr_cont_key=tr_cont_key, cts_expcode=cts_expcode
            )
    ...
    def get_historical_daily_data(
        ...
        ), "last_date는 first_date와 같거나, 이후 날짜여야 합니다"
        assert (
            last_date <= dtm.date.today()
        ), "last_date는 오늘과 같거나 이전이어야 합니다."
    ...
    def get_today_minute_data(self, asset_code: str) -> pd.DataFrame:
        ...
        while fetching:
            ...
            for item in output2:
                ...
                datetime = dtm.datetime.strptime(
                    f"{item['date']} {item['time']}", "%Y%m%d %H%M%S"
                )
    ...
    def get_pending_orders(self) -> List[EBestStockOrder]:
        """
        미체결 주문을 조회합니다.

        Returns:
            List[EBestStockOrder]: 미체결 주문 리스트
        """
    ...
    def get_today_order_history(self) -> List[EBestStockOrder]:
        """
        오늘 주문 내역을 조회합니다.

        Returns:
            List[EBestStockOrder]: 오늘 주문 내역 리스트
        """
        fetching = True
        tr_cont_key = ""
        cts_ordno = ""
    ...
    def _get_today_order_details(self) -> dict:
        ...
        def __find_order_type(
            ord_prc_ptn_code: str, ord_cndi_tp_code: str
        ) -> OrderType:
        ...
        for item in r["output3"]:
            ...
            result[order_no] = {
                ...
                "order_type": __find_order_type(
                    item["OrdprcPtnCode"], item["OrdCndiTpCode"]
                ),
            }
    ...
    def get_tick_data(self, asset_code) -> list:
        ...
        while fetching:
            ...
            if tr_cont == "N" or cts_time == "":
                ...
                tr_cont = "Y"
                self.logger.debug(
                    f"get_tick_data: sleep 1 for next page request. current length: {len(result)} cts_time={cts_time} tr_cont_key={tr_cont_key}"
                )
                time.sleep(1)
