...
class LokiLogProvider(LogProvider):
    def __init__(
        ...
        self.ingress_auth_max_retry = 10
        self.ingress_auth_retry_wait_time_s = 0.1
        # We are retrying 401 here because there could be a race condition
        # between pinging Loki and the auth token getting persisted and
        # accessible by the ingress. In that case the status code will be
        # 401 and thus need to retry.
        # Also retrying 500 for possible transient errors.
        self.ingress_auth_retry_status = {401, 500}
        self.loki_query_max_retry_time_s = 60

    async def __aenter__(self, *args, **kwargs):
        self.session = aiohttp.ClientSession()
        self.ingress_auth_current_retry_count = 0
        return self
    ...
    async def query(
        ...
        lines = []
        while (
            self.ingress_auth_current_retry_count < self.ingress_auth_max_retry
            and time.time() - query_start_time < self.loki_query_max_retry_time_s
        ):
            async with self.session.get(
                self.url,
                params=self.params,
                headers=self.headers,
                timeout=self.loki_query_max_retry_time_s,
            ) as resp:
                if resp.status in self.ingress_auth_retry_status:
                    self.ingress_auth_current_retry_count += 1
                    await asyncio.sleep(self.ingress_auth_retry_wait_time_s)
                    if (
                        self.ingress_auth_current_retry_count
                        < self.ingress_auth_max_retry
                    ):
                        continue
