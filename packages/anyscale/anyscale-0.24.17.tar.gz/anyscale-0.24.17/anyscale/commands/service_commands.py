...
@click.option(
    ...
    multiple=True,
    type=str,
    help="Environment variables to set for the job. The format is 'key=value'. This argument can be specified multiple times.",
)
def deploy(  # noqa: PLR0912, PLR0913
...
@click.option(
    ...
    default=None,
    type=str,
    help="Name of the service. When running in a workspace, this defaults to the workspace name.",
)
@click.option(
    "-f",
    "--config-file",
...
def status(name: Optional[str], config_file: Optional[str], json: bool, verbose: bool):
    ...
    if not verbose:
        ...
        status_dict.pop("query_url", None)
        status_dict.pop("query_auth_token", None)
